"""Windows UTF-8 bootstrap for OpenAmer entry points.
...
should be imported at the very top of every OpenAmer entry point
(``openamer``, ``openamer-agent``, ``openamer-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``openamer_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``openamer_bootstrap``, never ``openamer_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing OpenAmer modules.

    OpenAmer ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real OpenAmer source root
    ...
    """
    root = src_root or os.environ.get("OPENAMER_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``OPENAMER_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("OPENAMER_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import openamer_bootstrap``
# (or ``from openamer_bootstrap import apply_windows_utf8_bootstrap``) at
