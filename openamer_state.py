#!/usr/bin/env python3
"""
SQLite State Store for OpenAmer Agent.
...
from openamer_cli.sqlite_runtime import (
...
DEFAULT_DB_PATH = get_openamer_home() / "state.db"
...
# optimized via ``openamer sessions optimize-storage``. A legacy DB sits at
...
# openamer_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``openamer_cli.kanban_db.connect`` so
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    logger.warning(
        ...
        "OpenAmer-managed installs can repair the embedded runtime with "
        "`openamer update`. See `openamer doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see openamer_cli/kanban_db.py) would
...
def _backup_db_file(db_path: Path) -> Optional[Path]:
    ...
    connection's POSIX advisory locks (see ``openamer_cli.sqlite_safe_read``).
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (openamer_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_openamer_state.py:600-632)
        ...
        probe_session_id = f"_openamer_fts_health_probe_{time.time_ns()}"
        try:
...
# (``~/.openamer/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``openamer sessions optimize-storage`` on a capable host).
#
...
# `openamer sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `openamer db optimize`. This is
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship openamer_state without openamer_cli). A
    ...
    try:
        ...
        logger.debug(
            "openamer_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch openamer_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``openamer_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `openamer snapshot list` / "
                "`openamer snapshot restore <id>`.",
                path,
            )
            return None
...
class SessionDB:
    ...
    # With multiple openamer processes (gateway + CLI sessions + worktree agents)
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `openamer snapshot list` / "
                    f"`openamer snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`openamer db optimize`),
            ...
            # ``openamer_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `openamer update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._openamer_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._openamer_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `openamer sessions optimize-storage` on a host "
    ...
    #
    # `optimize_fts_storage()` (the `openamer sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`openamer sessions optimize-storage`) ──
    #
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `openamer sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `openamer db optimize`) must keep its EXISTING
    ...
    def _backfill_gateway_metadata_from_sessions_json(
        ...
        """
        sessions_file = get_openamer_home() / "sessions" / "sessions.json"
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: openamer_state already depends on agent.* (see
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another OpenAmer surface and jump
    ...
    def delete_session_if_empty(
        ...
        and ``openamer sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade OpenAmer, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one OpenAmer session.

        A OpenAmer session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a OpenAmer session is already bound to any Telegram DM topic.
