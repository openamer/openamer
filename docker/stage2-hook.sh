...
#
# Wired into the image as /etc/cont-init.d/01-openamer-setup by the
...
OPENAMER_HOME="${OPENAMER_HOME:-/opt/data}"
INSTALL_DIR="/opt/openamer"

# Drop to openamer via s6-setuidgid, but skip it when already non-root.
as_openamer() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid openamer "$@"; }
...
# starts non-root. The baked install tree under /opt/openamer is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $OPENAMER_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# `id -u openamer` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# openamer UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u openamer)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-openamer UID).
...
and the baked /opt/openamer install tree is intentionally root-owned and
...
The image remaps the openamer user to that UID/GID at boot and chowns the data
...
# --- Bootstrap OPENAMER_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid openamer mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom OPENAMER_HOME paths whose parents only
# root can create (e.g. `OPENAMER_HOME=/home/openamer/.openamer` in a Compose
...
# host's `docker` group (or root). The supervised openamer user (UID 10000)
...
# openamer process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes openamer. Idempotent across container
...
#   - openamer is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G openamer 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] openamer already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" openamer 2>/dev/null; then
        echo "[stage2] Added openamer to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group openamer failed; docker backend may fail with EACCES"
    fi
    break
done
...
#
# The canonical list of openamer-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_openamer_uid=$(id -u openamer)

path_has_symlink_component() {
    path="$1"
    root="${2:-$OPENAMER_HOME}"
...
chown_openamer_tree() {
    ...
    chown -R openamer:openamer "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$OPENAMER_HOME" 2>/dev/null)" != "$actual_openamer_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $OPENAMER_HOME (targeted) to openamer ($actual_openamer_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $OPENAMER_HOME: chown the directory itself (not its contents)
    # so openamer can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$OPENAMER_HOME"; then
        :
    else
        chown openamer:openamer "$OPENAMER_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $OPENAMER_HOME failed (rootless container?) — continuing"
    fi
    # OpenAmer-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by openamer (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$OPENAMER_HOME/$sub" ]; then
            chown_openamer_tree "$OPENAMER_HOME/$sub"
        fi
    done
fi
...
# Always reset ownership of $OPENAMER_HOME/profiles to openamer on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> openamer …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as openamer and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$OPENAMER_HOME/profiles" ]; then
    chown_openamer_tree "$OPENAMER_HOME/profiles"
fi

# Always reset ownership of $OPENAMER_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged openamer runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$OPENAMER_HOME/cron" ]; then
    chown_openamer_tree "$OPENAMER_HOME/cron"
fi
...
# openamer pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged openamer gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $OPENAMER_HOME is
...
# is negligible.
if [ -d "$OPENAMER_HOME/platforms/pairing" ]; then
    chown_openamer_tree "$OPENAMER_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$OPENAMER_HOME/pairing" ]; then
    chown_openamer_tree "$OPENAMER_HOME/pairing"
fi

# Reset ownership of openamer-owned top-level state files on every boot.
# The targeted data-volume chown above only covers openamer-owned
# *subdirectories*; loose state files living directly under $OPENAMER_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> openamer …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged openamer runtime then hits
...
# sweep so host-owned files in a bind-mounted $OPENAMER_HOME are never
...
# entries of openamer_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$OPENAMER_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$OPENAMER_HOME/$f"; then
            :
        else
            chown openamer:openamer "$OPENAMER_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the openamer runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$OPENAMER_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$OPENAMER_HOME/config.yaml"; then
        :
    else
        chown openamer:openamer "$OPENAMER_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$OPENAMER_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as openamer user ---
# Run as openamer via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $OPENAMER_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_openamer mkdir -p \
    "$OPENAMER_HOME/backups" \
    "$OPENAMER_HOME/cron" \
    "$OPENAMER_HOME/sessions" \
    "$OPENAMER_HOME/logs" \
    "$OPENAMER_HOME/logs/gateways" \
    "$OPENAMER_HOME/hooks" \
    "$OPENAMER_HOME/memories" \
    "$OPENAMER_HOME/skills" \
    "$OPENAMER_HOME/skins" \
    "$OPENAMER_HOME/plans" \
    "$OPENAMER_HOME/workspace" \
    "$OPENAMER_HOME/home" \
    "$OPENAMER_HOME/pairing" \
    "$OPENAMER_HOME/platforms/pairing" \
    "$OPENAMER_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/openamer/.install_method (see Dockerfile), NOT written here into
# $OPENAMER_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $OPENAMER_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.openamer:/opt/data) and sometimes shared with a
...
# 'openamer update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $OPENAMER_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/openamer stamp, so deleting the data-dir copy is safe).
if [ -f "$OPENAMER_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$OPENAMER_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$OPENAMER_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$OPENAMER_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$OPENAMER_HOME/$dest"; then
            :
        else
            as_openamer cp "$INSTALL_DIR/$src" "$OPENAMER_HOME/$dest"
        fi
    fi
}
...
if [ -f "$OPENAMER_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$OPENAMER_HOME/.env"; then
        :
    else
        chown openamer:openamer "$OPENAMER_HOME/.env" 2>/dev/null || true
        chmod 600 "$OPENAMER_HOME/.env" 2>/dev/null || true
    fi
fi
...
if [ ! -f "$OPENAMER_HOME/gateway_state.json" ] && \
    ...
    if refuse_symlinked_path "seed" "$OPENAMER_HOME/gateway_state.json"; then
        ...
        printf '{"gateway_state":"running"}\n' > "$OPENAMER_HOME/gateway_state.json"
        chown openamer:openamer "$OPENAMER_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$OPENAMER_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_openamer "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/openamer/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI OpenAmer spawns for the
...
# main-wrapper.sh propagates it into the supervised ``openamer`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-openamer, dashboard, etc.).
