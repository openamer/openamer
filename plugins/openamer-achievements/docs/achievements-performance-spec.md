# OpenAmer Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: openamer-achievements plugin
...
### `GET /api/plugins/openamer-achievements/achievements`
...
### `POST /api/plugins/openamer-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/openamer-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/openamer-achievements/overview`
...
- Persist plugin data under:
  - `~/.openamer/plugins/openamer-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.openamer/plugins/openamer-achievements/scan_checkpoint.json`
...
- `~/.openamer/plugins/openamer-achievements/`
