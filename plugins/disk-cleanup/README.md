...
Auto-tracks and cleans up ephemeral files created during OpenAmer Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$OPENAMER_HOME` and `/tmp/openamer-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `OPENAMER_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `OPENAMER_HOME` or `/tmp/openamer-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$OPENAMER_HOME/disk-cleanup/` is itself excluded
- `$OPENAMER_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
