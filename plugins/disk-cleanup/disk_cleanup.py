...
Rules:
  ...
  - empty dirs    → always delete (under OPENAMER_HOME)
...
def get_state_dir() -> Path:
    """State dir — separate from ``$OPENAMER_HOME/logs/``."""
    return get_openamer_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$OPENAMER_HOME/logs/``."""
...
# Paths under $OPENAMER_HOME that must NEVER be deleted by quick(),
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside OPENAMER_HOME)")
        return False
