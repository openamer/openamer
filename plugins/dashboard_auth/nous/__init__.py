...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``openamer-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``openamer_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set OPENAMER_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"openamer_session_pkce\"]``
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between OPENAMER_DASHBOARD_PORTAL_URL / OPENAMER_DASHBOARD_OAUTH_CLIENT_ID
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``OPENAMER_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("OPENAMER_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``OPENAMER_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("OPENAMER_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    can surface "Set OPENAMER_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``openamer_cli.web_server.should_require_auth`` +
