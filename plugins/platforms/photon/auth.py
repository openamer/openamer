...
Credential storage mirrors every other OpenAmer channel:

    * runtime SDK creds  -> ``~/.openamer/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.openamer/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a openamer dependency
...
# registers OpenAmer as its own client_id.
...
# Default name of the project OpenAmer provisions for the operator.
DEFAULT_PROJECT_NAME = "OpenAmer Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of openamer-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.openamer/auth.json`` honouring the active OpenAmer profile."""
    try:
        from openamer_constants import get_openamer_home
        return Path(get_openamer_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.openamer")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.openamer/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.openamer/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.openamer/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: openamer_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `openamer photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `openamer photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `openamer photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `openamer photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `openamer photon setup`)"
