// OpenAmer Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> OpenAmer): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (OpenAmer -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-OpenAmer-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `openamer photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so OpenAmer can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply OpenAmer'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local OpenAmer allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of OpenAmer, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-openamer-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when OpenAmer supplied them so a known-good
