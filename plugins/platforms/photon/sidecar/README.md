...
Small Node helper that bridges OpenAmer Agent to Photon's Spectrum SDK
(`spectrum-ts`).  OpenAmer is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-OpenAmer-Sidecar-Token`)
...
The OpenAmer plugin's `openamer photon setup` command runs `npm install`
here automatically.
