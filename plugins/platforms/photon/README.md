...
This plugin connects OpenAmer Agent to iMessage (and other Spectrum
...
TypeScript-only, OpenAmer runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-OpenAmer-Sidecar-Token`.
...
There is no separate `login` command; like every other OpenAmer channel,
...
Run `openamer photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.openamer/.env` (the same place every other
...
Management metadata lives in `~/.openamer/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `openamer photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `openamer photon status`, a DM and a group roundtrip,
