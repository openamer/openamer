"""ntfy platform adapter (OpenAmer plugin).
...
a OpenAmer dependency.

This adapter ships as a OpenAmer platform plugin under
``plugins/platforms/ntfy/``. The OpenAmer plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "openamer-in"              # subscribe topic (incoming)
          publish_topic: "openamer-out"     # optional — defaults to topic
...
_ECHO_TAG = "openamer-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``openamer cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a OpenAmer dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `openamer gateway status` without
