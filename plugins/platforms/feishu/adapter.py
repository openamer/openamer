...
# OpenAmer monkey-patches ``websockets.connect`` on the shared module to inject ping settings.
...
# (``loop``), and OpenAmer also monkey-patches ``websockets.connect`` on the shared ``websockets`` module to
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_openamer_home() / "feishu_seen_message_ids.json"
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            dedup_cache_size=max(32, env_int("OPENAMER_FEISHU_DEDUP_CACHE_SIZE", _DEFAULT_DEDUP_CACHE_SIZE)),
            text_batch_delay_seconds=env_float("OPENAMER_FEISHU_TEXT_BATCH_DELAY_SECONDS", _DEFAULT_TEXT_BATCH_DELAY_SECONDS),
            text_batch_split_delay_seconds=env_float("OPENAMER_FEISHU_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0),
            text_batch_max_messages=max(1, env_int("OPENAMER_FEISHU_TEXT_BATCH_MAX_MESSAGES", _DEFAULT_TEXT_BATCH_MAX_MESSAGES)),
            text_batch_max_chars=max(1, env_int("OPENAMER_FEISHU_TEXT_BATCH_MAX_CHARS", _DEFAULT_TEXT_BATCH_MAX_CHARS)),
            media_batch_delay_seconds=env_float("OPENAMER_FEISHU_MEDIA_BATCH_DELAY_SECONDS", _DEFAULT_MEDIA_BATCH_DELAY_SECONDS),
    ...
    def _get_sdk_executor(self) -> concurrent.futures.ThreadPoolExecutor:
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(max_workers=10, thread_name_prefix="openamer-feishu-sdk")
    ...
    async def _run_blocking(self, func, *args):
        ...
        profile OPENAMER_HOME override / secret scope (multiplexed dedup flush, thread lookup).
        """
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local OpenAmer gateway is already using this Feishu app_id"
    ...
    async def _send_exec_approval_prompt(self, prompt: ExecApprovalPrompt) -> SendResult:
        """Approval-button card; ``openamer_action`` in each button value lets the click callback
        ...
        try:
            ...
            actions = [
                _card_button(label, style or "default",
                             {"openamer_action": self._EA_CARD_ACTIONS[choice], "approval_id": approval_id})
    ...
    def _write_update_prompt_response(answer: str) -> None:
        response_path = get_openamer_home() / ".update_response"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that OpenAmer does not act on."""
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        if isinstance(action_value, dict):
            if action_value.get("openamer_action"):
                return self._handle_approval_card_action(event=event, action_value=action_value, loop=loop)
            if action_value.get("openamer_update_prompt_action"):
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return self._card_response()
        choice = _APPROVAL_CHOICE_MAP.get(action_value.get("openamer_action"), "deny")
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return self._card_response()
        answer = str(action_value.get("openamer_update_prompt_action", "") or "").strip().lower()
    ...
    async def _download_remote_document(self, file_url: str, *, default_ext: str, preferred_name: str) -> tuple[str, str]:
        ...
        async with create_ssrf_safe_async_client(
            ...
            response = await client.get(
                file_url, headers={"User-Agent": "Mozilla/5.0 (compatible; OpenAmerAgent/1.0)", "Accept": "*/*"},
            )
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by OpenAmer webhook mode")
...
def _begin_registration(domain: str = "feishu") -> dict:
    ...
    qr_url += ("&" if "?" in qr_url else "?") + "from=openamer&tp=openamer"
    return {
...
# static dict in openamer_cli/gateway.py, and the _send_feishu dispatch in tools/send_message_tool.py).
# ──────────────────────────────────────────────────────────────────────────
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    if not await asyncio.to_thread(_load_lark_oapi):
        return send_error("Feishu dependencies not installed. Run `openamer setup` to install Feishu support.")
    try:
...
def interactive_setup() -> None:
    ...
    from openamer_cli.config import remove_env_value, save_env_value
    from openamer_cli.setup import prompt_choice
    from openamer_cli.cli_output import prompt, print_header, print_info, print_success, print_warning
    from openamer_cli.setup_platforms import declines_reconfigure
    ...
    if access_idx == 2:
        ...
        if access_idx == 0:
            ...
            print_info("Unknown users can request access; approve with `openamer pairing approve`.")
        else:
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `openamer setup` to install Feishu support.", setup_fn=interactive_setup,
