...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        try:
            ...
            hermes_home = Path(get_openamer_home()).resolve()
        except Exception:
...
def interactive_setup() -> None:
    ...
    one optional public URL. Writes to ``~/.hermes/.env`` via ``openamer_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("openamer_cli.config not available; set LINE_* vars manually in ~/.hermes/.env")
        return
