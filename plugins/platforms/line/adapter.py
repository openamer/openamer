"""
LINE Messaging API platform adapter for OpenAmer Agent.
...
to OpenAmer Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``OPENAMER_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            openamer_home = Path(get_openamer_home()).resolve()
        except Exception:
            openamer_home = Path.home().joinpath(".openamer").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``openamer status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``openamer status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``openamer setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.openamer/.env`` via ``openamer_cli.config``.
    """
    print()
    ...
    try:
        from openamer_cli.config import get_env_var, set_env_var
    except ImportError:
        print("openamer_cli.config not available; set LINE_* vars manually in ~/.openamer/.env")
        return
