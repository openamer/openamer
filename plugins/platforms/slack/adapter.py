...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from openamer_cli.commands import is_gateway_known_command
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `openamer slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `openamer gateway setup` "
                "or in the active profile's ~/.openamer/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `openamer gateway setup` "
                "or add it to your active profile's ~/.openamer/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `openamer gateway setup` "
                "or in the active profile's ~/.openamer/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `openamer gateway setup` "
                "or add it to your active profile's ~/.openamer/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_openamer_home() / "slack_tokens.json"
        ...
        try:
            ...
            # OpenAmer has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by OpenAmer can "
                    "be removed from the Slack app manifest via "
                    "`openamer slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            # /model work directly without /openamer prefix). A single regex
            ...
            # manifest (see `openamer slack manifest`). In Socket Mode, Slack
            ...
            from openamer_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/openamer$")

            @self._app.command(_slash_pattern)
            async def handle_openamer_command(ack, command):
            ...
            for _action_id in (
                "openamer_approve_once",
                "openamer_approve_session",
                "openamer_approve_always",
                "openamer_deny",
            ):
            ...
            for _action_id in (
                "openamer_confirm_once",
                "openamer_confirm_always",
                "openamer_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("openamer_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^openamer_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("openamer_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'openamer slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: OpenAmer handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own OpenAmer session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # OpenAmer owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_openamer_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_openamer_no_thread_response"] = True
    ...
    async def _handle_slack_message(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_openamer_no_thread_response"):
        ...
        force_process = bool(event.get("_openamer_force_process"))
        ...
        # the shape produced by peer OpenAmer agents in Socket Mode on some
        ...
        try:
            ...
            actions = [
                {
                    ...
                    "style": "primary",
                    "action_id": "openamer_approve_once",
                    "value": session_key,
                },
            ]
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "openamer_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "openamer_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "openamer_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "openamer_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "openamer_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``openamer_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``openamer_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"openamer_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "openamer_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "openamer_confirm_once": "once",
            "openamer_confirm_always": "always",
            "openamer_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "openamer_approve_once": "once",
            "openamer_approve_session": "session",
            "openamer_approve_always": "always",
            "openamer_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "openamer_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/openamer <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/openamer
        ...
        if slash_name in {"openamer", ""}:
            # Legacy /openamer <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from openamer_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/openamer   ``).
        ...
        # "Running /cmd…" ack shown by handle_openamer_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/openamer <question>" must produce public replies so
...
# ``openamer_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_openamer_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``openamer_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under OPENAMER_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from openamer_cli.slack_cli import _build_full_manifest
            from openamer_constants import get_openamer_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="OpenAmer",
                bot_description="Your OpenAmer agent on Slack",
            )
            ...
            print_info(
                "   Re-run `openamer slack manifest --write` anytime to refresh after "
                "OpenAmer adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `openamer update`)",
                True,
            ):
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``openamer_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `openamer setup` to install Slack support.",
        # Interactive setup wizard — replaces openamer_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in openamer_cli/gateway.py.
