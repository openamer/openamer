"""
Email platform adapter for the OpenAmer gateway.

Allows users to interact with OpenAmer by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "openamer-agent" "version" "{_openamer_version}" '
            '"vendor" "openamer" '
            '"support-email" "noreply@openamer.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `openamer config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `openamer gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "OpenAmer Agent")
        ...
        msg_id = f"<openamer-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# openamer_cli/gateway.py, and the _send_email dispatch in
