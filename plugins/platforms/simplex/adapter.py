"""SimpleX Chat platform adapter (OpenAmer plugin).
...
This adapter ships as a OpenAmer platform plugin under
``plugins/platforms/simplex/``. The OpenAmer plugin loader scans the
...
Optional environment variables:
    ...
    OPENAMER_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``openamer setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside OpenAmer — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("OPENAMER_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``openamer setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.openamer/.env`` via
    ``openamer_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "openamer_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.openamer/.env"
        )
        return
