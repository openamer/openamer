...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "openamer-kanban-dot-triage",
    todo: "openamer-kanban-dot-todo",
    ready: "openamer-kanban-dot-ready",
    running: "openamer-kanban-dot-running",
    blocked: "openamer-kanban-dot-blocked",
    done: "openamer-kanban-dot-done",
    archived: "openamer-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-openamer-task";
  ...
  const DOCS_URL = "https://openamer.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://openamer.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "openamer.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="openamer-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])openamer-kanban-md-code\1/i.test(attrs)) {
      return ' class="openamer-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "openamer-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "openamer-kanban-md",
  ...
  // standard `drop` event and our `openamer-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("openamer-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("openamer-kanban-column--drop");
          if (target) target.classList.add("openamer-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("openamer-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("openamer-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("openamer-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__OPENAMER_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "openamer-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "openamer-kanban-attention",
        "openamer-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "openamer-kanban-attention-bar" },
        h("span", { className: "openamer-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "openamer-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "openamer-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "openamer-kanban-attention-row",
                  "openamer-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "openamer-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "openamer-kanban-attention-row-id" }, task.id),
                h("span", { className: "openamer-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "openamer-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "openamer-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "openamer-kanban-diag-action-btn",
      action.suggested ? "openamer-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " openamer-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".openamer-kanban-drawer-comment-row input, .openamer-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "openamer-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("openamer-kanban-diag", sevClass) },
      h("div", { className: "openamer-kanban-diag-header" },
        h("span", { className: "openamer-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "openamer-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "openamer-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "openamer-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "openamer-kanban-diag-data-row" },
                  h("span", { className: "openamer-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "openamer-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "openamer-kanban-diag-data-row" },
                h("span", { className: "openamer-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "openamer-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "openamer-kanban-diag-reassign-row" },
            h("span", { className: "openamer-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "openamer-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "openamer-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "openamer-kanban-diag-msg",
              msg.ok ? "openamer-kanban-diag-msg--ok" : "openamer-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "openamer-kanban-section" },
      h("div", { className: "openamer-kanban-section-head-row" },
        h("span", { className: "openamer-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "openamer-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "openamer-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "openamer-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "openamer-kanban-docs-link",
      title: "Open OpenAmer Kanban docs in a new tab",
      "aria-label": "OpenAmer Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `openamer kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "openamer-kanban-msg-ok" : "openamer-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "openamer-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "openamer-kanban-boardswitcher" },
      h("div", { className: "openamer-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "openamer-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "openamer-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "openamer-kanban-dialog-title" },
        ...
        h("div", { className: "openamer-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned OpenAmer profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "openamer-kanban-bulk" },
      h("span", { className: "openamer-kanban-bulk-count" },
      ...
      h("div", { className: "openamer-kanban-bulk-priority",
      ...
      h("div", { className: "openamer-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different OpenAmer profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "openamer-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("openamer-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("openamer-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "openamer-kanban-trash",
        dragOver ? "openamer-kanban-trash--drop" : "",
        props.draggingTaskId ? "openamer-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "openamer-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "openamer-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".openamer-kanban-card")) return true;
      if (target.closest && target.closest(".openamer-kanban-column-add")) return true;
      if (target.closest && target.closest(".openamer-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("openamer-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("openamer-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".openamer-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "openamer-kanban-columns",
        isScrollable ? "openamer-kanban-columns--scrollable" : "",
        isPanning ? "openamer-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("openamer-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("openamer-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "openamer-kanban-column",
        dragOver ? "openamer-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "openamer-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "openamer-kanban-col-check",
        ...
        h("span", { className: cn("openamer-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "openamer-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "openamer-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "openamer-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "openamer-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "openamer-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "openamer-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "openamer-kanban-lane" },
                  h("div", { className: "openamer-kanban-lane-head" },
                    h("span", { className: "openamer-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "openamer-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "openamer-kanban-card--stale-red";
    if (age >= tier.amber) return "openamer-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".openamer-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "openamer-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "openamer-kanban-card",
        props.selected ? "openamer-kanban-card--selected" : "",
        props.failed ? "openamer-kanban-card--failed" : "",
        props.draggingSource ? "openamer-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "openamer-kanban-card-content" },
          h("div", { className: "openamer-kanban-card-row" },
            h("label", {
              className: "openamer-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "openamer-kanban-card-check",
            ...
            h("span", { className: "openamer-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or openamer kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "openamer-kanban-warning-badge",
                    "openamer-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "openamer-kanban-needs-assignee",
          ...
          h("div", { className: "openamer-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "openamer-kanban-card-row openamer-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "openamer-kanban-assignee",
                            title: `Assigned to OpenAmer profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "openamer-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "openamer-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "openamer-kanban-ago",
  ...
  // centered modal reusing the openamer-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "openamer-kanban-dialog openamer-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "OpenAmer profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "OpenAmer profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "openamer-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "openamer-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "openamer-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "openamer-kanban-drawer-close",
        ...
        data ? h("div", { className: "openamer-kanban-drawer-comment-foot" },
          h("div", {
            className: "openamer-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "openamer-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "openamer-kanban-section" },
      h("div", { className: "openamer-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "openamer-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "openamer-kanban-drawer-body" },
      h("div", { className: "openamer-kanban-drawer-title" },
        h("span", { className: cn("openamer-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "openamer-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "openamer-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "openamer-kanban-section" },
            h("div", { className: "openamer-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "openamer-kanban-section" },
            h("div", { className: "openamer-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "openamer-kanban-done-no-result openamer-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "openamer-kanban-section" },
            h("div", { className: "openamer-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "openamer-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "openamer-kanban-section" },
        h("div", { className: "openamer-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "openamer-kanban-comment" },
            h("div", { className: "openamer-kanban-comment-head" },
              h("span", { className: "openamer-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "openamer-kanban-diag-action-btn",
      ...
      h("div", { className: "openamer-kanban-section" },
        h("div", { className: "openamer-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "openamer-kanban-comment" },
                h("div", { className: "openamer-kanban-comment-head" },
                  h("span", { className: "openamer-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "openamer-kanban-comment-ago" },
      ...
      h("div", { className: "openamer-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "openamer-kanban-event",
              isDiag ? "openamer-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "openamer-kanban-event-header" },
                  h("span", { className: "openamer-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "openamer-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "openamer-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "openamer-kanban-event-header-plain" },
                  h("span", { className: "openamer-kanban-event-kind" }, e.kind),
                  h("span", { className: "openamer-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "openamer-kanban-event-phantom-row" },
                  h("span", { className: "openamer-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "openamer-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "openamer-kanban-event-payload" },
    ...
    return h("div", { className: "openamer-kanban-section" },
      h("div", { className: "openamer-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "openamer-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `openamer-kanban-run--${r.outcome || r.status || "ended"}`
          : "openamer-kanban-run--active";
        return h("div", { key: r.id, className: cn("openamer-kanban-run", outcomeClass) },
          h("div", { className: "openamer-kanban-run-head" },
            h("span", { className: "openamer-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "openamer-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "openamer-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "openamer-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "openamer-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "openamer-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "openamer-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "openamer-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "openamer-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "openamer-kanban-pre openamer-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "openamer-kanban-meta-row" },
      h("span", { className: "openamer-kanban-meta-label" }, props.label),
      h("span", { className: "openamer-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "openamer-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "openamer-kanban-meta-row" },
        h("span", { className: "openamer-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "openamer-kanban-meta-value openamer-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "openamer-kanban-meta-row" },
        h("span", { className: "openamer-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "openamer-kanban-meta-value openamer-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "openamer-kanban-meta-row" },
        h("span", { className: "openamer-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "openamer-kanban-meta-value openamer-kanban-editable",
    ...
    return h("div", { className: "openamer-kanban-meta-row" },
      h("span", { className: "openamer-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "openamer-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "openamer-kanban-section" },
      h("div", { className: "openamer-kanban-section-head-row" },
        h("span", { className: "openamer-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "openamer-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "openamer-kanban-section" },
      h("div", { className: "openamer-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "openamer-kanban-deps-row" },
        h("span", { className: "openamer-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "openamer-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "openamer-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "openamer-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "openamer-kanban-dep-chip-x",
      ...
      h("div", { className: "openamer-kanban-deps-row" },
        h("span", { className: "openamer-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "openamer-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "openamer-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "openamer-kanban-msg-ok"
          : "openamer-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "openamer-kanban-section" },
      ...
      h("div", { className: "openamer-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "openamer-kanban-home-sub openamer-kanban-home-sub--on"
              : "openamer-kanban-home-sub",
          }, label);
        })
      )
    );
