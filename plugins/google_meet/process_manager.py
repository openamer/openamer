...
session-scoped state file under ``$OPENAMER_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $OPENAMER_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_openamer_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this openamer install, leave it first —
