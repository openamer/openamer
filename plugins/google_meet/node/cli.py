"""`openamer meet node ...` subcommand tree.

Wired into the existing ``openamer meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``openamer meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="openamer-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``openamer meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             openamer meet node approve <name> ws://<host>:{args.port} {token}")
        try:
