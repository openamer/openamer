...
``{title, url, description, position}`` rows every other OpenAmer web
provider produces.
...
prefers OpenAmer-managed xAI Grok OAuth (via ``openamer auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.openamer/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``openamer tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `openamer auth` to sign in with "
