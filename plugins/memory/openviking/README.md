...
- OpenViking server running and reachable from OpenAmer
...
connection values into OpenAmer, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.openamer/.env`; for a named profile use
...
OpenViking's server config is separate from OpenAmer:
...
OpenAmer-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `openamer` | OpenAmer peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, OpenAmer lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
OpenAmer sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
OpenAmer built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| OpenAmer action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because OpenAmer
...
file URIs, such as
`viking://user/peers/openamer/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/openamer/memories/preferences/mem_abc123.md`. Files
