...
- Hosted: API key from [app.supermemory.ai/integrations?connect=openamer](http://app.supermemory.ai/integrations?connect=openamer)
...
Before running `openamer memory setup`, add the local endpoint to
...
Then run `openamer memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$OPENAMER_HOME/supermemory.json`
...
| `container_tag` | `openamer` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `openamer-{identity}` → `openamer-coder`). |
...
`https://api.supermemory.ai`. OpenAmer resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: openamer`, and document writes stamp
`metadata.sm_source: openamer`. This is a **functional routing key, not telemetry**:
it groups OpenAmer-written memories into a dedicated "OpenAmer" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per OpenAmer profile:

```json
{
  "container_tag": "openamer-{identity}"
}
```

For a profile named `coder`, this resolves to `openamer-coder`. The default profile resolves to `openamer-default`. Without `{identity}`, all profiles share the same container.
