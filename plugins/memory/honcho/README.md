...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/openamer>
...
```bash
openamer memory setup honcho   # configure Honcho directly (works on a fresh install)
openamer memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
openamer config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.openamer/.env
```

> `openamer honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `openamer memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `openamer model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$OPENAMER_HOME/honcho.json` | Profile-local (isolated OpenAmer instances) |
| 2 | `~/.openamer/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active OpenAmer profile: `openamer` (default) or `openamer_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `openamer honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | OpenAmer session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`openamer-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`openamer`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-openamer-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening openamer in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — OpenAmer session ID (timestamp + hex). Every `openamer` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple OpenAmer profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`openamer`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active OpenAmer profile: `openamer` (default) or `openamer_<profile>` (e.g. `openamer -p coder` -> host key `openamer_coder`). Older `openamer.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `OPENAMER_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `openamer-agent`) |
...
|---------|-------------|
| `openamer memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `openamer honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `openamer memory setup`) |
| `openamer honcho status` | Show resolved config for active profile |
| `openamer honcho enable` / `disable` | Toggle Honcho for active profile |
| `openamer honcho mode <mode>` | Change recall or observation mode |
| `openamer honcho peer --user <name>` | Update user peer name |
| `openamer honcho peer --ai <name>` | Update AI peer name |
| `openamer honcho tokens --context <N>` | Set context token budget |
| `openamer honcho tokens --dialectic <N>` | Set dialectic max chars |
| `openamer honcho map <name>` | Map current directory to a session name |
| `openamer honcho sync` | Create host blocks for all OpenAmer profiles |
