...
```bash
openamer memory setup    # select "mem0"
```
...
Behavioral settings live in `$OPENAMER_HOME/mem0.json` (set them via `openamer memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.openamer/.env`.
...
| `user_id` | `openamer-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh OpenAmer session and call `mem0_search` — it connects to your server.
...
```bash
openamer memory setup mem0 --mode oss \
...
```bash
openamer memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$OPENAMER_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.openamer/mem0_qdrant"}}
  }
}
```
...
```bash
openamer memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$OPENAMER_HOME/mem0.json`).
