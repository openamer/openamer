# Contributing to OpenAmer Agent

Thank you for contributing to OpenAmer Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want OpenAmer to work everywhere.
...
Related: #38284 covers the agent-side analog — OpenAmer itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every OpenAmer install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `openamer skills browse` (labeled "official") and install it with `openamer skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [OpenAmer Discord](https://discord.gg/openamer). Users can install it with `openamer skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.openamer/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(openamer_home, config)` for setup-wizard integration
...
- Integrate with `openamer memory setup` via `post_setup()` — no need to touch core code
...
- Promote it in the [OpenAmer Discord](https://discord.gg/openamer) `#plugins-skills-and-skins` channel so users can find and install it
...
Use this only if you intentionally do not want OpenAmer' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `openamer` entrypoint from this venv; running the
system `python3 -m openamer_cli.main` can pick up unrelated system Python
packages.
...
export VIRTUAL_ENV="$HOME/.openamer/venvs/openamer-dev"
...
```bash
mkdir -p ~/.openamer/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.openamer/config.yaml
touch ~/.openamer/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.openamer/.env
```

### Run

```bash
# The standard installer already put `openamer` on PATH.
openamer doctor
openamer chat -q "Hello"
```

If you used the manual clone fallback, run `./openamer` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/openamer" ~/.local/bin/openamer
```
...
├── cli.py                    # OpenAmerCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (openamer-cli, openamer-telegram, etc.)
├── openamer_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── openamer_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, OpenAmer Portal
...
### User configuration (stored in `~/.openamer/`)

| Path | Purpose |
|------|---------|
| `~/.openamer/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.openamer/.env` | API keys and secrets |
| `~/.openamer/auth.json` | OAuth credentials (OpenAmer Portal) |
| `~/.openamer/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.openamer/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.openamer/state.db` | SQLite session database |
| `~/.openamer/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.openamer/cron/` | Scheduled job data |
| `~/.openamer/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`openamer_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.openamer/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (OpenAmer Portal OAuth, OpenRouter API key, or custom endpoint).
...
(for example `_OPENAMER_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.openamer`:
...
The user may skip setup and keep loading the skill. OpenAmer only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `openamer setup` or update `~/.openamer/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native OpenAmer tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "OpenAmer Agent" is the secondary collaborator. If the contributor's commit shows "OpenAmer Agent" as author because they used OpenAmer to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing OpenAmer tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `openamer --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

OpenAmer uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.openamer/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `openamer_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `openamer_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

OpenAmer runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the openamer wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `openamer_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `openamer_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `openamer_cli/gateway_windows.py::
...
OpenAmer has terminal access. Security matters.
...
2. **Test manually**: Run `openamer` and exercise the code path you changed
...
- **Discord**: [discord.gg/openamer](https://discord.gg/openamer) — for questions, showcasing projects, and sharing skills
