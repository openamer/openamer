...
// Mirrors @openamer/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @openamer/ink is bundled separately from packages/.
 ...
 *  answer when known (cached in OPENAMER_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.OPENAMER_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit OPENAMER_TUI_THEME / COLORFGBG signals above still win,
...
// `OPENAMER_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `OPENAMER_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `OPENAMER_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `OPENAMER_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default OpenAmer palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.OPENAMER_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.OPENAMER_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.OPENAMER_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into OPENAMER_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into OPENAMER_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (OpenAmer navy
  ...
  // the derived value already carries the identity seeds (e.g. OpenAmer navy).
  const activeRow =
