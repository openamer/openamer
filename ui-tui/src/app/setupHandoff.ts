import type { RunExternalProcess } from '@openamer/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`openamer ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching openamer: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`openamer ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
