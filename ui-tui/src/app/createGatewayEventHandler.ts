...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@openamer/ink'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.OPENAMER_TUI_THEME : undefined

  writeBootTheme(theme, process.env.OPENAMER_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to OPENAMER_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit OPENAMER_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the OPENAMER_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.OPENAMER_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.OPENAMER_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a OPENAMER_TUI_THEME the user
    ...
    delete process.env.OPENAMER_TUI_THEME
  }
 ...
 * is cached into OPENAMER_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * OPENAMER_TUI_LIGHT / OPENAMER_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.OPENAMER_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.OPENAMER_TUI_THEME || process.env.OPENAMER_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.OPENAMER_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.OPENAMER_TUI_BACKGROUND ||
      process.env.OPENAMER_TUI_THEME ||
      process.env.OPENAMER_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.OPENAMER_TUI_BACKGROUND || process.env.OPENAMER_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.OPENAMER_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // forging a brand-new one.  Mirrors classic CLI's `openamer -c` /
    // `openamer --tui` muscle memory and addresses the audit's "session
