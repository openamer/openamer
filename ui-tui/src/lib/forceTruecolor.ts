...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.OPENAMER_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep OpenAmer on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via OPENAMER_TUI_TRUECOLOR=1.
