...
const logDir = join(process.env.OPENAMER_HOME?.trim() || join(homedir(), '.openamer'), 'logs')
...
// a real ~/.openamer (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('openamer-tui: parent lifecycle log unavailable\n')
    }
  }
}
