...
import { Box, Text } from '@openamer/ink'
...
/**
 * User widget apps — OpenAmer authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$OPENAMER_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.openamer/plugins/`: files under OPENAMER_HOME execute
...
const widgetsDir = () => join(process.env.OPENAMER_HOME?.trim() || join(homedir(), '.openamer'), 'tui-widgets')
 ...
 *  every change, so a widget OpenAmer writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or OpenAmer) ever writes must hot-load too — a 10s poll here read
