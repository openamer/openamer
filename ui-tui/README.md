...
React + Ink terminal UI for OpenAmer. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `OPENAMER_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `openamer setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.openamer/.openamer_history` or under `OPENAMER_HOME`.
...
```text
ui-tui/
  packages/openamer-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external openamer setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.openamer/tui-parent.log
    ...
    types/
      openamer-ink.d.ts            type declarations for @openamer/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent OpenAmerCLI subprocess for slash commands
```
