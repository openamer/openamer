...
_OPENAMER_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (OPENAMER_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `openamer tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``openamer tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (OPENAMER_KANBAN_TASK env "
    ...
    # Coding posture (base OpenAmer — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in openamer_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full OpenAmer toolsets (CLI + messaging platforms)
    #
    ...
    # the `openamer send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "openamer-cron": {
        # Mirrors openamer-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `openamer tools` filters
    ...
    "openamer-gateway": {
        ...
        "tools": [],
        "includes": ["openamer-telegram", "openamer-discord", "openamer-whatsapp", "openamer-slack", "openamer-signal", "openamer-bluebubbles", "openamer-homeassistant", "openamer-email", "openamer-sms", "openamer-mattermost", "openamer-matrix", "openamer-dingtalk", "openamer-feishu", "openamer-wecom", "openamer-wecom-callback", "openamer-weixin", "openamer-qqbot", "openamer-webhook", "openamer-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    ...
    Bundle nesting is one level deep in practice (only ``openamer-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        ...
        if include_registry and name.startswith("openamer-"):
            platform_name = name[len("openamer-"):]
            try:
