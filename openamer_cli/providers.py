"""
Single source of truth for provider identity in OpenAmer Agent.
...
2. **OpenAmer overlays** — transport type, auth patterns, aggregator flags,
...
# -- OpenAmer overlay ----------------------------------------------------------
# OpenAmer-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class OpenAmerOverlay:
    """OpenAmer-specific provider metadata layered on top of models.dev."""
...
OPENAMER_OVERLAYS: Dict[str, OpenAmerOverlay] = {
    ...
    "openai-api": OpenAmerOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": OpenAmerOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": OpenAmerOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
    ...
    "copilot-acp": OpenAmerOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": OpenAmerOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": OpenAmerOverlay(
    ...
    "kimi-for-coding": OpenAmerOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": OpenAmerOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": OpenAmerOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": OpenAmerOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": OpenAmerOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": OpenAmerOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": OpenAmerOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": OpenAmerOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": OpenAmerOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": OpenAmerOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "kilo": OpenAmerOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="KILOCODE_BASE_URL",
    ),
    "huggingface": OpenAmerOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": OpenAmerOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": OpenAmerOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": OpenAmerOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": OpenAmerOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": OpenAmerOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": OpenAmerOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": OpenAmerOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "openamer", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + openamer extra
    ...
    if overlay is not None:
        # OpenAmer-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def resolve_provider_full(
    ...
    # 0.5 Exact OpenAmer provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from openamer_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
