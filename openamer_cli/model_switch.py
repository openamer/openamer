...
- ``openamer_cli.providers``        -- canonical provider identity + overlays
- ``openamer_cli.model_normalize``  -- per-provider name formatting
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``openamer config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from openamer_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'openamer model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'openamer doctor' — config issues detected:"
    ...
    # =================================================================
        ...
        try:
            from openamer_cli.config import load_config
            from openamer_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from openamer_cli.providers import resolve_user_provider as _ruser
    ...
    # openamer_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from openamer_cli.models import normalize_opencode_base_url
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from openamer_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from openamer_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # https://openamer-agent.nousresearch.com/docs/api/model-catalog.json so
    ...
    # --- 1. Check OpenAmer-mapped providers ---
    from openamer_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    ...
    for openamer_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (OPENAMER_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers OpenAmer may not route yet.
    ...
    # Build reverse mapping: models.dev ID → OpenAmer provider ID.
    # OPENAMER_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use OpenAmer IDs ("copilot").
    _mdev_to_openamer = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    # in PROVIDER_TO_MODELS_DEV or OPENAMER_OVERLAYS (keeps /model in sync
    # with `openamer model`).
    try:
        from openamer_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # OpenAmer writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # OpenAmer's own writer uses (mirrors section-4 grouping).
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. OpenAmer's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, openamer_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`openamer_cli.models.fetch_openrouter_models`, which filters the
