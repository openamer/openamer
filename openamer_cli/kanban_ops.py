"""Dispatcher and maintenance verbs for ``openamer kanban``: ``dispatch``,
...
from openamer_cli import kanban_db_workspace as kbw
from openamer_cli.kanban_output import _err, _fmt_ts, _print_json
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"successfully. Check profile health (venv, PATH, credentials) and `openamer "
                    f"kanban list --status ready` / `openamer kanban list --status blocked` for "
