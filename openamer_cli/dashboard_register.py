"""``openamer dashboard register`` — register a self-hosted dashboard OAuth client.

Automates what a user otherwise does by hand: open the OpenAmer Portal
...
as ``OPENAMER_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh OpenAmer Portal access token from the existing login
  ...
  3. Writes ``OPENAMER_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``OPENAMER_DASHBOARD_PORTAL_URL`` into ``~/.openamer/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``OPENAMER_DASHBOARD_PORTAL_URL`` env (used for testing against a
      ...
      2. The ``portal_base_url`` stored on the OpenAmer login — this is the
    ...
    try:
        from openamer_cli.auth import DEFAULT_openamer_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``openamer dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "OpenAmer Portal rejected the access token (401). "
                "Try `openamer auth add openamer` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
        ...
        raise RuntimeError(
            f"Could not reach OpenAmer Portal at {portal_base_url}: {exc.reason}"
        ) from exc
...
def _print_post_register_hint(
    *,
    ...
    from openamer_cli.config import get_env_path

    env_path = get_env_path()
    _cid = client_id
    print()
    print(f"  Wrote to {env_path}:")
    print("    OPENAMER_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    OPENAMER_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    OPENAMER_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
        "  Heads up — OpenAmer login only *engages* on a non-loopback bind. A plain\n"
        "  `openamer dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        try:
            ...
            host = "your-host"
        print("  To require OpenAmer login on your registered host, run the dashboard")
        print(f"  bound publicly (it must be reachable at https://{host}) and log in")
        print("  at its /login page.")
    else:
        print("  To require OpenAmer login (e.g. exposing on your LAN or a public host):")
        print("    openamer dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from openamer_cli.auth import AuthError, resolve_openamer_access_token
    from openamer_cli.config import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets OPENAMER_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        print(
            "✗ `openamer dashboard register` is not available in a managed/hosted "
            "install.\n"
            "  The dashboard OAuth client is provisioned by the hosting platform."
        )
        sys.exit(1)

    # 1. Resolve a fresh OpenAmer access token (refreshes if near expiry). Fail fast
    #    with a setup hint when the user isn't logged in.
    try:
        access_token = resolve_openamer_access_token()
    except AuthError as exc:
        if getattr(exc, "relogin_required", False):
            print("✗ You're not logged into OpenAmer Portal.")
            print("  Run `openamer setup` (or `openamer auth add openamer`) first, then retry.")
        else:
            print(f"✗ Could not resolve a OpenAmer Portal access token: {exc}")
        sys.exit(1)
    ...
    # OPENAMER_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (OPENAMER_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("OPENAMER_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    try:
        save_env_value("OPENAMER_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write OPENAMER_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  OPENAMER_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    # Persist the portal URL. Two cases:
    #   a) The user explicitly supplied a custom portal (--portal-url flag or
    #      OPENAMER_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    default_portal = "https://portal.openamer.com"
    existing_portal = None
    try:
        existing_portal = get_env_value("OPENAMER_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("OPENAMER_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # that same callback by taking OPENAMER_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as OPENAMER_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("OPENAMER_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("OPENAMER_DASHBOARD_PUBLIC_URL", public_url)
