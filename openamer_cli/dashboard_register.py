...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    try:
        from openamer_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `openamer auth add nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_dashboard_register(args) -> None:
    ...
    from openamer_cli.auth import AuthError, resolve_nous_access_token
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `openamer setup` (or `openamer auth add nous`) first, then retry.")
        else:
