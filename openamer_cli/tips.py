...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or openamer -c.",
    ...
    "Ctrl+Z suspends OpenAmer to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "openamer -c resumes your most recent CLI session. openamer -c \"project name\" resumes by title.",
    "openamer -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "openamer -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "openamer chat -t web,terminal enables only specific toolsets for a focused session.",
    "openamer chat -s github-pr-workflow preloads a skill at launch.",
    "openamer chat -q \"query\" runs a single non-interactive query and exits.",
    "openamer chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "openamer chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "openamer --yolo bypasses all dangerous command approval prompts for the entire session.",
    "openamer chat --source telegram tags the session for filtering in openamer sessions list.",
    "openamer -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "openamer doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "openamer dump outputs a compact setup summary — great for bug reports.",
    "openamer config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "openamer config edit opens config.yaml in your default editor.",
    "openamer config check scans for missing or stale configuration options.",
    "openamer sessions browse opens an interactive session picker with search.",
    "openamer sessions stats shows session counts by platform and database size.",
    "openamer sessions prune --older-than 30 cleans up old sessions.",
    "openamer skills search react --source skills-sh searches the skills.sh public directory.",
    "openamer skills check scans installed hub skills for upstream updates.",
    "openamer skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "openamer skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "openamer mcp add github --command npx adds MCP servers from the command line.",
    "openamer mcp serve runs OpenAmer itself as an MCP server for other agents.",
    "openamer auth add lets you add multiple API keys for credential pool rotation.",
    "openamer completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "openamer logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "openamer backup creates a zip backup of your entire OpenAmer home directory.",
    "openamer profile create coder creates an isolated profile that becomes its own command.",
    "openamer profile create work --clone copies your current config and keys to a new profile.",
    "openamer update syncs new bundled skills to ALL profiles automatically.",
    "openamer gateway install sets up OpenAmer as a system service (systemd/launchd).",
    "openamer memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as openamer -w).",
    ...
    "OPENAMER_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call OpenAmer tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'openamer profile create coder' creates the 'coder' command.",
    "openamer profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, OpenAmer prints a resume command with session ID and stats.",
    "openamer sessions export backup.jsonl exports all sessions for backup or analysis.",
    "openamer -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "openamer skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .openamer/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: openamer cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "OpenAmer runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "openamer gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to OpenAmer via a local macOS server.",
    ...
    "Context files (.openamer.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "OpenAmer loads project context from .openamer.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "openamer mcp opens an interactive picker of OpenAmer-approved MCPs you can install in one keystroke.",
    "openamer mcp catalog lists OpenAmer-approved MCP servers shipped with the repo.",
    "openamer mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — openamer tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.openamer/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "openamer chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "openamer plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "openamer acp runs OpenAmer as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "OPENAMER_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "openamer auth add openamer or openamer auth add openai-codex sets up OAuth-based providers.",
    ...
    "openamer status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.openamer/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make OpenAmer your own.",
    ...
    "Before a gateway session reset, OpenAmer auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.openamer/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope OpenAmer state via OPENAMER_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "OPENAMER_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in OPENAMER_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.openamer/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'openamer profile create ls' would be rejected.",
    "openamer profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "openamer claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "OPENAMER_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via openamer tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.openamer/.env.",
    "Skills from trusted repos (openamer) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.openamer/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — OpenAmer auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of OpenAmer config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while OpenAmer is working.',
    ...
    '/reload reloads ~/.openamer/.env into the running session — pick up new API keys without restarting.',
    ...
    "OPENAMER_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.openamer/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.openamer/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'openamer curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "openamer curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'openamer curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'openamer auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your OpenAmer subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'OPENAMER_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "OPENAMER_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'openamer dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'openamer dashboard embeds the full OpenAmer TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.openamer/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.openamer/dashboard-plugins/ — no npm build required.',
    ...
    'OPENAMER_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'OPENAMER_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'OPENAMER_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'OPENAMER_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.openamer/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.openamer/skills/ so drop-in skills appear without restarting the session.',
    ...
    'openamer -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'openamer chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'openamer chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'openamer chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "openamer chat --source tool tags programmatic chats so they don't clutter openamer sessions list.",
    'openamer dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'openamer sessions rename <ID> "new title" renames any past session; openamer sessions delete <ID> removes one.',
    'openamer import restores a full OpenAmer backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'openamer fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'openamer pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'openamer setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'openamer status --deep runs the full health sweep across every component; plain openamer status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'OPENAMER_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'OPENAMER_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.openamer/plugins/ — trust-gated by design.',
    "OPENAMER_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'OPENAMER_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'OPENAMER_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'OPENAMER_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'OPENAMER_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'OPENAMER_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'OPENAMER_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'OPENAMER_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'OPENAMER_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'OPENAMER_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'OPENAMER_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes OpenAmer block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from openamer sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.openamer/dashboard-plugins/.',
]
