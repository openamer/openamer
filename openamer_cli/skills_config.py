"""
Skills configuration for OpenAmer Agent.
`openamer skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.openamer/config.yaml under:
...
from openamer_cli.config import cfg_get, load_config, save_config
from openamer_cli.colors import Colors, color
from openamer_cli.platforms import PLATFORMS as _PLATFORMS
...
def skills_command(args=None):
    """Entry point for `openamer skills`."""
    from openamer_cli.curses_ui import curses_checklist
