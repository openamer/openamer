...
Pure leaf: imports nothing from ``openamer_cli.auth`` so the per-provider modules
...
# httpx is imported lazily (~30ms) because openamer_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("openamer_cli.auth.httpx.Client")
...
_nous_err = _provider_error_factory("openamer")
