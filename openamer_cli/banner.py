...
Pure display functions with no OpenAmerCLI state dependency.
"""
import json
import logging
import os
...
# (e.g. nix-built openamer — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/openamer/openamer.git"
_OFFICIAL_REPO_CANONICAL = "github.com/openamer/openamer"
...
def check_for_updates() -> Optional[int]:
    """Check whether a OpenAmer update is available.

    Two paths: if ``OPENAMER_REVISION`` is set (nix builds embed it), compare
    ...
    embedded_rev = os.environ.get("OPENAMER_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # OPENAMER_REVISION (that's nix-only). Returning None makes both the Rich
    ...
    if embedded_rev:
        ...
        # $OPENAMER_HOME/openamer-agent/ may be a stale copy from --clone-all;
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active OpenAmer git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$OPENAMER_HOME/openamer-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``openamer_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from openamer_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/openamer/openamer/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    OpenAmer checkout. Cached per-process. Release URL always points at the
    canonical openamer/openamer repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"OpenAmer Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else OPENAMER_CADUCEUS
    except Exception:
        _bskin = None
        _hero = OPENAMER_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]OpenAmer[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]OpenAmer[/]")

    if os.getenv("OPENAMER_YOLO_MODE"):
    ...
    try:
        from openamer_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from openamer_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else OPENAMER_AGENT_LOGO
        console.print(_logo)
        console.print()
