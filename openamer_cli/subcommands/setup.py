"""``openamer setup`` subcommand parser.
...
def build_setup_parser(subparsers, *, cmd_setup: Callable) -> None:
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure OpenAmer Agent with an interactive wizard. "
        "Run a specific section: openamer setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'openamer setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    setup_parser.add_argument(
        "--portal",
        action="store_true",
        help="One-shot OpenAmer Portal setup: log in via OAuth, pick a OpenAmer "
        "model, set OpenAmer as the inference provider, and opt into the Tool "
