"""``openamer gateway`` and ``openamer proxy`` subcommand parsers.
...
from openamer_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if OPENAMER_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy openamer.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy OpenAmer gateway unit files "
            "(e.g. openamer.service) left over from older installs. Profile "
            "units (openamer-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Authenticates as your OpenAmer Portal account (the connector derives the "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.openamer/.env. "
            "Requires being logged in (openamer setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.openamer/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. OpenAmer Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="openamer",
        help="Upstream provider: openamer or xai (default: openamer). See `openamer proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
