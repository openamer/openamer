"""``openamer dashboard`` / ``openamer serve`` subcommand parsers.
...
``openamer_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `openamer dashboard` / `openamer serve` cmdlines and SIGTERM them directly —
    # the same path `openamer update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running OpenAmer web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running OpenAmer web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the OpenAmer Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older OpenAmer desktop app shells (<= 0.15.x) spawn the
    # backend as `openamer dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "OpenAmer couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the OpenAmer Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the OpenAmer backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the OpenAmer backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # `openamer dashboard register` — register a self-hosted dashboard OAuth
    # client with OpenAmer Portal and write the client_id into ~/.openamer/.env.
    # Nested subparser so bare `openamer dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        "register",
        help="Register a self-hosted dashboard with OpenAmer Portal (writes the OAuth client ID to .env)",
        description=(
            "Register this install as a self-hosted dashboard with your OpenAmer "
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://openamer.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        default=None,
        help=(
            "Override the OpenAmer Portal base URL for registration (default: the "
