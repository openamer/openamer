"""
Interactive setup wizard for OpenAmer Agent.
...
_DOCS_BASE = "https://github.com/openamer/openamer/docs"
...
# display_openamer_home imported lazily at call sites (stale-module safety during openamer update)
...
from openamer_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from openamer_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure OpenAmer using environment variables or config commands:")
    print_info("  openamer config set model.provider custom")
    print_info("  openamer config set model.base_url http://localhost:8080/v1")
    print_info("  openamer config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'openamer setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from openamer_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'openamer setup')")


def _print_setup_summary(config: dict, openamer_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'openamer setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from openamer_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `openamer tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from openamer_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'openamer setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'openamer setup tts'"))
    else:
    ...
    # Spotify (OAuth via openamer auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'openamer setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {openamer_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('openamer setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('openamer setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('openamer setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('openamer setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('openamer setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('openamer config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('openamer config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('openamer config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('openamer', Colors.GREEN)}              Start chatting")
    print(f"   {color('openamer gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('openamer doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `openamer tools` and `openamer setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``openamer model``)
    ...
    provider added to ``openamer model`` is automatically available here.
    ...
    # Delegate to the shared openamer model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from openamer_cli.main import select_provider_and_model
    try:
    ...
    # on demand via `openamer auth add`, `openamer setup` vision, and
    # `openamer setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from openamer_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``openamer model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the OpenAmer auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via openamer setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'openamer setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where OpenAmer runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `openamer setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `openamer setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `openamer egress setup` then `openamer egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `openamer egress setup`."
            )
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `openamer setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.openamer/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from openamer_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_openamer_home(Path(get_openamer_home()))
    except Exception:
        pass
...
def _profile_name_from_openamer_home(openamer_home) -> str | None:
    """Return the active profile name when OPENAMER_HOME is a profile dir."""
    if openamer_home.parent.name == "profiles":
        return openamer_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where OpenAmer delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects OpenAmer to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from openamer_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://github.com/openamer/openamer/blob/main/website/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://github.com/openamer/openamer/blob/main/website/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  openamer config edit")
    print_info("   Open config in your editor:  openamer config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from openamer_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with OpenAmer from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'openamer setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     openamer config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_openamer_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: openamer gateway install --system")
                print_info("  Or run in foreground:  openamer gateway")
        else:
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   openamer gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `openamer setup tools` and `openamer tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from openamer_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from openamer_cli.config
    ...
    if section_key == "model":
        ...
        from openamer_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_openamer.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_openamer migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_openamer", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and OpenAmer.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure OpenAmer to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point OpenAmer at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point OpenAmer at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point OpenAmer at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point OpenAmer at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to OpenAmer equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing OpenAmer config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in OpenAmer.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ OpenAmer's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(openamer_home: Path) -> bool:
    ...
    print_info("OpenAmer can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: openamer claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=openamer_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: openamer claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing OpenAmer configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing OpenAmer config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in OpenAmer (use openamer claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    ``openamer setup`` and hunt for the quick-setup option.
    ...
    print_info("  Run `openamer portal info` to inspect routing.")
    print_info("  Run `openamer` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      openamer setup           — full or quick (auto-detected)
      openamer setup model     — just model/provider
      openamer setup tts       — just text-to-speech
      openamer setup terminal  — just terminal backend
      openamer setup gateway   — just messaging platforms
      openamer setup tools     — just tool configuration
      openamer setup agent     — just agent settings
    """
    from openamer_cli.config import is_managed, managed_error
    if is_managed():
    ...
    print(
        color(
            "│  Let's configure your OpenAmer Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, openamer_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have OpenAmer configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'openamer setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(openamer_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, openamer_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, openamer_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {openamer_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'openamer config edit'")
    ...
    # Tune later with `openamer setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, openamer_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'openamer setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    openamer setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  openamer setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``openamer tools`` (which
    ...
    try:
        ...
        from openamer_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``openamer setup agent`` /
    ``openamer config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, openamer_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `openamer update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        openamer tools")
        print_info("  Seed skills:         openamer skills opt-in --sync")
        print_info("  Add MCP servers:     openamer mcp add")
        print_info("  Enable plugins:      openamer plugins")
        print_info("  Tune agent settings: openamer setup agent")
        print()
        _print_setup_summary(config, openamer_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, openamer_home)


def _blank_slate_walkthrough(config: dict, openamer_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`openamer update` runs from re-injecting them. Opt back in any")
            print_info("time with `openamer skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `openamer tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `openamer plugins list` / `openamer plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `openamer plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `openamer mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `openamer mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   openamer tools")
    print_info("  Seed skills:         openamer skills opt-in --sync")
    print_info("  Add MCP servers:     openamer mcp add")
    print_info("  Tune agent settings: openamer setup agent")
    print()
...
def _run_quick_setup(config: dict, openamer_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'openamer setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect OpenAmer to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'openamer setup gateway'.")
