...
``<root>`` is the **shared OpenAmer root** (the parent of any active
...
``openamer -p <profile>`` joins the same board as the dispatcher that
...
the profile's OpenAmer home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``OPENAMER_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``OPENAMER_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``OPENAMER_KANBAN_DB`` — pin the database file path directly.
* ``OPENAMER_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``OPENAMER_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``OPENAMER_KANBAN_DB``,
``OPENAMER_KANBAN_WORKSPACES_ROOT``, and ``OPENAMER_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/openamer-kanban-v1-spec.pdf`` for the full
...
def _assert_not_delegated_child_mutation(path: "str | Path | None" = None) -> None:
    ...
    *path* is the board DB / metadata root being mutated; ``None`` keeps the
    ...
    try:
        ...
        delegated = bool(os.environ.get("OPENAMER_DELEGATED_CHILD_CONTEXT"))
    if delegated:
        raise PermissionError(
            "delegate_task child contexts cannot mutate Kanban tasks or boards"
        )
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active OPENAMER_HOME so dispatcher- and
...
# ``OPENAMER_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``OPENAMER_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("OPENAMER_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``OPENAMER_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("OPENAMER_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``OPENAMER_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "OPENAMER_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
# enough that kebab-case names like ``atm10-server`` or ``openamer-agent``
...
def kanban_home() -> Path:
    """Return the shared OpenAmer root that anchors the kanban board.

    Resolution order:

    1. ``OPENAMER_KANBAN_HOME`` env var when set and non-empty (explicit
    ...
    profile's ``OPENAMER_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("OPENAMER_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``openamer kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``OPENAMER_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``openamer kanban boards
    ...
    env = os.environ.get("OPENAMER_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``openamer kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``OPENAMER_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("OPENAMER_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``OPENAMER_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("OPENAMER_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``OPENAMER_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("OPENAMER_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``openamer kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``OPENAMER_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (openamer_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``OPENAMER_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("OPENAMER_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``openamer_cli.sqlite_safe_read``).
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``openamer gateway run --replace`` /
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from openamer_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see openamer_cli.sqlite_safe_read), so it must only run once the board has
    ...
    # rescue.
    from openamer_cli.sqlite_safe_read import has_live_connection
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources OpenAmer treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``openamer kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``OPENAMER_KANBAN_DB`` env → ``OPENAMER_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `openamer kanban list` mid-integrity-probe) block
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See openamer_state._WAL_INCOMPAT_MARKERS for detection logic.
                from openamer_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``openamer kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``OPENAMER_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from openamer_cli.sqlite_safe_read import file_length_matches_header
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from openamer_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``openamer --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        from openamer_cli import projects_db as _pdb
    ...
    # `openamer --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``openamer kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `openamer kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``openamer kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``openamer kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``openamer kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``openamer kanban block <id>``).  This is a deliberate handoff that
...
def release_stale_claims(
    ...
    *extended* (with a ``claim_extended`` event) instead of being
    ...
    for row in stale:
        ...
        # its OWN transaction. ``write_txn`` is not reentrant (it issues
        # ``BEGIN IMMEDIATE`` unconditionally), so nesting the call inside the
        # block above raises "cannot start a transaction within a transaction"
        # and the reclaim stops working entirely.
...
def complete_task(
    ...
    *,
    ...
    completion (``openamer kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``OPENAMER_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold OpenAmer' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside OpenAmer-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``openamer kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the OpenAmer checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, OpenAmer treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, OpenAmer creates/reuses that linked worktree. With
      no ``workspace_path``, OpenAmer anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, OpenAmer uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``OPENAMER_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a OpenAmer
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``openamer kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``openamer kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real OpenAmer profile.
    ...
    if not rows:
        return False
    try:
        from openamer_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from openamer_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real OpenAmer profile.
        # `_default_spawn` invokes ``openamer -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a OpenAmer
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `openamer kanban tail` — without
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from openamer_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _resolve_openamer_argv() -> list[str]:
    ...
    1. ``$OPENAMER_BIN`` — explicit operator override. Path-like values are
    ...
    env_bin = os.environ.get("OPENAMER_BIN", "").strip()
    if env_bin:
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``openamer -p <profile> chat -q ...`` subprocess.
    ...
    ``OPENAMER_KANBAN_DB`` / ``OPENAMER_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject OPENAMER_HOME so the worker reads the profile-scoped config.yaml
    ...
    try:
        env["OPENAMER_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via OPENAMER_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # OPENAMER_HOME never had profiles created.
        pass
    if task.tenant:
        env["OPENAMER_TENANT"] = task.tenant
    env["OPENAMER_KANBAN_TASK"] = task.id
    env["OPENAMER_KANBAN_WORKSPACE"] = workspace
    ...
    if task.branch_name:
        env["OPENAMER_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["OPENAMER_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["OPENAMER_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["OPENAMER_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["OPENAMER_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    env["OPENAMER_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["OPENAMER_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["OPENAMER_KANBAN_BOARD"] = resolved_board
    # OPENAMER_PROFILE is the author the kanban_comment tool defaults to.
    # `openamer -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["OPENAMER_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited OPENAMER_TUI=1
    ...
    # older openamer builds on PATH that predate the flag's precedence.
    env.pop("OPENAMER_TUI", None)
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("OPENAMER_HOME"))
    if worker_toolsets:
    ...
    # `openamer kanban log` on a specific board reads its own file and
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``openamer kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled OPENAMER_PROFILE values like "openamer-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default OpenAmer root exists

    Reads profile paths directly so this module has no import dependency on
    ``openamer_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from openamer_constants import get_default_openamer_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``openamer kanban assignees`` for the terminal.
