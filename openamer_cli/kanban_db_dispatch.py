...
Split out of ``openamer_cli.kanban_db``; origin-resident helpers are reached
...
if TYPE_CHECKING:
    from openamer_cli.kanban_db import Task
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a OpenAmer profile. Expected steady-state
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``openamer kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``openamer_cli.profiles.profile_exists``, or ``None`` when it cannot be
    ...
    """
    try:
        from openamer_cli.profiles import normalize_profile_name, profile_exists
    except Exception:
        return None
...
def _dispatch_profile_allowlist(normalize_profile_name) -> Optional[frozenset]:
    ...
    On a shared board (one ``kanban.db`` mounted across several OpenAmer homes),
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real OpenAmer profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (OpenAmer ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``OPENAMER_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``openamer -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``openamer kanban tail`` shows why the task looks stuck.
...
def _module_openamer_argv() -> list[str]:
    """Interpreter-bound OpenAmer CLI invocation (``openamer_cli.main`` is the
    console-script target — there is no top-level ``openamer`` package)."""
...
def _openamer_path_argv(path: str) -> list[str]:
    """argv for a resolved OpenAmer executable path. Windows batch shims
...
def _resolve_openamer_argv() -> list[str]:
    """Resolve the ``openamer`` invocation as argv for ``Popen``: ``$OPENAMER_BIN``
    (path-like -> absolute; bare names keep PATH semantics, never a
    same-directory file), then ``which("openamer")`` (Windows: safe PATH search,
    batch shims fall back to the module form), then ``sys.executable -m
    openamer_cli.main`` for shim-less environments (cron, systemd ``User=``,
    launchd). Mirrors ``gateway.run._resolve_openamer_bin``; local because
    ``openamer_cli`` sits below ``gateway`` in the dependency order.
    """
    import shutil
    ...
    openamer_bin = _safe_which_no_cwd("openamer") if _kb._IS_WINDOWS else shutil.which("openamer")
    if openamer_bin:
...
def _resolve_worker_cli_toolsets(openamer_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``OPENAMER_KANBAN_TASK`` is set.
    """
    ...
    try:
        ...
        secret_token = (
            set_secret_scope(build_profile_secret_scope(Path(openamer_home)))
...
def _retag_legacy_worker_sessions(workspaces_root_path: str) -> None:
    ...
    if workspaces_root_path in _retagged_workspace_roots:
        return
    try:
        from openamer_state_registry import acquire, release_or_close
...
def _worker_argv(task: Task, profile_arg: str, openamer_home: Optional[str]) -> list[str]:
    """Build the ``openamer -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_openamer_argv(),
        ...
        "--cli",
        # Workers run under a profile-scoped OPENAMER_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(openamer_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `openamer kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    ...
    ``OPENAMER_KANBAN_DB`` / ``OPENAMER_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from openamer_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    try:
        ...
        # OPENAMER_PROFILE (set below) instead.
        profile_home = None
    ...
    # without it the child's get_openamer_home() falls back to the DEFAULT
    # profile root because `openamer -p` applies its override before
    # openamer_constants is imported.
    if profile_home:
        env["OPENAMER_HOME"] = profile_home
    ...
    env["OPENAMER_SESSION_SOURCE"] = "kanban"
    ...
    # match after `openamer -p` rewrites OPENAMER_HOME (symlink / Docker layouts).
    env["OPENAMER_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["OPENAMER_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["OPENAMER_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["OPENAMER_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads OPENAMER_PROFILE for its default author; `-p` alone
    ...
    # `--cli` is the highest-precedence TUI override; dropping OPENAMER_TUI covers
    # older openamer builds on PATH that predate the flag's precedence.
    env.pop("OPENAMER_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("OPENAMER_HOME"))
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``openamer kanban dispatch`` — the standalone
...
from openamer_cli import kanban_db as _kb  # noqa: E402
from openamer_cli import kanban_db_connect as _kbc  # noqa: E402
from openamer_cli import kanban_db_workspace as _kbw  # noqa: E402
