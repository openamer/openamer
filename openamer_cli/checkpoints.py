"""`openamer checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.openamer/checkpoints/``.  Actions:

    openamer checkpoints               # same as `status`
    openamer checkpoints status        # total size, project count, breakdown
    openamer checkpoints list          # per-project checkpoint counts + workdir
    openamer checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    openamer checkpoints clear [-f]    # nuke the entire base (asks first)
    openamer checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    openamer checkpoints
    openamer checkpoints prune --retention-days 3 --max-size-mb 200
    openamer checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: openamer checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``openamer checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `openamer checkpoints` → status
