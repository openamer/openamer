...
Split out of ``openamer_cli/auth.py`` and re-exported there; origin helpers are imported lazily
inside each function so ``openamer_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from openamer_cli.auth_constants import _decode_jwt_claims
from utils import file_signature

# Log-record parity with the origin module (caplog tests pin "openamer_cli.auth").
logger = logging.getLogger("openamer_cli.auth")
...
def _is_pkce_row(row: Dict[str, Any]) -> bool:
    return str(row.get("source") or "").endswith("openamer_pkce")
...
def strip_cloned_single_use_oauth_grants(profile_dir: Path) -> Dict[str, Any]:
    ...
    Called after any path that copies credential files between profiles (``openamer profile create
    ...
    """
    from openamer_cli.auth import _same_path, _save_auth_store
    ...
    try:
        ...
        root_auth_path = get_default_openamer_root() / "auth.json"
...
# the heal for the life of ONE process, so every fresh `openamer` invocation and every new worker
...
def _oauth_heal_clean_mark_path() -> Optional[Path]:
    ...
    try:
        from openamer_cli.auth import _auth_file_path
...
def consume_oauth_heal_notices() -> List[str]:
    ...
    ``openamer auth list`` / ``openamer auth status`` print them so the user sees the consolidation.
    """
    from openamer_cli.auth import _oauth_heal_notices
...
def _oauth_identity(entry: Dict[str, Any]) -> Optional[str]:
    ...
    """
    from openamer_cli.auth import _nonempty_str
    if not isinstance(entry, dict):
        return None
...
def _adopt_oauth_material(target: Dict[str, Any], winner: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from openamer_cli.auth import _POOL_STATUS_FIELDS
    merged = dict(target)
...
def heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    row with no root counterpart (an independent ``openamer -p <p> auth add`` grant, or the only
...
class _HealPass:
    ...
    def heal_pool_rows(self) -> None:
        ...
        for row in self.p_rows:
            ...
            # (the ``openamer auth`` PKCE shape); a profile openamer_pkce-family row is its copy.
    ...
    def heal_profile_singleton(self, profile_singleton: Optional[Path]) -> None:
        if profile_singleton is None or not profile_singleton.exists():
            return
        from openamer_cli.auth import _is_same_auth_store
    ...
    def sync_root_singleton_with_pkce_row(self) -> None:
        """Keep root's singleton and its ``openamer_pkce``-seeded pool row in step.
        ...
        pkce_idx = next(
            (i for i, r in enumerate(self.r_rows)
             if _is_oauth_pool_payload(r) and r.get("source") == "openamer_pkce"), None)
        if pkce_idx is None:
            return
...
def _heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        # ~/.openamer/auth.json from a test that forgot to isolate HOME.
        real_home_env = os.environ.get("HOME", "")
        if real_home_env and _same_path(root_path, Path(real_home_env) / ".openamer" / "auth.json"):
            return None
    ...
    with _auth_store_lock():
        ...
        with _auth_store_lock(target_path=root_path):
            ...
            if summary["adopted"] and root_singleton is not None and singleton_row is not None:
                from agent.anthropic_credentials import _write_openamer_oauth_credentials
                _write_openamer_oauth_credentials(
