"""
Backup and import commands for openamer CLI.

`openamer backup` creates a zip archive of the entire ~/.openamer/ directory
(excluding the openamer-agent repo and transient files).

`openamer import` restores from a backup zip, overlaying onto the current
OPENAMER_HOME root.
"""
...
from openamer_constants import get_default_openamer_root, get_openamer_home, display_openamer_home
...
# ``openamer-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/openamer-agent/``
...
# OPENAMER_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "openamer-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under OPENAMER_HOME) —
...
# File names that ``openamer import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting it from the OpenAmer portal
...
# Reserved archive subtree for provider state that lives OUTSIDE OPENAMER_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside OPENAMER_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to openamer root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``openamer-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/openamer-agent/`` — must be preserved.
        if part == "openamer-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside OPENAMER_HOME.
...
# to the user as a hung `openamer update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # openamer_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the OpenAmer home directory."""
    openamer_root = get_default_openamer_root()

    if not openamer_root.is_dir():
        print(f"Error: OpenAmer home directory not found at {openamer_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"openamer-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"openamer-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_openamer_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(openamer_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(openamer_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``openamer-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "openamer-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(openamer_root)
    ...
    # outside OPENAMER_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_openamer_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: openamer import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a OpenAmer backup.
    ...
    # Look for telltale files that a openamer home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a OpenAmer backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.openamer/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a openamer dir name
        if prefix in {".openamer", "openamer"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_openamer_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (openamer_root / "config.yaml").exists()
        has_env = (openamer_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has OpenAmer configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under OPENAMER_HOME.
            ...
            # OpenAmer portal. Matched by basename so both the root profile and
            # named profiles (profiles/<name>/gateway_state.json) are covered.
            if Path(rel).name in _IMPORT_SKIP_NAMES:
                skipped_runtime.append(rel)
                continue

            target = openamer_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(openamer_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_openamer_home()}."
            )
        ...
        profiles_dir = openamer_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # openamer_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: openamer profile list  (after installing openamer)")

        # Guidance
        print()
        if not (openamer_root / "openamer-agent").is_dir():
            print("Note: The openamer-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: openamer update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  openamer -p {pname} gateway install")

        print("Done. Your OpenAmer configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and openamer backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to OPENAMER_HOME).
...
# — `openamer update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $OPENAMER_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside OPENAMER_HOME and the entry is silently
...
def _quick_snapshot_root(openamer_home: Optional[Path] = None) -> Path:
...
def create_quick_snapshot(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: openamer snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(openamer_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside OPENAMER_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``openamer update``.
    ...
    Args:
        ...
        openamer_home: Override for the OpenAmer home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(openamer_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for openamer backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_openamer_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, openamer_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``openamer_root`` to ``out_path``.
...
def _pre_update_backup_dir(openamer_home: Optional[Path] = None) -> Path:
    home = openamer_home or get_openamer_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of OPENAMER_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<OPENAMER_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``openamer update``) should continue even if the backup fails.
    """
    openamer_root = openamer_home or get_default_openamer_root()
    if not openamer_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(openamer_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, openamer_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `openamer claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of OPENAMER_HOME under ``backups/`` before a
    ``openamer claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``openamer import <archive>``.  Writes to
    ``<OPENAMER_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `openamer import` and the
