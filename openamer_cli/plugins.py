"""
OpenAmer Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with openamer-agent;
...
from openamer_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. OpenAmer' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by openamer_cli.kanban_db when a task
    ...
    # separate `openamer -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `openamer kanban dispatch`),
...
ENTRY_POINTS_GROUP = "openamer_agent.plugins"

_NS_PARENT = "openamer_plugins"
...
class PluginManifest:
    ...
    #              in ~/.openamer/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``openamer plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active OpenAmer profile name (e.g. ``"default"``).

        Derived from ``OPENAMER_HOME`` via
        :func:`openamer_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.openamer/profiles/<name>``, or ``"custom"`` when
        ``OPENAMER_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with OpenAmer core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``openamer honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``openamer <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`openamer_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_openamer_dotenv()`` startup — after
        ``~/.openamer/.env`` loads, before OpenAmer reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        OpenAmer' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``openamer model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: openamer_cli.main imports plugins indirectly
        from openamer_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.openamer/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to openamer_cli/. Two layouts are
        ...
        # 2. User plugins (~/.openamer/plugins/)
        user_dir = get_openamer_home() / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with openamer and must
            ...
            # all ~20 of them added several seconds to every `openamer`
            # invocation — including plain `openamer chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform OpenAmer ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `openamer plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``openamer plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`openamer plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `openamer plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``openamer_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``openamer_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="openamer-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``openamer tools`` TUI so plugin-provided toolsets appear
