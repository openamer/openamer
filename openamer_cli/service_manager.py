...
the existing module-level functions in openamer_cli.gateway and
openamer_cli.gateway_windows directly. This protocol is a thin facade
...
``openamer gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``openamer gateway start/stop/restart``).
    """
    ...
    # `openamer gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    detection always fails. Since every OpenAmer runtime call inside the
    container drops to openamer via ``s6-setuidgid``, that silent failure
...
# in ``openamer_cli.gateway`` (systemd/launchd) and ``openamer_cli.gateway_windows``
...
# active (set via the ``openamer -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in openamer_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from openamer_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from openamer_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from openamer_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from openamer_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in openamer_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from openamer_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from openamer_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from openamer_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from openamer_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``openamer_cli.gateway_windows`` (Scheduled Task /
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's OPENAMER_HOME blindly;
    derive the shared profile root from the current OPENAMER_HOME and map the
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the openamer user.
    ...
    is dispatched as the openamer user.
    ...
    with openamer ownership before triggering ``s6-svscanctl -a``,
    ...
    slot's supervise/ was openamer-owned.
    ...
    # when the parent slot's supervise/ is openamer-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``openamer -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`openamer profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        root OPENAMER_HOME profile" (the implicit profile that exists at
        the top of $OPENAMER_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``openamer_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$OPENAMER_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``OPENAMER_HOME``,
        ...
        allocator in ``openamer_cli.profiles._allocate_gateway_port``).
        ...
        # the Open WebUI helper) grabs the per-OPENAMER_HOME PID lock first;
        ...
        if profile == "default":
            gateway_cmd = "openamer gateway run --replace"
        else:
            gateway_cmd = f"openamer -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid openamer {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``openamer logs`` reads and what
        ...
        return (
            ...
            f': "${{OPENAMER_HOME:=/opt/data}}"\n'
            f'log_dir="$OPENAMER_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as openamer and crash-loops on
            ...
            f'exec s6-setuidgid openamer s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``openamer gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``openamer -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with openamer ownership
            ...
            # the openamer user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is openamer-owned (see ``_seed_supervise_skeleton``), and on
