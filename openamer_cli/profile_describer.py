...
Used by ``openamer profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``openamer_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the OpenAmer Agent kanban board.
...
Rules:
  ...
  - Never write "OpenAmer Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_openamer_home())
        else:
