"""
Profile management for multiple isolated OpenAmer instances.
...
Usage::
    ...
    openamer profile create coder          # fresh profile + bundled skills
    openamer profile create coder --clone  # also copy config, .env, SOUL.md, skills
    openamer profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    openamer -p coder chat                 # or via flag
    openamer profile use coder             # set as sticky default
    openamer profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See openamer_constants.get_subprocess_home().
    "home",
]
...
#
# Rationale per item:
#   openamer-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `openamer backup` archives
...
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "openamer-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when OPENAMER_HOME equals the
...
# into the export. The set below identifies the *known OpenAmer profile
# artifacts* at the root of OPENAMER_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``openamer_constants``.
...
_OPENAMER_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the openamer root, NOT to the current OPENAMER_HOME
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the OpenAmer installation itself or a common system binary.  "
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, openamer subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _OPENAMER_SUBCOMMANDS:
        return f"'{canon}' conflicts with a openamer subcommand"
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running OpenAmer. If we
    ...
    try:
        ...
        # not be migrated. The next `openamer doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``openamer -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``openamer profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``openamer -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``openamer profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``OPENAMER_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing OpenAmer config
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own OPENAMER_HOME.
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``openamer update`` skips re-seeding this profile's
    ...
    # Keys pages, `openamer -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this OpenAmer profile.\n"
    ...
    if not soul_path.exists():
        try:
            from openamer_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `openamer update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`openamer profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running OpenAmer (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `openamer profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `openamer -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches OPENAMER_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``openamer profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "OPENAMER_HOME": str(profile_dir)},
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running OpenAmer *backends* bound to this profile.
    ...
    names it, so find it by inspection: a OpenAmer backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``OPENAMER_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `openamer -p <canon> profile
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # ...or by OPENAMER_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("OPENAMER_HOME", "")
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``OPENAMER_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from openamer_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set OPENAMER_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("OPENAMER_HOME")
    try:
        os.environ["OPENAMER_HOME"] = str(profile_dir)
        from openamer_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["OPENAMER_HOME"] = old_home
        elif "OPENAMER_HOME" in os.environ:
            del os.environ["OPENAMER_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: openamer profile create {canon}"
        )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      OPENAMER_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-OpenAmer file the
      user may have lying alongside OPENAMER_HOME (#58394).
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: openamer profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.openamer itself — disallow
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a OPENAMER_HOME path string.

    Called early in the CLI entry point, before any openamer modules
    are imported, to set the OPENAMER_HOME environment variable.
    """
