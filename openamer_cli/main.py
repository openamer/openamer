#!/usr/bin/env python3
"""
OpenAmer CLI - Main entry point.

Usage:
    openamer                     # Interactive chat (default)
    openamer chat                # Interactive chat
    openamer gateway             # Run gateway in foreground
    openamer gateway start       # Start gateway as service
    openamer gateway stop        # Stop gateway service
    openamer gateway status      # Show gateway status
    openamer gateway install     # Install gateway service
    openamer gateway uninstall   # Uninstall gateway service
    openamer setup               # Interactive setup wizard
    openamer logout              # Clear stored authentication
    openamer status              # Show status of all components
    openamer cron                # Manage cron jobs
    openamer cron list           # List cron jobs
    openamer cron status         # Check if cron scheduler is running
    openamer doctor              # Check configuration and dependencies
    openamer honcho setup                    # Configure Honcho AI memory integration
    openamer honcho status                   # Show Honcho config and connection status
    openamer honcho sessions                 # List directory → session name mappings
    openamer honcho map <name>               # Map current directory to a session name
    openamer honcho peer                     # Show peer names and dialectic settings
    openamer honcho peer --user NAME         # Set user peer name
    openamer honcho peer --ai NAME           # Set AI peer name
    openamer honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    openamer honcho mode                     # Show current memory mode
    openamer honcho mode [hybrid|honcho|local]  # Set memory mode
    openamer honcho tokens                   # Show token budget settings
    openamer honcho tokens --context N       # Set session.context() token cap
    openamer honcho tokens --dialectic N     # Set dialectic result char cap
    openamer honcho identity                 # Show AI peer identity representation
    openamer honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    openamer honcho migrate                  # Step-by-step migration guide: OpenClaw native → OpenAmer + Honcho
    openamer version             Show version
    openamer update              Update to latest version
    openamer uninstall           Uninstall OpenAmer Agent
    openamer acp                 Run as an ACP server for editor integration
    openamer sessions browse     Interactive session picker with search

    openamer claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: openamer_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``openamer_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``openamer update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``openamer_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, openamer crashes on import and the user can't run
# ``openamer update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import openamer_bootstrap  # noqa: F401
...
from openamer_cli._subprocess_compat import suppress_platform_ver_console
...
# a prior ``openamer update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from openamer_cli.env_loader import ...`` / ``from openamer_cli.config import
...
# package directory, so if IT can't import, nothing else in openamer_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from openamer_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from openamer_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    ...
    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``openamer tools`` or
...
# that run *before* openamer_cli.config is importable. Mirrors the explicit
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``OPENAMER_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``openamer … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("OPENAMER_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``OPENAMER_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("OPENAMER_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`openamer --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from openamer_cli import __release_date__, __version__

    print(f"OpenAmer Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``openamer --version`` before config/logging imports on Termux."""
    if os.environ.get("OPENAMER_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from openamer_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from openamer_cli.subcommands.cron import build_cron_parser
from openamer_cli.subcommands.gateway import build_gateway_parser
from openamer_cli.subcommands.profile import build_profile_parser
from openamer_cli.subcommands.model import build_model_parser
from openamer_cli.subcommands.setup import build_setup_parser

from openamer_cli.subcommands.whatsapp import build_whatsapp_parser
from openamer_cli.subcommands.slack import build_slack_parser
from openamer_cli.subcommands.login import build_login_parser
from openamer_cli.subcommands.logout import build_logout_parser
from openamer_cli.subcommands.auth import build_auth_parser
from openamer_cli.subcommands.status import build_status_parser
from openamer_cli.subcommands.webhook import build_webhook_parser
from openamer_cli.subcommands.hooks import build_hooks_parser
...
from openamer_cli.subcommands.config import build_config_parser
from openamer_cli.subcommands.skin import build_skin_parser
...
from openamer_cli.subcommands.logs import build_logs_parser
from openamer_cli.subcommands.prompt_size import build_prompt_size_parser
from openamer_cli.subcommands.memory import build_memory_parser
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (openamer tools, openamer setup, openamer model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'openamer {command_name}' requires an interactive terminal.\n"
...
def _project_venv_dir() -> Path:
    """Return the project virtualenv directory, tolerating both naming norms.

    Managed installs (the Windows installer / desktop bootstrap) and the classic
    layout stamp a plain ``venv`` dir, while dev checkouts (VS Code, uv, a
    ``project_root/.venv`` original-development clone) use the hidden ``.venv``
    name.  Update/recovery/install code that points at the project venv must find
    whichever exists, or ``openamer update`` fails with "Failed to inspect Python
    interpreter from active virtual environment at venv\\Scripts\\python.exe"
    whenever the env lives under ``.venv``.

    Prefer ``.venv`` when it exists and the plain ``venv`` does not (dev
    checkout).  If both exist, keep the plain ``venv`` (managed norm) — never
    second-guess an explicitly provisioned install.
    """
    dot = PROJECT_ROOT / ".venv"
    plain = PROJECT_ROOT / "venv"
    if dot.is_dir() and not plain.is_dir():
        return dot
    return plain


# ---------------------------------------------------------------------------
# Profile override — MUST happen before any openamer module import.
#
# Many modules cache OPENAMER_HOME at import time (module-level constants).
...
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `openamer mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to OpenAmer' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo openamer -p <name>` against the invoking user's home.
    ...
    # after the subcommand (`openamer chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors openamer_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # run-script as OPENAMER_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``openamer gateway run`` to mean
    # "the root OPENAMER_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("OPENAMER_S6_SUPERVISED_CHILD"):
        try:
    ...
    # 3. If we found a profile, resolve and set OPENAMER_HOME
    if profile_name is not None:
        try:
            ...
            # A bug in profiles.py must NEVER prevent openamer from starting
            print(
...
# Bridge security.redact_secrets from config.yaml → OPENAMER_REDACT_SECRETS env
...
try:
    ...
    _cfg_path = get_openamer_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "OPENAMER_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["OPENAMER_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from openamer_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from openamer_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if openamer_constants not importable yet
...
from openamer_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to openamer_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (openamer_cli.main._model_flow_*) keep resolving unchanged.
from openamer_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_openamer_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("OPENAMER_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("OPENAMER_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from openamer_cli.auth import PROVIDER_REGISTRY
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo openamer {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo openamer {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  openamer --tui --resume {target}")
    if title:
        print(f'  openamer --tui -c "{title}"')
    print()
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/openamer-ink/package.json",
    "packages/openamer-ink/index.js",
    "packages/openamer-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``OPENAMER_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("OPENAMER_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``OPENAMER_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("OPENAMER_SKIP_NODE_BOOTSTRAP"):
        return
...
def _find_bundled_tui(openamer_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if openamer_cli_dir is None:
        openamer_cli_dir = Path(__file__).parent
    bundled = openamer_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``openamer update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("OPENAMER_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this OpenAmer checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `openamer update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the OpenAmer checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `openamer --tui`\n"
        "If the checkout is still inconsistent, run `openamer update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (OPENAMER_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("OPENAMER_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from openamer_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("OPENAMER_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with OPENAMER_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset OPENAMER_TUI_DIR (e.g. `unset OPENAMER_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # openamer_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from openamer_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("OPENAMER_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["OPENAMER_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("OPENAMER_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["OPENAMER_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("OPENAMER_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["OPENAMER_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["OPENAMER_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from openamer_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="openamer-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["OPENAMER_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["OPENAMER_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["OPENAMER_MODEL"] = model
        env["OPENAMER_INFERENCE_MODEL"] = model
    if provider:
        env["OPENAMER_TUI_PROVIDER"] = provider
        env["OPENAMER_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["OPENAMER_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["OPENAMER_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["OPENAMER_TUI_SKILLS"] = value
    if query:
        env["OPENAMER_TUI_QUERY"] = query
    if image:
        env["OPENAMER_TUI_IMAGE"] = image
    if checkpoints:
        env["OPENAMER_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["OPENAMER_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["OPENAMER_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["OPENAMER_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["OPENAMER_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["OPENAMER_ACCEPT_HOOKS"] = "1"
    ...
    # OPENAMER_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `openamer --tui` try to
    ...
    # still set OPENAMER_TUI_RESUME themselves.
    env.pop("OPENAMER_TUI_RESUME", None)
    if resume_session_id:
        env["OPENAMER_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `openamer update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``OPENAMER_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``openamer kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``openamer kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("OPENAMER_KANBAN_BOARD"):
        return
    try:
        from openamer_cli.kanban_db import get_current_board

        os.environ["OPENAMER_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def _sync_bundled_skills_quietly() -> None:
    ...
    dependency. OpenAmer still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``openamer … chat -q`` with stdout
    ...
    try:
        ...
        return False
    if os.environ.get("OPENAMER_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'openamer sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from openamer_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from openamer_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'openamer doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like OpenAmer isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  openamer setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'openamer setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from openamer_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["OPENAMER_YOLO_MODE"] = "1"
    ...
    if getattr(args, "ignore_user_config", False):
        os.environ["OPENAMER_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["OPENAMER_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["OPENAMER_SESSION_SOURCE"] = args.source
...
def cmd_proxy(args):
    ...
    from openamer_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with OpenAmer?")
        print()
    ...
    # has no creds.json.  Every subsequent `openamer gateway` then paid a 30s
    ...
    session_dir = get_openamer_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: openamer gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `openamer gateway` skips it cleanly instead of paying a 30s
        ...
        print("⚠ Pairing may not have completed. Run 'openamer whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``openamer whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``openamer_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from openamer_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from openamer_cli.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from openamer_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``openamer model``) and the setup wizard
    ...
    from openamer_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("OPENAMER_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'openamer model' for "
                "available providers, or run 'openamer doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in openamer_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``openamer model``
...
def _clear_stale_openai_base_url():
    ...
    """
    from openamer_cli.config import get_env_value, save_env_value, load_config
...
#
# OpenAmer uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `openamer model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `openamer model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`openamer_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from openamer_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``openamer model`` flow, then route aux tasks
    to them here.
    """
    from openamer_cli.config import load_config
    from openamer_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from openamer_cli.auth import _prompt_model_selection
    from openamer_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from openamer_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from openamer_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use OpenAmer URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from openamer_cli.curses_ui import curses_radiolist
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``openamer setup`` / ``openamer model``.
    ...
    """
    from openamer_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from openamer_cli.config import save_env_value
    from openamer_cli.secret_prompt import masked_secret_prompt
    ...
    from openamer_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `openamer setup` to configure {pconfig.name} again."
        )
        return "", True
...
def cmd_login(args):
    """Authenticate OpenAmer CLI with a provider."""
    from openamer_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from openamer_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from openamer_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from openamer_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from openamer_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from openamer_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``openamer slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: openamer slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `openamer slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from openamer_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from openamer_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from openamer_cli.projects_cmd import projects_command
...
def cmd_security(args):
    """Dispatch `openamer security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from openamer_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from openamer_cli.dump import run_dump
...
def cmd_config(args):
    ...
    from openamer_cli.config import config_command
...
def cmd_skin(args):
    ...
    from openamer_cli.skin_cmd import skin_command

    skin_command(args)


def cmd_backup(args):
    """Back up OpenAmer home directory to a zip file."""
    if getattr(args, "quick", False):
        from openamer_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from openamer_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a OpenAmer backup from a zip file."""
    from openamer_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    from openamer_cli.banner import format_banner_version_label
...
def cmd_uninstall(args):
    """Uninstall OpenAmer Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from openamer_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from openamer_cli.uninstall import run_gui_uninstall
    ...
    from openamer_cli.uninstall import run_uninstall
...
# Critical files that OpenAmer must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "openamer_cli/main.py",
    "openamer_cli/config.py",
    "openamer_cli/__init__.py",
    "openamer_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "openamer_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="openamer-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``openamer update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``openamer update``
    ...
    ``openamer_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "openamer_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $OPENAMER_HOME."""
    from openamer_constants import get_openamer_home
    return get_openamer_home() / "web-ui-build-stamp.json"
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the openamer venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — openamer venv python3, no nix-shell overhead
    ...
    # a openamer venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``openamer update`` to stash the
    lockfile — repeatedly.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "openamer_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``openamer web``).
    ...
    # build path usable on a stock `py -m openamer_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (openamer web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``openamer update`` can unconditionally call ``openamer desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``openamer desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $OPENAMER_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $OPENAMER_HOME."""
    from openamer_constants import get_openamer_home
    return get_openamer_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``openamer update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/OpenAmer.app/Contents/MacOS/OpenAmer"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "OpenAmer.exe",
            release_dir / "win-ia32-unpacked" / "OpenAmer.exe",
            release_dir / "win-arm64-unpacked" / "OpenAmer.exe",
        ]
    else:
        candidates = [
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture OpenAmer.exe
...
# OpenAmer.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture OpenAmer.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``OpenAmer.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 openamer-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built OpenAmer.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working OpenAmer.exe from backup.")
        print("    Your existing version was kept and still works. Run `openamer desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `openamer desktop --force-build` to rebuild, or re-run the OpenAmer")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/OpenAmer'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``OpenAmer.exe`` keeps an exclusive lock on
    ``release/win-unpacked/OpenAmer.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\OpenAmer.exe: Access is
    ...
    an unrelated "OpenAmer" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "OpenAmer is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../OpenAmer.app/Contents/MacOS/OpenAmer  ->  app bundle = .../OpenAmer.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``openamer desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ OpenAmer Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ OpenAmer Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the OPENAMER_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from openamer_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["OPENAMER_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["OPENAMER_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "openamer_root", None):
        env["OPENAMER_DESKTOP_OPENAMER_ROOT"] = str(Path(args.openamer_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["OPENAMER_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["OPENAMER_DESKTOP_CWD"] = os.getcwd()
    ...
    # `OPENAMER_DESKTOP_DISABLE_GPU=... openamer desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "OPENAMER_DESKTOP_DISABLE_GPU" not in os.environ:
        env["OPENAMER_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  openamer gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the OpenAmer-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → openamer-setup →
            ...
            if not source_mode:
                ...
                # holds OpenAmer.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose OpenAmer.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on OpenAmer.exe, close any")
                    print("  running OpenAmer desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> openamer desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "OpenAmer is
                ...
                # success on a OpenAmer.exe Windows cannot load (truncated PE from
    ...
    if source_mode:
        print("→ Launching OpenAmer Desktop from source build...")
    ...
    print(f"→ Launching packaged OpenAmer Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``openamer dashboard`` processes other than ourselves.

    ``openamer dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``openamer update`` replaces files on disk, the running
    ...
    ``openamer-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the OpenAmer Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``openamer serve`` as
    ...
    environment variable ``OPENAMER_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "openamer dashboard",
        "openamer_cli.main dashboard",
        "openamer_cli/main.py dashboard",
        # The headless backend (`openamer serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "openamer serve",
        "openamer_cli.main serve",
        "openamer_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            from openamer_cli._subprocess_compat import windows_hide_flags
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `openamer update`.
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `openamer update`.
    ...
    try:
        ...
        return
    db_path = get_openamer_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `openamer sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    openamer sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  openamer sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``openamer update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``openamer update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: openamer curator status)"
    )
...
_DASHBOARD_SYSTEMD_UNIT = "openamer-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: OpenAmer installs Linux services in the
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``openamer dashboard`` processes.

    Called at the end of ``openamer update`` (default ``reason``) and also
    from ``openamer dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``openamer update`` path), a detected
    ``openamer-dashboard.service`` is restarted through systemd instead of
    raw-killing its main PID.
    """
    if restart_managed and _restart_managed_dashboard_service(reason):
        return

    # When the OpenAmer Desktop Electron app spawns this dashboard as a
    # backend child, it sets OPENAMER_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("OPENAMER_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    openamer dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.openamer-update-staging"
    backup = f"{dst}.openamer-update-old"
...
def _update_via_zip(args):
    """Update OpenAmer Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `openamer update "
            f"--branch {branch}`, or update against main with `openamer update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="openamer-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"openamer-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for openamer-agent itself
        ...
        # GitHub ZIPs extract to openamer-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"openamer-agent-{branch}")
    ...
    from openamer_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: openamer skills list-modified  "
    ...
    try:
        from openamer_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from openamer_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_openamer_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_openamer_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "openamer-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if OpenAmer behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes openamer completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but OpenAmer couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but OpenAmer couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if OpenAmer behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but OpenAmer couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but OpenAmer couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `openamer update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_openamer_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_openamer_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``openamer update``, every profile is now current.
    """
    homes = []
...
# Install-scoped breadcrumbs live next to the venv (not under $OPENAMER_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``openamer update``.
    ...
    protocol stream (``openamer acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `openamer update` was interrupted mid-install — "
    ...
    try:
        ...
        if self_locked:
            print(
                "  OpenAmer is still running from the launcher that needs "
                "replacing. Close other OpenAmer windows, restart from a "
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``openamer update`` is still progressing even if pip/uv itself is silent.
    """
...
def _format_concurrent_instances_message(
    ...
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `openamer update --force` if you've already")
...
def _quarantine_running_openamer_exe(
    ...
    handles (recovers in <1s), or the OpenAmer Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       OpenAmer Desktop / gateway / REPL) and pointing to ``--force``.
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `openamer update`:")
    print(
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``openamer update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `openamer update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `openamer update` once the upstream issue is resolved.")
        return True
    if status == "indeterminate":
        print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``openamer update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `openamer update --force` after "
            "closing other openamer processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m openamer_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``openamer update``, not the venv we just wrote
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `openamer update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `openamer update --force` after closing other openamer processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from openamer_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$OPENAMER_HOME/bin/uv``, but on Termux the official
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `openamer update` is exactly the step
...
def _resolve_node_runtime_npm() -> str | None:
    ...
    """
    from openamer_constants import find_node_executable
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from openamer_constants import is_wsl
        ...
        if is_wsl() and path_npm and _is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `openamer update`.")
            failed = ["repo root"]
            if any(
    ...
    # need desktop during `openamer update`, so we install root-only first
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `openamer update`.")
        return list(labels)
    ...
    # hung. The chatty npm-deprecation noise during `openamer update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``openamer update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``openamer update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``openamer update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``openamer update --gateway``) the update is already
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``openamer update --check``: fetch and report without installing.
    ...
    """
    from openamer_cli.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``openamer update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from openamer_cli.config import format_docker_update_message
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``openamer update`` without requiring a reinstall.  The
    ...
    path_comment = (
        "# OpenAmer Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of OPENAMER_HOME under
      ``backups/`` (restorable via ``openamer import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from openamer_cli.backup import (
    ...
    try:
        from openamer_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    print(f"  Restore:  openamer import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``openamer update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``openamer update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run openamer from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.openamer-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".openamer-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m openamer_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "openamer_cli.main" in cmdline_low:
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other OpenAmer processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← OpenAmer Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the OpenAmer desktop app / other OpenAmer terminals, then re-run:"
    )
    lines.append("    openamer update")
    lines.append("  (or use `openamer update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("→ Stopping Windows gateway process(es) before updating OpenAmer...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m openamer_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: openamer gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``openamer gateway start`` uses (``gateway_windows._spawn_detached``).
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``openamer-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("openamer-gateway"):
            continue
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``openamer update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update OpenAmer Agent to the latest version.
    ...
    if is_managed():
        managed_error("update OpenAmer Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `openamer update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # (most commonly the Desktop app's `openamer serve` backend) will keep .pyd
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if repair_uv:
                    repair_env = {**os.environ, "VIRTUAL_ENV": str(_project_venv_dir())}
                    _install_python_dependencies_with_optional_fallback(
                ...
                if healthy_after:
                    ...
                    print("  Close all OpenAmer windows/gateways and re-run: openamer update")
            else:
                print("✓ Already up to date!")
            if runtime_repaired is not None and not _is_windows():
                ...
                print(
                    "  Any running OpenAmer gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in openamer_cli/config.py bricked
        # every user who ran ``openamer update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``openamer update`` again later once a fix lands.")
                    else:
        ...
        # build.  ``openamer desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``openamer desktop`` shouldn't be forced into a full
        # Electron build by ``openamer update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and _resolve_node_runtime_npm() and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "openamer_cli.main", "desktop", "--build-only"]
            ...
            # debuggable.
            #
            # Start the build subprocess with the OpenAmer-managed Node on PATH:
            # when `openamer update` runs inside the desktop updater chain
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT, env=_build_env)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `openamer desktop` to retry)")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_openamer_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # seed_profile_skills() uses subprocess with an explicit OPENAMER_HOME so
        # it is not affected by sync_skills()'s module-level OPENAMER_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's OPENAMER_HOME env var points at the default or a named profile.
        try:
            from openamer_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from openamer_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'openamer config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: openamer config migrate")
            else:
                print()
                print("Skipped. Run 'openamer config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from openamer_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `openamer curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``openamer update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from openamer_cli.tools_config import install_cua_driver
        ...
        # When running as ``openamer update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_openamer_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... openamer-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... openamer-gateway*)
            ...
            # Discover all openamer-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: openamer gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `openamer update` restart it automatically, allow\n"
                        ...
                        # path in `openamer gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: openamer gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: openamer -p <profile> gateway run  for each profile)"
                        )
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_openamer_units():
                ...
                print()
                print("    openamer gateway migrate-legacy")
                print()
        ...
        print("  openamer model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``openamer -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → openamer -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  openamer profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     openamer -p {name} chat")
                else:
        ...
        from openamer_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from openamer_constants import get_openamer_home as _hh
        ...
        from openamer_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `openamer profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → openamer -p {name}  ({wrapper})")
        print()
        ...
        from openamer_cli.profiles import profile_exists, validate_alias_name
        ...
        from openamer_cli.profiles import rename_profile
        ...
        from openamer_cli.profiles import export_profile
        ...
        from openamer_cli.profiles import import_profile
        ...
        import tempfile
        from openamer_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  openamer -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      openamer -p {plan.manifest.name} chat")
        ...
        from openamer_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `openamer profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"openamer -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from openamer_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from openamer_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _report_dashboard_status() -> int:
    """Print ``openamer dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``openamer dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No openamer dashboard processes running.")
        return 0

    print(f"{len(pids)} openamer dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``openamer dashboard register`` for OAuth.
    ...
    try:
        from openamer_cli.web_server import should_require_auth
    ...
    try:
        from openamer_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    try:
        from openamer_cli.config import load_config, save_config
        from openamer_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from openamer_cli.windows_ssh_runtime import read_token
    ...
    token_root = _get_openamer_home() / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``OPENAMER_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``openamer dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No openamer dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `openamer update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with openamer serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with OPENAMER_DESKTOP=1 plus
    # OPENAMER_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # OPENAMER_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `openamer dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited OPENAMER_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # OPENAMER_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (OPENAMER_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets OPENAMER_SERVE_HEADLESS below.
    if os.environ.get("OPENAMER_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("OPENAMER_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("OPENAMER_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("OPENAMER_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → OPENAMER_HOME points into
    ...
    if (
        ...
        and os.environ.get("OPENAMER_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "openamer_cli.main",
            "-p", "default",
        ...
        # OPENAMER_HOME.  We must resolve the root explicitly instead of just
        # dropping OPENAMER_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV OPENAMER_HOME=/opt/data`), so an unset
    ...
    # the same logs directory as every other OpenAmer surface.
    try:
        from openamer_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["OPENAMER_SERVE_HEADLESS"] = "1"
    elif "OPENAMER_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["OPENAMER_WEB_DIST"])
            if "OPENAMER_WEB_DIST" in os.environ
            else PROJECT_ROOT / "openamer_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom OPENAMER_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "OPENAMER_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # OPENAMER_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["OPENAMER_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ OPENAMER_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset OPENAMER_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads OPENAMER_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["OPENAMER_WEB_DIST"] = str(_dist_root)
    ...
    try:
        from openamer_cli.mcp_startup import start_background_mcp_discovery
    ...
    from openamer_cli.web_server import start_server
...
def cmd_gateway_enroll(args):
    ...
    from openamer_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from openamer_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from openamer_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter OpenAmer log files."""
    from openamer_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from openamer_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``openamer -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``openamer_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``openamer -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("OPENAMER_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that OPENAMER_YOLO_MODE is set before ANY
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["OPENAMER_SAFE_MODE"] = "1"
    os.environ["OPENAMER_IGNORE_USER_CONFIG"] = "1"
    os.environ["OPENAMER_IGNORE_RULES"] = "1"
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["OPENAMER_DEFER_AGENT_STARTUP"] = "1"
            os.environ["OPENAMER_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["OPENAMER_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `openamer --tui` is the hot path on phones. The full parser setup imports
...
def cmd_acp(args):
    """Launch OpenAmer Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from openamer_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from openamer_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from openamer_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from openamer_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from openamer_cli.skills_hub import skills_command
...
def cmd_mcp(args):
    from openamer_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from openamer_cli.claw import claw_command
...
def main():
    ...
    try:
        from openamer_cli.stdio import configure_windows_stdio
    ...
    # Self-heal a venv left half-built by an interrupted ``openamer update``
    ...
    # ``openamer skills install update``) merely defers recovery one launch;
    # under-matching (missing ``openamer -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in openamer_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from openamer_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from openamer_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `openamer model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    # `openamer proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    from openamer_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``openamer proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from openamer_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in openamer_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in openamer_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in openamer_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `openamer whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in openamer_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from openamer_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in openamer_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in openamer_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in openamer_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in openamer_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in openamer_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in openamer_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from openamer_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from openamer_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in openamer_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in openamer_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in openamer_cli/subcommands/security.py)
    # =========================================================================
    ...
    # =========================================================================
    # backup command  (parser built in openamer_cli/subcommands/backup.py)
    # =========================================================================
    ...
    from openamer_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    # =========================================================================
    # config command  (parser built in openamer_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in openamer_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in openamer_cli/subcommands/console.py)
    # =========================================================================
    ...
    from openamer_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    # subcommand — ``openamer --help``, ``openamer version``, ``openamer logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from openamer_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from openamer_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and OpenAmer shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from openamer_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from openamer_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in openamer_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in openamer_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `openamer computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `openamer tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `openamer computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the OpenAmer app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                version = ""
                try:
                    from openamer_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: openamer computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: openamer computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: openamer computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: openamer computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: openamer computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: openamer computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in openamer_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <openamer home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    def cmd_sessions(args):
        ...
        if action == "repair":
            ...
            if report.get("repaired"):
                ...
                print(f"    openamer sessions recover --source {source_hint} \\")
                print("        --inspect-only")
        ...
        if action == "recover":
            ...
            from openamer_cli.session_recovery import (
        ...
        if action == "list":
            from openamer_state import workspace_key as _ws_key
            ...
            from openamer_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from openamer_cli.session_export_md import redact_session_data
            ...
            if args.format == "html":
                if not args.output or args.output == "-":
                    ...
                    return
                from openamer_cli.session_export_html import (
                ...
                try:
                    if len(ids) == 1:
                        ...
                        out_dir = (
                            ...
                            else get_openamer_home() / "session-exports"
                        )
            ...
            # Markdown / QMD export
            from openamer_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_openamer_home() / "session-exports"
                ...
                if args.delete_after_verified:
                    for data, exported_path in exported_items:
                        ...
                        if not ok:
                            ...
                            db.close()
                            return
                    sessions_dir = get_openamer_home() / "sessions"
            ...
            # `openamer sessions prune`: no time window and no filters at all
            ...
            # Launch openamer --resume <id> by replacing the current process
            ...
            if result.get("vacuumed") is False:
                print("  (VACUUM was skipped or failed — run "
                      "`openamer sessions optimize` later to reclaim freed space.)")
    ...
    # =========================================================================
    # acp command  (parser built in openamer_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in openamer_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in openamer_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The OpenAmer-Setup.exe success screen tells users to
    # run `openamer desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in openamer_cli/subcommands/gui.py)
    # =========================================================================
    ...
    # e.g. ``openamer -c Pokemon Agent Dev`` → ``openamer -c 'Pokemon Agent Dev'``
    ...
    from openamer_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'openamer -c model' where 'model' is consumed as the
    ...
    # --yolo: set OPENAMER_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (openamer hooks list, cron
    ...
    # ``openamer egress start`` refusing when credential_source=bitwarden
