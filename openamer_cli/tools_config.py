"""
Unified tool configuration for OpenAmer Agent.

`openamer tools` and `openamer setup tools` both enter this module.
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``openamer tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`openamer_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from openamer_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _OPENAMER_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `openamer tools` → Video Generation, which walks
...
# `openamer tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from openamer_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `openamer tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``openamer tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from openamer_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``openamer tools`` checklist actually offers
    for ``platform``.
    ...
    so ``openamer tools`` never claims to add or remove a toolset the user was
...
from openamer_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "OpenAmer routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set OPENAMER_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "OPENAMER_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "OPENAMER_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "OPENAMER_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "OPENAMER_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "OPENAMER_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `openamer model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("OPENAMER_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the OpenAmer telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``openamer update`` and
      by ``openamer computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `openamer update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `openamer computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True) -> bool:
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `openamer update`'s refresh), capture the
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / OpenAmer process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in openamer-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/openamer/openamer:latest"
            )
            return
        ...
        try:
            from openamer_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: openamer auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `openamer auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: openamer auth spotify")
        except Exception as exc:
        ...
        try:
            from openamer_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: openamer plugins enable observability/langfuse")
        _print_info("    Restart OpenAmer for tracing to take effect.")
        _print_info("    Verify: openamer plugins list")
        ...
        try:
            ...
            _print_info("    Run later: openamer auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `openamer auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: openamer auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: openamer auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``openamer tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``openamer tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: openamer tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``openamer-discord``, which contains those
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``openamer-discord``) is an opt-in to the platform's native default-off
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"openamer-{platform}"
    ...
    # "openamer-cli" (which include all _OPENAMER_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "openamer-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``openamer tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `openamer tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"openamer-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("openamer-")}
    ...
    # shipped as a bundled plugin but user must opt in via `openamer tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `openamer tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by openamer tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # at runtime, not only during the next `openamer update`/`openamer doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `openamer tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (openamer-cli, openamer-telegram, etc.)
    ...
    # Opening `openamer tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `openamer tools` gets a silent no-op
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``openamer model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``openamer model`` aux-task
    ...
    """
    try:
        from openamer_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `openamer model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `openamer model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `openamer tools` and `openamer setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ OpenAmer Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://github.com/openamer/openamer/blob/main/website/docs/user-guide/features/tools", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, openamer mcp
