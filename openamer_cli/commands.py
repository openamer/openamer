"""Slash command definitions and autocomplete for the OpenAmer CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of OpenAmer config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal OpenAmer works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while OpenAmer is working", "Configuration",
    ...
    CommandDef("subscription", "View your OpenAmer plan and change it in the browser", "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", "Show your OpenAmer balance and manage billing on the portal", "Info"),
    ...
    CommandDef("update", "Update OpenAmer Agent to the latest version", "Info"),
    CommandDef("version", "Show OpenAmer Agent version", "Info", aliases=("v",)),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info",
               args_hint="[openamer|local]"),
...
# Telegram allows up to 100 BotCommands. OpenAmer ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full OpenAmer
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``openamer skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``openamer skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``openamer skills list`` and
...
# native slot, the alias spelling stays reachable via /openamer reset).
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/openamer <verb>`` subcommand.
...
def slack_app_manifest(request_url: str = "https://openamer-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /openamer handler.

    Maps both canonical names and aliases so /openamer bg do stuff works
    the same as /openamer background do stuff.

    Plugin-registered slash commands are included so ``/openamer <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from openamer_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
