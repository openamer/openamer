"""
Configuration management for OpenAmer Agent.

Config files are stored in ~/.openamer/ for easy access:
- ~/.openamer/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.openamer/.env         - API keys and secrets

This module provides:
- openamer config          - Show current configuration
- openamer config edit     - Open config in editor
- openamer config get      - Print a resolved configuration value
- openamer config set      - Set a specific value
- openamer config unset    - Remove a user configuration value
- openamer config wizard   - Re-run setup wizard
"""
...
from openamer_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``openamer config set`` (which rewrites
    ...
    ``config.yaml`` in place: openamer never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.openamer/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``openamer logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  openamer config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` OpenAmer makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. OpenAmer itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``openamer update``, the TUI build.
...
#   on every plugin install / ``openamer update``.
...
from openamer_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_openamer_home() / ".managed"
...
_NIX_UPDATE_MSG = (
    "Update OpenAmer through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``openamer_cli/`` — i.e. the git checkout for source
    installs, ``/opt/openamer`` inside the published image. It is a property of
    the running interpreter, NOT of ``$OPENAMER_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how OpenAmer was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.
    ...
    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.openamer``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$OPENAMER_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.openamer:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$OPENAMER_HOME``, a
    ...
    ``docker`` and ``openamer update`` refuses to run ("doesn't apply inside the
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $OPENAMER_HOME.
    try:
    ...
    #    host install whose shared $OPENAMER_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``openamer update``.
    try:
        method = (
            (get_openamer_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``openamer_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$OPENAMER_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/openamer`` in the published image, which instead bakes the stamp at
...
# Long-form text for ``openamer update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# openamer_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side OpenAmer, it doesn't update
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``openamer update`` inside Docker.
...
def format_managed_message(action: str = "modify this OpenAmer installation") -> str:
    ...
    return (
        f"Cannot {action}: this OpenAmer installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall OpenAmer."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from OPENAMER_HOME/.container-mode.
    ...
    container_mode_file = get_openamer_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "openamer-agent")
    exec_user = info.get("exec_user", "openamer")
    openamer_bin = info.get("openamer_bin", "/data/current-package/bin/openamer")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "openamer_bin": openamer_bin,
    }
...
# Re-export from openamer_constants — canonical definition lives there.
from openamer_constants import get_openamer_home, get_process_openamer_home  # noqa: F811,E402
...
def _resolve_openamer_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    ownership. The entrypoint chowns the top-level OPENAMER_HOME once, but
    subdirectories created at runtime by ``ensure_openamer_home()`` (especially
...
def _chown_to_openamer_uid(path) -> None:
    ...
    directories created by :func:`ensure_openamer_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_openamer_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the openamer group can
    share state with the gateway service.

    The mode can be overridden via the OPENAMER_HOME_MODE environment variable
    (e.g. OPENAMER_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse OPENAMER_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("OPENAMER_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_openamer_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When OpenAmer runs in a container with volume-mounted config files, forcing
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into OPENAMER_HOME, upgrading legacy empty templates.
...
# ensure_openamer_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_OPENAMER_HOME_ENSURED: set = set()


def ensure_openamer_home():
    """Ensure ~/.openamer directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``OPENAMER_HOME`` we only re-run
    ...
    ``get_openamer_home()`` and therefore re-run for the new path.
    """
    home = get_openamer_home()
    key = str(home)

    if key in _OPENAMER_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``openamer profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old OPENAMER_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_openamer_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _OPENAMER_HOME_ENSURED.add(key)


def _ensure_openamer_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"OPENAMER_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the OpenAmer-level retry loop that wraps the whole call.  Lower
        ...
        # app, ACP) in a code workspace, OpenAmer adds a coding operating brief
    ...
    "terminal": {
        ...
        #             OPENAMER_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force OPENAMER_HOME/home when it exists (old strict
        ...
        # skipped. When empty, OpenAmer auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), OpenAmer sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when OpenAmer
        ...
        # bundled OpenAmer image, which drops to the `openamer` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, OpenAmer sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and OpenAmer should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``openamer chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.openamer/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: openamer sweeps the checkpoint base at startup
        ...
        # ``openamer checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .openamer.md, or .cursorrules before OpenAmer applies
    ...
    # single read_file page exceeds these limits, OpenAmer truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so OpenAmer' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); openamer =
                                      # OpenAmer' compression threshold triggers
    ...
    # See tools/kanban_tools.py and openamer_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``openamer dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        "vision": {
            "provider": "auto",    # auto | openrouter | openamer | codex | custom
        ...
        # Invoked by ``openamer kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``openamer kanban decompose`` and the kanban
        ...
        # ``openamer profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `openamer model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `openamer` (and `openamer chat`) launches by default:
        ...
        # OpenAmer credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # can set `path`/`paths` to OPENAMER_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/openamer`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `openamer pets`. Disabled until
        ...
        "pet": {
            ...
            # get_openamer_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled OpenAmer Portal plugin reads
        ...
        # ``portal_url`` defaults to https://portal.openamer.com).
        "oauth": {
        ...
        # openamer-account-service at deploy time via the
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; OpenAmer appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.openamer/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.openamer/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, OpenAmer, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.openamer/cache/delegation/
    ...
    # response. If not, OpenAmer feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before OpenAmer auto-pauses the goal and
    ...
    "moa": {
        ...
        # <openamer_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # cron daemon.
    #
    # See `openamer curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `openamer curator run --consolidate` overrides
        ...
        # normally restored on every `openamer update`, so pruning them only
        ...
        # skipped), snapshot ~/.openamer/skills/ into
        # ~/.openamer/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `openamer curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for openamer-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *OpenAmer Agent*" header.
    ...
    # replace OpenAmer' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.openamer/shell-hooks-allowlist.json.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `openamer doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``openamer_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow OpenAmer to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $OPENAMER_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing OpenAmer Portal token — there is no token key here.
        "chronos": {
    ...
    # todos to ready, and fires `openamer -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `openamer` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `openamer kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     openamer-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core OpenAmer tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.openamer/logs/.
    ...
    # curated model lists for OpenRouter and OpenAmer Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a openamer-agent release.
    ...
    "gateway": {
        ...
        # producing ~/.openamer/sessions/sessions.json entirely.
        ...
        # `launchctl kickstart ai.openamer.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.openamer/.env,
        ...
        # behavior: files must live under the OpenAmer cache, under
        ...
        # cache (~/.openamer/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.openamer/state.db.
    ...
    "sessions": {
        ...
        # default of ``openamer sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.openamer/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `openamer sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `openamer update` prints a one-line notice with
        ...
        # ~/.openamer/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
    ...
    "updates": {
        ...
        #     DBs) into <OPENAMER_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``openamer backup``-style zip
        #     of OPENAMER_HOME into <OPENAMER_HOME>/backups/, restorable with
        #     ``openamer import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `openamer update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<OPENAMER_HOME>/lsp/bin/`` on first use.
    ...
    # `openamer tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.openamer/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.openamer/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.openamer/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, OpenAmer may use this encrypted
            ...
            # ~/.openamer/bin/ on first use.  When False you must install
            ...
            # `openamer secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.openamer/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. OpenAmer disables it for our users unless they
        # explicitly opt in here. When false (default), OpenAmer sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; OpenAmer also calls
    ...
    #
    # Configure with `openamer egress setup`.  Disabled by default — the rest of
    # OpenAmer works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.openamer/bin/ on
        ...
        # Together, DeepSeek, OpenAmer).  Wildcards (`*.foo.com`) are supported.
        "extra_allowed_hosts": [],
    },

    # OpenAmer Desktop (Electron app) launch options. These only affect
    # `openamer desktop`; they do not touch the CLI/gateway.
    "desktop": {
...
# selection step (OpenAmer Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "OPENAMER_BASE_URL": {
        "description": "OpenAmer Portal base URL override",
        "prompt": "OpenAmer Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials OpenAmer mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'openamer model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for OpenAmer Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for OpenAmer Subscribers only, used to derive vendor hosts, e.g. openamer.com -> firecrawl-gateway.openamer.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for OpenAmer Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit OpenAmer Subscriber access token for tool-gateway requests (optional; otherwise read from the OpenAmer auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use OpenAmer, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @openamer:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #openamer)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: openamer-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'openamer-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote OpenAmer API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote OpenAmer API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote OpenAmer API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `openamer config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.openamer.config`` entries, then checks
    ...
    try:
        ...
        # should never break `openamer update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but OpenAmer' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older OpenAmer versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``openamer_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (openamer_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (openamer_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (openamer_cli/setup.py)
    "known_plugin_toolsets", # written/read by openamer_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'openamer setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — OpenAmer won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, openamer send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'openamer doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("OPENAMER_HOME", "~/.openamer")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "openamer update / openamer -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("OPENAMER_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from OPENAMER_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$OPENAMER_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_openamer_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `openamer plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `openamer curator status` has no
    ...
    #   3. Creates `~/.openamer/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_openamer_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_openamer_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `openamer config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `openamer config set`"
                    )
    ...
    # behavior set it to true explicitly via `openamer config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from openamer_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from openamer_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: openamer config set <key> <value>")
    ...
    # `openamer update`, but only the version bump is persisted.
    ...
    # metadata.openamer.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.openamer/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_openamer_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``openamer config set``
    ...
    backends) — while display paths (``openamer status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _openamer_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.openamer/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.openamer/config.yaml.
    ...
    (which change ``OPENAMER_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/openamer/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_openamer_dotenv()
...
#   openai-codex (OAuth — openamer auth) — OpenAI Codex
#   openamer         (OAuth — openamer auth) — OpenAmer Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.openamer/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset OpenAmer writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.openamer/.env.
    ...
    interactive menu render (`openamer tools`, `openamer setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `openamer tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.openamer/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.openamer/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.openamer/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.openamer/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to OpenAmer — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known OpenAmer vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.openamer/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.openamer/.env`` over ``os.environ``.

    Used for OpenAmer-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ OpenAmer Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from openamer_cli.auth import get_anthropic_key
    ...
    print(color("  openamer config edit     # Edit config file", Colors.DIM))
    print(color("  openamer config set <key> <value>", Colors.DIM))
    print(color("  openamer setup           # Run setup wizard", Colors.DIM))
    print()
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``openamer config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``openamer config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            exposes this via ``openamer config set --force``.
    """
    if is_managed():
    ...
    # so a fresh `openamer config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_openamer_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `openamer config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `openamer config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: openamer config get <key> [--json]")
            print()
            print("Examples:")
            print("  openamer config get model")
            print("  openamer config get terminal.backend")
            print("  openamer config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: openamer config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  openamer config set model anthropic/claude-sonnet-4")
            print("  openamer config set terminal.backend docker")
            print("  openamer config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: openamer config unset <key>")
            print()
            print("Examples:")
            print("  openamer config unset model")
            print("  openamer config unset terminal.backend")
            print("  openamer config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'openamer config migrate' to add them")
        ...
        print("  openamer config           Show current configuration")
        print("  openamer config edit      Open config in editor")
        print("  openamer config get <key>          Print a resolved config value")
        print("  openamer config set <key> <value>   Set a config value")
        print("  openamer config unset <key>        Remove a config value")
        print("  openamer config check     Check for missing/outdated config")
        print("  openamer config migrate   Update config with new options")
        print("  openamer config path      Show config file path")
        print("  openamer config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``openamer config`` UI so users
