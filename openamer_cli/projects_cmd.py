"""``openamer project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$OPENAMER_HOME/projects.db`` store
(see :mod:`openamer_cli.projects_db`).
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'OpenAmer Agent'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``openamer project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: openamer project <action> [options]\n"
                "Run 'openamer project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `openamer project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `openamer project add-folder`.",
            file=sys.stderr,
        )
        return 1
