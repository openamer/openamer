...
# xAI's public API is OpenAI-compatible for the endpoints OpenAmer commonly
# uses. The Responses endpoint is included because OpenAmer' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via OpenAmer-managed OAuth credentials."""

    auth_hint = "openamer auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`openamer auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`openamer auth reset xai-oauth` or re-authenticate with "
                    "`openamer auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `openamer auth add xai-oauth --type oauth`."
            )
