"""CLI handlers for the ``openamer proxy`` subcommand."""
...
from openamer_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "openamer proxy requires aiohttp. Run `openamer setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"openamer auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting OpenAmer proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("OpenAmer proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: openamer proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``openamer proxy <subcommand>``."""
    ...
    print(
        "openamer proxy — local OpenAI-compatible proxy that attaches your\n"
