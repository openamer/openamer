"""Helpers for loading OpenAmer .env files consistently across entrypoints."""
...
def reset_secret_source_cache() -> None:
    """Forget which OPENAMER_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `openamer setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``openamer_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from openamer_cli.config import _sanitize_env_lines
    except ImportError:
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of OPENAMER_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of OpenAmer reads
    ...
    owns the once-per-OPENAMER_HOME guard, the post-apply ASCII
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `openamer model`
