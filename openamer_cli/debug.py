"""``openamer debug`` debug tools for OpenAmer Agent.

Currently supports:
    openamer debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.openamer/logs/*.log`` are not leaked into
                          the public paste service. Pass ``--no-redact``
                          to disable.
                          Pass ``--openamer`` to upload instead to OpenAmer-internal
                          storage (AWS S3) via a signed URL minted by the
                          OpenAmer account service: the bundle is private
                          (viewable only by OpenAmer staff / allowlisted mods via
...
_REDACTION_BANNER = (
    "[openamer debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.openamer/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``openamer debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``openamer debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `openamer debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "openamer-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``openamer debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.openamer/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``openamer debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "openamer-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----OpenAmerDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from openamer_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_openamer_home() / "logs" / filename) if filename else None
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``openamer debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``openamer dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``openamer dump`` is run
        internally.
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by both the paste.rs and OpenAmer-S3 paths)
# ---------------------------------------------------------------------------

# Bundle format identifier embedded in the OpenAmer-S3 JSON envelope. The
# discord-support viewer keys off this string to parse the bundle.
_openamer_BUNDLE_FORMAT = "openamer-debug-share/1"
...
def collect_share_bundle(
    ...
    OpenAmer-S3 path (``--openamer``).  Centralising it guarantees the OpenAmer bundle is
    built from the *same* force-redacted snapshots as the public paste path —
    redaction is the safety boundary, so the OpenAmer path must never see raw
    logs.
...
def build_openamer_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip-compress a :func:`collect_share_bundle` mapping into the OpenAmer envelope.

    The JSON shape is what the discord-support viewer (Repo 3) parses::

        {"format": "openamer-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
    ...
    envelope = {
        "format": _openamer_BUNDLE_FORMAT,
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``openamer debug share`` (CLI) and the
    ...
    # shared collector so the paste.rs and OpenAmer-S3 paths build identical,
    ...
    if redact:
        logger.info(
            "openamer debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    openamer = getattr(args, "openamer", False)
    ...
    print("To delete now:  openamer debug delete <url>")

    print("\nShare these links with the OpenAmer team for support.")


_openamer_PRIVACY_NOTICE = """\
⚠️  --openamer: This uploads your debug bundle to OpenAmer-INTERNAL storage (AWS S3),
    NOT a public paste service. The following is included:
  • System info (OS, Python/OpenAmer version, provider, which API keys are
  ...
  • The bundle is viewable only by OpenAmer staff (and allowlisted Discord mods)
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``openamer debug share --openamer``: upload the bundle to OpenAmer-S3.

    Collects the same force-redacted bundle as the paste path, gzips it into
    the OpenAmer envelope, requests a signed URL from NAS, uploads, and prints the
    private viewer link. On any failure falls back to a clear error that
    suggests ``--local``.
    """
    from openamer_cli.diagnostics_upload import share_to_nous

    print(_openamer_PRIVACY_NOTICE)
    if not _confirm_upload(args):
        return
    if not redact:
        print(
    ...
    if redact:
        logger.info(
            "openamer debug share --openamer: applied force-mode redaction before upload"
        )
    blob = build_openamer_bundle(bundle, redact=redact)

    print("Uploading to OpenAmer diagnostics storage...")
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `openamer debug share --local` to print the report instead, "
            "or `openamer debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)

    view_url = res.get("viewUrl") or res.get("view_url")
    print("\nDebug bundle uploaded to OpenAmer (private):")
    if view_url:
    ...
    print(
        "\nShare this private link with the OpenAmer team — only OpenAmer staff "
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: openamer debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'openamer debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``openamer debug`` call.
    ...
    # best-effort — any failure is swallowed so ``openamer debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: openamer debug <command>")
        print()
        print("Commands:")
        ...
        print("  --openamer       Upload to OpenAmer-internal storage (private, staff-only,")
