...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"OpenAmer could not verify your Nous Portal entitlement, so {capability} "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but OpenAmer cannot verify "
    ...
    if account_info.paid_service_access is None:
        detail = (
            f"OpenAmer could not verify your Nous Portal paid access, so {capability} "
            f"is unavailable."
        )
    ...
    if reason == "account_missing":
        return (
            f"OpenAmer could not find a Nous Portal account or organisation for this "
