"""
OpenAmer Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``openamer desktop`` / ``openamer gui``)
     Built inside the agent checkout under ``$OPENAMER_HOME/openamer-agent/``:
       ...
       - ``$OPENAMER_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/OpenAmer.app`` or ``~/Applications/OpenAmer.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\OpenAmer`` (NSIS per-user)
...
the app name ("OpenAmer"), separate from ``$OPENAMER_HOME``:
  - macOS:   ``~/Library/Application Support/OpenAmer``
  - Windows: ``%APPDATA%\\OpenAmer``
  - Linux:   ``$XDG_CONFIG_HOME/OpenAmer`` (default ``~/.config/OpenAmer``)
...
``openamer uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(openamer_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return openamer_home / "openamer-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "OpenAmer"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "OpenAmer"
...
def source_built_gui_artifacts(openamer_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``openamer desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``openamer-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(openamer_home)
    ...
    return [
        ...
        openamer_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "OpenAmer" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/OpenAmer.app"),
            home / "Applications" / "OpenAmer.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\OpenAmer).
            local_base / "Programs" / "OpenAmer",
            # Older / alternate layout some builds used.
            local_base / "openamer-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "OpenAmer")
    else:
        ...
        paths += [
            data_base / "applications" / "openamer.desktop",
            data_base / "applications" / "OpenAmer.desktop",
        ]
    return paths


def agent_is_installed(openamer_home: Path) -> bool:
    """Return True when a usable Python agent install exists under OPENAMER_HOME.
    ...
    if (agent_root / "openamer_cli").is_dir():
        return True
...
def gui_is_installed(openamer_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(openamer_home):
        if p.exists():
            return True
...
def gui_install_summary(openamer_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = openamer_home if openamer_home is not None else get_openamer_home()
...
def uninstall_gui(openamer_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``openamer-agent/openamer_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$OPENAMER_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove openamer' or "
            "'sudo dnf remove openamer'). AppImage builds are a single file you "
