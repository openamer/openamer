"""OpenAmer-managed uv and Python runtime repair.

OpenAmer owns its own uv binary at ``$OPENAMER_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$OPENAMER_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every OpenAmer
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.openamer-runtime/python``. A
...
from openamer_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".openamer-runtime"
_VENV_NAME = "venv"
...
def managed_uv_path() -> Path:
    """Return the path where OpenAmer keeps *its* uv binary.

    ``$OPENAMER_HOME/bin/uv`` on POSIX, ``$OPENAMER_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for OpenAmer-private uv Python commands."""
    target = (
        Path(install_dir)
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``openamer update`` runs
    the call site from the *old*, already-imported ``openamer_cli.main`` against
...
def update_managed_uv(
    *,
    ...
    Call this during ``openamer update`` so the managed copy stays current.
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the OpenAmer generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("openamer_cli.main")
    ...
    if holders:
        ...
        return True, f"other OpenAmer processes still hold the venv (PID {pids})"
    return False, ""
    ...
    try:
        ...
        if backup is not None:
            print(
                ...
                "keep it until all older OpenAmer processes have exited."
            )
        return RuntimeRepairResult(
            "repaired",
            sqlite_before=current.sqlite_version_string,
...
def _install_uv(target: Path) -> None:
    ...
    ``$OPENAMER_HOME/bin/`` instead of ``~/.local/bin/``.
    """
