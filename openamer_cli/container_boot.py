...
``$OPENAMER_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-openamer-setup
(the stage2 hook) has chowned the volume and seeded $OPENAMER_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$OPENAMER_HOME``,
    not under ``profiles/``). The dispatcher in ``openamer_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``openamer gateway start`` (no ``-p``) targets. Without it,
    bare ``openamer gateway start`` inside the container would land on
    ...
    ``$OPENAMER_HOME/gateway_state.json`` (sibling to the profile root,
    ...
    # ``openamer gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `openamer profile create` (config.yaml
            # is not — that comes later via `openamer setup`). Use it as the
            ...
            # via ``openamer -p default-named gateway start`` if they
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    if os.environ.get("OPENAMER_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the openamer args.
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``openamer dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted OPENAMER_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `openamer -p <profile> gateway start`
...
def main() -> int:
    ...
    # bind-mounted OPENAMER_HOME, both race to flock() the same s6-log lock
