"""
Gateway subcommand for openamer CLI.

Handles: openamer gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "openamer CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``openamer gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_openamer_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. OPENAMER_HOME
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``openamer update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            OpenAmer profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to OpenAmer profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from openamer_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "openamer_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m openamer_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``openamer update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting OpenAmer's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose OPENAMER_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``openamer update`` users
    # with no running gateway until they re-invoke ``openamer gateway``
    ...
    from openamer_cli._subprocess_compat import (
    ...
    # interpreter and captures the stable cwd + env overlay (OPENAMER_HOME,
    ...
    if sys.platform == "win32":
        try:
            from openamer_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / OPENAMER_HOME) the windowless
        # base interpreter needs to import openamer_cli.  Empty on POSIX, where
...
def _openamer_home_from_systemd_unit_file(system: bool = False) -> str | None:
    ...
    for line in text.splitlines():
        ...
        if body.startswith("OPENAMER_HOME="):
...
def _sync_openamer_home_from_systemd_unit(system: bool) -> None:
    ...
    if not unit_home:
        return
    current = os.environ.get("OPENAMER_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["OPENAMER_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}openamer gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}openamer gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from openamer_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: openamer gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `openamer gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``openamer gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from openamer_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (openamer_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (OPENAMER_HOME-scoped).
...
from openamer_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our OpenAmer Docker image — that one runs s6-overlay as
...
_SERVICE_BASE = "openamer-gateway"
SERVICE_DESCRIPTION = "OpenAmer Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current OPENAMER_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (OPENAMER_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary OPENAMER_HOME paths
...
def print_legacy_unit_warning() -> None:
    """Warn about legacy OpenAmer gateway unit files if any are installed.
    ...
    print_info("  These run alongside the current openamer-gateway service and")
...
def remove_legacy_openamer_units(
    ...
    print()
    print("Legacy OpenAmer gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: openamer gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo openamer gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    openamer gateway uninstall")
    print_info("    sudo openamer gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: openamer gateway install"
            )
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode OpenAmer often sets ``HOME`` to a profile-scoped directory, but
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m openamer_cli.main``, so module
    ...
    dir, or a clone that ``openamer update`` later relocates/removes — the path
    ...
    ``OPENAMER_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if OPENAMER_HOME
...
Group={group_name}
ExecStart={python_path} -m openamer_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m openamer_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="OPENAMER_HOME={openamer_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned OPENAMER_HOME is adopted before any compare/regenerate" at a single
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir OPENAMER_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved OPENAMER_HOME
    ...
    exported a throwaway ``OPENAMER_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``openamer gateway restart``
    with ``OPENAMER_HOME=/tmp/openamer-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="OPENAMER_HOME=..."`` lines and launchd
    ``<key>OPENAMER_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'OPENAMER_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>OPENAMER_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp OPENAMER_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: OPENAMER_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported OPENAMER_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single OPENAMER_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # OPENAMER_HOME into the unit (manual E2E homes like /tmp/openamer-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current OpenAmer install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo openamer gateway uninstall --system")
    print_info("         openamer gateway install")
    print_info("         openamer gateway start")
...
def systemd_install(
    ...
    # Existing system units already pin OPENAMER_HOME; adopt it before any
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}openamer gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}openamer gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}openamer gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # OPENAMER_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `openamer gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `openamer gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}openamer gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}openamer gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}openamer gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}openamer gateway start{scope_flag}"
        )
...
# process invocation (each ``openamer gateway start/stop/status`` call).
...
#      background process (the `nohup openamer gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_openamer_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_openamer_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m openamer_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active OPENAMER_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "openamer_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup openamer gateway run --replace`
    ...
    """
    from openamer_cli._subprocess_compat import windows_detach_popen_kwargs
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup openamer gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>openamer_cli.main</string>",
    ]
    if profile_arg:
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current OpenAmer install"
    )
    return True
...
def launchd_stop():
    ...
    # `openamer gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # OpenAmer PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the OpenAmer PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current OpenAmer install")
    else:
        print("⚠ Service definition is stale relative to the current OpenAmer install")
        print("  Run: openamer gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: openamer gateway stop")
            else:
    ...
    if deep:
        log_file = get_openamer_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``openamer gateway run`` is always a misconfiguration, so we
    ...
    print()
    print("    openamer gateway restart")
    print()
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``openamer gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``openamer gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other OPENAMER_HOME roots.
    """
    ...
    print("  Use 'openamer gateway restart' to replace it,")
    print("  or 'openamer gateway stop' first.")
    print("  Or use 'openamer gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # /restart) rather than through `openamer gateway restart` which already
    ...
    # `openamer gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           🧬 OpenAmer Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``openamer gateway run`` enters through this CLI wrapper, not through
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where OpenAmer delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. OpenAmer connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: openamer pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and OpenAmer will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``openamer setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run openamer under
        WSL.
    """
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_openamer_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from openamer_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'openamer pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: openamer pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'openamer gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `openamer gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_openamer_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `openamer pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `openamer pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "OpenAmerAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``openamer_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from openamer_cli import setup as _s
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: openamer gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: openamer gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: openamer gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo openamer gateway install --system"
                        )
                    print_info("  Or run in foreground:  openamer gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: openamer gateway run")
                print_info(
                    "  For persistence:   tmux new -s openamer 'openamer gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: openamer gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup openamer gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'openamer gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`openamer_cli.service_manager` are caught and surfaced as
    ...
    if profile is None:
        ...
        # OPENAMER_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``openamer gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``openamer gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    # `docker logs` and ${OPENAMER_HOME}/logs/gateways/<profile>/current,
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: openamer gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: openamer gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s openamer 'openamer gateway run'"
                )
                print()
            ...
            print()
            print(
                "  openamer gateway run                              # direct foreground"
            )
            print(
            ...
            from openamer_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  openamer profile create <name>     # creates the s6 service slot")
                print("  openamer -p <name> gateway start   # bring it up via s6")
                print("  openamer status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: openamer gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: openamer gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: openamer gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  openamer profile delete <name>     # tears down the s6 service slot")
                print("  openamer -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `openamer -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: openamer gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_OPENAMER_GATEWAY") == "1":
            print_error(
                ...
                "Use `openamer gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_OPENAMER_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `openamer gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: openamer gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  openamer gateway install")
                else:
                    print("To install as a service:")
                    print("  openamer gateway install")
                    print("  sudo openamer gateway install --system")
            else:
                ...
                if is_termux():
                    ...
                    print(
                        "  openamer gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  openamer gateway install  # Install as user service")
                    print(
                        "  sudo openamer gateway install --system  # Install as boot-time system service"
                    )
...
def ensure_gateway_service(context: str = "setup") -> bool:
    """Install and start a user-scope gateway service without prompting (``openamer setup``/``import``).
    ...
    if is_container():
        ...
        print_info("   openamer gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: openamer gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: openamer gateway install")
    except Exception as e:
