"""
Doctor command for openamer CLI.

Diagnoses issues with OpenAmer Agent setup.
"""
...
from openamer_cli.config import get_project_root, get_openamer_home, get_env_path
...
_DHH = display_openamer_home()  # user-facing display path (e.g. ~/.openamer or ~/.openamer/profiles/coder)

# Load environment variables from ~/.openamer/.env so API key checks work
...
from openamer_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.openamer/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("OPENAMER_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from openamer_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches openamer_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``openamer --version`` reporting a stale version while
    ...
    """
    try:
        from openamer_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != openamer_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'openamer update', or set "
            "openamer_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-openamer and dashboard static services are up
    ...
    for static in ("main-openamer", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `openamer profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `openamer doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from openamer_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.openamer/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 OpenAmer Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`openamer doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from openamer_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: OpenAmer already refuses to enable WAL on fresh DBs.
            ...
            check_warn(
                ...
                "(run `openamer update`; fixed versions: 3.51.3+ / 3.50.7 / "
    ...
    # Detect drift between pyproject.toml and openamer_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.openamer/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        # matches openamer_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'openamer setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'openamer setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'openamer setup' to create one")
                issues.append("Run 'openamer setup' to create .env")
    
    # Check ~/.openamer/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from openamer_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from openamer_cli.providers import (
            ...
            if isinstance(user_providers, dict):
                from openamer_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'openamer config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from openamer_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.openamer/.env or run 'openamer setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'openamer setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'openamer config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from openamer_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from openamer_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'openamer setup' to migrate config")
                else:
                    issues.append("Run 'openamer doctor --fix' or 'openamer setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'openamer doctor --fix'")
        except Exception:
            pass
        ...
        try:
            import yaml
            from openamer_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
        ...
        try:
            ...
            from openamer_cli.config import load_env as _load_env_depr
    ...
    try:
        ...
        if openamer_status.get("logged_in"):
            check_ok("OpenAmer Portal auth", "(logged in)")
        else:
            check_warn("OpenAmer Portal auth", "(not logged in)")
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses OpenAmer' own device-code flow — the Codex CLI is
    ...
    # disrupt the already-printed OpenAmer/Codex/Gemini/MiniMax rows above.
    try:
        from openamer_cli.auth import get_xai_oauth_auth_status
    ...
    for subdir_name in expected_subdirs:
        subdir_path = openamer_home / subdir_name
    ...
    soul_path = openamer_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give OpenAmer a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# OpenAmer Agent Persona\n\n"
                "<!-- Edit this file to customize how OpenAmer communicates. -->\n\n"
                "You are OpenAmer, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = openamer_home / "memories"
    ...
    state_db_path = openamer_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from openamer_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'openamer doctor --fix' "
                        "(or 'openamer sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from openamer_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'openamer doctor --fix' "
                        "(or 'openamer sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = openamer_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'openamer doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "openamer"
        ...
        _cmd_link = _cmd_link_dir / "openamer"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(openamer not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/openamer → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/openamer points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/openamer → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/openamer — run 'openamer doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/openamer exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/openamer not found",
                    "(openamer command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/openamer → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/openamer symlink — run 'openamer doctor --fix'")
    ...
    try:
        from openamer_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_path.exists():
            ...
            # `openamer update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `openamer doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable OPENAMER_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a OpenAmer one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'openamer config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``openamer doctor``.
    ...
    # never the legitimate source for `openamer doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'openamer setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = OPENAMER_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: openamer skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = OPENAMER_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: openamer memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: openamer memory setup",
                    "No Honcho API key — run 'openamer memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run openamer memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: openamer memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: openamer memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from openamer_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "openamer -p" in content:
                            _m = _re.search(r"openamer -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'openamer doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
