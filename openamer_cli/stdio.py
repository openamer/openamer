...
on Japanese locales, etc.).  OpenAmer's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``openamer_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. OpenAmer is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``OPENAMER_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("OPENAMER_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the OpenAmer-managed Git install directories so
    ...
    # install without OpenAmer-managed PortableGit).
    ...
    # stdin is re-configured for completeness; OpenAmer's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and OpenAmer's
    ``openamer config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching OpenAmer (User env var in Windows Settings, or
    export in a PowerShell profile) and OpenAmer picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known OpenAmer-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Patch-up strategy: add the known OpenAmer-managed tool directories to our
