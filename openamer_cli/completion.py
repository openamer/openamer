"""Shell completion script generation for openamer CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# OpenAmer Agent bash completion
# Add to ~/.bashrc:
...
{top_cmds_str}
            ...
            _describe 'openamer command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# OpenAmer Agent fish completion",
        "# Add to your config:",
