"""
Status command for openamer CLI.

Shows the status of all OpenAmer Agent components.
"""
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``openamer config``'s
...
def show_status(args):
    """Show status of all OpenAmer Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ OpenAmer Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: openamer model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: openamer auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: openamer auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: openamer model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from openamer_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from openamer_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   openamer gateway")
    ...
    jobs_file = get_openamer_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    print(color("  Run 'openamer doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'openamer setup' to configure", Colors.DIM))
    print()
