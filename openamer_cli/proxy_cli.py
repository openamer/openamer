"""CLI handlers for ``openamer egress ...``.
...
The top-level command is ``openamer egress``.  Note that the inbound OAuth
reverse-proxy command (``openamer proxy``) lives elsewhere in
``openamer_cli/main.py`` — different direction, different purpose.
"""
...
# ---------------------------------------------------------------------------
# Argparse wiring — called from openamer_cli.main
# ---------------------------------------------------------------------------
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``openamer_cli.main`` as part of building the top-level
    ``openamer egress`` parser.
    """

    # dest='egress_command' — keeps this subparser tree disjoint from the
    # inbound OAuth ``openamer proxy`` subparser (which uses dest='proxy_command').
    ...
    setup.add_argument(
        ...
        help="Do not restart a running daemon after setup; you'll need to run "
             "`openamer egress restart` yourself for changes to take effect.",
    )
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if args.from_bitwarden:
        ...
        if not bw_cfg.get("enabled"):
            ...
            console.print(
                "  Run `openamer secrets bitwarden setup` first, or omit "
                "--from-bitwarden."
            )
            return 1
        try:
    ...
    if rotate and existing:
        ...
        if _sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running OpenAmer sandbox.  They will start "
    ...
    # `openamer egress reload` apply future ruleset changes without a
    ...
    # rotation), running `openamer egress setup` again WITHOUT
    ...
    if do_restart:
        try:
            ...
            console.print(
                "  Run [cyan]openamer egress start[/cyan] manually before "
        ...
        console.print(
            ...
            "changed.  Run [cyan]openamer egress restart[/cyan] (or "
    ...
    console.print(
        "  Start:   [cyan]openamer egress start[/cyan]\n"
        "  Restart: [cyan]openamer egress restart[/cyan]  (after any re-setup)\n"
        "  Reload:  [cyan]openamer egress reload[/cyan]   (apply ruleset edits "
        "in-place, no restart)\n"
        "  Status:  [cyan]openamer egress status[/cyan]\n"
        "  Stop:    [cyan]openamer egress stop[/cyan]\n"
        "  Disable: [cyan]openamer egress disable[/cyan]"
    )
    return 0
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print(
            "[yellow]proxy.enabled is false — run `openamer egress setup` "
            "first.[/yellow]"
        )
        return 1
    ...
    if credential_source == "bitwarden" and not refresh_bw:
        if bool(proxy_cfg.get("allow_env_fallback", False)):
            ...
            console.print(
                ...
                "back to env credentials with `openamer egress setup "
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            ...
            console.print(
                ...
                "`openamer egress setup --no-bitwarden` to switch back to "
        ...
        if not (bw_cfg or {}).get("project_id"):
            ...
            console.print(
                "  Run `openamer secrets bitwarden setup` to configure the "
                "project, or switch back via `openamer egress setup "
                "--no-bitwarden`."
            )
            return 1
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    ``openamer egress restart`` instead: the daemon reads real credentials
    ...
    console.print(
        ...
        "still need `openamer egress restart` — the daemon reads real "
...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    if bool(proxy_cfg.get("enabled")) and not status.configured:
        lines.extend(["", "Next: run `openamer egress setup` to mint tokens and write proxy.yaml."])
    elif bool(proxy_cfg.get("enabled")) and not (status.pid and status.listening):
        lines.extend(["", "Next: run `openamer egress start` before launching Docker sandboxes."])
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            ...
            "[cyan]openamer egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print(
            "[yellow](no config generated — run `openamer egress setup`)[/yellow]"
        )
        return 1
