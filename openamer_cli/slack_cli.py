"""``openamer slack ...`` CLI subcommands.

Today only ``openamer slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ openamer slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of OpenAmer. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a OpenAmer deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps OpenAmer on Slack's older Assistant messaging
    ...
    """
    from openamer_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with OpenAmer in threads and DMs.",
        }
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your OpenAmer agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``openamer_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$OPENAMER_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "OpenAmer")
    ...
    """
    name = getattr(args, "name", None) or "OpenAmer"
    description = getattr(args, "description", None) or "Your OpenAmer agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "openamer slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"openamer slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "openamer slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "openamer slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your OpenAmer app\n"
