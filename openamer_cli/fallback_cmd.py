"""
openamer fallback — manage the fallback provider chain.
...
Subcommands:
  openamer fallback [list]   Show the current fallback chain (default when no subcommand)
  openamer fallback add      Pick provider + model via the same picker as `openamer model`,
                           then append the selection to the chain
  openamer fallback remove   Pick an entry to delete from the chain
  openamer fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.openamer/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `openamer model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  openamer fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/openamer/openamer/blob/main/website/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `openamer model`, then append the selection to the chain."""
    from openamer_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `openamer model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `openamer fallback list` to view, or `openamer fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``openamer fallback [subcommand]``."""
