"""Implementation of the ``openamer bundles`` CLI subcommand.

Mirrors the structure of ``openamer_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `openamer bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  openamer bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``openamer bundles`` argparse tree.
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `openamer bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``openamer bundles <subcommand>`` to the right handler."""
