...
Pure logic: walks a OpenAmer config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `openamer doctor` and a future `openamer migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a OpenAmer config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a OpenAmer config and return retirement issues.
...
def apply_migration(
    ...
    from openamer_cli.config import require_readable_config_before_write
