"""Per-profile view of ``openamer_cli.models``' module-level catalog slots.
...
files. In a multiplexed gateway every turn runs under a OPENAMER_HOME override, so a single module slot
...
from openamer_constants import get_openamer_home_override, openamer_home_key
...
def profile_slot_get(module: Any, attr: str, default: Any = None) -> Any:
    ...
    if get_openamer_home_override() is None:
        return getattr(module, attr)
    return _SLOTS_BY_HOME.get((openamer_home_key(), attr), default)
...
def profile_slot_set(module: Any, attr: str, value: Any) -> None:
    if get_openamer_home_override() is None:
        ...
        _SLOTS_BY_HOME[(openamer_home_key(), attr)] = value
