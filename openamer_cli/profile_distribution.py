"""Profile distributions — shareable, packaged OpenAmer profiles via git.

A distribution is a OpenAmer profile published as a git repository (or
...
* ``openamer profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``openamer skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``openamer profile``, not a parallel tree):

    openamer profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "openamer_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".openamer_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "openamer-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            openamer_requires=str(data.get("openamer_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.openamer_requires:
            out["openamer_requires"] = self.openamer_requires
        if self.author:
...
def check_openamer_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires OpenAmer {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``openamer profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a OpenAmer profile distribution."
            )
...
def plan_install(
    ...
    from openamer_cli import __version__ as openamer_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a OpenAmer distribution."
        )

    # Version check up-front so we fail fast
    check_openamer_requires(manifest.openamer_requires, openamer_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.openamer).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="openamer_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `openamer profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`openamer profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="openamer_dist_update_") as tmp:
        plan = plan_install(
