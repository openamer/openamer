"""Normalized OpenAmer Portal account entitlement helpers."""
...
OpenAmerAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]

# Free tool-pool coverage categories. Kept byte-for-byte aligned with the
# Portal's TOOL_COVERAGE_CATEGORIES (openamer-account-service
...
_account_info_cache: tuple[str, float, "OpenAmerPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class OpenAmerPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class OpenAmerPaidServiceAccessInfo:
...
@dataclass(frozen=True)
class OpenAmerPortalAccountInfo:
    logged_in: bool
    source: OpenAmerAccountInfoSource
    fresh: bool
    ...
    openamer_client: Optional[str] = None
    ...
    subscription: Optional[OpenAmerPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[OpenAmerPaidServiceAccessInfo] = None
    tool_access: Optional[OpenAmerToolAccessInfo] = None
...
def openamer_portal_billing_url(account_info: Optional[OpenAmerPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized OpenAmer account snapshot."""
    try:
    ...
    if not isinstance(base, str) or not base.strip():
        base = DEFAULT_openamer_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def openamer_portal_topup_url(account_info: Optional[OpenAmerPortalAccountInfo] = None) -> str:
    ...
    """
    base_billing = openamer_portal_billing_url(account_info)  # {base}/billing
...
def format_openamer_portal_entitlement_message(
    account_info: Optional[OpenAmerPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
    coverage_category: Optional[str] = None,
) -> Optional[str]:
    """Return user-facing guidance for a missing OpenAmer tool-gateway entitlement.
    ...
    """
    billing_url = openamer_portal_billing_url(account_info)
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current OpenAmer Portal "
    ...
    if account_info is None:
        return (
            f"OpenAmer could not verify your OpenAmer Portal entitlement, so {capability} "
            f"is unavailable. Run `openamer model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"OpenAmer inference credentials are configured, but OpenAmer cannot verify "
                f"your OpenAmer Portal paid access for {capability}. Log in with "
                f"`openamer model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to OpenAmer Portal to use {capability}: run `openamer model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"OpenAmer could not verify your OpenAmer Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `openamer model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"OpenAmer could not find a OpenAmer Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `openamer model` to "
            f"authenticate again; if the problem persists, contact OpenAmer support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `openamer model` to refresh OpenAmer."
        return message

    return (
        f"Your OpenAmer Portal account does not currently have paid service access, "
...
def _no_paid_access_message(
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your OpenAmer Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current OpenAmer Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your OpenAmer Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your OpenAmer Portal account has no usable paid credits{credit_detail}, so "
...
def _fresh_account_info(
    *,
    ...
    try:
        from openamer_cli.auth import get_provider_auth_state, resolve_openamer_access_token

        access_token = resolve_openamer_access_token()
        ...
        payload = _fetch_openamer_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[OpenAmerPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque OpenAmer keys."""
    try:
        entry = _select_openamer_pool_entry()
        if entry is None:
            return None
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_openamer_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _select_openamer_pool_entry() -> Optional[Any]:
...
def _fetch_openamer_account_info(
    access_token: str,
    portal_base_url: Optional[str] = None,
) -> dict[str, Any]:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from openamer_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = OpenAmerPaidServiceAccessInfo(
        allowed=paid_access,
    ...
    return OpenAmerPortalAccountInfo(
        ...
        openamer_client=_coerce_str(claims.get("openamer_client")),
        portal_base_url=portal_base_url,
...
def _tool_access_from_value(value: Any) -> Optional[OpenAmerToolAccessInfo]:
    """Parse a Portal ``tool_access`` object (from the JWT claim or the account
    API) into :class:`OpenAmerToolAccessInfo`. Fails closed: a non-object value
    ...
    return OpenAmerToolAccessInfo(enabled=enabled, coverage=coverage)


def _subscription_from_payload(value: Any) -> Optional[OpenAmerPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return OpenAmerPortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[OpenAmerPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return OpenAmerPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
