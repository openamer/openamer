"""``openamer logs`` — view and filter OpenAmer log files.
...
Usage examples::
    ...
    openamer logs                    # last 50 lines of agent.log
    openamer logs -f                 # follow agent.log in real time
    openamer logs errors             # last 50 lines of errors.log
    openamer logs gateway -n 100    # last 100 lines of gateway.log
    openamer logs gui -f            # follow gui.log (dashboard/pty/ws)
    openamer logs desktop -f        # follow desktop.log (Electron app boot/backend)
    openamer logs --level WARNING    # only WARNING+ lines
    openamer logs --session abc123   # filter by session ID substring
    openamer logs --component tools  # only tool-related lines
    openamer logs --since 1h         # lines from the last hour
    openamer logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_openamer_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when OpenAmer runs — try 'openamer chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_openamer_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_openamer_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_openamer_home()}/logs/")
        return

    print(f"Log files in {display_openamer_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'openamer chat' to generate logs)")
