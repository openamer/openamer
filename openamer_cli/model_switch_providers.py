...
Split out of ``openamer_cli/model_switch.py``; every moved name is re-imported there so
``openamer_cli.model_switch.<name>`` keeps resolving (and monkeypatching) as before."""
...
from openamer_cli.providers import custom_provider_aliases, custom_provider_slug, get_label
from utils import base_url_host_matches

# Log-record parity with the origin module.
logger = logging.getLogger("openamer_cli.model_switch")
...
def _save_discovered_models_to_config(
    ...
    from openamer_cli.model_switch import _extra_headers_from_config
...
def _discovered_catalog_stale(entry: dict, model_ids: list[str]) -> bool:
    ...
    A mapping OpenAmer itself discovered (entry flag or legacy in-mapping sentinel) is ours to
...
def _fetch_picker_live_models(
    ...
    from openamer_cli.models import _get_ollama_native_headers, cached_fetch_api_models, fetch_api_models
    from openamer_cli.models_local import (
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    from openamer_cli.model_switch import list_authenticated_providers
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from openamer_cli.models import update_provider_cache_entry
...
def _iter_builtin_candidates(models_dev_data: dict, excluded: set, seen: set):
    """Yield ``(openamer_id, mdev_id, pconfig, env_vars)`` for section-1 rows.
    ...
    from openamer_cli.auth import PROVIDER_REGISTRY, is_runtime_provider_routable
    from openamer_cli.models import _AGGREGATOR_PROVIDERS
    from openamer_cli.providers import ALIASES
    for openamer_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        alias_target = ALIASES.get(openamer_id)
        if alias_target and alias_target != openamer_id and alias_target in _AGGREGATOR_PROVIDERS:
            continue
        try:
            from providers import get_provider_profile
            prof = get_provider_profile(openamer_id)
            if prof is not None and prof.name != openamer_id:
                continue
        except Exception:
            pass
        if openamer_id.lower() in seen:
            continue
        if openamer_id.lower() in excluded or mdev_id.lower() in excluded:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(openamer_id)
        if (pconfig and pconfig.auth_type != "api_key") or not is_runtime_provider_routable(openamer_id):
            continue
        env_vars = list(pconfig.api_key_env_vars) if pconfig and pconfig.api_key_env_vars else pdata.get("env", [])
        if isinstance(env_vars, list):
            yield openamer_id, mdev_id, pconfig, env_vars
...
def _raw_pool_usable(openamer_id: str) -> bool:
    """Section-1 pool check: only consult the pool when auth.json lists a raw entry."""
    try:
        from openamer_cli.auth import _load_auth_store
        store = _load_auth_store()
        if store and store.get("credential_pool", {}).get(openamer_id):
            return _credential_pool_is_usable(openamer_id, raw_pool_present=True)
    except Exception:
        pass
    return False
...
def _overlay_has_env_creds(pid: str, openamer_slug: str, overlay, read_env) -> bool:
...
def _live_or_curated_ids(slug: str, curated: dict, *fallback_keys: str, merge_models_dev: bool = True) -> list:
    """``cached_provider_model_ids`` (the SAME disk-cached list ``openamer model`` builds), falling
    back to the curated list (merged with models.dev for preferred providers) when live is empty."""
    from openamer_cli.models import _MODELS_DEV_PREFERRED, _merge_with_models_dev, cached_provider_model_ids
...
def _nous_picker_model_ids(curated: dict, force_fresh_nous_tier: bool) -> list:
    ...
    try:
        from openamer_cli.models_pricing import get_pricing_for_provider
        from openamer_cli.models import (
        ...
        pass
    try:
        from openamer_cli.models_pricing import nous_policy_allowed_ids, restrict_to_nous_policy
...
def _free_tier_nous_row(row: dict) -> dict | None:
    ...
    from openamer_cli import anon_auth
...
def _display_prefix(name: str) -> str:
    """Text before the per-model separator OpenAmer's own writer uses ("—" / " - ")."""
...
def _discover_endpoint_models(
    ...
    if probe_live:
        ...
        try:
            from openamer_cli.models import cached_fetch_api_models
...
def _collect_authed_provider_slugs(
    ...
    (heavier detection)."""
    from openamer_cli.model_switch import _scoped_key_env
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from openamer_cli.auth import PROVIDER_REGISTRY
    from openamer_cli.providers import OPENAMER_OVERLAYS
    from openamer_cli.models import CANONICAL_PROVIDERS
    ...
    for openamer_id, _mdev_id, _pconfig, env_vars in _iter_builtin_candidates(models_dev_data, excluded_set, seen):
        if _any_env(env_vars, _scoped_key_env) or _raw_pool_usable(openamer_id):
            _emit(openamer_id, openamer_id)

    mdev_to_openamer = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in OPENAMER_OVERLAYS.items():
        openamer_slug = mdev_to_openamer.get(pid, pid)
        if _skip(seen, excluded_set, pid, openamer_slug) or overlay.auth_type == "aws_sdk":
            continue
        if (
            _overlay_has_env_creds(pid, openamer_slug, overlay, _scoped_key_env)
            or _auth_store_has_provider(pid, openamer_slug) or _pool_usable(openamer_slug)):
            _emit(openamer_slug, pid, openamer_slug)
...
class _PickerBuild:
    ...
    models.dev, 2 OpenAmer-only overlays, 2b canonical providers missed by 1/2, 3 ``providers:``
    entries + 3b the bare active custom endpoint, 4 ``custom_providers:`` entries. Row-builder
    imports of ``openamer_cli.auth/models`` stay lazy so tests can patch those modules."""
...
def _lap_builtin_rows(b: _PickerBuild, data: dict, user_providers: dict) -> None:
    ...
    from openamer_cli.model_switch import _declared_model_ids, _scoped_key_env
    from agent.models_dev import get_provider_info
    for openamer_id, mdev_id, pconfig, env_vars in _iter_builtin_candidates(data, b.excluded, b.seen_slugs):
        ...
        if not (_any_env(env_vars, _scoped_key_env) or _raw_pool_usable(openamer_id)):
            continue
        model_ids = _live_or_curated_ids(openamer_id, b.curated)
        ...
        configured = user_providers.get(openamer_id) if isinstance(user_providers, dict) else None
        ...
        b.add_builtin_row(
            openamer_id, display_name, b.current_provider in (openamer_id, mdev_id), model_ids, "built-in")


def _overlay_has_creds(b: _PickerBuild, pid: str, openamer_slug: str, overlay) -> bool:
    ...
    if overlay.auth_type == "aws_sdk":
        has_creds = _has_aws_sdk_creds_for_listing(openamer_slug, b.current_provider)
    else:
        from openamer_cli.model_switch import _scoped_key_env
        has_creds = _overlay_has_env_creds(pid, openamer_slug, overlay, _scoped_key_env)
    ...
    if not has_creds and overlay.auth_type == "external_process":
        try:
            ...
            _ext_status = get_auth_status(openamer_slug) or {}
    ...
    has_creds = has_creds or _auth_store_has_provider(pid, openamer_slug)
    if not has_creds:
        ...
        try:
            if _credential_pool_is_usable(openamer_slug):
                ...
                try:
                    from agent.credential_pool import load_pool
                    has_creds = load_pool(openamer_slug).has_credentials()
                except Exception:
                    pass
        except Exception as exc:
    ...
    if not has_creds and openamer_slug == "anthropic":
        ...
        try:
            from agent.anthropic_credentials import read_claude_code_credentials, read_openamer_oauth_credentials
            openamer_creds = read_openamer_oauth_credentials()
            cc_creds = read_claude_code_credentials()
            if (openamer_creds and openamer_creds.get("accessToken")) or (cc_creds and cc_creds.get("accessToken")):
                has_creds = True
        except Exception as exc:
...
def _lap_overlay_rows(b: _PickerBuild, data: dict) -> None:
    """Section 2: OpenAmer-only providers (nous, openai-codex, copilot, opencode-go, ...)."""
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from openamer_cli.providers import OPENAMER_OVERLAYS

    # OPENAMER_OVERLAYS keys may be models.dev IDs ("github-copilot") while config.yaml uses
    # OpenAmer IDs ("copilot").
    mdev_to_openamer = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in OPENAMER_OVERLAYS.items():
        openamer_slug = mdev_to_openamer.get(pid, pid)
        if _skip(b.seen_slugs, b.excluded, pid, openamer_slug):
            continue
        if not _overlay_has_creds(b, pid, openamer_slug, overlay):
            continue
        if openamer_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            from openamer_cli.models import cached_provider_model_ids
            model_ids = cached_provider_model_ids(openamer_slug)
        elif overlay.auth_type == "aws_sdk":
            model_ids = _aws_live_or_curated_ids(openamer_slug, b.curated, openamer_slug, pid)
            ...
            tier_row = _free_tier_nous_row({"name": get_label(openamer_slug), "models": []})
            ...
            model_ids = _live_or_curated_ids(openamer_slug, b.curated, openamer_slug, pid)
        b.add_builtin_row(
            openamer_slug, get_label(openamer_slug), b.current_provider in (openamer_slug, pid), model_ids, "openamer")
...
def _lap_user_provider_rows(b: _PickerBuild, user_providers: dict) -> None:
    ...
    from openamer_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from openamer_cli.config import coerce_provider_id, is_provider_enabled
...
def _lap_custom_provider_rows(b: _PickerBuild, custom_providers: list) -> None:
    ...
    their own rows."""
    from openamer_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from openamer_cli.config import coerce_provider_id
...
def _build_curated_lists(current_provider: str, current_base_url: str, current_model: str) -> dict[str, list[str]]:
    """Curated model lists keyed by openamer provider id, plus the dynamic ones (nous manifest,
    Ollama Cloud, LM Studio live probe)."""
    from openamer_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS, get_curated_nous_model_ids
    ...
    if "lmstudio" not in curated and (os.environ.get("LM_API_KEY") or os.environ.get("LM_BASE_URL") or is_current_lmstudio):
        from openamer_cli.models_local import fetch_lmstudio_models
...
def list_authenticated_providers(
    ...
    ("built-in", "openamer", "canonical", "user-config", "model-config").
    ...
    from openamer_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # as (openamer_id / mdev_id / canonical slug).
    b = _PickerBuild(
    ...
    if current_model:
        for row in results:
            ...
            if current_model not in models:
                from openamer_cli.models import _model_requires_account_discovery
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    try:
        from openamer_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    OpenRouter's list is replaced with :func:`openamer_cli.models.fetch_openrouter_models` (curated
    ...
    from openamer_cli.model_switch import list_authenticated_providers
    from openamer_cli.models import fetch_openrouter_models
