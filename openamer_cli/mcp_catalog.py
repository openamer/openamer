"""MCP catalog — curated, OpenAmer-approved MCP servers shipped with the repo.
...
entries via ``openamer mcp catalog`` or the interactive ``openamer mcp picker``,
and install them with ``openamer mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into openamer-agent. Presence in the
  ``optional-mcps/`` directory = OpenAmer approval. No community tier, no trust
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``openamer mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.openamer/.env`` (the
...
from openamer_constants import get_openamer_home, get_optional_mcps_dir
...
from openamer_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.openamer/.env.
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this OpenAmer install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this OpenAmer understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their OpenAmer is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this OpenAmer
        understands. Update OpenAmer to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_openamer_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.openamer/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.openamer/.env via save_env_value()."""
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``openamer mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `openamer mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `openamer mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `openamer mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `openamer mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `openamer auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`openamer auth {entry.auth.provider}` if you have not "
    ...
    from openamer_cli.mcp_config import _save_mcp_server
    ...
    print(color(
        ...
        f"Start a new OpenAmer session to load its tools.",
        Colors.GREEN,
    ))
