"""
Cron subcommand for openamer CLI.
...
# commands at execution time when ``_OPENAMER_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: openamer gateway install", Colors.DIM))
    print(color("                    sudo openamer gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  openamer cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'openamer cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: openamer gateway restart")
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec openamer "
                        "openamer cron ...`). Fix ownership to match the "
        ...
        print("    openamer gateway install    # Install as a user service")
        print("    sudo openamer gateway install --system  # Linux servers: boot-time system service")
        print("    openamer gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: openamer cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)
