"""MCP picker — interactive `openamer mcp picker` (also the default `openamer mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``openamer mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `openamer plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from openamer_cli.cli_output import prompt_yes_no
from openamer_cli.curses_ui import curses_single_select
from openamer_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new OpenAmer session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from openamer_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `openamer mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: openamer mcp install <name>    Picker: openamer mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their OpenAmer is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer OpenAmer — run `openamer update` "
...
def show_catalog() -> None:
    """`openamer mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`openamer mcp picker` (and default `openamer mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`openamer mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `openamer mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
