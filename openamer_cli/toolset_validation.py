...
the tool registry or launching OpenAmer (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``openamer-cli`` to the non-existent ``openamer``. ``resolve_toolset('openamer')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``openamer-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``openamer`` instead of ``openamer-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        for name in names:
            ...
            if is_valid_toolset(name):
                valid_count += 1
                continue
            suggestion = f"openamer-{platform}"
            hint = (
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `openamer tools` to reconfigure."
        )
    return warnings
