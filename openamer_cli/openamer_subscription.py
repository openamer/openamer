"""Helpers for OpenAmer subscription managed-tool capabilities."""
...
from openamer_cli.config import get_env_value, load_config
...
# Maps a tools_config provider's ``managed_openamer_feature`` to the tool-pool
# coverage category (openamer_cli.openamer_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `openamer tools` picker scope its entitlement gate to the selected backend, so a
# free-tool-pool user is allowed image gen but denied video gen at select time —
# consistent with the per-category feature gates in get_openamer_subscription_features.
...
class OpenAmerSubscriptionFeatures:
    ...
    @property
    def web(self) -> OpenAmerFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> OpenAmerFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> OpenAmerFeatureState:
        return self.features["tts"]

    @property
    def stt(self) -> OpenAmerFeatureState:
        return self.features["stt"]

    @property
    def browser(self) -> OpenAmerFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> OpenAmerFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> OpenAmerFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[OpenAmerFeatureState]:
...
def _local_stt_backend_available() -> bool:
    ...
    ``apply_openamer_managed_defaults`` from flipping a working local setup
    to the managed gateway.
    """
    if get_env_value("OPENAMER_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_openamer_subscription_features(
    config: Optional[Dict[str, object]] = None,
    *,
    force_fresh: bool = False,
) -> OpenAmerSubscriptionFeatures:
    if config is None:
    ...
    openamer_auth_present = bool(account_info and account_info.logged_in)
    ...
    # requires `pip install faster-whisper`. For OpenAmer subscribers we'd
    # rather route through the managed OpenAI audio gateway — see
    # apply_openamer_managed_defaults below.
    ...
    # Tool Gateway via `openamer model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("OPENAMER_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("OPENAMER_LOCAL_STT_COMMAND"))
    ...
    features = {
        ...
        "image_gen": OpenAmerFeatureState(
            key="image_gen",
            ...
            current_provider="FAL" if direct_fal else ("OpenAmer Subscription" if image_managed else ""),
            explicit_configured=direct_fal,
        ),
        "video_gen": OpenAmerFeatureState(
            key="video_gen",
            ...
            current_provider="FAL" if direct_fal_video else ("OpenAmer Subscription" if video_managed else ""),
            explicit_configured=direct_fal_video,
        ),
        "tts": OpenAmerFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
...
def apply_openamer_managed_defaults(
    ...
    *,
    ...
    # (requires `pip install faster-whisper`); for OpenAmer subscribers we
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    All lists are empty when the user is not a paid OpenAmer subscriber or
    is not using OpenAmer as their provider.
    """
    ...
    # Quick provider check without the heavy get_openamer_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "openamer":
        return [], [], []
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from openamer_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    try:
        account_info = get_openamer_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
    ...
    source_label = "free tool pool" if pool_only else "OpenAmer subscription"
    ...
    if pool_only:
        title = "Your free OpenAmer tool pool — pick the tools to enable:"
    else:
        title = (
            "Your OpenAmer subscription includes the Tool Gateway — "
...
# ---------------------------------------------------------------------------
# Inline OpenAmer Portal login for the Tool Gateway picker (`openamer tools`)
# ---------------------------------------------------------------------------


def ensure_openamer_portal_access(
    *,
    capability: str = "the OpenAmer Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the OpenAmer Tool Gateway, logging in if
    needed.

    Used by ``openamer tools`` when a user selects a OpenAmer-managed Tool Gateway
    backend (e.g. "Firecrawl (OpenAmer Portal)").  Unlike ``openamer model``'s OpenAmer
    login, this:
    ...
    It only performs the OpenAmer Portal device-code OAuth (when the user isn't
    ...
    if info is None or not info.logged_in:
        if not _run_openamer_portal_login_only(capability=capability):
            return False
        try:
            info = get_openamer_portal_account_info(force_fresh=True)
        except Exception:
...
def _run_openamer_portal_login_only(*, capability: str) -> bool:
    """Run the OpenAmer Portal device-code OAuth and persist credentials only.
    ...
    try:
        from openamer_cli.auth import (
            _auth_store_lock,
            _load_auth_store,
            _openamer_device_code_login,
            _read_shared_openamer_state,
            _save_auth_store,
            _save_provider_state,
            _sync_openamer_pool_from_auth_store,
            _try_import_shared_openamer_state,
            _write_shared_openamer_state,
        )
    except Exception as exc:  # pragma: no cover - defensive
        print(f"  Could not start OpenAmer Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a OpenAmer Portal login.")
    try:
        proceed = input("  Log in to OpenAmer Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped OpenAmer Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to OpenAmer.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing OpenAmer OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                auth_state = _try_import_shared_openamer_state(timeout_seconds=15.0)
        ...
        _sync_openamer_pool_from_auth_store()
        print("  OpenAmer Portal login successful.")
        return True
    except KeyboardInterrupt:
        print("\n  Login cancelled.")
        return False
    except SystemExit:
        # _openamer_device_code_login raises SystemExit on subscription_required;
        # it already printed billing guidance.
        return False
    except Exception as exc:
        print(f"  OpenAmer Portal login failed: {exc}")
        return False
