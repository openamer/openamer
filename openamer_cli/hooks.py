"""openamer hooks — inspect and manage shell-script hooks.

Usage::

    openamer hooks list
    openamer hooks test <event> [--for-tool X] [--payload-file F]
...
def hooks_command(args) -> None:
    """Entry point for ``openamer hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: openamer hooks {list|test|revoke|doctor}")
        print("Run 'openamer hooks --help' for details.")
        return
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `openamer hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `openamer hooks test` and `openamer hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (OpenAmer wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `openamer hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `openamer hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `openamer hooks doctor`.")
