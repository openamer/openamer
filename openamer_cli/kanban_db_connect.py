...
def _resolve_busy_timeout_ms() -> int:
    ...
    return _kb._env_int("OPENAMER_KANBAN_BUSY_TIMEOUT_MS", DEFAULT_BUSY_TIMEOUT_MS, minimum=1)
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``openamer gateway run --replace`` / ``gateway restart`` invoked from a shell
...
def repair_db(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> RepairResult:
    ...
    ``openamer kanban repair`` can pick its exit code. ``OperationalError``
...
def connect(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> sqlite3.Connection:
    ...
    :func:`kanban_db_path` (``OPENAMER_KANBAN_DB`` -> ``OPENAMER_KANBAN_BOARD`` ->
    ...
    with _cross_process_init_lock(path):
        ...
        # See #12508.
        from openamer_state import preflight_db_writability
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    if getattr(conn, "in_transaction", False):
        ...
        savepoint = f"openamer_nested_{secrets.token_hex(8)}"
