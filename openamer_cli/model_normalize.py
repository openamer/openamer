...
- **Aggregators** (OpenRouter, OpenAmer, AI Gateway, Kilo Code) need
...
- **DeepSeek** accepts ``deepseek-chat`` (V3), ``deepseek-reasoner``
  ...
  OpenAmer revisions folded every non-reasoner input into
...
# slug used by aggregator APIs (OpenRouter, OpenAmer, etc.).
#
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to OpenAmer' canonical ids."""
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical OpenAmer provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``openamer_cli.models.normalize_provider()``.
    ...
    if provider in {"copilot", "copilot-acp"}:
        try:
            from openamer_cli.models import normalize_copilot_model_id
