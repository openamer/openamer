...
sidebar_position: 4
title: "贡献指南"
description: "如何为 OpenAmer Agent 做贡献 — 开发环境配置、代码风格、PR 流程"
---

# 贡献指南

感谢您为 OpenAmer Agent 做贡献！本指南涵盖开发环境配置、代码库结构说明以及 PR 合并流程。
...
- 构建自定义/本地工具而不修改 OpenAmer 核心？从 [构建 OpenAmer 插件](../developer-guide/plugins/index.md) 开始
- 为 OpenAmer 本身构建新的内置核心工具？从 [添加工具](./adding-tools.md) 开始
...
对大多数贡献者来说，最好的开发启动方式和用户安装方式相同：运行标准安装器，然后在它克隆出的仓库里开发。安装器会创建 OpenAmer venv、配置 `openamer` 命令、为 `openamer update` 写入安装方式标记，并把完整 git 项目克隆到 `$HERMES_HOME/openamer-agent`（通常是 `~/.openamer/openamer-agent`）。这样你的开发环境会和 CLI、updater、lazy dependency installer、gateway、docs 默认假设的布局一致。
...
```bash
# 标准安装器已经把 `openamer` 放到了 PATH 上。
openamer doctor
openamer chat -q "Hello"
```

如果你使用了手动克隆备用路径，可以在 checkout 中运行 `./openamer`，或显式把这个 clone 的 venv 链接到 PATH：
...
OpenAmer 官方支持 **Linux、macOS、WSL2 以及原生 Windows（通过 PowerShell 安装）**。原生 Windows 使用 [Git for Windows](https://git-scm.com/download/win) 提供的 Git Bash 执行 shell 命令。部分功能依赖 POSIX 内核原语，已做条件限制：dashboard 内嵌的 PTY 终端面板（`/chat` 标签页）仅支持 WSL2。如果您主要在 Windows 上开发，推送前请运行 Windows 陷阱（footgun）lint（`scripts/check-windows-footguns.py`）。
...
2. **手动测试**：运行 `openamer` 并验证您修改的代码路径
