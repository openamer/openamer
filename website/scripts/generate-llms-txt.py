#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the OpenAmer docs site.
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use OpenAmer as a Python Library", None),
        ("guides/use-mcp-with-openamer", "Use MCP with OpenAmer", None),
        ("guides/use-voice-mode-with-openamer", "Use Voice Mode with OpenAmer", None),
        ("guides/use-soul-with-openamer", "Use SOUL.md with OpenAmer", None),
        ("guides/build-a-openamer-plugin", "Build a OpenAmer Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with OpenAmer"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# OpenAmer Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by OpenAmer. A terminal-native "
        ...
        "backends. Works with OpenAmer Portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/openamer/"
        "openamer-agent/main/scripts/install.sh | bash`  "
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# OpenAmer Agent — Full Documentation\n",
        (
            "This file is the entire OpenAmer Agent documentation concatenated for LLM "
