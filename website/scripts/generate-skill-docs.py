...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/openamer-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    openamer_meta = (fm.get("metadata") or {}).get("openamer") or {}
    tags = openamer_meta.get("tags") or []
    related = openamer_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `openamer skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with OpenAmer Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "OpenAmer ships with a large built-in skill library copied into `~/.openamer/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "OpenAmer also syncs bundled skills on `openamer update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.openamer/skills/` tree, it is still shipped with OpenAmer; restore it with `openamer skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with openamer-agent — install via openamer skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with openamer-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
