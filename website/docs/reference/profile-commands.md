...
This page covers all commands related to [OpenAmer profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `openamer profile` without a subcommand shows help.
...
## `openamer profile use`

```bash
openamer profile use <name>
```

Sets `<name>` as the active profile. All subsequent `openamer` commands (without `-p`) will use this profile.
...
openamer profile use default
```

## `openamer profile create`

```bash
openamer profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `openamer profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `openamer update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.openamer`), use `openamer skills opt-out` / `openamer skills opt-in`. |
...
openamer profile create mybot

# Clone config only from current profile
openamer profile create work --clone

# Clone everything from current profile
openamer profile create backup --clone-all

# Clone config from a specific profile
openamer profile create work2 --clone-from work

# Clone everything from a specific profile
openamer profile create work2-backup --clone-from work --clone-all
```

## `openamer profile describe`

```bash
openamer profile describe [<name>] [options]
```
...
openamer profile describe researcher

# Set it explicitly
openamer profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
openamer profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
openamer profile describe --all --auto
```

## `openamer profile delete`

```bash
openamer profile delete <name> [options]
```
...
```bash
openamer profile delete mybot
openamer profile delete mybot --yes
```
...
## `openamer profile show`

```bash
openamer profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's OpenAmer home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ openamer profile show work
Profile: work
Path:    ~/.openamer/profiles/work
...
## `openamer profile alias`

```bash
openamer profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your OpenAmer installation.
...
```bash
openamer profile alias work
# Creates/updates ~/.local/bin/work

openamer profile alias work --name mywork
# Creates ~/.local/bin/mywork

openamer profile alias work --remove
# Removes the wrapper script
```

## `openamer profile rename`

```bash
openamer profile rename <old-name> <new-name>
```
...
```bash
openamer profile rename mybot assistant
# ~/.openamer/profiles/mybot → ~/.openamer/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `openamer profile export`

```bash
openamer profile export <name> [options]
```
...
```bash
openamer profile export work
# Creates work.tar.gz in the current directory

openamer profile export work -o ./work-2026-03-29.tar.gz
```

## `openamer profile import`

```bash
openamer profile import <archive> [options]
```
...
```bash
openamer profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

openamer profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`openamer profile export` / `import` are still the right commands for
...
### `openamer profile install`

```bash
openamer profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `openamer -p telemetry`). |
...
openamer profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
openamer profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
openamer profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
openamer profile install ./telemetry/
```

### `openamer profile update`

```bash
openamer profile update <name> [--force-config] [--yes]
```
...
### `openamer profile info`

```bash
openamer profile info <name>
```

Prints the profile's distribution manifest — name, version, required
OpenAmer version, author, env var requirements, the source URL/path, and
...
`openamer profile list` also shows the distribution name and version in a
`Distribution` column, and `openamer profile show <name>` / `delete <name>`
...
openamer profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
openamer profile install https://github.com/your-org/internal-assistant.git
```
...
`openamer_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
OpenAmer version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host OpenAmer can clone from.
3. Tell recipients to run `openamer profile install <your-repo-url>`.
...
## `openamer -p` / `openamer --profile`

```bash
openamer -p <name> <command> [options]
openamer --profile <name> <command> [options]
```

Global flag to run any OpenAmer command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
openamer -p work chat -q "Check the server status"
openamer --profile dev gateway start
openamer -p personal skills list
openamer -p work config edit
```

## `openamer completion`

```bash
openamer completion <shell>
```
...
- `openamer profile <TAB>` — subcommands (list, use, create, etc.)
- `openamer profile use <TAB>` — profile names
- `openamer -p <TAB>` — profile names
