...
description: "Frequently asked questions and solutions to common issues with OpenAmer Agent"
---
...
### What LLM providers work with OpenAmer?

OpenAmer Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **[your hosted provider](/integrations/nous-portal)** — the OpenAmer project's subscription gateway — 300+ models plus web/image/TTS/browser through one OAuth login (recommended for newcomers)
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `openamer auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `openamer model` or by editing `~/.openamer/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run OpenAmer in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run OpenAmer inside WSL2
...
- let OpenAmer use the resulting MCP browser tools

This is more reliable than trying to force OpenAmer core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with OpenAmer](../guides/use-mcp-with-openamer.md#wsl2-bridge-openamer-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). OpenAmer Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.openamer/`.

### Can I use it offline / with local models?

Yes. Run `openamer model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← OpenAmer minimum; set this to match your server's actual context window
```
...
OpenAmer persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in OpenAmer — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::
...
OpenAmer Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same OpenAmer Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use OpenAmer programmatically:
...
#### `openamer: command not found` after installation
...
**Cause:** OpenAmer requires Python 3.11 or newer.
...
**Cause:** OpenAmer builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when OpenAmer runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** OpenAmer auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.openamer/config.yaml`:
...
**Solution:** Exit your session and use `openamer model` from your terminal to add new providers:

```bash
# Exit the OpenAmer chat session first (Ctrl+C or /quit)
...
After adding a new provider via `openamer model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `openamer model` (from terminal) |
| Enter/change API keys | `openamer model` (from terminal) |
...
# Or set directly
openamer config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.openamer/.env` for conflicting entries.
:::
...
- Using `openamer chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or OpenAmer detected the wrong context length for your model.
...
openamer chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, OpenAmer may have the wrong context length for your model. Check what it detected:
...
**Cause:** OpenAmer detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — OpenAmer never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `openamer chat`
...
cat ~/.openamer/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `openamer gateway setup`
- Check gateway logs: `cat ~/.openamer/logs/gateway.log | tail -50`
...
Configure in `~/.openamer/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.openamer/openamer-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `openamer gateway start` fails
...
tmux new -s openamer 'openamer gateway run'
# Reattach later: tmux attach -t openamer

# Option 3: Background via nohup
nohup openamer gateway run > ~/.openamer/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'openamer gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `openamer gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
openamer gateway install    # Re-snapshots your current PATH
openamer gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.openamer.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `openamer chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `openamer chat -t "terminal"`
...
cd ~/.openamer/openamer-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.openamer/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
openamer config show | grep -A 12 mcp_servers

# Restart OpenAmer or reload MCP after config changes
openamer chat
```

See also:
- [MCP (Model Context Protocol)](/user-guide/features/mcp)
- [Use MCP with OpenAmer](/guides/use-mcp-with-openamer)
...
:::warning
If an MCP server crashes mid-request, OpenAmer will report a timeout. Check the server's own logs (not just OpenAmer logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `openamer profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

### What happens when I run `openamer update`?

`openamer update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `openamer update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.openamer/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** OpenAmer can route subagents to a different model automatically. Set this in `~/.openamer/config.yaml`:
...
Now when you tell OpenAmer "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can OpenAmer do this?

**Current limitation:** OpenAmer profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, OpenAmer reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `openamer skills config` settings don't seem to take effect.

**Solution:** Use `openamer skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`openamer gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** OpenAmer creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting OpenAmer to another machine
...
2. On the **source machine**, create a full backup:
   ...
   openamer backup
   ```
   This creates a zip of your entire `~/.openamer/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/openamer-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/openamer-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   openamer import ~/openamer-backup-<timestamp>.zip
   ```

4. On the new machine, run `openamer setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
openamer profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
openamer profile import ./work-backup.tar.gz work
```
...
### `openamer backup` vs `openamer profile export`

| Feature | `openamer backup` | `openamer profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.openamer` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='openamer-agent' ~/.openamer/ newmachine:~/.openamer/
```

:::tip
`openamer backup` produces a consistent snapshot even while OpenAmer is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the OpenAmer installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a OpenAmer-specific issue — it's a shell config permissions problem.
...
openamer config show | head -20

# Re-run model selection
openamer model

# Or test with a known-good model
openamer chat -q "hello" --model anthropic/claude-opus-4.7
```
...
2. **Ask the community:** [the OpenAmer project Discord](https://discord.gg/nousresearch)
