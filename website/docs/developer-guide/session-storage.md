...
OpenAmer Agent uses a SQLite database (`~/.openamer/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `openamer_state.py`
...
Multiple openamer processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.openamer/state.db
