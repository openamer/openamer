...
description: "How OpenAmer builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

OpenAmer deliberately separates:
...
2. **context** — caller-supplied `system_message` plus project context files (`.openamer.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
```
# Layer 1: Agent Identity (from ~/.openamer/SOUL.md)
You are OpenAmer, an AI assistant created by the OpenAmer project.
...
The platform hint (Layer 10 above) is the per-surface guidance OpenAmer
...
`SOUL.md` lives at `~/.openamer/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
```
You are OpenAmer Agent, an intelligent AI assistant created by the OpenAmer project.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_openamer_md(cwd_path)       # 1. .openamer.md / OPENAMER.md (walks to git root)
    ...
    # SOUL.md from OPENAMER_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.openamer.md`, `OPENAMER.md` | CWD up to git root | OpenAmer-native project config |
...
- **YAML frontmatter stripped** — `.openamer.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, OpenAmer concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.openamer.md` / `OPENAMER.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs OpenAmer already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.openamer/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.openamer/MEMORY.md` and `~/.openamer/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.openamer.md`, `OPENAMER.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking OpenAmer.
- Ephemeral overlays such as `OPENAMER_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how OpenAmer assembles prompts for everyone, change Python and treat it as a code contribution
