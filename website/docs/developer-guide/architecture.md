...
├── openamer_constants.py       # HERMES_HOME, profile-aware paths
...
| **Profile isolation** | Each profile (`openamer -p <name>`) gets its own HERMES_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
