...
sidebar_position: 1
title: "Architecture"
description: "OpenAmer Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of OpenAmer Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ openamer_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # OpenAmerCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── openamer_state.py           # SQLite session/state database with FTS5
├── openamer_constants.py       # OPENAMER_HOME, profile-aware paths
...
│
├── openamer_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `openamer` subcommands (large file)
...
│   ├── skills_config.py      # openamer skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # openamer tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # openamer gateway start/stop
│
...
```text
User input → OpenAmerCLI.process_input()
...
Three discovery sources: `~/.openamer/plugins/` (user), `.openamer/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `openamer plugins` or `config.yaml`.
...
Exposes OpenAmer as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`openamer -p <name>`) gets its own OPENAMER_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
