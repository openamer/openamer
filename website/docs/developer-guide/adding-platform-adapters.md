...
This guide covers adding a new messaging platform to the OpenAmer gateway. A platform adapter connects OpenAmer to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.openamer/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core OpenAmer code. Your plugin is a directory with two files:

```
~/.openamer/plugins/my-platform/
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `openamer config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-openamer-config) below).
...
| `openamer config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `openamer status` | Shows plugin platforms with `(plugin)` tag |
| `openamer gateway setup` | Plugin platforms appear in setup menu |
| `openamer tools` / `openamer skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.openamer/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `openamer gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `openamer cron run` separate from `openamer gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `openamer config`

`openamer_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
version: 1.0.0
description: >
  My Platform gateway adapter for OpenAmer Agent.
author: Your Name
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #openamer)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
:::note
This checklist is for adding a platform directly to the OpenAmer core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`openamer_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`openamer_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`openamer_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`openamer_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`openamer_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`openamer_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
| `website/docs/reference/toolsets-reference.md` | openamer-newplat toolset |
