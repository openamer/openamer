...
description: "Three protocols for driving openamer-agent from external programs: ACP, the TUI gateway JSON-RPC, and the OpenAI-compatible HTTP API"
---

# Programmatic Integration

OpenAmer ships three protocols for driving the agent from external programs — IDE plugins, custom UIs, CI pipelines, embedded sub-agents. Pick the one that matches your transport and consumer.
...
`openamer acp` starts a stdio JSON-RPC server speaking ACP. Used in production by VS Code (Zed Industries' ACP extension), Zed, and any JetBrains IDE with an ACP plugin.
...
```bash
openamer acp                  # serve ACP on stdio
openamer acp --bootstrap      # print install snippet for an ACP-capable IDE
```
...
`tui_gateway/server.py` is the protocol the Ink TUI (`openamer --tui`) and the embedded dashboard PTY bridge talk to. Any external host can speak the same protocol over stdio (or WebSocket via `tui_gateway/ws.py`).
...
Every command in the Pi-mono RPC spec ([issue #360](https://github.com/NousResearch/openamer-agent/issues/360)) has a TUI-gateway equivalent:

| Pi command | OpenAmer equivalent |
|------------|-------------------|
...
`gateway/platforms/api_server.py` exposes openamer over HTTP for any client that already speaks the OpenAI format. Useful when you want a web frontend, a curl-driven CI runner, or a non-Python consumer.
...
GET  /v1/models                  Lists openamer-agent
GET  /api/model/options          Provider-aware picker inventory
GET  /health, /health/detailed
```

Setup, headers (`X-OpenAmer-Session-Id`, `X-OpenAmer-Session-Key`), and frontend wiring: [API Server](../user-guide/features/api-server).

### Model catalog surfaces

The OpenAI-compatible API intentionally keeps `GET /v1/models` minimal: it is
the compatibility endpoint frontends expect, not the full OpenAmer provider/model
picker catalog.

If an external control plane needs OpenAmer' curated provider rows, per-model
...
- Dashboard backend REST: `GET /api/model/options` with `X-OpenAmer-Session-Token`
...
`model.options` when you are building a OpenAmer-aware model picker.
...
- **You're writing a custom desktop / web / TUI host and want every OpenAmer feature** (slash commands, approvals, clarify, multi-agent, session branching) → TUI gateway JSON-RPC.
...
Provider-aware resolution (the same model name picks the right format for whatever provider you're on) is built in. See `openamer_cli/model_switch.py`.

---

## A note on `--mode rpc`

OpenAmer does not have a `--mode rpc` flag. The three protocols above already cover the use cases — ACP for IDE-protocol clients, the TUI gateway for stdio JSON-RPC hosts, and the API server for HTTP. If you find a real gap that none of them fill, open an issue with the concrete consumer you're building.
