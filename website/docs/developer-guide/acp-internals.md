...
The ACP adapter wraps OpenAmer' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
openamer acp / openamer-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.openamer/.env
  -> configure stderr logging
  -> construct OpenAmerACPAgent
...
- `allow_once` -> OpenAmer `once`
- `allow_always` -> OpenAmer `always`
- reject options -> OpenAmer `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps OpenAmer tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["openamer-acp"])
...
Instead it reuses OpenAmer' runtime resolver:

- `acp_adapter/auth.py`
- `openamer_cli/runtime_provider.py`

So ACP advertises and uses the currently configured OpenAmer provider/credentials. It also always advertises a terminal setup auth method (`openamer-setup`, args `--setup`) so first-run ACP clients can open OpenAmer' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.openamer/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `openamer-acp` toolset definition
- `openamer_cli/main.py` — `openamer acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `openamer-acp` script
