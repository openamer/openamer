...
description: "How to build a memory provider plugin for OpenAmer Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give OpenAmer Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `openamer plugins`.
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          openamer_home (str): Active OPENAMER_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `openamer memory setup` | **Yes** |
| `save_config(values, openamer_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `openamer memory setup`:
...
Every field in `get_config_schema()` is prompted during `openamer memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$OPENAMER_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, openamer_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(openamer_home) / "my-provider.json"
...
the `messages` parameter; OpenAmer will continue calling them with the legacy
signature.
...
All storage paths **must** use the `openamer_home` kwarg from `initialize()`, not hardcoded `~/.openamer`:

```python
# CORRECT — profile-scoped
from openamer_constants import get_openamer_home
data_dir = get_openamer_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.openamer/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `openamer my-provider status`, `openamer my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `openamer <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `openamer --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: openamer my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the openamer my-provider argparse tree.
