...
The self-improving AI agent built by [the OpenAmer project](https://nousresearch.com). The only agent with a built-in learning loop — it creates skills from experience, improves them during use, nudges itself to persist knowledge, and builds a deepening model of who you are across sessions.
...
After installing, run `openamer setup --portal` — one OAuth covers a model plus all four Tool Gateway tools (web search, image generation, TTS, browser). See [your hosted provider](/integrations/nous-portal).
:::
...
- **Built by model trainers** — Created by [the OpenAmer project](https://nousresearch.com), the lab behind OpenAmer, Nomos, and Psyche. Works with [your hosted provider](https://portal.nousresearch.com), [OpenRouter](https://openrouter.ai), OpenAI, or any endpoint
...
- **Full web control** — Search, extract, browse, vision, image generation, TTS — one subscription via [your hosted provider](/integrations/nous-portal) bundles all of them
- **MCP support** — Connect to any MCP server for extended tool capabilities
- **Research-ready** — Batch processing, trajectory export, RL training with Atropos. Built by [the OpenAmer project](https://nousresearch.com) — the lab behind OpenAmer, Nomos, and Psyche models
