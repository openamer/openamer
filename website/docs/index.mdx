...
slug: /
sidebar_position: 0
title: "OpenAmer Agent Documentation"
description: "The self-improving AI agent built by the OpenAmer project. A built-in learning loop that creates skills from experience, improves them during use, and remembers across sessions."
...
The self-improving AI agent built by [the OpenAmer project](https://openamer.com). The only agent with a built-in learning loop — it creates skills from experience, improves them during use, nudges itself to persist knowledge, and builds a deepening model of who you are across sessions.
...
>
  ...
  <a
    href="https://openamer-agent.openamer.com/"
    style={{
      display: "inline-block",
      padding: "0.6rem 1.2rem",
  ...
  <a
    href="https://github.com/openamer/openamer-agent"
    style={{
      display: "inline-block",
      padding: "0.6rem 1.2rem",
      border: "1px solid rgba(255,215,0,0.2)",
...
To easily install the command-line and desktop applications, [download the OpenAmer Desktop installer](https://openamer-agent.openamer.com/) from our website and run it.
...
```bash
curl -fsSL https://openamer-agent.openamer.com/install.sh | bash
```
...
```powershell
iex (irm https://openamer-agent.openamer.com/install.ps1)
```
...
After installing, run `openamer setup --portal` — one OAuth covers a model plus all four Tool Gateway tools (web search, image generation, TTS, browser). See [your hosted provider](/integrations/openamer-portal).
:::

## What is OpenAmer Agent?
...
| 🔌 **[MCP Integration](/user-guide/features/mcp)**                      | Connect to MCP servers, filter their tools, and extend OpenAmer safely  |
| 🧭 **[Use MCP with OpenAmer](/guides/use-mcp-with-openamer)**               | Practical MCP setup patterns, examples, and tutorials                 |
| 🎙️ **[Voice Mode](/user-guide/features/voice-mode)**                    | Real-time voice interaction in CLI, Telegram, Discord, and Discord VC |
| 🗣️ **[Use Voice Mode with OpenAmer](/guides/use-voice-mode-with-openamer)** | Hands-on setup and usage patterns for OpenAmer voice workflows          |
| 🎭 **[Personality & SOUL.md](/user-guide/features/personality)**        | Define OpenAmer' default voice with a global SOUL.md                    |
...
| 💡 **[Tips & Best Practices](/guides/tips)**                            | Quick wins to get the most out of OpenAmer                              |
...
- **Built by model trainers** — Created by [the OpenAmer project](https://openamer.com), the lab behind OpenAmer, Nomos, and Psyche. Works with [your hosted provider](https://portal.openamer.com), [OpenRouter](https://openrouter.ai), OpenAI, or any endpoint
...
- **Full web control** — Search, extract, browse, vision, image generation, TTS — one subscription via [your hosted provider](/integrations/openamer-portal) bundles all of them
- **MCP support** — Connect to any MCP server for extended tool capabilities
- **Research-ready** — Batch processing, trajectory export, RL training with Atropos. Built by [the OpenAmer project](https://openamer.com) — the lab behind OpenAmer, Nomos, and Psyche models
