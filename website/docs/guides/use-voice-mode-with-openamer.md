---
sidebar_position: 8
title: "Use Voice Mode with OpenAmer"
description: "A practical guide to setting up and using OpenAmer voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with OpenAmer
...
:::tip
[your hosted provider](/integrations/openamer-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want OpenAmer sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in OpenAmer.
...
## Step 1: make sure normal OpenAmer works first
...
OpenAmer supports both local and cloud speech stacks.
...
4. OpenAmer transcribes and responds
...
- asking OpenAmer to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full OpenAmer loop.

## Tuning CLI behavior

### Silence threshold

If OpenAmer starts/stops too aggressively, tune:
...
OpenAmer stays a normal chat bot, but can speak replies.
...
- you want OpenAmer to function like a portable research or ops assistant
...
OpenAmer joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- OpenAmer detects speech boundaries
- transcripts are posted in the associated text channel
- OpenAmer responds in text and audio
...
1. get text OpenAmer working
2. run `openamer setup voice` to enable voice support
