...
3. Sets `model.provider: nous` in `~/.openamer/config.yaml`
...
openamer auth add nous --type oauth
...
```bash
openamer config set model.provider nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/NousResearch/openamer-agent/issues) — most gaps are routing config we can update.
...
```bash
openamer auth logout nous       # wipes the local refresh token
