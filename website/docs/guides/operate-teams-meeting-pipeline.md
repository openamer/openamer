...
```bash
openamer teams-pipeline validate
```
...
```bash
openamer teams-pipeline token-health
openamer teams-pipeline token-health --force-refresh
```
...
```bash
openamer teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
openamer teams-pipeline maintain-subscriptions
openamer teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: OpenAmer cron (recommended if you already run the OpenAmer gateway)

OpenAmer ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.openamer/scripts/`. First create the script:

```bash
mkdir -p ~/.openamer/scripts
cat > ~/.openamer/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec openamer teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.openamer/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
openamer cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/openamer-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=OpenAmer Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=openamer
EnvironmentFile=/etc/openamer/env
ExecStart=/usr/local/bin/openamer teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/openamer-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run OpenAmer Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now openamer-teams-pipeline-maintain.timer
systemctl list-timers openamer-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/openamer teams-pipeline maintain-subscriptions >> /var/log/openamer/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.openamer/.env` at the top of a wrapper script that crontab calls.
...
```bash
openamer teams-pipeline subscriptions   # should show expirationDateTime advanced
openamer teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
openamer teams-pipeline list
openamer teams-pipeline list --status failed
openamer teams-pipeline show <job-id>
```

### Replay a stored job

```bash
openamer teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
openamer teams-pipeline fetch --meeting-id <meeting-id>
openamer teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `openamer teams-pipeline maintain-subscriptions --dry-run`
- inspect `openamer teams-pipeline list --status failed`
...
- run `openamer teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `openamer teams-pipeline run`
...
- [ ] `openamer teams-pipeline validate` returns an OK snapshot
- [ ] `openamer teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (OpenAmer cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
