...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `openamer send`."
---

# Pipe Script Output to Messaging Platforms

`openamer send` is a small, scriptable CLI that pushes a message to any
messaging platform OpenAmer is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | openamer send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `openamer
...
openamer send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
openamer send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
openamer send --list

# Filter by platform
openamer send --list telegram
```
...
Any platform OpenAmer ships adapters for works as a target:
...
`openamer send` resolves the message body in this order:

1. **Positional argument** — `openamer send --to telegram "hi"`
2. **`--file PATH`** — `openamer send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | openamer send --to telegram`

When stdin is a TTY (no pipe), OpenAmer does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  openamer send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `openamer send` reuses your OpenAmer config, the same script works on
any host where OpenAmer is installed — no need to export bot tokens into
...
`openamer send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  openamer send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | openamer send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.openamer/bin/openamer send \
...
```bash
./train.py --epochs 200 && \
  openamer send --to telegram "training done" || \
  openamer send --to telegram "training failed (exit $?)"
```
...
openamer send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(openamer send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `openamer send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `openamer send` calls
the platform's REST endpoint directly using credentials from
`~/.openamer/.env` and `~/.openamer/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `openamer gateway start` and retry.
...
# Machine-readable
openamer send --list --json
```

The listing is built from `~/.openamer/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`openamer gateway
...
| Approach | Multi-platform | Reuses OpenAmer creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `openamer send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`openamer send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `openamer send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `openamer send` shares with cron delivery.
