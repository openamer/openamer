...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `openamer setup --portal`. See [your hosted provider](/integrations/openamer-portal).
:::
...
2. **OpenAmer spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.openamer/cron/output/` and you can read them anytime.
:::
...
OpenAmer will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by OpenAmer Agent
```
...
Before creating cron jobs, ensure OpenAmer has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell OpenAmer what you want:
...
OpenAmer will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell OpenAmer to delegate each topic to a sub-agent:
...
OpenAmer will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
