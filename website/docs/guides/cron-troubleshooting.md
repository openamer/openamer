...
description: "Diagnose and fix common OpenAmer cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`openamer gateway` for foreground, or `openamer gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `openamer cron tick`.
...
```bash
date
openamer cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.openamer/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.openamer/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.openamer/.env` |
...
| `local` | Write access to `~/.openamer/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `openamer cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `openamer skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `openamer skills list`.
...
2. `~/.openamer/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `openamer cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the OpenAmer config directory). Verify:
```bash
ls ~/.openamer/scripts/your-script.py   # Must exist
openamer cron edit <job_id> --script ~/.openamer/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `openamer skills install <skill-name>`.
...
```bash
ps aux | grep openamer
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.openamer/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.openamer/cron/jobs.json
chmod 600 ~/.openamer/cron/jobs.json   # Your user should own it
```
...
```bash
openamer cron list                    # Show all jobs, states, next_run times
openamer cron run <job_id>            # Schedule for next tick (for testing)
openamer cron edit <job_id>           # Fix configuration issues
openamer logs                         # View recent OpenAmer logs
openamer skills list                  # Verify installed skills
```
