...
This guide walks you through running OpenAmer Agent on a [your hosted provider](https://portal.openamer.com) subscription end to end — from signing up to verifying that every tool routes correctly. If you just want the overview of what the Portal is and what's in the subscription, see the [your hosted provider integration page](/integrations/openamer-portal). This page is the task script.

## Prerequisites

- OpenAmer Agent installed ([Quickstart](/getting-started/quickstart))
...
Open [portal.openamer.com/manage-subscription](https://portal.openamer.com/manage-subscription), sign up, and pick a plan.
...
3. Sets `model.provider: openamer` in `~/.openamer/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where OpenAmer is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
openamer setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
openamer auth add openamer --type oauth
# Then re-run `openamer setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "OpenAmer Agent release notes" and summarize the top 3 hits.
```

You should see OpenAmer call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`openamer setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
openamer config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick OpenAmer-4 for agent work
...
The Portal's own [info page](https://portal.openamer.com/info) carries this warning too — it's the official a hosted provider guidance, not just a OpenAmer-side opinion.

## 6. (Optional) Customize Tool Gateway routing

The gateway is opt-in per tool, not all-or-nothing. If you already have a Browserbase account and want to keep using it while routing web search and image generation through OpenAmer, that's supported:

```bash
openamer tools
# → Web search       → "OpenAmer Subscription"     (recommended)
# → Image generation → "OpenAmer Subscription"     (recommended)
# → Browser          → "Browserbase"           (your existing key)
# → TTS              → "OpenAmer Subscription"     (recommended)
```

These rows appear in `openamer tools` even before you've logged into your hosted provider — if you pick "OpenAmer Subscription" without an active session, OpenAmer runs the Portal login inline (without changing your inference provider or your other tools).
...
```bash
openamer setup voice
# → pick "OpenAmer Subscription" for TTS
# → pick a speech-to-text backend (local faster-whisper is free, no setup)
```

Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and OpenAmer will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
openamer cron create "every day at 9am" \
...
If you use [OpenAmer profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.openamer/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `openamer portal info` shows "not logged in" after `openamer setup --portal`
...
```bash
openamer config set model.provider openamer
```
...
```bash
openamer tools
# pick "OpenAmer Subscription" for any tool you want gateway-routed
```
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so OpenAmer doesn't replay it endlessly. Just log in again:

```bash
openamer auth add openamer
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/openamer/openamer/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`openamer portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:

- `model.provider` set to `openrouter`/`anthropic`/etc. instead of `openamer`
- An OAuth refresh failure that fell back to a different configured provider
- Multiple OpenAmer profiles where you're using the wrong one (check `openamer profile list`)

### Want to revoke and start clean

```bash
openamer auth logout openamer       # wipes the local refresh token
...
- **[your hosted provider integration page](/integrations/openamer-portal)** — Overview of what's in the subscription
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple OpenAmer configurations
