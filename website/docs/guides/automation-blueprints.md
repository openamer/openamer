...
openamer cron create "0 2 * * *" \
  "You are a project manager triaging the openamer/openamer GitHub repo.

1. Run: gh issue list --repo openamer/openamer --state open --json number,title,labels,author,createdAt --limit 30
...
openamer cron create "0 9 * * 1" \
  "Scan the openamer/openamer repo for documentation drift.

1. Run: gh pr list --repo openamer/openamer --state merged --json number,title,files,mergedAt --limit 30
