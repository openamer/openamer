...
This tutorial walks you through setting up a Telegram bot powered by OpenAmer Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **OpenAmer Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.openamer/.env`

:::tip
A $5/month VPS is plenty for running the gateway. OpenAmer itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team OpenAmer Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_openamer_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by OpenAmer Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.openamer/.env`:
...
```
[Gateway] Starting OpenAmer Gateway...
...
# View live logs
journalctl --user -u openamer-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `openamer gateway install` to update the plist.
:::
...
```bash
openamer gateway stop && openamer gateway start
```
...
openamer pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.openamer/.env`:
...
Control how much detail the bot shows when using tools. In `~/.openamer/config.yaml`:
...
Customize how the bot communicates by editing `~/.openamer/SOUL.md`:

For a full guide, see [Use SOUL.md with OpenAmer](/guides/use-soul-with-openamer).
...
```bash
# From the CLI
openamer cron list          # View all scheduled jobs
openamer cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u openamer-gateway` (Linux) or `~/.openamer/logs/gateway.log` (macOS) |
| Cron job output | `~/.openamer/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.openamer/cron/jobs.json` |
| Pairing data | `~/.openamer/pairing/` |
| Session history | `~/.openamer/sessions/` |
