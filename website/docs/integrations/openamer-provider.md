...
Don't have a subscription yet? [portal.openamer.com/manage-subscription](https://portal.openamer.com/manage-subscription) — sign up, then come back and run the command above.
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes OpenAmer Agent's tool calls through OpenAmer-managed infrastructure. Five backends, one login:
...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.openamer/auth.json` is the only credential on disk, and OpenAmer mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
The Portal's own [model info page](https://portal.openamer.com/info) carries the same warning, so this isn't a OpenAmer-side opinion — it's the official guidance from the OpenAmer project.
...
1. Opens your browser to portal.openamer.com for OAuth login
...
If you don't have a subscription yet, sign up at [portal.openamer.com/manage-subscription](https://portal.openamer.com/manage-subscription) first.

### Existing install — add Portal alongside other providers

If you already have OpenAmer configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `openamer model` between sessions — the Portal becomes one of your available providers, not your only one.

### Headless / SSH / remote setup

OAuth needs a browser, but the loopback callback runs on the machine where OpenAmer is running. For remote hosts, see [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding).

### Profile setup

If you use [OpenAmer profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
`openamer portal` (with no subcommand) is the human-readable alias for `openamer auth add openamer --type oauth` — it logs you in, lets you pick a a hosted provider model, sets a hosted provider as your inference provider, and offers the Tool Gateway opt-in (identical to `openamer setup --portal`, and the same a hosted provider flow as the first-time quick setup).
...
```
  ...
  ───────────
  Auth:    ✓ logged in
  Portal:  https://portal.openamer.com
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through OpenAmer, that's supported. Use `openamer tools` to pick backends per tool:

```bash
openamer tools
# → Web search       → "OpenAmer Subscription"
# → Image generation → "OpenAmer Subscription"
# → Browser          → "Browserbase"  (your existing key)
# → TTS              → "OpenAmer Subscription"
```

The Tool Gateway is opt-in per tool, not all-or-nothing. The managed backends show up in `openamer tools` whether or not you're logged into your hosted provider — if you pick "OpenAmer Subscription" before authenticating, OpenAmer runs the Portal login inline (it won't change your inference provider or touch your other tools). See the [Tool Gateway docs](/user-guide/features/tool-gateway) for the full per-tool configuration matrix.

### Subscription management

Manage your plan, view usage, or upgrade/cancel at any time:

- **Web:** [portal.openamer.com/manage-subscription](https://portal.openamer.com/manage-subscription)
- **CLI shortcut:** `openamer portal open` (opens the same page in your default browser)

## Configuration reference

After `openamer setup --portal`, `~/.openamer/config.yaml` will look like:
...
```yaml
web:
  backend: openamer       # web search/extract routes through Tool Gateway
...
The OAuth refresh token is stored separately at `~/.openamer/auth.json` (not in `config.yaml` — credentials and configuration are kept separate by design).

## Token handling

OpenAmer mints a short-lived JWT from your stored Portal refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.

If the Portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is **quarantined locally** so OpenAmer stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `openamer auth add openamer` to log in again; the quarantine clears on the next successful login.

## Troubleshooting

### `openamer portal info` shows "not logged in"
...
Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `openamer auth add openamer` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
If a model is genuinely missing, [open an issue](https://github.com/openamer/openamer/issues) — we surface the Portal's catalog to OpenAmer and gaps usually mean a routing config we can update.
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-OpenAmer tools (other agents, scripts, third-party clients)
...
- **[Profiles](/user-guide/profiles)** — Multiple OpenAmer configurations sharing one Portal login
