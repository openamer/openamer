...
secrets:
  command:
    ...
    # command: "pass show openamer/env"
    # command: "secret-tool lookup service openamer-env"
```
...
- The helper's **stderr is discarded** — vault CLI diagnostics can carry secret material, so they never reach OpenAmer' output. Failures log structured fields only (exit code / signal / errno), never the command string.
...
| `helper command failed` | Non-zero exit, timeout, spawn failure | Run the helper manually in a shell to see its real error (OpenAmer discards its stderr on purpose) |
