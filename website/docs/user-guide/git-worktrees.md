...
title: "Git Worktrees"
description: "Run multiple OpenAmer agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

OpenAmer Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with OpenAmer so each session has a clean, isolated working directory.

## Why Use Worktrees with OpenAmer?

OpenAmer treats the **current working directory** as the project root:

- CLI: the directory where you run `openamer` or `openamer chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.openamer/config.yaml`
...
git worktree add ../repo-feature feature/openamer-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/openamer-experiment` checked out in that directory

Now you can `cd` into the new worktree and run OpenAmer there:

```bash
cd ../repo-feature

# Start OpenAmer in the worktree
openamer
```
...
git worktree add ../repo-experiment-a feature/openamer-a
git worktree add ../repo-experiment-b feature/openamer-b
```
...
Each OpenAmer process:

- Works on its own branch (`feature/openamer-a` vs `feature/openamer-b`).
...
- OpenAmer checkpoint data under `~/.openamer/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per OpenAmer experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/openamer-checkpoints-docs`, `feature/openamer-refactor-tests`.
- **Commit frequently**
...
- **Avoid running OpenAmer from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `openamer -w` (Automatic Worktree Mode)

OpenAmer has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `openamer/openamer-<hash>`).
...
```bash
openamer -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `openamer -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each OpenAmer session its own clean checkout.
