...
Connect OpenAmer to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
OpenAmer supports two WeCom integration modes:
...
> Run `openamer gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. OpenAmer decrypts the message, queues it for the agent
...
(Use `openamer gateway start` only after `openamer gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in OpenAmer and the token the
...
in `~/.openamer/.env` values around `=` will also break signature checks. After
fixing, restart `openamer gateway run`.
...
Check `openamer gateway run` logs for the bound host/port. If the adapter bound to
