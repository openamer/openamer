...
sidebar_position: 12
title: "Google Chat"
description: "Set up OpenAmer Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect OpenAmer Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your OpenAmer
...
> Run `openamer gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only OpenAmer can read (e.g.,
`~/.openamer/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a openamer restart)
...
- Principal: `openamer-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — OpenAmer calls
...
- **App name**: whatever you want users to see ("OpenAmer" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/openamer-chat-events`.
...
`ADDED_TO_SPACE` event that OpenAmer uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure OpenAmer

Add the Google Chat section to `~/.openamer/.env`:
...
Install the dependencies the Google Chat adapter needs (no OpenAmer extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "OpenAmer is thinking…" marker, then
...
`~/.openamer/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "OpenAmer is thinking…").
...
Thread support: when a user replies inside a thread, OpenAmer detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate OpenAmer session.
...
3. Download the JSON. Move it onto the host that runs OpenAmer.
4. Register the client with OpenAmer (run under the profile you want it scoped to):
...
openamer -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's OpenAmer home (e.g.
`~/.openamer/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.openamer/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.openamer/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, OpenAmer isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `openamer gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: OpenAmer will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.openamer/google_chat_user_tokens/<sanitized_email>.json` (filesystem
