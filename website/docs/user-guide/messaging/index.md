...
description: "Chat with OpenAmer from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with OpenAmer from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with OpenAmer](/guides/use-voice-mode-with-openamer).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [your hosted provider](/integrations/openamer-portal) subscription bundles all of them.
:::
...
```mermaid
flowchart TB
    subgraph Gateway["OpenAmer Gateway"]
...
For group chats, hooks, and automation flows, OpenAmer supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. OpenAmer keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; OpenAmer does not hide failures just because the text resembles a silence token.
...
```bash
openamer gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
openamer gateway              # Run in foreground
openamer gateway setup        # Configure messaging platforms interactively
openamer gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo openamer gateway install --system   # Linux only: install a boot-time system service
openamer gateway start        # Start the default service
openamer gateway stop         # Stop the default service
openamer gateway status       # Check default service status
openamer gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.openamer/config.yaml"
gateway:
...
```bash
openamer gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. OpenAmer sends heartbeats
...
| `/update` | Update OpenAmer Agent to the latest version |
...
with the `session_reset` section in `~/.openamer/config.yaml`:
...
Configure per-platform overrides in `~/.openamer/gateway.json`:
...
openamer pairing approve telegram XKGH5N7P

# Other pairing commands:
openamer pairing list          # View pending + approved users
openamer pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, OpenAmer appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.openamer/config.yaml`:
...
Off by default. When enabled, OpenAmer prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.openamer/config.yaml`:
...
```bash
openamer gateway install               # Install as user service
openamer gateway start                 # Start the service
openamer gateway stop                  # Stop the service
openamer gateway status                # Check status
journalctl --user -u openamer-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo openamer gateway install --system
sudo openamer gateway start --system
sudo openamer gateway status --system
journalctl -u openamer-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit OpenAmer installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit openamer-gateway` (or `sudo systemctl edit openamer-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `openamer update`. When `openamer update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart openamer-gateway` command (it never blocks on an interactive password prompt).
...
```bash
openamer gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `openamer update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo openamer update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/openamer-gateway`:

```
openamer ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed openamer-gateway*, /usr/bin/systemctl --no-ask-password start openamer-gateway*, /usr/bin/systemctl --no-ask-password restart openamer-gateway*
```
:::
...
```bash
openamer gateway install               # Install as launchd agent
openamer gateway start                 # Start the service
openamer gateway stop                  # Stop the service
openamer gateway status                # Check status
tail -f ~/.openamer/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.openamer.gateway.plist`. It includes three environment variables:
...
|----------|---------|--------------|
| CLI | `openamer-cli` | Full access |
| Telegram | `openamer-telegram` | Full tools including terminal |
| Discord | `openamer-discord` | Full tools including terminal |
| WhatsApp | `openamer-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `openamer-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `openamer-slack` | Full tools including terminal |
| Google Chat | `openamer-google_chat` | Full tools including terminal |
| Signal | `openamer-signal` | Full tools including terminal |
| SMS | `openamer-sms` | Full tools including terminal |
| Email | `openamer-email` | Full tools including terminal |
| Home Assistant | `openamer-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `openamer-mattermost` | Full tools including terminal |
| Matrix | `openamer-matrix` | Full tools including terminal |
| DingTalk | `openamer-dingtalk` | Full tools including terminal |
| Feishu/Lark | `openamer-feishu` | Full tools including terminal |
| WeCom | `openamer-wecom` | Full tools including terminal |
| WeCom Callback | `openamer-wecom-callback` | Full tools including terminal |
| Weixin | `openamer-weixin` | Full tools including terminal |
| BlueBubbles | `openamer-bluebubbles` | Full tools including terminal |
| QQBot | `openamer-qqbot` | Full tools including terminal |
| Yuanbao | `openamer-yuanbao` | Full tools including terminal |
| Microsoft Teams | `openamer-teams` | Full tools including terminal |
| API Server | `openamer-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `openamer-webhook` | Full tools including terminal |
| Raft | `openamer-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.openamer/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
