...
sidebar_position: 16
title: "Yuanbao"
description: "Connect OpenAmer Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect OpenAmer to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.openamer/.env`:
...
All standard OpenAmer commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.openamer/logs/gateway.log`
...
4. Consider enabling verbose logging: `OPENAMER_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. OpenAmer automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
OPENAMER_LOG_LEVEL=debug openamer gateway
```
...
```bash
openamer chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
