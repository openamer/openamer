...
Schedule tasks to run automatically with natural language or cron expressions. OpenAmer exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to OpenAmer itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
At creation, an unpinned job (one you don't give an explicit `provider`/`model`) follows the global default selected by `openamer model` — and OpenAmer **snapshots** that provider and model on the job. If the global default later changes, the job **fails closed**: it skips the run, makes no inference call, and sends an alert telling you to pin the provider/model explicitly (`cronjob action=update job_id=… provider=… model=…`) to proceed. This prevents an unattended job from silently inheriting a switch to a paid provider/model and spending money you didn't intend (#44585). To make a job deliberately track your global default, pin it to the new values after changing them. `openamer setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [your hosted provider](/integrations/openamer-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. OpenAmer disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
openamer cron create "every 2h" "Check server status"
openamer cron create "every 1h" "Summarize new feed items" --skill blogwatcher
openamer cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask OpenAmer normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

OpenAmer will use the unified `cronjob` tool internally.
...
openamer cron create "every 1d at 09:00" \
...
```bash
openamer cron edit <job_id> --schedule "every 4h"
openamer cron edit <job_id> --prompt "Use the revised task"
openamer cron edit <job_id> --skill blogwatcher --skill maps
openamer cron edit <job_id> --add-skill maps
openamer cron edit <job_id> --remove-skill blogwatcher
openamer cron edit <job_id> --clear-skills
```
...
```bash
openamer cron list
openamer cron pause <job_id_or_name>
openamer cron resume <job_id_or_name>
openamer cron run <job_id_or_name>
openamer cron remove <job_id_or_name>
openamer cron edit <job_id_or_name> [...flags]
openamer cron status
openamer cron tick
```
...
sudo openamer gateway install --system   # Linux: boot-time system service for servers
openamer gateway             # Or run in foreground
...
On each tick OpenAmer:

1. loads jobs from `~/.openamer/cron/jobs.json`
...
A file lock at `~/.openamer/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.

### Execution history

OpenAmer records each claimed cron attempt in the profile-local
`~/.openamer/cron/executions.db` before executor or provider dispatch. Attempts
move through `claimed`, `running`, and one immutable terminal state:
`completed`, `failed`, or `unknown`. After restart, OpenAmer marks an abandoned
...
Inspect recent attempts with `openamer cron runs [job-id] --limit 20` (alias:
...
| `"local"` | Save to local files only (`~/.openamer/cron/output/`) | Default on CLI |
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.openamer/cron/output/`), but no message is sent to the delivery target.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.openamer/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to OpenAmer directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

OpenAmer will write the check script to `~/.openamer/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.openamer/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.openamer/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.openamer/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.openamer/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, OpenAmer reads Job 1's most recent output from `~/.openamer/cron/output/{job1_id}/*.md`
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `openamer tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `openamer tools` cron-platform config wins; otherwise OpenAmer falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether OpenAmer should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.openamer/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.openamer/scripts/.feed-changed.last"
...
```bash
#!/bin/bash
# ~/.openamer/scripts/flag-ready.sh
...
:::tip
OpenAmer's own `~/.openamer/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/openamer/openamer/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
