...
description: "Durable SQLite-backed task board for coordinating multiple OpenAmer profiles"
---
...
OpenAmer Kanban is a durable task board, shared across all your OpenAmer profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.openamer/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.openamer/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `openamer kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `openamer kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/openamer-kanban-v1-spec.pdf` in the repository.
...
called `default` (DB at `~/.openamer/kanban.db` for back-compat). Users who
...
openamer kanban boards list

# Create a new board.
openamer kanban boards create atm10-server \
    --name "ATM10 Server" \
...
openamer kanban --board atm10-server list
openamer kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
openamer kanban boards switch atm10-server
openamer kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
openamer kanban boards rename atm10-server "ATM10 (Prod)"
...
`openamer dashboard` → Kanban tab shows a board switcher at the top as soon
...
openamer kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
openamer kanban watch

# 5. See the board (you)
openamer kanban list
openamer kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `openamer kanban show t_abcd`.
...
until one comes up — `openamer kanban create` warns about this at creation
time.

Running `openamer kanban daemon` as a separate process is **deprecated**;
...
openamer kanban create "nightly ops review" \
    --assignee ops \
...
```bash
openamer kanban complete t_abc t_def t_hij --result "batch wrap"
openamer kanban archive  t_abc t_def t_hij
openamer kanban unblock  t_abc t_def
openamer kanban block    t_abc "need input" --ids t_def t_hij
```
...
### Why tools instead of shelling to `openamer kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `openamer kanban complete` *inside* the container, where `openamer` isn't installed and `~/.openamer/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.openamer/kanban.db` regardless of terminal backend.
...
```json
{
  ...
  "verification": ["pytest tests/openamer_cli/test_kanban_db.py -q"],
...
**Agent-side prevention:** Before the worker exits, OpenAmer injects up to two
...
```bash
openamer kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

openamer kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `openamer skills list` to see what's available); there's no runtime install.
...
```bash
openamer kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `openamer kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `openamer kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `openamer profile create --description "..."`, `openamer profile describe <name> --text "..."`, `openamer profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.openamer/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `openamer profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.openamer/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard OpenAmer dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
openamer kanban init                                     # create kanban.db + print daemon hint
openamer kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
openamer kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
openamer kanban show <id> [--json]
openamer kanban assign <id> <profile>                    # or 'none' to unassign
openamer kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
openamer kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
openamer kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
openamer kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
openamer kanban diagnostics [--json]                     # board health snapshot (alias: diag)
openamer kanban link <parent_id> <child_id>
openamer kanban unlink <parent_id> <child_id>
openamer kanban claim <id> [--ttl SECONDS]
openamer kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
openamer kanban complete <id>... [--result "..."]
openamer kanban block <id> "<reason>" [--ids <id>...]
openamer kanban unblock <id>...
openamer kanban archive <id>...

openamer kanban tail <id>                                # follow a single task's event stream
openamer kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
openamer kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
openamer kanban runs <id> [--json]                       # attempt history (one row per run)
openamer kanban assignees [--json]                       # profiles on disk + per-assignee task counts
openamer kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
openamer kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `openamer gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
openamer kanban stats [--json]                           # per-status + per-assignee counts
openamer kanban log <id> [--tail BYTES]                  # worker log from ~/.openamer/kanban/logs/
openamer kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
openamer kanban notify-list [<id>] [--json]
openamer kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
openamer kanban context <id>                             # what a worker sees
openamer kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
openamer kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
openamer kanban create "nightly backup audit" \
...
Every `openamer kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `openamer chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `openamer_cli.kanban.run_slash()` entry point that reuses the `openamer kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `openamer kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.openamer/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`openamer kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `openamer kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/openamer-kanban-v1-spec.pdf`.
...
```bash
openamer kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
...
```bash
openamer kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
openamer kanban notify-list
openamer kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
openamer kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
openamer kanban runs t_abcd
...
**Bulk close caveat.** `openamer kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`openamer kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `openamer kanban heartbeat $TASK` to signal liveness during long operations. |
...
`openamer kanban tail <id>` shows these for a single task. `openamer kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.openamer/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/openamer-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
