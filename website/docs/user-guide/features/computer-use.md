...
OpenAmer Agent can drive your desktop — clicking, typing, scrolling,
...
```
openamer computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `openamer computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `openamer tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or OpenAmer app). `openamer computer-use doctor` will tell you which permission is missing. |
...
```
openamer -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.openamer/config.yaml`.

## `openamer computer-use doctor` — your first triage stop

`openamer computer-use doctor` runs cua-driver's structured
...
agent is acting here." Each OpenAmer run declares its own cua-driver
**session id** (something like `openamer-3a7b9c14d2e8`); the cursor's
...
OpenAmer intentionally keeps its skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the OpenAmer-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the OpenAmer skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **OpenAmer
...
OpenAmer applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.openamer/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. OpenAmer applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  OpenAmer agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the OpenAmer agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. OpenAmer warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive OpenAmer from inside the RDP / console
...
upstream. **OpenAmer disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) OpenAmer sets
...
When it's on, `openamer computer-use doctor` reports `telemetry: enabled`;
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for OpenAmer testing (see notes).
...
- `openamer computer-use status` prints the resolved binary path and
  version.
- `openamer computer-use doctor` confirms the binary is reachable and
...
- **OpenAmer spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no OpenAmer
  change needed either way.
- **Local builds skip the version check.** OpenAmer warns when the
...
**First action when anything's off: run `openamer computer-use doctor`.**
...
Run `openamer computer-use install` to fetch the cua-driver binary, or
run `openamer tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `openamer computer-use doctor` will
...
- **OpenAmer-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  OpenAmer `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects OpenAmer (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (OpenAmer among them)
