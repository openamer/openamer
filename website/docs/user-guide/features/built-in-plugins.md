...
3. **Project** — `./.openamer/plugins/<name>/` (requires `HERMES_ENABLE_PROJECT_PLUGINS=1`)
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HERMES_HOME` or `/tmp/openamer-*`, track it silently as `test` / `temp` / `cron-output`. |
...
**Safety** — cleanup only ever touches paths under `HERMES_HOME` or `/tmp/openamer-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.
...
- Unlock state is persisted to `$HERMES_HOME/plugins/openamer-achievements/state.json`
...
**State files** — live under `$HERMES_HOME/plugins/openamer-achievements/`:
...
**Opting out:** Delete or rename `plugins/openamer-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.openamer/plugins/openamer-achievements/` that ships no dashboard. The plugin's state files under `$HERMES_HOME/plugins/openamer-achievements/` survive — reinstalling preserves your unlock history.
