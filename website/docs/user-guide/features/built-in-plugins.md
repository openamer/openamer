...
description: "Plugins shipped with OpenAmer Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

OpenAmer ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.openamer/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a OpenAmer Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.openamer/plugins/<name>/`
3. **Project** — `./.openamer/plugins/<name>/` (requires `OPENAMER_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `openamer_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `openamer memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `openamer plugins list` and the interactive `openamer plugins` UI), but none load until you explicitly enable them:

```bash
openamer plugins enable disk-cleanup
```

Or via `~/.openamer/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer OpenAmer. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
openamer plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `openamer plugins enable <name>`.
...
| `openamer-achievements` | dashboard tab | Steam-style collectible badges generated from your real OpenAmer session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `openamer memory` and `openamer plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `OPENAMER_HOME` or `/tmp/openamer-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under OPENAMER_HOME | always | Never |
...
**State** — everything lives at `$OPENAMER_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `OPENAMER_HOME` or `/tmp/openamer-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `openamer plugins enable disk-cleanup` (or check the box in `openamer plugins`).

**Disabling again:** `openamer plugins disable disk-cleanup`.
...
**Enabling:** `openamer plugins enable security-guidance` (or check the box in `openamer plugins`).

**Disabling again:** `openamer plugins disable security-guidance`.
...
Traces OpenAmer turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of OpenAmer' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `openamer logs`.
...
```bash
openamer tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart OpenAmer and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
openamer plugins enable observability/langfuse
```

Then put the credentials in `~/.openamer/.env`:

```bash
OPENAMER_LANGFUSE_PUBLIC_KEY=pk-lf-...
OPENAMER_LANGFUSE_SECRET_KEY=sk-lf-...
OPENAMER_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "OpenAmer turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `OPENAMER_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the OpenAmer session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `openamer chat` session lives under one Langfuse session.

**Verify:**

```bash
openamer plugins list                 # observability/langfuse should show "enabled"
openamer chat -q "hello"              # check the Langfuse UI for a "OpenAmer turn" trace
```
...
| `OPENAMER_LANGFUSE_RELEASE` | — | Release/version tag |
| `OPENAMER_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `OPENAMER_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `OPENAMER_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

OpenAmer-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — OpenAmer-prefixed wins when both are set.
...
**Disabling:** `openamer plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.openamer/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
openamer plugins enable google_meet
...
**Disabling:** `openamer plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.openamer/cache/google_meet/` until you remove them.

### openamer-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real OpenAmer session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with OpenAmer feature changes.

**How it works:**

- Scans your entire `~/.openamer/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$OPENAMER_HOME/plugins/openamer-achievements/state.json`
...
| Secret | Hidden until OpenAmer detects the first related signal in your history |

**API** — routes mount under `/api/plugins/openamer-achievements/`:
...
**State files** — live under `$OPENAMER_HOME/plugins/openamer-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across OpenAmer updates. |
...
**Enabling:** Nothing to enable — `openamer-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `openamer dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/openamer-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.openamer/plugins/openamer-achievements/` that ships no dashboard. The plugin's state files under `$OPENAMER_HOME/plugins/openamer-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other OpenAmer plugin — see [Build a OpenAmer Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.openamer/plugins/<name>/`
- Manifest source is reported as `bundled` in `openamer plugins list`
