...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `openamer tools`.
...
OpenAmer Agent generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `openamer tools` and persists in `config.yaml`.
...
If you have a paid [your hosted provider](https://portal.openamer.com) subscription, you can use image generation through the **[Tool Gateway](tool-gateway.md)** without a FAL API key. Your model selection persists across both paths. New installs can run `openamer setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **OpenAmer Subscription** as the image-gen backend via `openamer tools`.
...
Navigate to **🎨 Image Generation**, pick your backend (OpenAmer Subscription or FAL.ai), then the picker shows all supported models in a column-aligned table — arrow keys to navigate, Enter to select:
...
image_gen:
  ...
  use_gateway: false            # true if using a hosted provider Subscription
```

### GPT-Image Quality

The `fal-ai/gpt-image-1.5` and `fal-ai/gpt-image-2` request quality is pinned to `medium` (~$0.034–$0.06/image at 1024×1024). We don't expose the `low` / `high` tiers as a user-facing option so that your hosted provider billing stays predictable across all users — the cost spread between tiers is 3–22×. If you want a cheaper option, pick Klein 9B or Z-Image Turbo; if you want higher quality, use Nano Banana Pro or Recraft V4 Pro.
...
as a tool the chat model may call, and OpenAmer cannot force the call — the
...
3. **Submission** — `_submit_fal_request()` routes via direct FAL credentials or the managed a hosted provider gateway.
...
- **Requires credentials** for the active backend (FAL `FAL_KEY` / a hosted provider Subscription, `OPENAI_API_KEY`, xAI OAuth, `KREA_API_KEY`)
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; OpenAmer materializes them to the local cache so delivery still works after expiry
