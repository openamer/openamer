...
Type `@` followed by a reference to inject content directly into your message. OpenAmer expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- OpenAmer env: `$OPENAMER_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$OPENAMER_HOME/skills/.hub/`
