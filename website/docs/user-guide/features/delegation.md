...
Top-level model calls run in the background automatically. OpenAmer returns a handle immediately so the conversation can continue, then posts the result back as a new message. An orchestrator subagent waits for its own workers so it can synthesize their results before returning.
...
When a top-level agent provides a `tasks` array, OpenAmer returns one background handle, runs the subagents in parallel, and posts one consolidated result after every child finishes. An orchestrator subagent waits for its batch in the current turn so it can synthesize the results.
...
When a background delegation finishes, OpenAmer stores its completion event in
the active profile's `state.db` before publishing it to the normal fresh-turn
queue. If OpenAmer restarts after completion but before delivery, the pending
...
OpenAmer cannot prove whether its external side effects happened. Pending and
...
With a hard cap configured, if a subagent times out having made **zero** API calls (usually: provider unreachable, auth failure, or tool-schema rejection), `delegate_task` writes a structured diagnostic to `~/.openamer/logs/subagent-timeout-<session>-<timestamp>.log` containing the subagent's config snapshot, credential-resolution trace, and any early error messages. Much easier to root-cause than the previous silent-timeout behavior.
:::
...
```
<openamer_home>/cache/delegation/live/<delegation_id>/task-<n>.log
```
...
```bash
tail -f ~/.openamer/cache/delegation/live/deleg_ab12cd34/task-0.log
```
...
Top-level model-facing `delegate_task` calls run in the background automatically where the session supports later delivery. OpenAmer returns a handle immediately, and the result re-enters the conversation after the child or batch finishes. Orchestrator subagents wait for their workers in the current turn because they must synthesize those results before returning. Stateless request/response endpoints fall back to synchronous execution when they cannot deliver a detached result later.
...
- A OpenAmer process restart does **not** resume a running child. Its attempt becomes `unknown` because OpenAmer cannot prove which side effects happened.
