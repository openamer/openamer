...
When OpenAmer Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.openamer/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.openamer/config.yaml` (switchable per session
via `/personality`).
...
If you use OpenAmer' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.openamer/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. OpenAmer Agent's deliverable
