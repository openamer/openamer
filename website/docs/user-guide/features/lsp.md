...
OpenAmer runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. OpenAmer
...
1. OpenAmer captures a baseline of current diagnostics for the file.
...
…). OpenAmer auto-detects the binary on PATH or in
`<OPENAMER_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point OpenAmer at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<OPENAMER_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`openamer lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — OpenAmer installs both packages together when you
run `openamer lsp install typescript` or auto-install fires on first
use.

## CLI

```
openamer lsp status          # service state + per-server install status
openamer lsp list            # registry, optionally --installed-only
openamer lsp install <id>    # eagerly install one server
openamer lsp install-all     # try every server with a known recipe
openamer lsp restart         # tear down running clients
openamer lsp which <id>      # print resolved binary path
```

`openamer lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <OPENAMER_HOME>/lsp/bin
...
When `install_strategy: auto`, OpenAmer installs binaries into
`<OPENAMER_HOME>/lsp/bin/`. NPM packages land in
`<OPENAMER_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully OpenAmer-owned and is
...
Servers are kept alive for the life of the OpenAmer process. There's
...
**`openamer lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<OPENAMER_HOME>/lsp/bin/`. Run
`openamer lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `openamer lsp status`**
...
When `openamer lsp status` shows a `Backend warnings` section, install
...
`~/.openamer/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.openamer/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `openamer lsp restart` to clear the set;
