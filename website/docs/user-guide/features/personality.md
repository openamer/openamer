...
description: "Customize OpenAmer Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

OpenAmer Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `OPENAMER_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who OpenAmer is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

OpenAmer now seeds a default `SOUL.md` automatically in:

```text
~/.openamer/SOUL.md
```

More precisely, it uses the current instance's `OPENAMER_HOME`, so if you run OpenAmer with a custom home directory, it will use:

```text
$OPENAMER_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- OpenAmer creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- OpenAmer loads `SOUL.md` only from `OPENAMER_HOME`
- OpenAmer does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, OpenAmer falls back to a built-in default identity
...
If OpenAmer loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `OPENAMER_HOME`, the personality belongs to the OpenAmer instance itself.

That also makes it easier to teach users:
- "Edit `~/.openamer/SOUL.md` to change OpenAmer' default personality."
...
- how OpenAmer should handle uncertainty, disagreement, or ambiguity
...
## What OpenAmer injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, OpenAmer falls back to a built-in default identity ("You are OpenAmer Agent, an intelligent AI assistant created by the OpenAmer project..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
OpenAmer ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain OpenAmer, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives OpenAmer its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.openamer/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.openamer/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-openamer)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how OpenAmer speaks
- `display.skin` and `/skin` affect how OpenAmer looks in the terminal
