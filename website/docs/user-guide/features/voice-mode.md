...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with OpenAmer Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

OpenAmer Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with OpenAmer](/guides/use-voice-mode-with-openamer).

## Prerequisites

Before using voice features, make sure you have:

1. **OpenAmer Agent installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `openamer model` or set your preferred provider credentials in `~/.openamer/.env`
3. **A working base setup** — run `openamer` to verify the agent responds to text before enabling voice
...
cd ~/.openamer/openamer-agent && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.openamer/openamer-agent && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.openamer/openamer-agent && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.openamer/openamer-agent && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`openamer chat`) and the **TUI** (`openamer --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.openamer/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
openamer                # Start the interactive CLI
```
...
1. Start the CLI with `openamer` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.openamer/config.yaml` (default: `ctrl+b`).
:::
...
```bash
openamer gateway        # Start the gateway (connects to configured platforms)
openamer gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@openamerbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.openamer/.env`:
```bash
...
```bash
openamer gateway        # Start with existing configuration
```
...
If you are running OpenAmer inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.openamer/.env`
...
- Review gateway logs: `tail -f ~/.openamer/logs/gateway.log`
