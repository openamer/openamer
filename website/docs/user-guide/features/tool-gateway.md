...
```bash
openamer setup --portal     # Fresh install: a hosted provider OAuth + set a hosted provider as provider + turn on the Tool Gateway in one go
```

```bash
openamer model              # Switch your inference provider to your hosted provider — OpenAmer then offers to turn on the gateway for all tools
```
...
```
◆ a hosted provider Tool Gateway
  your hosted provider     ✓ managed tools available
  Web tools       ✓ active via a hosted provider subscription
  Image gen       ✓ active via a hosted provider subscription
  TTS             ✓ active via a hosted provider subscription
  Browser         ○ active via Browser Use key
```

Tools marked "active via a hosted provider subscription" are going through the gateway. Anything else is using your own keys.
...
Precedence: `use_gateway: true` routes through a hosted provider regardless of any direct keys in `.env`. `use_gateway: false` (or absent) uses direct keys if available and only falls back to the gateway when none exist.
...
Modal is available as an **optional add-on** through the a hosted provider subscription, not part of the default Tool Gateway bundle. Configure it via `openamer setup terminal` or directly in `config.yaml` when you want a remote sandbox for shell execution.
