...
`~/.openamer/config.yaml`, `~/.openamer/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/openamer`:

```text
/etc/openamer/
├── config.yaml     # managed config layer (wins over ~/.openamer/config.yaml)
└── .env            # managed env layer (wins over ~/.openamer/.env + shell)
```
...
The location can be relocated with the `OPENAMER_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `OPENAMER_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by OpenAmer.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export OPENAMER_MANAGED_DIR=/opt/org/openamer-policy
```

:::warning
A user who can set `OPENAMER_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `openamer doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/openamer/config.yaml` (managed) | `/etc/openamer/.env` (managed) |
| 2 | `~/.openamer/config.yaml` (user) | `~/.openamer/.env` (user) |
...
```bash
openamer config        # shows a header naming the managed source + the pinned keys
openamer doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, OpenAmer refuses and names the source:

```bash
$ openamer config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/openamer/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `openamer config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/openamer
...
sudo tee /etc/openamer/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/openamer
sudo chmod 0644 /etc/openamer/config.yaml /etc/openamer/.env
```

Changes take effect on the next OpenAmer start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`openamer doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs OpenAmer as `root`), managed scope is advisory.
