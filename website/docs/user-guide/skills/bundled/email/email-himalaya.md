...
This skill is separate from the OpenAmer Email gateway adapter. The gateway
adapter lets people email the agent and uses OpenAmer' built-in IMAP/SMTP
...
To reply non-interactively from OpenAmer, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from OpenAmer)** — pipe the message via stdin:
