...
| | |
|---|---|
| Source | Optional — install with `openamer skills install official/mcp/fastmcp` |
...
| Related skills | [`openamer-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-openamer-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into OpenAmer or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to OpenAmer. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.openamer/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.openamer/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is OpenAmer integration, either:

- configure the server in `~/.openamer/config.yaml` using the `native-mcp` skill, or
...
### OpenAmer cannot see the deployed server

The server-building part may be correct while the OpenAmer config is not. Load the `native-mcp` skill and configure the server in `~/.openamer/config.yaml`, then restart OpenAmer.
