...
| | |
|---|---|
| Source | Optional — install with `openamer skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${OPENAMER_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${OPENAMER_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `openamer`)    |
