...
| | |
|---|---|
| Source | Optional — install with `openamer skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), OpenAmer Agent |
| License | MIT |
...
The script reads `${OPENAMER_HOME:-~/.openamer}/.env` for two optional defaults:
...
Helper script: `~/.openamer/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${OPENAMER_HOME:-~/.openamer}/.env`.
