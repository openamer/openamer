...
| | |
|---|---|
| Source | Optional — install with `openamer skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by OpenAmer Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during OpenAmer terminal calls (desktop app flow only)
...
### Service Account (recommended for OpenAmer)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${OPENAMER_HOME:-~/.openamer}/.env` (the skill will prompt for this on first load).
...
## OpenAmer Execution Pattern (desktop app flow)

OpenAmer terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/openamer-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/openamer-op.sock"
