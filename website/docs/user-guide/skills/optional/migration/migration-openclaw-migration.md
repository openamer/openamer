---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into OpenAmer"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into OpenAmer"
---
...
Import an OpenClaw setup (memories, skills) into OpenAmer.
...
# OpenClaw -> OpenAmer Migration

Use this skill when a user wants to move their OpenClaw setup into OpenAmer Agent with minimal manual cleanup.
...
```bash
openamer claw migrate              # Full interactive migration
...
openamer claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `openamer setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_openamer.py` to:

- import `SOUL.md` into the OpenAmer home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into OpenAmer memory entries
- merge OpenClaw command approval patterns into OpenAmer `command_allowlist`
- migrate OpenAmer-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to OpenAmer working-directory configuration
- copy OpenClaw skills into `~/.openamer/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen OpenAmer workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.openamer/tts/`
- archive non-secret docs that do not have a direct OpenAmer destination
...
- `scripts/openclaw_to_openamer.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.openamer/skills/migration/openclaw-migration/scripts/openclaw_to_openamer.py`

Do not guess a shorter path like `~/.openamer/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.openamer/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of OpenAmer-compatible secrets, currently:
...
OpenAmer CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in OpenAmer. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a OpenAmer workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing OpenAmer skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.openamer/skills/migration/openclaw-migration/scripts/openclaw_to_openamer.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.openamer/skills/migration/openclaw-migration/scripts/openclaw_to_openamer.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.openamer/skills/migration/openclaw-migration/scripts/openclaw_to_openamer.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.openamer/skills/migration/openclaw-migration/scripts/openclaw_to_openamer.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.openamer/skills/migration/openclaw-migration/scripts/openclaw_to_openamer.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.openamer/skills/migration/openclaw-migration/scripts/openclaw_to_openamer.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of OpenAmer unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty OpenAmer targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean OpenAmer destination. Unsupported auth blobs must still be reported as skipped.
...
- OpenAmer persona state imported
- OpenAmer memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.openamer/skills/openclaw-imports/`
