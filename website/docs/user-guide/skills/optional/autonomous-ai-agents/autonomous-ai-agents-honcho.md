---
title: "Honcho — Configure and troubleshoot Honcho memory for OpenAmer"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for OpenAmer"
---
...
Configure and troubleshoot Honcho memory for OpenAmer.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `openamer skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for OpenAmer

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every OpenAmer profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/openamer#running-honcho-locally-with-openamer

### Verify

```bash
openamer honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this OpenAmer profile's AI peer
...
Honcho models conversations as interactions between **peers**. OpenAmer creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this OpenAmer instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each OpenAmer run |
| `global` | Single session across all directories |

Manual override: `openamer honcho map my-project-name`
...
Each OpenAmer profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block openamer.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `openamer.coder` host block in `honcho.json`
...
```bash
openamer honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for OpenAmer when Honcho memory is active.
...
Config file: `$OPENAMER_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `openamer honcho setup`. Ensure `memory.provider: honcho` is in `~/.openamer/config.yaml`.

### Memory not persisting across sessions
Check `openamer honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `openamer profile create <name> --clone`. For existing profiles: `openamer honcho sync`.
...
| Command | Description |
|---------|-------------|
| `openamer honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `openamer honcho status` | Show resolved config, connection test, peer info for active profile |
| `openamer honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `openamer honcho disable` | Disable Honcho for the active profile |
| `openamer honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `openamer honcho peers` | Show peer identities across all profiles |
| `openamer honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `openamer honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `openamer honcho sessions` | List known directory-to-session-name mappings |
| `openamer honcho map <name>` | Map current working directory to a Honcho session name |
| `openamer honcho identity` | Seed AI peer identity or show both peer representations |
| `openamer honcho sync` | Create host blocks for all OpenAmer profiles that don't have one yet |
| `openamer honcho migrate` | Step-by-step migration guide from OpenClaw native memory to OpenAmer + Honcho |
| `openamer memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `openamer memory status` | Show active memory provider and config |
| `openamer memory off` | Disable external memory provider |
