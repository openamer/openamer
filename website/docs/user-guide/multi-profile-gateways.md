...
If you only run one OpenAmer agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more OpenAmer agents that should all
...
(`ai.openamer.gateway-<name>.plist`) or systemd user service
(`openamer-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
openamer profile create coder
openamer profile create personal-bot
openamer profile create research
...
```bash
openamer config set gateway.multiplex_profiles true
openamer gateway restart
```

Equivalently, in the default profile's `~/.openamer/config.yaml`:
...
You do **not** run `openamer gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `openamer gateway start` / `run` is a
...
home). `openamer status` reports the multiplexer and the profiles it serves;
`openamer status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/openamer-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: openamer-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    openamer gateway "$action"
  else
    openamer -p "$profile" gateway "$action"
  fi
}
...
```bash
openamer-gateways start      # start every configured profile
openamer-gateways stop       # stop every configured profile
openamer-gateways restart    # restart all
openamer-gateways status     # status across all
openamer-gateways list       # delegates to `openamer gateway list`
```

:::tip
The `default` profile is targeted with `openamer gateway <action>` (no `-p`),
not `openamer -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `openamer -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.openamer.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/openamer-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.openamer.gateway.plist` /
`openamer-gateway.service`.
...
```bash
# Default profile
tail -f ~/.openamer/logs/gateway.log
tail -f ~/.openamer/logs/gateway.error.log

# Named profile
tail -f ~/.openamer/profiles/<name>/logs/gateway.log
tail -f ~/.openamer/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.openamer/logs/gateway.log ~/.openamer/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
openamer logs -f                  # follow default profile
openamer -p coder logs -f         # follow one profile
openamer logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
openamer profile list             # profiles + model + gateway state
openamer-gateways status          # full status across every profile
launchctl list | grep openamer    # macOS — PIDs and labels
systemctl --user list-units 'openamer-gateway-*'   # Linux — units
```
...
```
~/.openamer/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.openamer/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
openamer config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.openamer/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=openamer --why="gateways running" \
  sleep infinity &
...
`openamer-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.openamer/.env ~/.openamer/profiles/*/.env
```

## Updating the code

`openamer update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `openamer gateway start` after a previous `openamer gateway stop`. The
...
```bash
ps -ef | grep "openamer_cli.*-p <profile>"
cat ~/.openamer/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.openamer.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.openamer.gateway-<profile>.plist

# Linux
systemctl --user restart openamer-gateway-<profile>.service
```

### Health check

```bash
openamer doctor                  # default profile
openamer -p <profile> doctor     # one profile
```
