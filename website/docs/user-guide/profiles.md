...
Run multiple independent OpenAmer agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate OpenAmer home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up OpenAmer state.
...
```bash
openamer profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own OpenAmer profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `openamer setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
openamer profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `openamer profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
Copies your current profile's `config.yaml`, `.env`, `SOUL.md`, and skills into the new profile. Same API keys, model, and capabilities, but fresh sessions and memory. Edit `~/.openamer/profiles/work/.env` for different API keys, or `~/.openamer/profiles/work/SOUL.md` for a different personality.
...
```bash
openamer profile create work-backup --clone-from coder --clone-all
```
...
The alias works with every openamer subcommand — it's just `openamer -p <name>` under the hood.
...
```bash
openamer -p coder chat
openamer --profile=coder doctor
openamer chat -p coder -q "hello"    # works in any position
```

### Sticky default (`openamer profile use`)

```bash
openamer profile use coder
openamer chat                   # now targets coder
openamer tools                  # configures coder's tools
openamer profile use default    # switch back
```

Sets a default so plain `openamer` commands target that profile. Like `kubectl config use-context`.
...
- **`openamer profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives OpenAmer its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory OpenAmer was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.openamer/profiles/coder/.env

# Edit assistant's tokens
nano ~/.openamer/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates openamer-gateway-coder systemd/launchd service
assistant gateway install     # creates openamer-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `openamer profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `openamer -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.openamer/profiles/coder/SOUL.md
```
...
default for **future CLI/gateway runs** (same as `openamer profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`openamer update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
openamer profile list           # show all profiles with status
openamer profile show coder     # detailed info for one profile
openamer profile rename coder dev-bot   # rename (updates alias + service)
openamer profile export coder   # export to coder.tar.gz
openamer profile import coder.tar.gz   # import from archive
```

## Deleting a profile

```bash
openamer profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `openamer profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.openamer`). To remove everything, use `openamer uninstall`.
:::
...
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, OpenAmer keeps it as the real user home by default so tools like
