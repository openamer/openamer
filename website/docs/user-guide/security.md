...
OpenAmer Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, OpenAmer checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.openamer/config.yaml`:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that OpenAmer refuses to run them **regardless** of:
...
Configure the timeout in `~/.openamer/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.openamer/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.openamer/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` openamer/gateway | Self-termination prevention |
...
Commands approved with "always" are saved to `~/.openamer/config.yaml`:
...
Do not ask the agent to `patch` `~/.openamer/cron/jobs.json` directly. Use the `cronjob` tool, [`openamer cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other OpenAmer control files when write safety blocks direct edits.
...
When running the messaging gateway, OpenAmer controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.openamer/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.openamer/.env to allow open access,
...
For more flexible authorization, OpenAmer includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `openamer pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.openamer/config.yaml`:
...
openamer pairing list

# Approve a pairing code
openamer pairing approve telegram ABC12DEF

# Revoke a user's access
openamer pairing revoke telegram 123456789

# Clear all pending codes
openamer pairing clear-pending
```

:::tip Docker users: run pairing commands as the `openamer` user
The official Docker image runs the gateway as the unprivileged `openamer` user
...
Always pass `-u openamer`:

```bash
docker exec -u openamer openamer-agent openamer pairing approve telegram ABC12DEF
```
...
When using the `docker` terminal backend, OpenAmer applies strict security hardening to every container.
...
Container resources are configurable in `~/.openamer/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.openamer/sandboxes/docker/<task_id>/`
...
| **terminal** (local) | Blocks explicit OpenAmer infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- OpenAmer infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
OpenAmer integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run OpenAmer under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.openamer/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.openamer/logs/` for unauthorized access attempts
10. **Keep updated** — run `openamer update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.openamer/.env`:
...
TERMINAL_SSH_USER=openamer
TERMINAL_SSH_KEY=~/.ssh/openamer_agent_key
```
...
OpenAmer ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `openamer_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `openamer doctor` for the full remediation.
- **`openamer doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
```bash
openamer doctor --ack <advisory-id>
```
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. OpenAmer installs these **lazily** on first use rather than eagerly under `openamer-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `openamer tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `openamer tools`.
