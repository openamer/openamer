...
description: "Run OpenAmer Agent directly on an Android phone with Termux"
---

# OpenAmer on Android with Termux
...
OpenAmer Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop OpenAmer from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `openamer` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `openamer` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/openamer" "$PREFIX/bin/openamer"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `openamer` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.openamer/.env`.
...
### `openamer doctor` says ripgrep or Node is missing
