...
description: "Install and deploy OpenAmer Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
OpenAmer Agent ships a Nix flake & a NixOS module.
...
**For non-NixOS users**, this only changes the install step. Everything after (`openamer setup`, `openamer gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage openamer the same way you manage any other NixOS service.
:::
...
After `nix profile install`, `openamer`, `openamer-agent`, and `openamer-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `openamer setup` walks you through provider selection, `openamer gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.openamer/`.

:::warning Messaging platforms (Discord, Telegram, Slack)
The default package includes ALL libraries openamer-agent might need. if you want a smaller variant, check the other flake outputs. 
...
# /etc/nixos/flake.nix (or your system flake)
{
  ...
  outputs = { nixpkgs, openamer-agent, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        openamer-agent.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
# configuration.nix
{ config, ... }: {
  services.openamer-agent = {
    enable = true;
    settings.model.default = "anthropic/claude-sonnet-4";
    environmentFiles = [ config.sops.secrets."openamer-env".path ];
    addToSystemPackages = true;
  };
}
```

That's it. `nixos-rebuild switch` creates the `openamer` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o openamer /dev/stdin /var/lib/openamer/env
```

```nix
services.openamer-agent.environmentFiles = [ "/var/lib/openamer/env" ];
```
:::
...
:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `openamer` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.
...
Users listed in `hostUsers` are automatically added to the `openamer` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo openamer chat` manually.
:::
...
systemctl status openamer-agent

# Watch logs (Ctrl+C to stop)
journalctl -u openamer-agent -f

# If addToSystemPackages is true, test the CLI
openamer version
openamer config       # shows the generated config
```
...
```nix
# base.nix
services.openamer-agent.settings = {
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, OpenAmer defaults to OpenRouter.
:::
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."openamer-env".path ]` |
| Give the agent a personality | `${services.openamer-agent.stateDir}/.openamer/SOUL.md` | manage the file directly |
...
| Override the openamer package | `package` | `inputs.openamer-agent.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/openamer"` |
...
{
  sops = {
    defaultSopsFile = ./secrets/openamer.yaml;
    age.keyFile = "/home/user/.config/sops/age/keys.txt";
    secrets."openamer-env" = { format = "yaml"; };
  };

  services.openamer-agent.environmentFiles = [
    config.sops.secrets."openamer-env".path
  ];
}
```
...
```yaml
# secrets/openamer.yaml (encrypted with sops)
openamer-env: |
...
```nix
{
  age.secrets.openamer-env.file = ./secrets/openamer-env.age;

  services.openamer-agent.environmentFiles = [
    config.age.secrets.openamer-env.path
  ];
}
```
...
{
  services.openamer-agent = {
    authFile = config.sops.secrets."openamer/auth.json".path;
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). OpenAmer looks for specific filenames by convention:
...
```nix
{
  services.openamer-agent.documents = {
...
```nix
{
  services.openamer-agent.mcpServers = {
    filesystem = {
      command = "npx";
      args = [ "-y" "@modelcontextprotocol/server-filesystem" "/data/workspace" ];
    };
...
```nix
{
  services.openamer-agent.mcpServers.remote-api = {
...
Set `auth = "oauth"` for servers using OAuth 2.1. OpenAmer implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.

```nix
{
  services.openamer-agent.mcpServers.my-oauth-server = {
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, OpenAmer prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u openamer` (native):

```bash
# Container mode
docker exec -it openamer-agent \
...
```bash
openamer mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.openamer/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/openamer/.openamer/mcp-tokens/
# Ensure: chown openamer:openamer, chmod 0600
```
...
```nix
{
  services.openamer-agent.mcpServers.analysis = {
    command = "npx";
...
When openamer runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `openamer setup` | Config is declarative — edit `settings` in your Nix config |
| `openamer config edit` | Config is generated from `settings` |
| `openamer config set <key> <value>` | Config is generated from `settings` |
| `openamer gateway install` | The systemd service is managed by NixOS |
| `openamer gateway uninstall` | The systemd service is managed by NixOS |
...
When container mode is enabled, openamer runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-openamer-agent-0.1.0  ──►  /nix/store/... (ro)
~/.openamer -> /var/lib/openamer/.openamer       (symlink bridge, per hostUsers)
/var/lib/openamer/                    ──►  /data/          (rw)
...
The Nix-built binary works inside the Ubuntu container because `/nix/store` is bind-mounted — it brings its own interpreter and all dependencies, so there's no reliance on the container's system libraries. The container entrypoint resolves through a `current-package` symlink: `/data/current-package/bin/openamer gateway run --replace`. On `nixos-rebuild switch`, only the symlink is updated — the container keeps running.

### What Persists Across What

| Event | Container recreated? | `/data` (state) | `/home/openamer` | Writable layer (`apt`/`pip`/`npm`) |
|---|---|---|---|---|
| `systemctl restart openamer-agent` | No | Persists | Persists | Persists |
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the openamer package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/openamer` is preserved (these are bind mounts).

If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/openamer-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current openamer package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `openamer plugins install` needed.

### Directory Plugins (`extraPlugins`)

For plugins that are just a source tree with `plugin.yaml` + `__init__.py` (e.g., [openamer-lcm](https://github.com/stephenschoettler/openamer-lcm)):

```nix
services.openamer-agent.extraPlugins = [
...
For pip-packaged plugins that register via `[project.entry-points."openamer_agent.plugins"]` (e.g., [rtk-openamer](https://github.com/ogallotti/rtk-openamer)):

```nix
services.openamer-agent.extraPythonPackages = [
...
The package's `site-packages` is added to PYTHONPATH in the openamer wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in openamer-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.

```nix
# Enable Discord, Telegram, Slack
services.openamer-agent.extraDependencyGroups = [ "messaging" ];
```
...
```nix
services.openamer-agent.settings.plugins.enabled = [
  "openamer-lcm"
  "rtk-rewrite"
];
```
...
:::note
A build-time collision check prevents plugin packages from shadowing core openamer dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the openamer-agent service |
| `package` | `package` | `openamer-agent` | The openamer-agent package to use |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.openamer` symlink to the service stateDir and are auto-added to the `openamer` group |
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | OpenAmer binary + all Nix deps |
| `/data` | `/var/lib/openamer` | `rw` | All state, config, workspace |
| `/home/openamer` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |
...
cd /etc/nixos && nix flake update openamer-agent
...
```bash
systemctl status openamer-agent
docker ps -a --filter name=openamer-agent
docker inspect openamer-agent --format='{{.State.Status}}'
docker exec -it openamer-agent bash
docker exec openamer-agent readlink /data/current-package
docker exec openamer-agent cat /data/.container-identity
```
...
```bash
sudo systemctl stop openamer-agent
docker rm -f openamer-agent
sudo rm /var/lib/openamer/.container-identity
sudo systemctl start openamer-agent
```
...
```bash
# Native mode
sudo -u openamer cat /var/lib/openamer/.openamer/.env

# Container mode
docker exec openamer-agent cat /data/.openamer/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec openamer-agent readlink /data/current-package)
```
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u openamer-agent` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
| Container recreated unexpectedly | `extraVolumes`, `extraOptions`, or `image` changed | Expected — writable layer resets. Reinstall packages or use a custom image |
| `openamer version` shows old version | Container not restarted | `systemctl restart openamer-agent` |
| Permission denied on `/var/lib/openamer` | State dir is `0750 openamer:openamer` | Use `docker exec` or `sudo -u openamer` |
| `nix-collect-garbage` removed openamer | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "openamer-agent"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
| `unable to find user openamer` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart openamer-agent` |
