"""Shared constants for OpenAmer Agent.
...
def set_openamer_home_override(path: str | Path | None) -> Token:
    """Set a context-local OpenAmer home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _OPENAMER_HOME_OVERRIDE.set(value)


def reset_openamer_home_override(token: Token) -> None:
    """Restore the previous context-local OpenAmer home override."""
    _OPENAMER_HOME_OVERRIDE.reset(token)


def get_openamer_home_override() -> str | None:
    """Return the active context-local OpenAmer home override, if any."""
    override = _OPENAMER_HOME_OVERRIDE.get()
...
def _openamer_home_from_env() -> Path:
    """Resolve OPENAMER_HOME from the process environment only.

    Reads the ``OPENAMER_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_openamer_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_openamer_home`
    and :func:`get_process_openamer_home` so the two never drift.
    """
...
def _warn_profile_fallback_once() -> None:
    ...
    Guard: if a non-default profile is sticky-active but ``OPENAMER_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_openamer_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[OPENAMER_HOME fallback] OPENAMER_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass OPENAMER_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_openamer_home() -> Path:
    """Return the OpenAmer home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_openamer_home_override`) → ``OPENAMER_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``OPENAMER_HOME`` is unset but an ``active_profile`` file indicates
    ...
    if not os.environ.get("OPENAMER_HOME", "").strip():
        _warn_profile_fallback_once()

    return _openamer_home_from_env()


def get_process_openamer_home() -> Path:
    """Return the OpenAmer home for the running process, ignoring task overrides.

    Unlike :func:`get_openamer_home`, this never follows the context-local
    override set by :func:`set_openamer_home_override`.  It resolves only the
    process ``OPENAMER_HOME`` env var (falling back to the platform default),
...
def get_default_openamer_root() -> Path:
    """Return the root OpenAmer directory for profile-level operations.

    In standard deployments this is the platform-native OpenAmer home
    (``~/.openamer`` on POSIX, ``%LOCALAPPDATA%\\openamer`` on native Windows).

    In Docker or custom deployments where ``OPENAMER_HOME`` points outside
    ``~/.openamer`` (e.g. ``/opt/data``), returns ``OPENAMER_HOME`` directly
    — that IS the root.

    In profile mode where ``OPENAMER_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.openamer/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_openamer_home()
    ...
    try:
        ...
        # OPENAMER_HOME is under ~/.openamer (normal or profile mode)
        return native_home
    except ValueError:
        pass
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``OPENAMER_OPTIONAL_SKILLS``.
    """
    override = os.getenv("OPENAMER_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_openamer_home() / "optional-skills"


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers.

    Mirrors :func:`get_optional_skills_dir` for the MCP catalog (OpenAmer-approved
    Model Context Protocol servers shipped with the repo but disabled by
    default). Packaged installs may ship ``optional-mcps`` outside the Python
    package tree and expose it via ``OPENAMER_OPTIONAL_MCPS``.
    """
    override = os.getenv("OPENAMER_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_openamer_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``OPENAMER_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<OPENAMER_HOME>/skills`` last-resort
    """
    override = os.getenv("OPENAMER_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_openamer_dir(new_subpath: str, old_name: str) -> Path:
    ...
    Args:
        new_subpath: Preferred path relative to OPENAMER_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to OPENAMER_HOME (e.g. ``"image_cache"``).
...
_OPENAMER_NODE_TARGET_MAJOR = int(os.environ.get("OPENAMER_NODE_TARGET_MAJOR", "22"))
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%OPENAMER_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_openamer_home()
    index_url = f"https://nodejs.org/dist/latest-v{_OPENAMER_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_OPENAMER_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_openamer_managed_node() -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "OPENAMER_HOME": str(get_openamer_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for OpenAmer-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy OpenAmer-managed installs.

    This is for OpenAmer-owned subprocesses that should not be broken by a bad,
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("OPENAMER_REAL_HOME") or os.getenv("OPENAMER_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding OpenAmer profile HOME.

    ``OPENAMER_HOME`` scopes OpenAmer state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={OPENAMER_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{OPENAMER_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{OPENAMER_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply OpenAmer' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["OPENAMER_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when OpenAmer itself runs inside WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under OPENAMER_HOME.

    Replaces the ``get_openamer_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, openamer_logging.py, openamer_time.py, etc.).
    """
    return get_openamer_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under OPENAMER_HOME."""
    return get_openamer_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under OPENAMER_HOME."""
