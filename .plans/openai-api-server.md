# OpenAI-Compatible API Server for OpenAmer Agent
...
SSE streaming. By exposing this endpoint, openamer-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  openamer-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.openamer/.env`
2. Run `openamer gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with openamer-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns openamer-agent as a model) |
...
Authorization: Bearer openamer-api-key-here
...
But openamer-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "openamer-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured openamer-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of openamer-agent's core prompt
...
Once implemented, openamer-agent works as a drop-in backend for:
