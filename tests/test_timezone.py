"""
Tests for timezone support (openamer_time module + integration points).
...
import openamer_time


def _reset_openamer_time_cache():
    """Reset the openamer_time module cache (replacement for removed reset_cache)."""
    openamer_time._cached_tz = None
    openamer_time._cached_tz_name = None
    openamer_time._cache_resolved = False


# =========================================================================
# openamer_time.now() — core helper
# =========================================================================

class TestOpenAmerTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_openamer_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["OPENAMER_TIMEZONE"] = "Asia/Kolkata"
        result = openamer_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["OPENAMER_TIMEZONE"] = "UTC"
        result = openamer_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["OPENAMER_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="openamer_time"):
            result = openamer_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["OPENAMER_TIMEZONE"] = "UTC"
        _reset_openamer_time_cache()
        r1 = openamer_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["OPENAMER_TIMEZONE"] = "Asia/Kolkata"
        _reset_openamer_time_cache()
        r2 = openamer_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["OPENAMER_TIMEZONE"] = "Europe/London"
        tz = openamer_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["OPENAMER_TIMEZONE"] = "Not/A/Timezone"
        tz = openamer_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When OPENAMER_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) OPENAMER_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("OPENAMER_TIMEZONE=" + os.environ.get("OPENAMER_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "OPENAMER_TIMEZONE=NOT_SET" in result["output"], (
            "OPENAMER_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When OPENAMER_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("OPENAMER_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=openamer_tz) which shifted
        absolute time when system-local tz != OpenAmer tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_openamer_tz(self):
        """Already-aware datetimes should be normalized to OpenAmer tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in OpenAmer tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of OpenAmer tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), OpenAmer is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when OpenAmer tz
        ...
        # Use a OpenAmer timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _openamer_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["OPENAMER_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_openamer_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of OpenAmer timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["OPENAMER_TIMEZONE"] = "US/Eastern"
        _reset_openamer_time_cache()
