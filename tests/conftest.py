"""Shared fixtures for the openamer-agent test suite.
...
2. **Isolated OPENAMER_HOME.** OPENAMER_HOME points to a per-test tempdir so
   code reading ``~/.openamer/*`` via ``get_openamer_home()`` can't see the
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and OPENAMER_HOME to per-test tempdirs so code that
    reads ``~/.openamer/*`` can't touch the real one, and pins TZ/LANG so
    ...
    #    fixture. Any code in the codebase reading ``~/.openamer/*`` via
    #    ``Path.home() / ".openamer"`` instead of ``get_openamer_home()``
    #    is a bug to fix at the callsite.
    fake_openamer_home = tmp_path / "openamer_test"
    fake_openamer_home.mkdir()
    (fake_openamer_home / "sessions").mkdir()
    (fake_openamer_home / "cron").mkdir()
    (fake_openamer_home / "memories").mkdir()
    (fake_openamer_home / "skills").mkdir()
    monkeypatch.setenv("OPENAMER_HOME", str(fake_openamer_home))
    ...
    #    ~/.openamer/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import openamer_cli.plugins as _plugins_mod
...
@pytest.fixture()
def mock_config():
    """Return a minimal openamer config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``openamer-gateway`` process
...
# correlated with a ``tests/openamer_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> openamer-gateway``
...
def _wal_is_usable() -> bool:
    """True when OpenAmer will actually put a database into WAL mode here.

    OpenAmer refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running OpenAmer can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a OpenAmer managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``openamer_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_openamer_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_openamer_home`` fixture
    redirects ``OPENAMER_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.openamer/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``openamer_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where OpenAmer falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — OpenAmer uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart openamer-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting openamer/python patterns are also blocked.
    """
    ...
    def _is_blocked_systemctl(cmd) -> bool:
        ...
        if "systemctl" not in cmd_str:
            return False
        if not _matches_openamer_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch openamer-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m openamer_cli.main".
                if (
                    "openamer" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live openamer-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting openamer/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `openamer update` (or the
        # equivalent `python -m openamer_cli.main update`).  These commands
        ...
        if "update" in low and (
            # openamer update / openamer update --gateway / setsid bash -c ... openamer update
            ("openamer" in low and "update" in low.split())
            or
            # python -m openamer_cli.main update --gateway
            ("openamer_cli" in low and "update" in low.split())
            or
            # venv/bin/openamer update  (absolute path variant used in tests)
            (".venv/bin/openamer" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`openamer update` against the real checkout, fetching "
