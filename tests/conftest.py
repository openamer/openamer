...
2. **Isolated OPENAMER_HOME.** OPENAMER_HOME points to a per-test tempdir so
   code reading ``~/.hermes/*`` via ``get_openamer_home()`` can't see the
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # the real ~/.hermes/kanban.db instead of the per-test OPENAMER_HOME.
    "HERMES_KANBAN_DB",
    "HERMES_KANBAN_BOARD",
    "HERMES_KANBAN_HOME",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    #    ``~/.hermes/*`` via ``get_openamer_home()`` now gets the tempdir.
    #
    ...
    #    ``Path.home() / ".hermes"`` instead of ``get_openamer_home()``
    ...
    monkeypatch.setenv("OPENAMER_HOME", str(fake_hermes_home))
