...
binary download are exercised manually by `openamer secrets bitwarden
...
def openamer_home(tmp_path, monkeypatch):
    """Point OpenAmer at an isolated home directory."""
    home = tmp_path / ".openamer"
    home.mkdir()
    monkeypatch.setenv("OPENAMER_HOME", str(home))
    # Some modules cache get_openamer_home; clear if needed.
...
def test_install_bws_rejects_malicious_member(openamer_home, monkeypatch):
...
def test_install_bws_happy_path(openamer_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(openamer_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(openamer_home, monkeypatch):
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under openamer_home/cache/."""
    home = tmp_path / ".openamer"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
