"""Contract tests for the Docker image's immutable /opt/openamer install tree."""
...
def test_dockerfile_makes_opt_openamer_readonly_for_openamer_user() -> None:
    ...
    assert "chown -R root:root /opt/openamer" not in text
    assert "chmod -R a+rX /opt/openamer" not in text
    assert "chmod -R a-w /opt/openamer" not in text
...
def test_dockerfile_does_not_chown_install_trees_to_openamer() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+openamer:openamer\s+/opt/openamer/\.venv",
        r"chown\s+-R\s+openamer:openamer\s+/opt/openamer/ui-tui",
        r"chown\s+-R\s+openamer:openamer\s+/opt/openamer/gateway",
        r"chown\s+-R\s+openamer:openamer\s+/opt/openamer/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/openamer must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/openamer/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $OPENAMER_HOME data volume (which a host install may also use).
    The stamp is created by root in the shim-wiring RUN block; the openamer
    user can't modify it (go-w from the --chmod on the source COPY).
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/openamer/.install_method" in text

    # The stamp must be in the RUN block that wires the exec shim.
    shim_block = re.search(
        r"RUN mkdir -p /opt/openamer/bin && \\\n"
        r"(?:.*\\\n)+?"
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    assert '"$OPENAMER_HOME/lazy-packages"' in stage2, (
