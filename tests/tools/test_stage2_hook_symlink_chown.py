...
def test_stage2_uses_symlink_safe_helper_for_hermes_home_trees(stage2_text: str) -> None:
    assert 'chown_hermes_tree "$OPENAMER_HOME/$sub"' in stage2_text
    assert 'chown_hermes_tree "$OPENAMER_HOME/profiles"' in stage2_text
    assert 'chown_hermes_tree "$OPENAMER_HOME/cron"' in stage2_text
    assert 'chown -R hermes:hermes "$OPENAMER_HOME/$sub"' not in stage2_text
    assert 'chown -R hermes:hermes "$OPENAMER_HOME/profiles"' not in stage2_text
    assert 'chown -R hermes:hermes "$OPENAMER_HOME/cron"' not in stage2_text
