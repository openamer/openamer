...
class TestGptQualityPinnedToMedium:
    ...
    and cannot be overridden via config. Pinning keeps OpenAmer Portal billing
    ...
    def test_config_quality_setting_is_ignored(self, image_tool):
        ...
        with patch("openamer_cli.config.load_config",
...
class TestManagedGatewayErrorTranslation:
    """4xx from the OpenAmer managed gateway should be translated to a user-actionable message."""

    def test_4xx_translates_to_value_error_with_remediation(self, image_tool, monkeypatch):
        """403 from managed gateway → ValueError mentioning FAL_KEY + openamer tools."""
        ...
        managed_gateway.openamer_user_token = "test-token"
...
class TestManagedKreaRouting:
    ...
    def test_routes_native_krea_model_to_krea_plugin_in_managed_mode(
        ...
        monkeypatch.setattr(
            "openamer_cli.plugins._ensure_plugins_discovered", lambda *a, **k: None
        )
