...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        openamer_home.mkdir()
        (openamer_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"OPENAMER_HOME": str(openamer_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(openamer_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.openamer/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir()
        (openamer_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir()
        (openamer_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir()
        (openamer_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.openamer/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        openamer_home = tmp_path / ".openamer"
        (openamer_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"OPENAMER_HOME": str(openamer_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.openamer")

        assert mounts[0]["container_path"] == "/home/user/.openamer/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.openamer/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.openamer/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape OPENAMER_HOME."""
        monkeypatch.setenv("OPENAMER_HOME", str(tmp_path / ".openamer"))
        (tmp_path / ".openamer").mkdir()

        # Create a sensitive file one level above openamer_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside openamer_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside OPENAMER_HOME must still be registered."""
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir()
        monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))
        (openamer_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_openamer_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of OPENAMER_HOME must be allowed."""
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir()
        subdir = openamer_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside OPENAMER_HOME pointing outside must be rejected."""
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir()
        monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))

        # Create a sensitive file outside openamer_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside openamer_home pointing outside
        symlink = openamer_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes OPENAMER_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, openamer_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape OPENAMER_HOME."""
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir()
        monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))
        ...
        sensitive.write_text("{}")
        self._write_config(openamer_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(openamer_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside OPENAMER_HOME via config must still mount."""
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir()
        monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))

        (openamer_home / "oauth.json").write_text("{}")
        self._write_config(openamer_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir()
        (openamer_home / "cache" / "documents").mkdir(parents=True)
        (openamer_home / "cache" / "audio").mkdir(parents=True)
        (openamer_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.openamer/cache/documents" in paths
        assert "/root/.openamer/cache/audio" in paths
        assert "/root/.openamer/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.openamer/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_openamer_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir()
        # Use legacy dir name with content — get_openamer_dir prefers
        # populated old over new.
        legacy_doc = openamer_home / "document_cache"
        legacy_img = openamer_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(openamer_home / "document_cache") in host_paths
        assert str(openamer_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.openamer/cache/documents" in container_paths
        assert "/root/.openamer/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        openamer_home = tmp_path / ".openamer"
        img_dir = openamer_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.openamer/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.openamer")
            == "/home/agent/.openamer/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        openamer_home = tmp_path / ".openamer"
        (openamer_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(openamer_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        openamer_home = tmp_path / ".openamer"
        doc_dir = openamer_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        openamer_home = tmp_path / ".openamer"
        ss_dir = openamer_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.openamer/cache/screenshots/session_abc/screen1.png"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — OPENAMER_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping OPENAMER_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"OPENAMER_HOME": str(home)}):
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.openamer/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.openamer/google_token.json" in paths
        assert "/root/.openamer/.env" not in paths
