...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("openamer_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("openamer_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("openamer-verify-", "openamer-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "openamer-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/openamer-verify-example.py",
            "rm -f /tmp/openamer-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/openamer-verify-example.py",
            "rm -f /tmp/nested/../openamer-verify-example.py",
            "rm -f /tmp/./openamer-verify-example.py",
            "rm -f /tmp//openamer-verify-example.py",
            "rm -f /tmp/a/../../tmp/openamer-verify-example.py",
            "rm -f /var/tmp/openamer-verify-example.py",
            "rm -f /tmp/unrelated.py",
            "rm -f /tmp/openamer-verify-*",
            "rm -f /tmp/openamer-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/openamer-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/openamer-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_cmd_del_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd /c del /f /q C:\tmp\openamer-victim\file.txt"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_cmd_rmdir_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd.exe /k rmdir /s /q C:\tmp\openamer-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows cmd destructive delete"
    ...
    def test_powershell_remove_item_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"powershell -NoProfile -Command Remove-Item -Recurse -Force C:\tmp\openamer-victim"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_pwsh_rm_alias_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"pwsh -c rm -Recurse -Force C:\tmp\openamer-victim"
        )
        assert dangerous is True
        assert key is not None
        assert "delete" in desc.lower()
    ...
    def test_powershell_bare_remove_item_requires_approval(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            r"powershell Remove-Item -Recurse -Force C:\tmp\openamer-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows PowerShell destructive delete"
...
class TestTeePattern:
    ...
    def test_tee_openamer_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.openamer/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_openamer_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $OPENAMER_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_openamer_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$OPENAMER_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestOpenAmerConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.openamer/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.openamer/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.openamer/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.openamer/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.openamer/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.openamer/config.yaml")
        assert dangerous is True
        assert "openamer config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.openamer/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_openamer_home_config(self):
    ...
    def test_sed_in_place_absolute_openamer_home_env(self):
        env_path = get_openamer_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_openamer_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $OPENAMER_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.openamer/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_openamer_home_config(self):
        config_path = get_openamer_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.openamer/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_openamer_home_env(self):
        env_path = get_openamer_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.openamer/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.openamer/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.openamer/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.openamer/config.yaml"
        )
        assert dangerous is True
        assert key != "in-place edit of OpenAmer config/env (perl/ruby)"

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.openamer/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-OpenAmer config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_openamer_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $OPENAMER_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.openamer/config.yaml/.env must require approval — the
    ...
    def test_cp_to_openamer_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.openamer/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / OpenAmer-home prefixes must fold to ~/ and
    ~/.openamer/ in dangerous-command detection.
    ...
    SSH, and OpenAmer config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_openamer_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/OPENAMER_HOME so the fold is verified on the POSIX CI runner too."""
    ...
    def test_windows_openamer_home_config_folds(self, monkeypatch):
        # OpenAmer home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("OPENAMER_HOME", r"C:\Users\tester\.openamer")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.openamer\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.openamer/openamer-agent && source venv/bin/activate && python -m openamer_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m openamer_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m openamer_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "openamer_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart openamer-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_openamer_gateway_stop_detected(self):
        cmd = "openamer gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_openamer_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `openamer` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "openamer -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_openamer_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "openamer --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_openamer_gateway_multiple_flags_detected(self):
        cmd = "openamer -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_openamer_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "openamer -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_openamer_gateway_start_not_flagged(self):
        cmd = "openamer gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_openamer_detected(self):
        """pkill targeting openamer/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_openamer_detected(self):
        cmd = "killall openamer"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_sed_config_dangerous(self):
        """In-place edit of the OpenAmer security config via IFS must be caught."""
        cmd = "sed${IFS}-i ~/.openamer/config.yaml"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep openamer) bypasses the pkill/killall name-matching
    ...
    def test_pkill_openamer_still_detected(self):
        """Existing pkill pattern must not regress."""
        cmd = "pkill -9 openamer"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_kill_dollar_pidof_detected(self):
        """`kill $(pidof openamer)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        cmd = "kill -TERM $(pidof openamer_cli.main)"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()

    def test_kill_backtick_pidof_detected(self):
        cmd = "kill -9 `pidof openamer`"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True


class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the OpenAmer service
    label achieves the same effect as `openamer gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_stop_openamer_detected(self):
        cmd = "launchctl stop ai.openamer.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "openamer" in desc.lower()

    def test_launchctl_kickstart_openamer_detected(self):
        cmd = "launchctl kickstart -k system/ai.openamer.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_openamer_detected(self):
        cmd = "launchctl bootout system/ai.openamer.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_openamer_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.openamer.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-OpenAmer label is out of scope for the
