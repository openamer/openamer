...
Older images made /opt/openamer/ui-tui and /opt/openamer/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV OPENAMER_TUI_DIR=/opt/openamer/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R openamer:openamer /opt/openamer/ui-tui" not in text
    assert "chown -R openamer:openamer /opt/openamer/node_modules" not in text
