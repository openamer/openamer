...
from acp_adapter.server import OpenAmerACPAgent
...
class FakeAgent:
    def __init__(self):
        ...
        self.enabled_toolsets = ["openamer-acp"]
...
def test_acp_real_agent_gets_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "openamer_cli.config",
        mod("openamer_cli.config", load_config=lambda: {"model": {"default": "m", "provider": "p"}}),
    )
    monkeypatch.setitem(
        sys.modules,
