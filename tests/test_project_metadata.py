...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [
            ...
            if f"openamer-agent[{extra}]" in spec
        ]
...
def test_pyproject_aiohttp_pins_match_lazy_slack_pin():
    ...
    If the two drift, `openamer update` resolves the pyproject pin and
    ...
    assert not mismatches, (
        ...
        "LAZY_DEPS['platform.slack'] to avoid openamer update downgrading "
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `openamer update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `openamer update` downgrades the "
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (openamer_cli/dingtalk_auth.py)
...
def test_nemo_relay_extra_uses_supported_official_distribution_range():
    ...
    assert not any(
        spec == "openamer-agent[nemo-relay]"
        for spec in optional_dependencies["all"]
    )
