...
def _make_agent():
    openamer_home = Path(tempfile.mkdtemp(prefix="openamer-test-home-"))
    (openamer_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._openamer_home", openamer_home),
