...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    openamer_state = ModuleType("openamer_state")
    openamer_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "openamer_state", openamer_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "OpenAmer"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "OpenAmer"
