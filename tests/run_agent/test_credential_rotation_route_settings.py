...
def test_credential_rotation_replaces_route_scoped_tls_settings():
    ...
    with patch("openamer_cli.config.load_config_readonly", return_value=config):
...
def test_credential_rotation_does_not_carry_global_headers_across_routes():
    ...
    with (
        patch("openamer_cli.config.load_config_readonly", return_value=config),
        patch(
            "openamer_cli.config.get_compatible_custom_providers",
...
def test_credential_rotation_preserves_route_significant_trailing_segments():
    ...
    with patch("openamer_cli.config.load_config_readonly", return_value={}):
        AIAgent._swap_credential(agent, entry)
