...
class TestDeveloperRoleSwap:
    ...
    def test_developer_role_via_openamer_portal(self, monkeypatch):
        agent = _make_agent(
            monkeypatch,
            "openamer",
...
class TestBuildApiKwargsopenamerportal:
    def test_includes_openamer_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == openamer_portal_tags(session_id=agent.session_id)
...
# ── Chat completions response handling (OpenRouter/OpenAmer) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_openamer_when_no_openrouter(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_openamer_auth", return_value=openamer_auth), \
    ...
    def test_custom_endpoint_when_no_nous(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/OpenAmer keys are available.
