...
Run from the PR worktree:
    ...
    cd ~/.openamer/openamer-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.openamer/openamer-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.openamer/openamer-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a openamer-agent checkout"
)
...
# Isolated OPENAMER_HOME for the config write.
