...
def _inject_fake_openamer_cli(monkeypatch):
    """Inject fake openamer_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("openamer_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("openamer_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_openamer_cli = types.ModuleType("openamer_cli")
    fake_openamer_cli.config = fake_config_mod
    fake_openamer_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "openamer_cli", fake_openamer_cli)
    monkeypatch.setitem(sys.modules, "openamer_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "openamer_cli.memory_setup", fake_setup_mod)
