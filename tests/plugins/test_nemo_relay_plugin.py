...
def test_nemo_relay_plugin_emits_llm_tool_and_exports_atif(tmp_path, monkeypatch):
    ...
    base = {
        ...
        "task_id": "t1",
        "turn_id": "turn-1",
        "telemetry_schema_version": "openamer.observer.v1",
    }
    ...
    assert (tmp_path / "atif" / "openamer-atif-s1.json").exists()
...
def test_nemo_relay_plugin_emits_approval_marks(monkeypatch):
    ...
    assert "openamer.approval.request" in mark_names
    assert "openamer.approval.response" in mark_names
...
def test_nemo_relay_plugin_emits_unmatched_fallback_marks(monkeypatch):
    ...
    mark_names = [event[1] for event in fake.events if event[0] == "scope.event"]
    assert "openamer.api.response.unmatched" in mark_names
    assert "openamer.api.error" in mark_names
    assert "openamer.tool.response.unmatched" in mark_names
...
def test_nemo_relay_plugin_metadata_promotes_trajectory_and_subagent_ids(monkeypatch):
    ...
    turn_mark = next(event for event in fake.events if event[0] == "scope.event" and event[1] == "openamer.turn.start")
    ...
    start_mark = next(event for event in fake.events if event[0] == "scope.event" and event[1] == "openamer.subagent.start")
    ...
    stop_mark = next(event for event in fake.events if event[0] == "scope.event" and event[1] == "openamer.subagent.stop")
...
def test_nemo_relay_plugin_reparents_child_session_scope_for_embedded_atif(monkeypatch):
    ...
    child_push = next(
        ...
        if event[0] == "scope.push" and event[1] == "openamer-session-child-session"
    )
    child_kwargs = child_push[3]
    assert child_kwargs["handle"] == ("scope", "openamer-session-parent-session")
...
def test_nemo_relay_plugin_skips_embedded_child_atif_file_by_default(tmp_path, monkeypatch):
    ...
    assert (tmp_path / "atif" / "openamer-atif-parent-session.json").exists()
    assert not (tmp_path / "atif" / "openamer-atif-child-session.json").exists()
...
def test_nemo_relay_plugin_can_write_embedded_child_atif_file_in_all_mode(tmp_path, monkeypatch):
    ...
    assert (tmp_path / "atif" / "openamer-atif-parent-session.json").exists()
    assert (tmp_path / "atif" / "openamer-atif-child-session.json").exists()
...
""",
    ...
    assert "Use OpenAmer-owned [[dynamic_plugins]]" in caplog.text
...
""",
    ...
    assert "openamer-session-s2" in scope_push_names
...
""",
    ...
    assert not (tmp_path / "direct-atif" / "openamer-atif-s1.json").exists()
...
""",
    ...
    assert "plugin.clear" not in event_names
    assert "atif.register" in event_names
    assert (tmp_path / "direct-atif" / "openamer-atif-s1.json").exists()
...
def test_nemo_relay_downstream_unwrap_matches_real_middleware_wrapper_shape(monkeypatch):
    ...
    # ``openamer_cli.middleware._run_execution_chain`` hands to a middleware
    ...
    from openamer_cli import middleware
