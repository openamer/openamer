...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated OPENAMER_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="openamer_e2e_47967_")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("openamer_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["OPENAMER_HOME"] = prev_home
