...
for the credential stores while leaving arbitrary ``OPENAMER_HOME`` files
...
def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into OPENAMER_HOME's
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at OPENAMER_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == OPENAMER_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at OPENAMER_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in OPENAMER_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_openamer_files_outside_home_not_blocked(
    ...
    project.mkdir()
    # auth.json outside OPENAMER_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside OPENAMER_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, OPENAMER_HOME = <root>/profiles/<name>, but
