"""Tests for agent.portal_tags — OpenAmer Portal request tag contract."""

from __future__ import annotations


def test_openamer_client_tag_includes_current_version():
    """The client tag must reflect openamer_cli.__version__ verbatim."""
    from openamer_cli import __version__
    from agent.portal_tags import openamer_client_tag

    assert openamer_client_tag() == f"client=openamer-client-v{__version__}"


def test_openamer_client_tag_format():
    """The client tag has the exact shape OpenAmer Portal expects."""
    from agent.portal_tags import openamer_client_tag

    tag = openamer_client_tag()
    assert tag.startswith("client=openamer-client-v")
    # No spaces, no commas — single tag value
    assert " " not in tag
    assert "," not in tag


def test_openamer_portal_tags_contains_product_and_client():
    """Every OpenAmer Portal request gets BOTH the product tag and the version tag."""
    from agent.portal_tags import openamer_client_tag, openamer_portal_tags

    tags = openamer_portal_tags()
    assert "product=openamer-agent" in tags
    assert openamer_client_tag() in tags
    assert len(tags) == 2


def test_openamer_portal_tags_returns_fresh_list():
    """Callers mutate the returned list; we must not share state across calls."""
    from agent.portal_tags import openamer_portal_tags

    a = openamer_portal_tags()
    a.append("client=test-mutation")
    b = openamer_portal_tags()
...
def test_openamer_portal_tags_appends_conversation_when_session_id_given():
    """A session id adds a third, high-cardinality conversation tag."""
    from agent.portal_tags import conversation_tag, openamer_portal_tags

    tags = openamer_portal_tags(session_id="sess-42")
    assert "product=openamer-agent" in tags
    assert conversation_tag("sess-42") in tags
    assert len(tags) == 3


def test_openamer_portal_tags_omits_conversation_without_session_id():
    """Base tag set stays at two tags when no session id is available."""
    from agent.portal_tags import openamer_portal_tags

    for empty in (None, ""):
        tags = openamer_portal_tags(session_id=empty)
        assert len(tags) == 2
...
def test_ambient_context_tags_calls_without_explicit_session_id():
    """set_conversation_context makes bare openamer_portal_tags() carry the tag.

    This is the mechanism auxiliary calls (compression, titles, vision, MoA
    slots) rely on — they call openamer_portal_tags() with no argument.
    """
    from agent.portal_tags import (
        conversation_tag,
        openamer_portal_tags,
    ...
    assert not any(t.startswith("conversation=") for t in openamer_portal_tags())
...
def test_ambient_context_wins_over_explicit_session_id():
    ...
    try:
        tags = openamer_portal_tags(session_id="segment-2")
...
def test_ambient_context_set_none_clears():
    ...
    for empty in (None, ""):
        ...
        try:
            ...
            assert len(openamer_portal_tags()) == 2
        finally:
            reset_conversation_context(token)
...
def test_ambient_context_propagates_via_thread_context_helper():
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as ex:
            plain = ex.submit(openamer_portal_tags).result()
            propagated = ex.submit(
                propagate_context_to_thread(openamer_portal_tags)
            ).result()
    finally:
        reset_conversation_context(token)
...
def test_auxiliary_client_openamer_extra_body_uses_helper():
    """auxiliary_client.OPENAMER_EXTRA_BODY must match the canonical helper output."""
    from agent.auxiliary_client import OPENAMER_EXTRA_BODY
    from agent.portal_tags import openamer_portal_tags

    assert OPENAMER_EXTRA_BODY == {"tags": openamer_portal_tags()}


def test_openamer_provider_profile_uses_helper():
    """The OpenAmer provider profile (main agent loop) must use the canonical tags."""
    from agent.portal_tags import openamer_portal_tags
    from providers import get_provider_profile

    profile = get_provider_profile("openamer")
    assert profile is not None
    body = profile.build_extra_body()
    assert body["tags"] == openamer_portal_tags()


def test_openamer_sticky_key_matches_conversation_tag():
...
def test_openamer_sticky_key_falls_back_to_explicit_session_id():
