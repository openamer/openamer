"""Tests for build_openamer_credits_snapshot (L6-A, magnitudes-only)."""

from __future__ import annotations

from agent.account_usage import build_openamer_credits_snapshot
...
def test_healthy():
    ...
    snap = build_openamer_credits_snapshot(info)
    assert snap is not None
    assert snap.available is True
    assert snap.plan == "Pro"
    assert snap.provider == "openamer"
    assert snap.title == "OpenAmer credits"
...
def test_money_rule_no_percent():
    info = _account(
        ...
        subscription=OpenAmerPortalSubscriptionInfo(plan="Pro"),
    )
    snap = build_openamer_credits_snapshot(info)
    assert snap is not None
...
def test_logged_out():
    ...
    assert build_openamer_credits_snapshot(info) is None
