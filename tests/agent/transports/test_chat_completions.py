...
class TestChatCompletionsBasic:
    ...
    @pytest.mark.parametrize("provider", ["openamer", "openrouter"])
    ...
    def test_convert_messages_strips_internal_scaffolding_markers(self, transport):
        """OpenAmer-internal ``_``-prefixed markers must never reach the wire.
...
class TestChatCompletionsBuildKwargs:
    ...
    def test_openamer_tags(self, transport):
    ...
    def test_openamer_omits_disabled_reasoning(self, transport):
        from providers import get_provider_profile
        profile = get_provider_profile("openamer")
        msgs = [{"role": "user", "content": "Hi"}]
        kw = transport.build_kwargs(
        ...
        # OpenAmer rejects enabled=false; reasoning omitted entirely
    ...
    def test_gemma_does_not_receive_thinking_config(self, transport):
        ...
        # when OpenAmer has reasoning enabled, the field must be omitted for
...
class TestChatCompletionsKimi:
    ...
    def test_moonshot_tool_schemas_are_sanitized_by_model_name(self, transport):
        """Aggregator routes (OpenAmer, OpenRouter) hit Moonshot by model name, not base URL."""
        tools = [
            {
                "type": "function",
                "function": {
...
class TestChatCompletionsNormalize:
    ...
    def test_refusal_field_promoted_to_content_filter(self, transport):
        """OpenAI-compatible proxies (e.g. OpenAmer Portal fronting Anthropic) can
...
class TestChatCompletionsGeminiNativeExtraBodyStrip:
    """Profile extra_body (e.g. OpenAmer portal tags) must not reach a native
    Gemini endpoint — Google's REST API rejects unknown fields with HTTP 400.
    """

    def _openamer_profile(self):
        from providers import get_provider_profile
        return get_provider_profile("openamer")
    ...
    def test_tags_stripped_when_endpoint_is_native_gemini(self, transport):
        kw = transport.build_kwargs(
            ...
            [{"role": "user", "content": "hi"}],
            None,
            provider_profile=self._openamer_profile(),
            base_url="https://generativelanguage.googleapis.com/v1beta",
            session_id="s1",
            max_tokens=None,
        )
    ...
    def test_tags_preserved_on_openamer_endpoint(self, transport):
        kw = transport.build_kwargs(
            "openamer-3-405b",
            [{"role": "user", "content": "hi"}],
            None,
