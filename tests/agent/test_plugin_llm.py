...
""",
        ...
        from openamer_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir()
        (openamer_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))
        from openamer_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from openamer_cli.plugins import PluginContext, PluginManifest, PluginManager
