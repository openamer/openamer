...
Prior to this change, aggregator users (OpenRouter / OpenAmer Portal) had aux
...
class TestResolveAutoMainFirst:
    ...
    def test_openamer_main_uses_main_model_for_aux(self, monkeypatch):
        """OpenAmer Portal main user → aux uses their picked OpenAmer model, not free-tier MiMo."""
        # No OPENROUTER_API_KEY → ensures if main failed we'd fall to chain
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="openamer",
        ), patch(
            "agent.auxiliary_client._read_main_model",
        ...
        assert mock_resolve.call_args.args[0] == "openamer"
...
class TestResolveVisionMainFirst:
    ...
    def test_openamer_main_vision_uses_paid_openamer_vision_backend(self):
        """Paid OpenAmer main → aux vision uses the dedicated OpenAmer vision backend."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="openamer",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_openamer_main_vision_uses_free_tier_openamer_vision_backend(self):
        """Free-tier OpenAmer main → aux vision uses MiMo omni, not the text main model."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="openamer",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_copilot_vision_sets_vision_header(self, monkeypatch):
        ...
        with patch(
            ...
            "openamer_cli.copilot_auth.copilot_request_headers",
    ...
    def test_main_unavailable_vision_falls_through_to_aggregators(self):
        """Main provider fails → fall back to OpenRouter/OpenAmer strict backends."""
        ...
        assert provider in {"openrouter", "openamer"}
    ...
    def test_explicit_provider_override_still_wins(self):
        ...
        with patch(
            ...
            "agent.auxiliary_client._resolve_task_provider_model",
            return_value=("openamer", None, None, None, None),  # explicit override
        ), patch(
            "agent.auxiliary_client._resolve_strict_vision_backend"
        ) as mock_strict:
            mock_strict.return_value = (MagicMock(), "openamer-default-model")

            from agent.auxiliary_client import resolve_vision_provider_client

            provider, client, model = resolve_vision_provider_client()

        # Explicit "openamer" override → uses strict backend, NOT main model path
        assert provider == "openamer"
        mock_strict.assert_called_once_with("openamer", None)
...
class TestResolveVisionCustomProvider:
    ...
    it returns ``(None, None)`` and vision falls through to OpenRouter / OpenAmer,
