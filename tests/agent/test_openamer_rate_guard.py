"""Tests for agent/openamer_rate_guard.py — cross-session OpenAmer Portal rate limit guard."""
...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    openamer_home = str(tmp_path / ".openamer")
    os.makedirs(openamer_home, exist_ok=True)
...
class TestRecordNousRateLimit:
    ...
    def test_records_with_header_reset(self, rate_guard_env):
        from agent.openamer_rate_guard import record_openamer_rate_limit, _state_path

        headers = {"x-ratelimit-reset-requests-1h": "1800"}
        record_openamer_rate_limit(headers=headers)
    ...
    def test_falls_back_to_default_cooldown(self, rate_guard_env):
        ...
        record_openamer_rate_limit(headers=None)
    ...
    def test_custom_default_cooldown(self, rate_guard_env):
        ...
        record_openamer_rate_limit(headers=None, default_cooldown=120.0)
    ...
    def test_creates_directory_if_missing(self, rate_guard_env):
        ...
        record_openamer_rate_limit(headers={"retry-after": "10"})
...
class TestNousRateLimitRemaining:
    ...
    def test_returns_none_when_no_file(self, rate_guard_env):
        ...
        assert openamer_rate_limit_remaining() is None

    def test_returns_remaining_seconds_when_active(self, rate_guard_env):
        from agent.openamer_rate_guard import record_openamer_rate_limit, openamer_rate_limit_remaining

        record_openamer_rate_limit(headers={"x-ratelimit-reset-requests-1h": "600"})
        remaining = openamer_rate_limit_remaining()
    ...
    def test_returns_none_when_expired(self, rate_guard_env):
        from agent.openamer_rate_guard import openamer_rate_limit_remaining, _state_path
...
class TestClearNousRateLimit:
    ...
    def test_clears_existing_file(self, rate_guard_env):
        ...
        record_openamer_rate_limit(headers={"retry-after": "600"})
        assert openamer_rate_limit_remaining() is not None
...
class TestFormatRemaining:
    ...
    def test_seconds(self):
        from agent.openamer_rate_guard import format_remaining
...
class TestParseResetSeconds:
    ...
    def test_case_insensitive_headers(self, rate_guard_env):
        from agent.openamer_rate_guard import _parse_reset_seconds
...
class TestAuxiliaryClientIntegration:
    ...
    def test_try_openamer_skips_when_rate_limited(self, rate_guard_env, monkeypatch):
        from agent.openamer_rate_guard import record_openamer_rate_limit

        # Record a rate limit
        record_openamer_rate_limit(headers={"retry-after": "600"})

        # Mock _read_openamer_auth to return valid creds (would normally succeed)
        import agent.auxiliary_client as aux
        monkeypatch.setattr(aux, "_read_openamer_auth", lambda: {
            "access_token": "test-token",
            "inference_base_url": "https://api.openamer.test/v1",
        })

        result = aux._try_nous()
        assert result == (None, None)

    def test_try_openamer_works_when_not_rate_limited(self, rate_guard_env, monkeypatch):
        import agent.auxiliary_client as aux

        # No rate limit recorded — _try_nous should proceed normally
        # (will return None because no real creds, but won't be blocked
        # by the rate guard)
        monkeypatch.setattr(aux, "_read_openamer_auth", lambda: None)
        result = aux._try_nous()
        assert result == (None, None)
...
class TestIsGenuineNousRateLimit:
    ...
    OpenAmer Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, OpenAmer).
    ...
    def test_exhausted_hourly_bucket_in_429_headers_is_genuine(self):
        from agent.openamer_rate_guard import is_genuine_openamer_rate_limit
        ...
        assert is_genuine_openamer_rate_limit(headers=headers) is True
    ...
    def test_healthy_headers_on_429_are_upstream_capacity(self):
        # Classic upstream-capacity symptom: OpenAmer edge reports plenty of
        ...
        assert is_genuine_openamer_rate_limit(headers=headers) is False

    def test_bare_429_with_no_headers_is_upstream(self):
        from agent.openamer_rate_guard import is_genuine_openamer_rate_limit

        assert is_genuine_openamer_rate_limit(headers=None) is False
        assert is_genuine_openamer_rate_limit(headers={}) is False
        assert is_genuine_openamer_rate_limit(
    ...
    def test_last_known_state_with_exhausted_bucket_triggers_genuine(self):
        ...
        last_state = parse_rate_limit_headers(prior_headers, provider="openamer")
        assert is_genuine_openamer_rate_limit(
