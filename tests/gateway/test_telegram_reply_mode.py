...
class TestTelegramYamlConfigLoading:
    ...
    def _write_config(self, tmp_path, content: str):
        ...
        openamer_home.mkdir()
        (openamer_home / "config.yaml").write_text(content, encoding="utf-8")
    ...
    def test_top_level_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        openamer_home = self._write_config(tmp_path, "telegram:\n  reply_to_mode: off\n")
    ...
    def test_extra_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        openamer_home = self._write_config(
            tmp_path, "telegram:\n  extra:\n    reply_to_mode: \"off\"\n"
        )
        monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))
        monkeypatch.delenv("TELEGRAM_REPLY_TO_MODE", raising=False)
...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/NousResearch/openamer-agent/issues/23994:
    reply_to_mode 'off' was ignored when sending via OpenAmer-created DM topic
