...
async def _dispatch(runner, event):
    ...
    with patch("openamer_cli.plugins.invoke_hook", return_value=[]), \
...
async def test_prose_still_accepted_for_open_ended_clarify():
    ...
    result = await _dispatch(runner, _event("call it openamer-ux"))
    ...
    assert entry is not None
    assert entry.event.is_set()
    assert entry.response == "call it openamer-ux"
    _clear_clarify_state()
