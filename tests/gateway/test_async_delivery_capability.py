...
class TestAsyncDeliverySupported:
    ...
    def test_set_false_is_unsupported(self):
        ...
        try:
            ...
            assert get_session_env("OPENAMER_SESSION_PLATFORM") == "api_server"
        finally:
            clear_session_vars(tokens)
...
class TestDeclareStatelessChannel:
    """``openamer -z`` and cron cannot receive a completion after their turn ends.

    Cron clears the ``OPENAMER_SESSION_*`` routing keys, so an async delegation's
