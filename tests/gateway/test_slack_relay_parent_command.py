...
@pytest.mark.parametrize(
    ("wire_text", "expected"),
    [
        ("/openamer sethome", "/sethome"),
        ("/openamer\tsethome", "/sethome"),
        (
            "/openamer model gpt-5.6 --provider openai",
...
def test_slack_relay_parent_freeform_text_matches_native_adapter():
    event = _event_from_wire(_wire("/openamer explain this"))
...
def test_non_slack_relay_message_is_not_rewritten():
    event = _event_from_wire(_wire("/openamer sethome", platform="discord"))

    assert event.text == "/openamer sethome"
    assert event.message_type == MessageType.COMMAND
