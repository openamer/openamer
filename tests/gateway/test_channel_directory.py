...
def _isolate_channel_aliases(tmp_path_factory):
    ...
    ~/.openamer/channel_aliases.json never leaks into directory tests. Tests
...
class TestBuildSlack:
    ...
    def test_response_not_ok_missing_scope_falls_back_quietly(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"OPENAMER_HOME": str(tmp_path)}), caplog.at_level("WARNING"):
            entries = asyncio.run(_build_slack(_make_slack_adapter({"T1": client})))
    ...
    def test_repeated_workspace_errors_are_warning_throttled(
        ...
        with patch.dict(os.environ, {"OPENAMER_HOME": str(tmp_path)}), caplog.at_level("DEBUG"):
