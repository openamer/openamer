...
def _run_gateway_import(openamer_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
...
def test_config_gateway_timeout_wins_over_stale_env(openamer_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(openamer_home, agent_cfg={
...
def test_config_timezone_wins_over_stale_env(openamer_home: Path) -> None:
    _write_config(openamer_home, timezone="America/Los_Angeles")
    _write_env(openamer_home, {"OPENAMER_TIMEZONE": "UTC"})

    env = _run_gateway_import(openamer_home, initial_env={})

    assert env.get("OPENAMER_TIMEZONE") == "America/Los_Angeles"


def test_env_value_survives_when_config_omits_key(openamer_home: Path) -> None:
