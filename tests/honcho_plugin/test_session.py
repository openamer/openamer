...
class TestHonchoSession:
    def _make_session(self):
        return HonchoSession(
            ...
            assistant_peer_id="openamer-assistant",
...
class TestPeerLookupHelpers:
    ...
    def test_search_context_uses_peer_perspective_message_search(self):
        ...
        honcho_client.search.return_value = [
            SimpleNamespace(content="Robert runs neuralancer", peer_id="openamer", session_id="s-old", id="m1"),
    ...
    def test_search_context_explicit_ai_peer_searches_ai_perspective(self):
        ...
        honcho_client = MagicMock()
        honcho_client.search.return_value = [
            SimpleNamespace(content="Assistant note", peer_id="openamer", session_id="s1", id="m1"),
        ]
        with patch.object(HonchoSessionManager, "honcho", new_callable=lambda: property(lambda s: honcho_client)):
...
class TestConcludeToolDispatch:
    ...
    def test_honcho_profile_can_target_explicit_peer_id(self):
        ...
        provider._manager.get_peer_card.assert_called_once_with("telegram:123", peer="openamer")
    ...
    def test_honcho_reasoning_can_target_explicit_peer_id(self):
        ...
        result = provider.handle_tool_call(
            "honcho_reasoning",
            {"query": "who are you", "peer": "openamer"},
        )
...
class TestToolsModeInitBehavior:
    ...
    def _make_provider_with_config(self, recall_mode="tools", init_on_session_start=False,
        ...
        with patch("plugins.memory.honcho.client.HonchoClientConfig.from_global_config", return_value=cfg), \
             ...
             patch("openamer_constants.get_openamer_home", return_value=MagicMock()):
...
class TestPerSessionMigrateGuard:
    ...
    per-session creates a fresh Honcho session every OpenAmer run. Uploading
...
class TestDialecticLifecycleSmoke:
    ...
    def test_full_multi_turn_session(self):
        ...
        responses = iter([
            "prewarm: user is eri, works on openamer",      # session-start prewarm
