...
from openamer_cli.profiles import _get_default_openamer_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "openamer"
        assert config.workspace_id == "openamer"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.openamer/honcho.json
        monkeypatch.setenv("OPENAMER_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"openamer": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"openamer": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"openamer": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="openamer-agent"
        ):
            result = config.resolve_session_name("/home/user/openamer-agent/subdir")
        assert result == "openamer-agent"
...
class TestResolveConfigPath:
    def test_prefers_openamer_home_when_exists(self, tmp_path):
        openamer_home = tmp_path / "openamer"
        openamer_home.mkdir()
        local_cfg = openamer_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: OPENAMER_HOME points at ~/.openamer/profiles/<name>, so
        # _get_default_openamer_home() must resolve back to ~/.openamer — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".openamer"
        ...
        assert _get_default_openamer_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_openamer_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".openamer".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".openamer"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "openamer_coder"
        assert profile_host_key("default") == "openamer"
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            try:
                ...
                if saved is not None:
                    sys.modules["openamer_cli.profiles"] = saved
                else:
                    sys.modules.pop("openamer_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="openamer_coder")
        assert config.host == "openamer_coder"
        assert config.workspace_id == "openamer"  # shared workspace
        assert config.ai_peer == "openamer_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="openamer")
        assert config.host == "openamer"
        assert config.workspace_id == "openamer"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "openamer": {"aiPeer": "openamer", "peerName": "alice"},
                "openamer_coder": {
                    "aiPeer": "openamer_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="openamer_coder", config_path=config_file,
        )
        assert config.host == "openamer_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "openamer_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="openamer_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "openamer_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "openamer.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="openamer_dreamer",
            config_path=config_file,
        )
        assert config.host == "openamer_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "openamer_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"openamer": {"enabled": True, "aiPeer": "openamer"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"openamer": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_openamer_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="openamer",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("openamer_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("openamer_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_openamer_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="openamer",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("openamer_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_openamer_home() / "config.yaml"
    ...
    def test_honcho_json_timeout_does_not_thrash_singleton(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "cloud-key",
            "hosts": {"openamer": {"workspace": "ws", "requestTimeout": 120}},
        }))
        ...
        with patch("plugins.memory.honcho.client.resolve_config_path", return_value=config_file), \
             patch("openamer_cli.profiles.get_active_profile_name", return_value="default"):
            with patch("honcho.Honcho", return_value=fake_honcho_1) as mock_h1:
            ...
            config_file.write_text(json.dumps({
                "apiKey": "cloud-key",
                "hosts": {"openamer": {"workspace": "ws", "requestTimeout": 240}},
            }))
            st = config_file.stat()
...
class TestDialecticDepthParsing:
    ...
    def test_depth_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "***",
            "dialecticDepth": 1,
            "hosts": {"openamer": {"dialecticDepth": 3}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
