...
from openamer_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(openamer_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <openamer_home>/plugins/<name> and opt it in."""
    plugin_dir = openamer_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENAMER_HOME", str(tmp_path / "openamer_empty"))
