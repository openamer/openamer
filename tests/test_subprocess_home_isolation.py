...
OpenAmer state stays profile-scoped through OPENAMER_HOME. Host subprocesses should
...
See: https://github.com/NousResearch/openamer-agent/issues/25114
See: https://github.com/NousResearch/openamer-agent/issues/36144
See: https://github.com/NousResearch/openamer-agent/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for openamer_constants.get_subprocess_home()."""
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        openamer_home = real_home / ".openamer" / "profiles" / "coder"
        profile_home = openamer_home / "home"
    ...
    def test_real_home_falls_back_to_os_account_when_home_is_profile(self, tmp_path, monkeypatch):
        ...
        from openamer_constants import get_real_home

        assert get_real_home() != str(profile_home)

    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        self._container_mode(monkeypatch)
        base = tmp_path / ".openamer" / "profiles"
        ...
        monkeypatch.setenv("OPENAMER_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("OPENAMER_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_openamer_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_openamer_home_override(profile)
        try:
            assert get_openamer_home() == profile
            release.set()
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_no_injection_when_openamer_home_unset(self, monkeypatch):
        monkeypatch.delenv("OPENAMER_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["OPENAMER_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from openamer_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from openamer_cli.profiles import create_profile
...
class TestPythonProcessUnchanged:
    ...
    def test_path_home_unchanged_after_subprocess_home_resolved(
        ...
        assert sub_home in (None, str(openamer_home / "home"), original_home)
