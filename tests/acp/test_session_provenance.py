...
def test_root_session_no_compression(db):
    ...
    assert prov["currentOpenAmerSessionId"] == "root1"
    assert prov["rootOpenAmerSessionId"] == "root1"
    assert prov["parentOpenAmerSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_openamer_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentOpenAmerSessionId"] == "old"
    assert prov["rootOpenAmerSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousOpenAmerSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootOpenAmerSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootOpenAmerSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_openamer_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousOpenAmerSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"openamer"}
    assert "sessionProvenance" in meta["openamer"]
    assert meta["openamer"]["sessionProvenance"]["currentOpenAmerSessionId"] == "root1"
