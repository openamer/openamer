...
from openamer_cli.active_sessions import active_session_registry_snapshot
from openamer_cli.browser_connect import ChromeDebugLaunch
...
def test_session_create_rejects_at_active_session_limit(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert second["error"]["message"] == (
            "OpenAmer is at the active session limit (1/1). "
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["OPENAMER_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("OPENAMER_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("OPENAMER_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("OPENAMER_VOICE", "1")
    # setenv (not delenv) — the handler writes OPENAMER_VOICE_TTS directly, and
    ...
    monkeypatch.setenv("OPENAMER_VOICE_TTS", "0")
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("OPENAMER_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("OPENAMER_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("OPENAMER_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_folds_project_into_focus_posture(monkeypatch):
    ...
    monkeypatch.delenv("OPENAMER_TUI_TOOLSETS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("OPENAMER_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "openamer_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    # _get_platform_tools (a non-configurable platform toolset in openamer-cli's
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("OPENAMER_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "openamer_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("OPENAMER_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("OPENAMER_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("OPENAMER_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("OPENAMER_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "openamer_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown OPENAMER_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("openamer_state.SessionDB", lambda db_path=None: profile_db)
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    ...
    monkeypatch.setenv("OPENAMER_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set OPENAMER_INFERENCE_MODEL as a provision-time seed;
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    # `openamer --tui -m <model>` sets OPENAMER_MODEL/OPENAMER_INFERENCE_MODEL as a
    ...
    # permanently.
    monkeypatch.setenv("OPENAMER_MODEL", "one-shot/model")
    monkeypatch.setenv("OPENAMER_INFERENCE_MODEL", "one-shot/model")
    monkeypatch.setattr(
...
def test_apply_model_switch_persist_override_false_never_persists(monkeypatch):
    ...
    monkeypatch.setattr(
        "openamer_cli.model_switch.switch_model", lambda **kw: result
    )
    monkeypatch.setattr(
        "openamer_cli.model_switch.resolve_persist_behavior",
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("OPENAMER_MODEL", "sonnet")
    monkeypatch.delenv("OPENAMER_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("OPENAMER_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "openamer_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_load_fallback_model_merges_chain_providers_first(monkeypatch):
    # Parity with OpenAmerCLI / gateway: fallback_providers stays first and keeps
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "openamer_cli.models.fetch_openrouter_models",
...
def test_ensure_session_db_row_stamps_profile_name(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("openamer_state.SessionDB", _ProfileDB)
    monkeypatch.setattr(server, "_resolve_model", lambda: "test-model")
...
def test_config_set_fast_updates_live_agent_session_scoped(monkeypatch):
    ...
    monkeypatch.setattr(
        "openamer_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("OPENAMER_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("OPENAMER_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    ...
    assert server.os.environ["OPENAMER_GATEWAY_SESSION"] == "1"
    assert server.os.environ["OPENAMER_EXEC_ASK"] == "1"
    assert server.os.environ["OPENAMER_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("openamer_cli.main._has_any_provider_configured", lambda: False)
...
def test_config_set_model_requires_confirmation_for_expensive_model(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setattr(
        "openamer_cli.model_switch.switch_model", lambda **_kwargs: result
    )
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing OPENAMER_INFERENCE_PROVIDER on a switch leaked the new
    ...
    try:
        ...
        assert os.environ["OPENAMER_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "OPENAMER_TUI_PROVIDER" not in os.environ
        assert "OPENAMER_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write OPENAMER_MODEL / OPENAMER_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("OPENAMER_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("OPENAMER_MODEL", raising=False)
    monkeypatch.delenv("OPENAMER_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("openamer_cli.model_switch.switch_model", fake_switch_model)
    ...
    try:
        ...
        assert os.environ["OPENAMER_TUI_PROVIDER"] == "openai-codex"
        assert "OPENAMER_MODEL" not in os.environ
        assert "OPENAMER_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_config_set_model_once_keeps_env_and_records_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("OPENAMER_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setenv("OPENAMER_MODEL", "old/model")
    monkeypatch.setattr(
    ...
    try:
        ...
        assert os.environ["OPENAMER_INFERENCE_PROVIDER"] == "openrouter"
        assert os.environ["OPENAMER_MODEL"] == "old/model"
    finally:
        server._sessions.clear()
...
def test_config_set_model_session_switch_clears_pending_once_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setattr("openamer_cli.model_switch.switch_model", lambda **_kwargs: result)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda *args, **kwargs: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".openamer" / "desktop-attachments" / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == "@file:.openamer/desktop-attachments/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".openamer" / "desktop-attachments" / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == "@file:.openamer/desktop-attachments/outside.txt"
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert not (workspace / ".openamer" / "desktop-attachments").exists()
    finally:
        server._sessions.pop("sid", None)
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        assert resp["result"]["ref_text"] == "@file:`.openamer/desktop-attachments/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "OpenAmer TUI Status" in out
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("openamer_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "openamer_cli.commands.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("openamer_state")
    ...
    monkeypatch.setitem(sys.modules, "openamer_state", fake_mod)
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_openamer_home
    # fixture pins OPENAMER_HOME to a temp dir; the handler should append
    # /sessions to it.
...
def test_session_list_honors_params_profile_opens_profile_db(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("openamer_state.SessionDB", ProfileDB)
...
def test_session_most_recent_honors_params_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_profile_home", lambda p: profile_home if p == "mlperf" else None)
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("openamer_state.SessionDB", ProfileDB2)
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `openamer model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `openamer model` and the classic CLI /model picker.
    ...
    with patch(
        "openamer_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("openamer_cli.models.provider_model_ids") as live_fetch:
...
def test_model_options_hides_unconfigured_providers_by_default(monkeypatch):
    from openamer_cli.inventory import ConfigContext

    calls = []

    monkeypatch.setattr(server, "_resolve_model", lambda: "")
    monkeypatch.setattr(
        "openamer_cli.inventory.load_picker_context",
    ...
    monkeypatch.setattr(
        "openamer_cli.inventory.build_models_payload",
...
def test_model_options_preserves_canonical_custom_row_after_agent_init(monkeypatch):
    ...
    monkeypatch.setattr(
        "openamer_cli.runtime_provider.canonical_custom_identity",
        canonical,
    )
    monkeypatch.setattr(
        "openamer_cli.model_switch.list_authenticated_providers",
    ...
    monkeypatch.setattr(
        "openamer_cli.auth.is_provider_explicitly_configured",
        lambda _slug: False,
    )
    monkeypatch.setattr("openamer_cli.inventory._apply_pricing", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("openamer_cli.inventory._apply_capabilities", lambda *_args, **_kwargs: None)
...
def test_model_save_key_uses_credential_lifecycle_and_picker_context(monkeypatch):
    ...
    monkeypatch.setattr(
        "openamer_cli.auth.PROVIDER_REGISTRY",
        {
    ...
    monkeypatch.setattr(
        "openamer_cli.credential_lifecycle.save_provider_env_credential",
        save_credential,
    )
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``openamer --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"openamer_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "openamer_cli.browser_connect.launch_chrome_debug",
                return_value=ChromeDebugLaunch(),
            ),
            patch("openamer_cli.browser_connect.local_port_in_use", return_value=False),
            patch("openamer_cli.browser_connect.manual_chrome_debug_command", return_value=None),
            patch(
...
def test_browser_manage_connect_squatted_port_launches_on_alternate(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with (
            patch("openamer_cli.browser_connect.launch_chrome_debug", side_effect=_launch),
            patch("openamer_cli.browser_connect.local_port_in_use", return_value=True),
            patch("openamer_cli.browser_connect.find_free_debug_port", return_value=9223),
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_openamer_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.openamer/.env
    ...
    with patch.dict(sys.modules, {"openamer_cli.config": fake}):
...
def test_make_agent_waits_for_shared_mcp_discovery(monkeypatch):
    ...
    from openamer_cli import mcp_startup
...
def test_session_save_writes_under_openamer_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the OpenAmer profile
    ...
    Regression: the gateway handler wrote ``openamer_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are OpenAmer.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("openamer_conversation_*.json"))
    ...
    assert payload["model"] == "openamer-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are OpenAmer."
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from cli._openamer_home, which
    # is captured at import time — patch it directly (set_openamer_home_override
...
def test_tts_stream_begin_requires_working_provider(monkeypatch):
    monkeypatch.setenv("OPENAMER_VOICE_TTS", "1")
...
def test_tts_stream_begin_and_stop_lifecycle(monkeypatch):
    ...
    monkeypatch.setenv("OPENAMER_VOICE_TTS", "1")
    monkeypatch.setenv("OPENAMER_VOICE", "0")  # no barge-in monitor (no mic)
    playback_stops: list = []
...
def test_build_persist_message_quotes_paths_containing_spaces(tmp_path):
    ...
    img_dir = tmp_path / "Application Support" / "OpenAmer" / "composer-images"
