"""Tests for openamer_state.py — SessionDB SQLite CRUD, FTS5 search, export."""
...
from openamer_state import SCHEMA_SQL, SCHEMA_VERSION, SessionDB
...
class TestSessionLifecycle:
    ...
    def test_db_initializes_without_fts5_module(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("openamer_state.sqlite3.connect", connect_without_fts)
    ...
    def test_fts_runtime_restores_triggers_after_no_fts_open(
        ...
        monkeypatch.setattr("openamer_state.sqlite3.connect", real_connect)
    ...
    def test_base_fts_rebuilds_after_trigger_repair_without_trigram(
        ...
        monkeypatch.setattr("openamer_state.sqlite3.connect", connect_without_trigram)
        restored = SessionDB(db_path=db_path)
        try:
            assert restored._fts_enabled is True
...
class TestTimestampPreservation:
    ...
    def test_branch_copy_roundtrip_preserves_timestamps(self, db):
        ...
        # openamer_cli/cli_commands_mixin.py and tui_gateway/server.py.
        for msg in history:
            db.append_message(
                "child",
...
class TestFTS5Search:
    ...
    def test_sanitize_fts5_query_runtime_is_bounded(self):
        ...
        from openamer_state import MAX_FTS5_QUERY_CHARS, SessionDB
...
class TestSchemaInit:
    def test_wal_mode(self, db):
        ...
        from openamer_state import is_sqlite_wal_reset_vulnerable
    ...
    def test_schema_version(self, db):
        from openamer_state import SCHEMA_VERSION
    ...
    def test_v9_migration_skips_v10_trigram_backfill_before_v11_rebuild(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("openamer_state.sqlite3.connect", connect_with_trace)
        migrated_db = SessionDB(db_path=db_path)
        try:
    ...
    def test_schema_sql_is_source_of_truth(self, db):
        ...
        """
        from openamer_state import SCHEMA_SQL
...
class TestConcurrentWriteSafety:
    ...
    def test_sqlite_timeout_is_at_least_30s(self, db):
        ...
        import inspect
        from openamer_state import SessionDB as _SessionDB
    ...
    def test_optimize_fts_storage_transitions_to_v23(self, tmp_path):
        ...
        try:
            ...
            assert db.get_meta("fts_storage_version") == str(
                openamer_state.FTS_STORAGE_VERSION
            )
            assert db._conn.execute(
                "SELECT version FROM schema_version"
...
class TestApplyWalProbe:
    ...
    def test_macos_checkpoint_fullsync_barrier_applied(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(openamer_state.sys, "platform", "darwin")
    ...
    def test_checkpoint_fullsync_barrier_skipped_off_darwin(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(openamer_state.sys, "platform", "linux")
...
def test_refresh_compression_lock_requires_holder_and_preserves_reclaimability(db, monkeypatch):
    ...
    monkeypatch.setattr(openamer_state.time, "time", lambda: 1000.0)
    ...
    monkeypatch.setattr(openamer_state.time, "time", lambda: 1005.0)
    ...
    monkeypatch.setattr(openamer_state.time, "time", lambda: 1016.0)
...
def test_starved_refresher_revives_its_own_unclaimed_lease(db, monkeypatch):
    ...
    monkeypatch.setattr(openamer_state.time, "time", lambda: 1050.0)
    assert db.refresh_compression_lock("s1", "holder-a", ttl_seconds=10.0) is True
...
def test_refresh_cannot_resurrect_a_lock_already_reclaimed(db, monkeypatch):
    ...
    monkeypatch.setattr(openamer_state.time, "time", lambda: 1020.0)
    assert db.try_acquire_compression_lock("s1", "holder-b", ttl_seconds=10.0) is True
