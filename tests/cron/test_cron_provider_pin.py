...
openamer/claude-fable-5), the job would silently inherit it on its next tick and
...
resolve_runtime_provider against a temp OPENAMER_HOME) and the create_job
...
class TestProviderDriftGuard:
    ...
    def test_b_unpinned_snapshot_differs_fails_closed(self, tmp_path):
        ...
        job = _base_job(provider_snapshot="openrouter")
        success, output, final_response, error, agent_constructed = \
            _run_with_current_provider(job, "openamer", tmp_path)
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("openamer_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
                prompt="do a thing", schedule="every 1 hour", provider="openamer"
            )

        # Explicit provider → pinned → no snapshot needed, and resolution skipped.
        assert job["provider"] == "openamer"
