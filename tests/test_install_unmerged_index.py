...
The ``openamer update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m openamer_cli.main gateway run`` — image name
    ``pythonw``, not ``openamer.exe`` — so the ``taskkill /IM openamer.exe`` guard
    ...
    # The openamer.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM openamer.exe' in text
