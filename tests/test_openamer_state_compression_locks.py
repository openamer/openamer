...
def test_non_expired_lock_from_dead_pid_is_reclaimed(
    ...
    monkeypatch.setattr(openamer_state.os, "name", "posix")
    ...
    monkeypatch.setattr(
        openamer_state, "psutil", SimpleNamespace(pid_exists=process_is_gone)
    )
...
def test_dead_pid_reclaim_via_os_kill_fallback_when_psutil_missing(
    ...
    monkeypatch.setattr(openamer_state, "psutil", None)
    ...
    monkeypatch.setattr(openamer_state.os, "kill", process_is_gone)
...
def test_probe_doubt_keeps_lease_until_ttl(
    ...
    monkeypatch.setattr(
        openamer_state, "psutil", SimpleNamespace(pid_exists=probe_blows_up)
    )
...
def test_windows_reclaims_dead_holder_via_psutil_probe(
    ...
    monkeypatch.setattr(openamer_state.os, "name", "nt")
    probed: list[int] = []
    ...
    monkeypatch.setattr(
        openamer_state, "psutil", SimpleNamespace(pid_exists=_pid_exists)
    )
    monkeypatch.setattr(
        openamer_state.os,
        "kill",
