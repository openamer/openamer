...
Added 2026-09-23, two further bug classes:

1. The general form of the above: apply_and_test() had no check that a
   *module level binding* survives the edit. All three of its original checks
   passed on the corrupted result. The binding-survival gate now catches the
...
2. apply_and_test() opened files in text mode, so on Windows every LF became
   CRLF on write. Four of the six SAFE_TARGETS are pure LF, so the first
   applied proposal would have rewritten a whole file's line endings -- a
   237-line diff dressed up as a one-line improvement.
   `test_apply_and_test_preserves_lf_line_endings` pins the byte output.
"""
