...
def _find_scanner() -> str:
    """Locate scan_skill_safety.py in either layout.

    Live tree: it sits beside this test ($OPENAMER_HOME/scripts/).
    Repo tree: this test lives in tests/scripts/ while the scanner lives in
    scripts/ — so walk up two levels and try scripts/ there.
    """
    candidates = [
        os.path.join(HERE, "scan_skill_safety.py"),
        os.path.join(os.path.dirname(os.path.dirname(HERE)), "scripts",
...
def test_patterns_are_lowercase():
    ...
    if bad:
        ...
        raise AssertionError(
            f"{len(bad)} detector pattern(s) contain an uppercase literal and "
            f"can never match a lowercased line")
...
def test_case_is_load_bearing():
    ...
    assert rx_ok.search(low), (
        "the shipped lowercase outbound-post pattern fails to match a real "
        "lowercased 'curl -X POST' line")
    assert not rx_dead.search(low), (
        "an uppercase pattern matched a lowercased line — the premise of this "
        "guard is wrong")
...
def main():
    ...
    try:
        ...
        print()
        # Called (not `and ok`): they assert, so a failure raises and pytest
        # reports it. Under `main()` they run as plain checks for CLI use.
        test_patterns_are_lowercase()
        test_case_is_load_bearing()
