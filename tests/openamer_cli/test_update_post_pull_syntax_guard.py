"""Tests for the post-pull syntax guard in ``openamer update``.

When a bad commit lands on ``main`` with a syntax error in a critical file
(e.g. orphan merge-conflict markers in ``openamer_cli/config.py``), the CLI
becomes unbootable — every ``openamer`` invocation imports those files at
...
markers in ``openamer_cli/config.py``; users who ran ``openamer update`` in
the 7-minute window before #28458 landed could not run any ``openamer``
command afterward.
"""
...
def test_capture_head_sha_returns_stripped_sha(monkeypatch, tmp_path):
    ...
    assert openamer_main._capture_head_sha(["git"], tmp_path) == "deadbeefcafe"
...
def test_capture_head_sha_returns_none_on_git_failure(monkeypatch, tmp_path):
    ...
    assert openamer_main._capture_head_sha(["git"], tmp_path) is None
...
def _populate_critical_tree(root: Path, *, broken_file: str | None = None) -> None:
    ...
    for relpath in openamer_main._UPDATE_CRITICAL_FILES:
        path = root / relpath
...
def test_validate_critical_files_syntax_detects_conflict_markers(tmp_path):
    ...
    _populate_critical_tree(tmp_path, broken_file="openamer_cli/config.py")
...
def test_validate_critical_files_syntax_detects_break_in_main_py(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="openamer_cli/main.py")
...
def test_validate_critical_files_syntax_detects_break_in_web_server(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="openamer_cli/web_server.py")
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except openamer_constants.py
    for relpath in openamer_main._UPDATE_CRITICAL_FILES:
        if relpath == "openamer_constants.py":
            continue
...
# release; if a future ``openamer update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
...
def test_production_tree_passes_syntax_guard():
    ...
    ok, failing_path, error = openamer_main._validate_critical_files_syntax(repo_root)

    assert ok is True, (
        f"Critical-path file {failing_path} fails to parse on current main; "
        f"openamer update would brick users. Error: {error}"
    )
