...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_openamer_oauth_login_pure",
        lambda: {
            "access_token": token,
            "refresh_token": "refresh-token",
    ...
    entry = next(item for item in entries if item["source"] == "manual:openamer_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:openamer_pkce"
    assert entry["refresh_token"] == "refresh-token"
    assert entry["expires_at_ms"] == 1711234567000


def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    """openamer auth add qwen-oauth must set active_provider in auth.json.
    ...
    monkeypatch.setattr(
        "openamer_cli.auth.resolve_qwen_runtime_credentials",
    ...
    # pool entry from pool.add_entry() still present for openamer auth list
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "openamer_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "openamer_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``openamer auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``openamer auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("openamer_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from openamer_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from openamer_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """openamer auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``openamer auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from openamer_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`openamer logout --provider openai-codex` must still clear model.provider.
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `openamer logout` should target configured Codex if auth has no active provider."""
    openamer_home = tmp_path / "openamer"
    monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (openamer_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((openamer_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from openamer_cli.auth import _reset_config_provider
    ...
    with patch("openamer_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from openamer_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "openamer_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("openamer_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("openamer_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("openamer_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "openamer_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    updated = json.loads((openamer_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from openamer_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from openamer_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `openamer auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("OPENAMER_HOME", str(tmp_path / "openamer"))
    openamer_home = tmp_path / "openamer"
    openamer_home.mkdir(parents=True, exist_ok=True)

    # Pre-existing suppression (simulating a prior `openamer auth remove`)
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((openamer_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`openamer auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.openamer/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.openamer/.env (not the shell), the
    ...
    (openamer_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (openamer_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `openamer auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from openamer_cli.auth import is_source_suppressed
    from openamer_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `openamer auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source OpenAmer reads from has a
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import openamer_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_qwen_suppression(tmp_path, monkeypatch):
    ...
    import openamer_cli.auth as ha
...
def test_seed_from_singletons_respects_openamer_pkce_suppression(tmp_path, monkeypatch):
    """anthropic openamer_pkce must not re-seed from ~/.openamer/.anthropic_oauth.json when suppressed."""
    openamer_home = tmp_path / "openamer"
    openamer_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))

    import yaml
    (openamer_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (openamer_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["openamer_pkce"]},
    }))

    # Stub the readers so only openamer_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_openamer_oauth_credentials", lambda: {
    ...
    # openamer_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "openamer_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (openamer_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        "openamer_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "openamer_cli.copilot_auth.get_copilot_api_token",
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `openamer auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `openamer auth add openai-codex`)
