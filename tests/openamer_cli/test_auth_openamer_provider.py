"""Regression tests for OpenAmer OAuth refresh and inference JWT interactions."""
...
from openamer_cli.auth import AuthError, get_provider_auth_state, resolve_openamer_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from openamer_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENAMER_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_openamer_ca_bundle_env_falls_back(self, monkeypatch):
        from openamer_cli.auth import _resolve_verify

        monkeypatch.setenv("OPENAMER_CA_BUNDLE", "/nonexistent/openamer-ca.pem")
...
def _setup_openamer_auth(
    ...
    *,
    ...
    (openamer_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_openamer_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    tmp_path,
    monkeypatch,
):
    import openamer_cli.auth as auth_mod

    openamer_home = tmp_path / "openamer"
    token = _invoke_jwt(seconds=3600)
    _setup_openamer_auth(
        openamer_home,
        access_token=token,
    ...
    creds = auth_mod.resolve_openamer_runtime_credentials()

    assert creds["api_key"] == token
    assert creds["source"] == auth_mod.OPENAMER_AUTH_PATH_INVOKE_JWT
    assert creds["auth_path"] == auth_mod.OPENAMER_AUTH_PATH_INVOKE_JWT

    payload = json.loads((openamer_home / "auth.json").read_text())
    ...
    pool_entries = payload["credential_pool"]["openamer"]
    assert len(pool_entries) == 1
    assert pool_entries[0]["agent_key"] == token
    assert pool_entries[0]["source"] == auth_mod.OPENAMER_DEVICE_CODE_SOURCE


def test_resolve_openamer_runtime_credentials_env_override_wins_live_not_persisted(
    tmp_path,
    monkeypatch,
    shared_store_env,
):
    """OPENAMER_INFERENCE_BASE_URL is a LIVE override, not a persisted one.
    ...
    network_url = "https://inference-api.openamer.com/v1"
    ...
    monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))
    monkeypatch.setenv("OPENAMER_INFERENCE_BASE_URL", override_url)
    ...
    monkeypatch.setattr("openamer_cli.auth._refresh_access_token", _fake_refresh_access_token)
    ...
    payload = json.loads((openamer_home / "auth.json").read_text())
    assert payload["providers"]["openamer"]["inference_base_url"] == network_url
    assert payload["providers"]["openamer"]["inference_base_url"] != override_url
    assert payload["credential_pool"]["openamer"][0]["inference_base_url"] == network_url

    shared_payload = json.loads((shared_store_env / "openamer_auth.json").read_text())
    assert shared_payload["inference_base_url"] == network_url


def test_resolve_openamer_runtime_credentials_invoke_jwt_is_idempotent(
    tmp_path,
    monkeypatch,
):
    import openamer_cli.auth as auth_mod
    ...
    monkeypatch.setattr(auth_mod, "_write_shared_openamer_state", _unexpected_shared_write)
    monkeypatch.setattr(
        auth_mod,
    ...
    assert (
        payload["providers"]["openamer"]["agent_key_obtained_at"]
        == original_obtained_at
    )


def test_resolve_openamer_runtime_credentials_trusts_invoke_jwt_exp_over_stale_metadata(
    tmp_path,
    monkeypatch,
):
    import openamer_cli.auth as auth_mod
...
def test_resolve_openamer_runtime_credentials_does_not_apply_agent_key_ttl_to_invoke_jwt(
    tmp_path,
    monkeypatch,
):
    import openamer_cli.auth as auth_mod
    ...
    assert creds["source"] == auth_mod.OPENAMER_AUTH_PATH_INVOKE_JWT
    payload = json.loads((openamer_home / "auth.json").read_text())
    assert payload["providers"]["openamer"]["agent_key"] == token
    assert payload["credential_pool"]["openamer"][0]["agent_key"] == token


def test_resolve_openamer_runtime_credentials_refreshes_legacy_agent_key_to_invoke_jwt(
    tmp_path,
    monkeypatch,
):
    import openamer_cli.auth as auth_mod
    ...
    assert payload["credential_pool"]["openamer"][0]["agent_key"] == refreshed_token


def test_resolve_openamer_runtime_credentials_reauths_when_invoke_scope_missing(
    tmp_path,
    monkeypatch,
):
    import openamer_cli.auth as auth_mod
    ...
    payload = json.loads((openamer_home / "auth.json").read_text())
    assert payload["providers"]["openamer"]["agent_key"] is None
    assert "credential_pool" not in payload or not payload["credential_pool"].get("openamer")


def test_openamer_device_code_login_does_not_retry_legacy_scope_when_invoke_refused(monkeypatch):
    import openamer_cli.auth as auth_mod
    ...
    with pytest.raises(httpx.HTTPStatusError):
        auth_mod._openamer_device_code_login(
            portal_base_url="https://portal.example.com",
            inference_base_url="https://inference.example.com/v1",
            open_browser=False,
            timeout_seconds=1,
    ...
    assert scopes == [auth_mod.DEFAULT_openamer_SCOPE]
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    result = auth_mod._openamer_device_code_login(
        portal_base_url="https://portal.example.com",
        inference_base_url="https://inference.example.com/v1",
        open_browser=False,
        timeout_seconds=1,
...
def test_openamer_inference_auth_logs_do_not_include_secret_values(
    tmp_path,
    monkeypatch,
    caplog,
):
    ...
    caplog.set_level(logging.INFO, logger="openamer_cli.auth")
    auth_mod.resolve_openamer_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_openamer_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_openamer_auth_status() should find OpenAmer credentials in the pool
    even when the auth store has no OpenAmer provider entry — this is the
    ...
    openamer_home = tmp_path / "openamer"
    openamer_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no OpenAmer provider entry
    (openamer_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))

    # Seed the credential pool with a OpenAmer entry
    from agent.credential_pool import PooledCredential, load_pool
    pool = load_pool("openamer")
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    entry = PooledCredential.from_dict("openamer", {
        "access_token": token,
...
def test_get_openamer_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from openamer_cli.auth import get_openamer_auth_status, invalidate_openamer_auth_status_cache
...
def test_get_openamer_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    """get_openamer_auth_status() falls back to auth store when credential
    pool is empty.
    """
    from openamer_cli.auth import get_openamer_auth_status

    openamer_home = tmp_path / "openamer"
    _setup_openamer_auth(openamer_home, access_token="at-123")
    monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))
    monkeypatch.setattr(
...
def test_get_openamer_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    from openamer_cli.auth import get_openamer_auth_status
    from agent.credential_pool import PooledCredential, load_pool

    openamer_home = tmp_path / "openamer"
    _setup_openamer_auth(openamer_home, access_token="at-fresh")
    monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))

    pool = load_pool("openamer")
    stale = PooledCredential.from_dict("openamer", {
...
def test_get_openamer_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    from openamer_cli.auth import get_openamer_auth_status
    ...
    def _boom(**kwargs):
        raise AuthError("Refresh session has been revoked", provider="openamer", relogin_required=True)

    monkeypatch.setattr("openamer_cli.auth.resolve_openamer_runtime_credentials", _boom)
...
def test_get_openamer_auth_status_empty_returns_not_logged_in(tmp_path, monkeypatch):
    """get_openamer_auth_status() returns logged_in=False when both pool
...
def test_refresh_token_persisted_when_refreshed_jwt_lacks_invoke_scope(tmp_path, monkeypatch):
    ...
    state_after_failure = get_provider_auth_state("openamer")
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    """A revoked/invalid OpenAmer refresh token must not be replayed forever."""
    ...
    auth_mod._write_shared_openamer_state(shared_state)
    ...
    state_after_failure = auth_mod.get_provider_auth_state("openamer")
    assert state_after_failure is not None
    ...
    assert auth_mod._read_shared_openamer_state() is None
    payload = json.loads((openamer_home / "auth.json").read_text())
    assert payload.get("credential_pool", {}).get("openamer") == []
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    openamer_home = tmp_path / "openamer"
    _setup_openamer_auth(openamer_home, refresh_token="refresh-old")
    monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))
    from agent.credential_pool import load_pool
    ...
    with pytest.raises(AuthError, match="Invalid refresh token"):
        auth_mod.resolve_openamer_access_token()
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `openamer model` → OpenAmer Portal → Skip (keep current) after
    ...
    unconditionally after login, which flipped model.provider to "openamer" while
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return openamer_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        fake_auth_state = {
            "access_token": "fake-openamer-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.openamer.com",
            "portal_base_url": "https://portal.openamer.com",
        ...
        def _check_openamer_free_tier(**kwargs):
            free_tier_calls.append(kwargs)
            return None

        monkeypatch.setattr(models_mod, "check_openamer_free_tier", _check_openamer_free_tier)
        monkeypatch.setattr(
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, OpenAmer creds still saved."""
        import argparse
        import yaml
        from openamer_cli.auth import PROVIDER_REGISTRY, _login_nous

        openamer_home, config_path, auth_path = self._setup_home_with_openrouter(
        ...
        _login_nous(args, PROVIDER_REGISTRY["openamer"])
        ...
        # auth.json: active_provider restored to openrouter, but OpenAmer creds saved
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "openrouter"
        assert "openamer" in auth_after["providers"]
        assert auth_after["providers"]["openamer"]["access_token"] == "fake-openamer-token"
    ...
    def test_picking_model_switches_to_nous(self, tmp_path, monkeypatch):
        """User picks a OpenAmer model → provider flips to openamer with that model."""
        import argparse
        import yaml
        from openamer_cli.auth import PROVIDER_REGISTRY, _login_nous
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        instead of leaving it as openamer."""
        import argparse
        import yaml
        from openamer_cli.auth import PROVIDER_REGISTRY, _login_nous
        ...
        auth_path = openamer_home / "auth.json"
        auth_after = json.loads(auth_path.read_text())
        # active_provider should NOT be set to "openamer" after Skip
        assert auth_after.get("active_provider") in {None, ""}
        # But OpenAmer creds are still saved
        assert "openamer" in auth_after.get("providers", {})


# =============================================================================
# persist_openamer_credentials: shared helper for CLI + web dashboard login paths
# =============================================================================


def _full_state_fixture() -> dict:
    """Shape of the dict returned by _openamer_device_code_login /
    refresh_openamer_oauth_from_state. Used as helper input."""
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    return {
        "portal_base_url": "https://portal.example.com",
...
def test_persist_openamer_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    """Helper must populate BOTH credential_pool.openamer AND providers.openamer.

    Regression guard: before this helper existed, `openamer auth add openamer`
    wrote only the pool. After the OpenAmer agent_key's 24h TTL expired, the
    401-recovery path in run_agent.py called resolve_openamer_runtime_credentials
    which reads providers.openamer, found it empty, raised AuthError, and the
    agent failed with "Non-retryable client error". Both stores must stay
    in sync at write time.
    """
    from openamer_cli.auth import persist_openamer_credentials, OPENAMER_DEVICE_CODE_SOURCE
    ...
    entry = persist_openamer_credentials(state)

    assert entry is not None
    assert entry.provider == "openamer"
    assert entry.source == OPENAMER_DEVICE_CODE_SOURCE

    payload = json.loads((openamer_home / "auth.json").read_text())

    # providers.openamer populated with the full state (new behaviour)
    singleton = payload["providers"]["openamer"]
    ...
    # credential_pool.openamer has exactly one canonical device_code entry
    pool_entries = payload["credential_pool"]["openamer"]
    assert len(pool_entries) == 1, pool_entries
    pool_entry = pool_entries[0]
    assert pool_entry["source"] == OPENAMER_DEVICE_CODE_SOURCE
    assert pool_entry["agent_key"] == state["agent_key"]
    assert pool_entry["inference_base_url"] == "https://inference.example.com/v1"


def test_persist_openamer_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    """End-to-end: after persisting via the helper, resolve_openamer_runtime_credentials
    must succeed (not raise "OpenAmer is not logged into OpenAmer Portal").

    This is the exact path that run_agent.py's `_try_refresh_openamer_client_credentials`
    calls after a OpenAmer 401 — before the fix it would raise AuthError because
    providers.openamer was empty.
    """
    from openamer_cli.auth import (
        persist_openamer_credentials,
        resolve_openamer_runtime_credentials,
    )
    ...
    persist_openamer_credentials(_full_state_fixture())
    ...
    # doesn't raise "OpenAmer is not logged into OpenAmer Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_openamer_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.openamer and lets seeding
    ...
    persist_openamer_credentials(first)
    ...
    persist_openamer_credentials(second)

    payload = json.loads((openamer_home / "auth.json").read_text())

    # providers.openamer reflects the latest write (singleton semantics)
    assert payload["providers"]["openamer"]["access_token"] == second_token
    assert payload["providers"]["openamer"]["agent_key"] == second_token

    # credential_pool.openamer has exactly one entry, carrying the latest agent_key
    pool_entries = payload["credential_pool"]["openamer"]
    assert len(pool_entries) == 1, pool_entries
    assert pool_entries[0]["source"] == OPENAMER_DEVICE_CODE_SOURCE
...
def test_persist_openamer_credentials_reloads_pool_after_singleton_write(tmp_path, monkeypatch):
...
def test_persist_openamer_credentials_embeds_custom_label(tmp_path, monkeypatch):
    """User-supplied ``--label`` round-trips through providers.openamer and the pool.

    Previously `openamer auth add openamer --type oauth --label <name>` silently
    dropped the label because persist_openamer_credentials() ignored it and
    _seed_from_singletons always auto-derived via label_from_token().  The
    fix stashes the label inside providers.openamer so seeding prefers it.
    """
    from openamer_cli.auth import persist_openamer_credentials, OPENAMER_DEVICE_CODE_SOURCE
    ...
    entry = persist_openamer_credentials(_full_state_fixture(), label="my-personal")
    assert entry is not None
    assert entry.source == OPENAMER_DEVICE_CODE_SOURCE
    assert entry.label == "my-personal"

    # providers.openamer carries the label so re-seeding on the next load_pool
    # doesn't overwrite it with the auto-derived fingerprint.
    payload = json.loads((openamer_home / "auth.json").read_text())
    assert payload["providers"]["openamer"]["label"] == "my-personal"


def test_persist_openamer_credentials_custom_label_survives_reseed(tmp_path, monkeypatch):
    ...
    persist_openamer_credentials(_full_state_fixture(), label="work-acct")
...
def test_persist_openamer_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    entry = persist_openamer_credentials(_full_state_fixture())
    assert entry is not None
    ...
    # No "label" key embedded in providers.openamer when the caller didn't supply one.
    payload = json.loads((openamer_home / "auth.json").read_text())
    assert "label" not in payload["providers"]["openamer"]


def test_refresh_token_reuse_detection_surfaces_actionable_message():
    """Regression for #15099.

    When the OpenAmer Portal server returns ``invalid_grant`` with
    ``error_description`` containing "reuse detected", OpenAmer must surface an
    ...
    re-authenticate" string led users to report this as a OpenAmer persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from openamer_cli.auth import _refresh_access_token
    ...
    assert "openamer auth add openamer" in message.lower()
...
def test_refresh_token_reuse_error_code_is_terminal():
    """OpenAmer may return refresh_token_reused as the OAuth error code itself."""
    from openamer_cli import auth as auth_mod
    ...
    assert exc_info.value.relogin_required is True
    assert auth_mod._is_terminal_openamer_refresh_error(exc_info.value) is True


def test_refresh_token_exchange_sends_refresh_token_header():
    """OpenAmer refresh tokens must be sent in a header so sandbox proxies can
    ...
    assert client.kwargs["headers"]["x-openamer-refresh-token"] == "refresh-1"
...
# =============================================================================
# Shared OpenAmer token store — cross-profile persistence (Codex-style auto-import)
# =============================================================================


@pytest.fixture
def shared_store_env(tmp_path, monkeypatch):
    """Redirect OPENAMER_SHARED_AUTH_DIR to a tmp_path.

    Required for every test that exercises the shared OpenAmer store — the
    ...
    monkeypatch.setenv("OPENAMER_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without OPENAMER_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.openamer/shared/`` across CI runs.
    """
    from openamer_cli.auth import _openamer_shared_store_path

    monkeypatch.delenv("OPENAMER_SHARED_AUTH_DIR", raising=False)

    with pytest.raises(RuntimeError, match="shared OpenAmer auth store"):
        _openamer_shared_store_path()


def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """OPENAMER_SHARED_AUTH_DIR must redirect the path."""
    from openamer_cli.auth import _openamer_shared_store_path, OPENAMER_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("OPENAMER_SHARED_AUTH_DIR", str(custom_dir))

    path = _openamer_shared_store_path()
    assert path == custom_dir / OPENAMER_SHARED_STORE_FILENAME


def test_shared_store_read_missing_returns_none(shared_store_env):
    """Missing file → ``_read_shared_openamer_state()`` returns None."""
    from openamer_cli.auth import _read_shared_openamer_state

    assert _read_shared_openamer_state() is None


def test_shared_store_read_malformed_returns_none(shared_store_env):
    """Unreadable / non-JSON file → None, not an exception."""
    from openamer_cli.auth import _openamer_shared_store_path, _read_shared_openamer_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from openamer_cli.auth import _openamer_shared_store_path, _write_shared_openamer_state
    ...
    assert not _openamer_shared_store_path().is_file()


def test_persist_openamer_credentials_mirrors_to_shared_store(
    tmp_path, monkeypatch, shared_store_env,
):
    """persist_openamer_credentials must populate BOTH per-profile auth.json
    AND the shared store, so a future profile's `openamer auth add openamer
    ...
    payload = json.loads((openamer_home / "auth.json").read_text())
    assert "openamer" in payload.get("providers", {})
    ...
    assert str(_openamer_shared_store_path()).startswith(str(shared_store_env))
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from openamer_cli.auth import _try_import_shared_openamer_state

    assert _try_import_shared_openamer_state() is None
...
def test_try_import_shared_returns_none_on_refresh_failure(
    ...
    portal down), _try_import_shared_openamer_state must return None so the
    login flow falls back to a fresh device-code run.
    """
    from openamer_cli import auth as auth_mod

    # Seed the shared store
    auth_mod._write_shared_openamer_state(_full_state_fixture())
    ...
    monkeypatch.setattr(auth_mod, "refresh_openamer_oauth_from_state", _boom)

    assert auth_mod._try_import_shared_openamer_state() is None
    assert auth_mod._read_shared_openamer_state() is None
...
def test_try_import_shared_persists_rotated_token_when_jwt_validation_fails(
    ...
    shared_after = auth_mod._read_shared_openamer_state()
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    every field persist_openamer_credentials() needs.
    """
    from openamer_cli import auth as auth_mod
    ...
    monkeypatch.setattr(auth_mod, "refresh_openamer_oauth_from_state", _fake_refresh)

    result = auth_mod._try_import_shared_openamer_state()
    ...
    assert result["client_id"] == "openamer-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different OPENAMER_HOME) sees the same shared state and can rehydrate
    ...
    auth_mod.persist_openamer_credentials(_full_state_fixture())

    # Profile A's auth.json has openamer
    a_payload = json.loads((profile_a / "auth.json").read_text())
    assert "openamer" in a_payload.get("providers", {})

    # Profile B: fresh OPENAMER_HOME, no auth yet, but the shared store
    # persists — _read_shared_openamer_state() must still return the tokens.
    ...
    # B's own auth.json has no openamer
    b_payload = json.loads((profile_b / "auth.json").read_text())
    assert "openamer" not in b_payload.get("providers", {})

    # But the shared store is visible
    shared = auth_mod._read_shared_openamer_state()
    assert shared is not None
    assert shared["refresh_token"] == "refresh-tok"

    # And a successful rehydrate + persist lands openamer into profile B
    ...
    auth_mod.persist_openamer_credentials(result)

    b_payload = json.loads((profile_b / "auth.json").read_text())
    assert "openamer" in b_payload.get("providers", {})
    assert b_payload["providers"]["openamer"]["refresh_token"] == "b-refresh-tok"
...
def test_runtime_refresh_uses_newer_shared_token_before_local_stale_token(
    ...
    """A sibling profile may rotate the single-use OpenAmer refresh token.
    ...
    profile_state = auth_mod.get_provider_auth_state("openamer")
...
def test_runtime_credentials_merges_shared_token_before_empty_local_access_token(
    ...
    ``resolve_openamer_access_token()`` already merges shared OAuth state before
    giving up. The runtime path must do the same so sibling profiles that
    share a valid OpenAmer login do not dead-end on an empty local auth.json.
    """
    from openamer_cli import auth as auth_mod
    ...
    auth_payload["providers"]["openamer"]["access_token"] = None
    ...
    shared_state["scope"] = auth_mod.DEFAULT_openamer_SCOPE
    shared_state["inference_base_url"] = auth_mod.DEFAULT_openamer_INFERENCE_URL
    auth_mod._write_shared_openamer_state(shared_state)
    ...
    assert creds["api_key"] == shared_token
    assert creds["base_url"] == auth_mod.DEFAULT_openamer_INFERENCE_URL
...
def test_runtime_shared_recovery_recomputes_routing_before_force_refresh(
    ...
    auth_path = profile_b / "auth.json"
    auth_payload = json.loads(auth_path.read_text())
    local_state = auth_payload["providers"]["openamer"]
    ...
    def _refresh(**kwargs):
        ...
        return {
            ...
            "expires_in": 7200,
            "scope": auth_mod.DEFAULT_openamer_SCOPE,
            "inference_base_url": auth_mod.DEFAULT_openamer_INFERENCE_URL,
        }

    monkeypatch.setattr(auth_mod, "_refresh_access_token", _refresh)

    creds = auth_mod.resolve_openamer_runtime_credentials(force_refresh=True)
...
def test_runtime_refresh_persists_routing_before_jwt_validation_failure(
    ...
    refreshed_url = auth_mod.DEFAULT_openamer_INFERENCE_URL
    monkeypatch.setattr(
        auth_mod,
        "_refresh_access_token",
        lambda **_kwargs: {
    ...
    shared_state = auth_mod._read_shared_openamer_state()
...
def test_runtime_shared_recovery_honors_inference_env_override(
    ...
    profile_state = auth_mod.get_provider_auth_state("openamer")
    assert profile_state is not None
    assert profile_state["inference_base_url"] == auth_mod.DEFAULT_openamer_INFERENCE_URL
...
def test_managed_gateway_access_token_uses_newer_shared_token(
    ...
    assert auth_mod.resolve_openamer_access_token() == "shared-fresh-access"
...
class TestStalePortalBaseUrlMigration:
    """_migrate_stale_openamer_portal_url auto-corrects stale portal_base_url on load."""

    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from openamer_cli.auth import _load_auth_store, DEFAULT_openamer_PORTAL_URL
        ...
        auth_file.write_text(json.dumps({
            ...
            "providers": {
                "openamer": {
                    "portal_base_url": "https://api.openamer.com",
        ...
        openamer = store["providers"]["openamer"]
        assert openamer["portal_base_url"] == DEFAULT_openamer_PORTAL_URL
    ...
    def test_ignores_other_providers(self, tmp_path, monkeypatch):
        ...
        store = _load_auth_store(auth_file)
        assert "openamer" not in store.get("providers", {})

    def test_noop_when_openamer_state_not_dict(self, tmp_path, monkeypatch):
        ...
        auth_file.write_text(json.dumps({
            "version": 1,
            "active_provider": "openamer",
            "providers": {"openamer": None},
        }))

        store = _load_auth_store(auth_file)
        assert store["providers"]["openamer"] is None
    ...
    def test_runtime_fallback_for_invalid_portal_url(self, tmp_path, monkeypatch):
        ...
        store["providers"]["openamer"]["portal_base_url"] = "https://api.openamer.com"
        ...
        token = auth_mod.resolve_openamer_access_token()
        assert token == "refreshed-access"
        assert len(refresh_calls) == 1
        assert refresh_calls[0] == auth_mod.DEFAULT_openamer_PORTAL_URL
    ...
    def test_runtime_accepts_localhost(self, tmp_path, monkeypatch):
        ...
        auth_file = tmp_path / "auth.json"
        store = json.loads(auth_file.read_text())
        store["providers"]["openamer"]["portal_base_url"] = "http://localhost:8080/"
        auth_file.write_text(json.dumps(store, indent=2))
    ...
    def test_runtime_credentials_fallback_for_invalid_portal_url(self, tmp_path, monkeypatch):
        """resolve_openamer_runtime_credentials also rejects an off-allowlist portal host.
        ...
        resolve_openamer_access_token so the whole class is covered, not just the
        ...
        store = json.loads(auth_file.read_text())
        store["providers"]["openamer"]["portal_base_url"] = "https://evil.example.com"
        auth_file.write_text(json.dumps(store, indent=2))
    ...
    def test_runtime_credentials_rejects_http_for_production_portal(
        ...
        auth_file = openamer_home / "auth.json"
        store = json.loads(auth_file.read_text())
        store["providers"]["openamer"]["portal_base_url"] = (
            "http://portal.openamer.com"
        )
        auth_file.write_text(json.dumps(store, indent=2))
        ...
        auth_mod.resolve_openamer_runtime_credentials()
        assert refresh_calls == [auth_mod.DEFAULT_openamer_PORTAL_URL]
