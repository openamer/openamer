"""Tests for get_openamer_session_validity — the /api/status classifier NAS reads
...
from openamer_cli.auth import (
    OPENAMER_SESSION_TERMINAL,
    OPENAMER_SESSION_UNKNOWN,
    OPENAMER_SESSION_VALID,
    get_openamer_session_validity,
)


def _clear_cache():
    auth.invalidate_openamer_auth_status_cache()
...
def test_valid_when_logged_in(monkeypatch):
    ...
    monkeypatch.setattr(auth, "get_openamer_auth_status", lambda: {"logged_in": True})
    assert get_openamer_session_validity() == OPENAMER_SESSION_VALID
...
def test_terminal_on_persisted_quarantine_marker(monkeypatch):
    ...
    monkeypatch.setattr(auth, "get_openamer_auth_status", lambda: {"logged_in": False})
    assert get_openamer_session_validity() == OPENAMER_SESSION_TERMINAL
...
def test_terminal_on_relogin_required_status(monkeypatch):
    ...
    monkeypatch.setattr(auth, "get_openamer_auth_status", lambda: {
...
def test_unknown_when_no_provider_state(monkeypatch):
    """No OpenAmer provider state at all → 'unknown' (never terminal)."""
    monkeypatch.setattr(auth, "get_provider_auth_state", lambda p: None)
    monkeypatch.setattr(auth, "get_openamer_auth_status", lambda: {"logged_in": False})
    assert get_openamer_session_validity() == OPENAMER_SESSION_UNKNOWN
...
def test_status_exception_is_unknown_not_terminal(monkeypatch):
    ...
    monkeypatch.setattr(auth, "get_openamer_auth_status", _boom)
    assert get_openamer_session_validity() == OPENAMER_SESSION_UNKNOWN
