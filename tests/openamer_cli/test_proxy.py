"""Tests for the `openamer proxy` subcommand and its upstream adapters."""
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic OPENAMER_HOME."""
    auth_path = openamer_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from openamer_cli.proxy.server import create_app  # noqa: E402
...
def test_server_forwards_chat_completions():
    async def run():
        ...
        try:
            ...
            assert "OpenAmer-4-70B" in req["body"]
        finally:
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("OPENAMER_HOME", str(tmp_path))
    from openamer_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from openamer_cli.proxy.cli import cmd_proxy_list_providers
