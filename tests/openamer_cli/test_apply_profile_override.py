...
When OPENAMER_HOME is set to the hermes root (e.g. systemd hardcodes
OPENAMER_HOME=/root/.hermes), _apply_profile_override must still read
...
class TestApplyProfileOverrideHermesHomeGuard:
    ...
    Verifies that OPENAMER_HOME pointing to the hermes root does NOT suppress
    ...
    def test_hermes_home_at_root_with_active_profile_is_redirected(
        ...
        """OPENAMER_HOME=/root/.hermes + active_profile=coder must redirect
        OPENAMER_HOME to .../profiles/coder.

        Bug scenario from #22502: systemd sets OPENAMER_HOME to the hermes root
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    bare ``hermes gateway run`` (no ``-p``) to mean "the root OPENAMER_HOME
    ...
    def test_supervised_child_does_not_follow_active_profile(
        ...
        monkeypatch.setenv("OPENAMER_HOME", str(hermes_root))
        ...
        assert os.environ.get("OPENAMER_HOME") == str(hermes_root), (
