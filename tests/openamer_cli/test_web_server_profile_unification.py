...
profile switcher can target any profile's OPENAMER_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via OPENAMER_HOME.
"""
import pytest
import yaml
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_openamer_home):
    ...
    default_home = get_openamer_home()
...
class TestProfileScopedMcp:
    ...
    def test_mcp_probe_runs_inside_profile_scope(
        ...
        import openamer_cli.mcp_config as mcp_config
        from openamer_constants import get_openamer_home
        ...
        def fake_probe(name, config, connect_timeout=30, details=None):
            seen["home"] = str(get_openamer_home())
            return [("tool-a", "desc")]
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same OPENAMER_HOME the rest
        ...
        monkeypatch.setattr(
            web_server,
            "_spawn_openamer_action",
            lambda subcommand, name: calls.append(list(subcommand)) or _FakeProc(),
        )
        monkeypatch.setattr(
            "openamer_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid(*args, **kwargs):
            ...
            seen_homes.append(str(get_openamer_home()))
            return None
        ...
        assert resp.json()["openamer_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_openamer_home(self, isolated_profiles, monkeypatch):
        import openamer_cli.web_server as web_server

        monkeypatch.setattr(
            "openamer_cli.main._make_tui_argv",
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("OPENAMER_HOME") != str(isolated_profiles["worker_beta"])
