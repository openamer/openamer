"""Tests for the Kanban DB layer (openamer_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated OPENAMER_HOME with an empty kanban DB."""
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real OpenAmer profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="openamer-task", assignee="daily")
...
def test_scratch_workspace_created_under_openamer_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
        assert task is not None
...
def test_dispatch_worktree_task_persists_materialized_workspace_and_branch(kanban_home, tmp_path, monkeypatch):
    ...
    import openamer_cli.profiles as profiles
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "OpenAmer-managed scratch dir should be cleaned up"
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """OpenAmer' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` OpenAmer' DB/metadata/logs on task completion.
    """
...
# spawned with `openamer -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's OPENAMER_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's OPENAMER_HOME."""
    ...
    def test_default_install_anchors_at_home_dot_openamer(
        ...
        # Standard install: OPENAMER_HOME == ~/.openamer, no profile active.
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.openamer/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.openamer/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.openamer/kanban.db.
        default_home = tmp_path / ".openamer"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # OPENAMER_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `openamer -p coder`).
    ...
    def test_docker_custom_openamer_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: OPENAMER_HOME points outside ~/.openamer.
        # `get_default_openamer_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".openamer"`.
        custom_root = tmp_path / "opt" / "openamer"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: OPENAMER_HOME=/opt/openamer/profiles/coder;
        # `get_default_openamer_root()` walks up to /opt/openamer because
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # OPENAMER_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile OPENAMER_HOME and reads.
        monkeypatch.setenv("OPENAMER_HOME", str(profile_home))
        with kb.connect() as conn:
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see openamer_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    (see openamer-agent issue #22032).
    ...
    with _patch("openamer_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'openamer kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_openamer_argv()
#
# Workers spawned by the dispatcher must use a `openamer` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["openamer", ...]`
...
def test_resolve_openamer_argv_prefers_path_shim(monkeypatch):
    """When `openamer` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
...
def test_resolve_openamer_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
...
def test_resolve_openamer_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
...
def test_resolve_openamer_argv_openamer_bin_bare_name_uses_path(monkeypatch, tmp_path):
    ...
    cwd_openamer = tmp_path / "openamer"
    cwd_openamer.write_text("wrong\n", encoding="utf-8")
    cwd_openamer.chmod(cwd_openamer.stat().st_mode | stat.S_IXUSR)
    path_openamer = tmp_path / "bin" / "openamer"
    path_openamer.parent.mkdir()
    path_openamer.write_text("right\n", encoding="utf-8")
...
def test_resolve_openamer_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m openamer_cli.main`.

    Pins the correct module name (NOT `openamer` — there is no top-level
    `openamer` package). Regression for #23198: the original PR shipped
    `python -m openamer` which fails with `No module named openamer` on every
    invocation.
    """
    import shutil
    import sys
...
def test_resolve_openamer_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `openamer_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m openamer_cli.main --version`
    ...
    assert "OpenAmer Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="openamer_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from openamer_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch(
            "openamer_cli.sqlite_safe_read.os.path.getsize", side_effect=fake_getsize
        ):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from openamer_cli.kanban_db import connect, write_txn
    import openamer_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from openamer_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("openamer_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("openamer_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.kanban_db.os.name", "nt")
    with patch("openamer_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("openamer_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("openamer_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "openamer_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("openamer_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "openamer_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("openamer_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("openamer_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("openamer_cli.kanban_db._record_worker_exit"):
            with patch("openamer_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
