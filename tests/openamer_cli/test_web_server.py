"""Tests for openamer_cli.web_server and related config utilities."""
...
def _install_example_plugin(_isolate_openamer_home):
    ...
    #      and start serving requests against a torn-down OPENAMER_HOME.
    app = web_server.app
...
class TestSessionTokenInjection:
    """The desktop shell mints OPENAMER_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENAMER_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("OPENAMER_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_openamer_home):
        ...
        from openamer_constants import get_openamer_home
        from openamer_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(openamer_state, "DEFAULT_DB_PATH", get_openamer_home() / "state.db")
    ...
    def test_status_active_session_count_uses_read_only_db(self, monkeypatch, tmp_path):
        ...
        fake_db_path.touch()
        monkeypatch.setattr(openamer_state, "DEFAULT_DB_PATH", fake_db_path)
        ...
        monkeypatch.setattr("openamer_state.SessionDB", _FakeDB)
    ...
    def test_status_active_session_count_fresh_install_returns_zero(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(openamer_state, "DEFAULT_DB_PATH", tmp_path / "absent.db")

        def _boom(*a, **k):
            raise AssertionError("SessionDB must not be constructed when db file is absent")

        monkeypatch.setattr("openamer_state.SessionDB", _boom)
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        ...
        config_path = get_openamer_home() / "hindsight" / "config.json"
    ...
    def test_post_memory_provider_setup_persists_values_without_activation(self):
        from openamer_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide OPENAMER_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from openamer_constants import get_openamer_home

        path = get_openamer_home() / "honcho.json"
    ...
    def test_memory_provider_config_honors_profile_param(self, monkeypatch, tmp_path):
        ...
        own = json.loads((get_openamer_home() / "honcho.json").read_text(encoding="utf-8"))
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_openamer_home() / "images" / "nope.png"
        resp = self.client.get("/api/media", params={"path": str(missing)})
        assert resp.status_code == 404

    def test_get_media_requires_auth(self):
        from openamer_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_chat_image_upload_writes_to_default_profile_images(self):
        ...
        assert target.parent == get_openamer_home() / "images"
    ...
    def test_list_triggers_config_gated_auto_archive(self):
        ...
        # The helper imports load_config lazily from openamer_cli.config; patch there.
    ...
    def test_update_openamer_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        assert "docker pull openamer/openamer:latest" in data["message"]
        ...
        assert any("docker pull openamer/openamer:latest" in line for line in status_data["lines"])
    ...
    def test_config_schema_merges_custom_command_tts_provider(self):
        ...
        from openamer_cli.config import load_config, save_config
        from openamer_cli.web_server import CONFIG_SCHEMA
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from openamer_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from openamer_cli.web_server import (
    ...
    def test_ops_backup_uses_hosted_openamer_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENAMER_HOME", str(hosted_home))
        captured = {}
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from openamer_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from openamer_cli.config import save_env_value
        from openamer_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://github.com/openamer/openamer/blob/main/website/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        from openamer_cli.web_server import _build_catalog_entry

        teams = _build_catalog_entry("teams")
        assert teams["docs_url"] == (
            "https://github.com/openamer/openamer/blob/main/website/docs/user-guide/messaging/teams"
        )
    ...
    def test_google_chat_messaging_metadata_links_setup_guide(self):
        ...
        assert google_chat["docs_url"] == (
            "https://github.com/openamer/openamer/blob/main/website/docs/user-guide/messaging/google_chat"
        )
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/openamer_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from openamer_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from openamer_cli.web_server import _parse_model_ids
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from openamer_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_migrates_a_legacy_plaintext_key(self):
        ...
        endpoint again.
        """
        from openamer_cli.config import custom_endpoint_key_env, get_env_value, load_config, save_config
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from openamer_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from openamer_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_approvals_mode_options_match_config_values(self):
        ...
        are 'manual', 'smart', and 'off' (see openamer_cli/config.py).
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from openamer_cli.web_server import _build_schema_from_config
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from openamer_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_openamer_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_openamer_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_openamer_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_openamer_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_openamer_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_openamer_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_openamer_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_openamer_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_openamer_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_openamer_home() / "profiles" / "builder"
        token = set_openamer_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_openamer_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import openamer_cli.skills_config as skills_config
        import openamer_cli.web_server as web_server
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from openamer_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from openamer_cli.web_server import _denormalize_config_from_web
        from openamer_cli.config import save_config
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from openamer_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_openamer_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
            reset_openamer_home_override(token)
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_openamer_home):
        ...
        monkeypatch.setattr(
            openamer_state, "DEFAULT_DB_PATH", get_openamer_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``openamer_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in openamer_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_openamer_home):
        ...
        # Pin the SessionDB to the isolated OPENAMER_HOME so each test
...
class TestPluginAPIAuth:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_openamer_home, _install_example_plugin):
        ...
        installs it into the per-test ``OPENAMER_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into OPENAMER_HOME by the class-level
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        OPENAMER_HOME override scopes a request to another profile."""
        from openamer_constants import (
            reset_openamer_home_override,
            set_openamer_home_override,
        )
...
class TestPtyWebSocket:
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from openamer_cli.pty_bridge import PtyUnavailableError
        ...
        import openamer_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from openamer_cli import web_server as ws_mod
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_openamer_home, _install_example_plugin):
        ...
        the per-test ``OPENAMER_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
