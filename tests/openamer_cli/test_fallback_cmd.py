"""Tests for `openamer fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate OPENAMER_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".openamer" / "config.yaml"
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from openamer_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    def test_list_empty(self, isolated_home, capsys):
        ...
        assert "openamer fallback add" in out
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("openamer_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("openamer_cli.main._require_tty"):
            from openamer_cli.fallback_cmd import cmd_fallback_add
...
class TestClearCommand:
    def test_clear_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from openamer_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `openamer fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "openamer_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
