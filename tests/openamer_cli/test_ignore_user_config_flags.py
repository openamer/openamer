...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("OPENAMER_IGNORE_USER_CONFIG", "OPENAMER_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``OPENAMER_IGNORE_USER_CONFIG=1``.
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With OPENAMER_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("OPENAMER_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    ...
    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting OPENAMER_IGNORE_RULES=1 flips OpenAmerCLI.ignore_rules True."""
        monkeypatch.setenv("OPENAMER_IGNORE_RULES", "1")
        ...
        obj.ignore_rules = ignore_rules or os.environ.get("OPENAMER_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("OPENAMER_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.OpenAmerCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``openamer_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("OPENAMER_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("OPENAMER_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("OPENAMER_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("OPENAMER_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("OPENAMER_IGNORE_USER_CONFIG") == "1"
        assert "OPENAMER_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "OPENAMER_IGNORE_USER_CONFIG" not in os.environ
        assert "OPENAMER_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        ...
        import inspect
        import openamer_cli.main as hm
        src = inspect.getsource(hm)
        assert "OPENAMER_IGNORE_USER_CONFIG" in src
        assert "OPENAMER_IGNORE_RULES" in src
