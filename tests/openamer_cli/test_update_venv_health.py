...
Covers three additions to ``openamer update``:
...
def test_venv_health_missing_venv_unhealthy_on_managed_install(tmp_path):
    ...
    (tmp_path / ".openamer-bootstrap-complete").write_text("done")
    with patch.object(cli_main, "PROJECT_ROOT", tmp_path):
        healthy, detail = cli_main._venv_core_imports_healthy()
...
def test_detect_venv_python_finds_backend(_winp, tmp_path):
    ...
    fake_psutil = types.SimpleNamespace(
        process_iter=lambda attrs: iter(
            [
                _proc(101, venv_py, "python.exe", ["python.exe", "-m", "openamer_cli.main", "serve"]),
    ...
    fake_psutil = types.SimpleNamespace(
        process_iter=lambda attrs: iter(
            [
                ...
                _proc(555, venv_py, "openamer.exe"),
            ]
        ),
        Process=lambda *a, **k: me,
    )
...
def test_detect_venv_python_catches_outside_venv_trampoline(_winp, tmp_path):
    ...
    clearly runs OpenAmer from this install → must still be flagged as a holder
    ...
    fake_psutil = types.SimpleNamespace(
        process_iter=lambda attrs: iter(
            [
                ...
                # `-m openamer_cli.main serve` with the install root as cwd
                _proc(
                    202,
                    base_py,
                    "python.exe",
                    [base_py, "-m", "openamer_cli.main", "serve"],
                    cwd=str(tmp_path),
                ),
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_venv_openamer_cli_cmdline_outside_install_not_matched(_winp, tmp_path):
    """A openamer_cli.main process belonging to a DIFFERENT install (neither
...
def _run_update_until_guard(args):
    ...
    with patch.object(cli_main, "_is_windows", return_value=True), patch.object(
        ...
        cli_main,
        "_detect_venv_python_processes",
        return_value=[(101, "python.exe", "python.exe -m openamer_cli.main serve")],
    ), patch.object(
