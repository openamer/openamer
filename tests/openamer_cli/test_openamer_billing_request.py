"""Tests for the openamer_cli.openamer_billing HTTP client's response handling.
...
from openamer_cli import openamer_billing as nb
...
def test_post_subscription_preview_request(monkeypatch):
    with _capture(monkeypatch) as seen:
        nb.post_subscription_preview(subscription_type_id="openamer-chat-plan-40")
    assert seen["method"] == "POST"
    assert seen["url"] == "https://portal.example/api/billing/subscription/preview"
    assert seen["data"] == {"subscriptionTypeId": "openamer-chat-plan-40"}


def test_put_pending_change_tier_change_request(monkeypatch):
    with _capture(monkeypatch) as seen:
        nb.put_subscription_pending_change(subscription_type_id="openamer-chat-plan-10")
    ...
    assert seen["data"] == {
        "type": "tier_change",
        "subscriptionTypeId": "openamer-chat-plan-10",
    }
...
def test_post_subscription_upgrade_sends_idempotency_key(monkeypatch):
    with _capture(monkeypatch) as seen:
        nb.post_subscription_upgrade(
            subscription_type_id="openamer-chat-plan-40", idempotency_key="abc-123"
        )
    assert seen["method"] == "POST"
...
def test_post_subscription_upgrade_blank_key_raises():
    with pytest.raises(nb.BillingError) as ei:
        nb.post_subscription_upgrade(
            subscription_type_id="openamer-chat-plan-40", idempotency_key="  "
        )
...
def test_403_cli_billing_disabled_stays_generic_with_portal_url(monkeypatch):
    ...
    assert ei.value.portal_url == "https://portal.openamer.com/billing"
...
def test_urlerror_dns_maps_to_network_error(monkeypatch):
    ...
    assert "Could not reach OpenAmer Portal" in str(ei.value)
