...
def test_expected_machines_per_host(host, loadable, not_loadable):
    with patch("openamer_cli.main._windows_native_machine", return_value=host):
...
def test_expected_machines_unknown_host_is_permissive():
    ...
    with patch("openamer_cli.main._windows_native_machine", return_value="RISCV64"):
        expected = cli_main._expected_windows_pe_machines()
...
# MACHINE_ATTRIBUTES.UserEnabled — mirrors openamer_cli.main.
...
def test_gate_rolls_back_corrupt_exe_and_purges_cache(tmp_path, monkeypatch, capsys):
    ...
    with patch("openamer_cli.main._windows_native_machine", return_value="AMD64"), \
         ...
         patch("openamer_cli.main._desktop_stamp_path", return_value=stamp):
        verified, rolled_back = cli_main._ensure_desktop_exe_launchable(desktop_dir, exe)
...
def test_gate_fails_clearly_without_backup(tmp_path, monkeypatch, capsys):
    ...
    with patch("openamer_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("openamer_cli.main._desktop_stamp_path", return_value=tmp_path / "stamp.json"):
        verified, rolled_back = cli_main._ensure_desktop_exe_launchable(desktop_dir, exe)
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    ...
    with patch("openamer_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("openamer_cli.main._stop_desktop_processes_locking_build", return_value=[]), \
         ...
         patch("openamer_cli.main._windows_native_machine", return_value="AMD64"), \
         patch("openamer_cli.main.subprocess.run", return_value=pack_ok), \
