"""Regression tests for OpenAmer Portal inference_base_url host-allowlist validation.
...
client. ``_validate_openamer_inference_url_from_network()`` blocks any URL
...
4. The env-var override path (``OPENAMER_INFERENCE_BASE_URL``) is NOT
...
from openamer_cli.auth import (
    DEFAULT_openamer_INFERENCE_URL,
    _ALLOWED_openamer_INFERENCE_HOSTS,
...
class TestValidatorRules:
    def test_allowlisted_https_host_returned(self):
        url = "https://inference-api.openamer.com/v1"
        assert _validate_openamer_inference_url_from_network(url) == url

    def test_trailing_slash_stripped(self):
        url = "https://inference-api.openamer.com/v1/"
        assert _validate_openamer_inference_url_from_network(url) == url.rstrip("/")

    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="openamer_cli.auth"):
            assert (
                _validate_openamer_inference_url_from_network("https://attacker.com/v1")
                is None
            )
        assert any("attacker.com" in rec.message for rec in caplog.records)

    def test_subdomain_of_allowlist_host_rejected(self):
        """*.openamer.com is NOT in the allowlist — exact hostname only.

        A subdomain takeover or DNS hijack of *.openamer.com would
        otherwise pass — keep the gate tight.
        """
        assert (
            _validate_openamer_inference_url_from_network(
                "https://evil.inference-api.openamer.com/v1"
            )
            is None
        )
    ...
    def test_http_scheme_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="openamer_cli.auth"):
            assert (
                _validate_openamer_inference_url_from_network(
                    "http://inference-api.openamer.com/v1"
                )
                is None
            )
    ...
    def test_file_scheme_rejected(self):
        assert (
            _validate_openamer_inference_url_from_network("file:///etc/passwd") is None
        )
    ...
    def test_empty_string_rejected(self):
        assert _validate_openamer_inference_url_from_network("") is None

    def test_whitespace_only_rejected(self):
        assert _validate_openamer_inference_url_from_network("   ") is None

    def test_none_rejected(self):
        assert _validate_openamer_inference_url_from_network(None) is None

    def test_non_string_rejected(self):
        assert _validate_openamer_inference_url_from_network(12345) is None  # type: ignore[arg-type]
        assert _validate_openamer_inference_url_from_network({"url": "x"}) is None  # type: ignore[arg-type]

    def test_malformed_url_rejected(self):
        """Even garbled input must fall back safely, not raise."""
        assert (
            _validate_openamer_inference_url_from_network("not://a real url at all")
            is None
        )

    def test_default_inference_url_is_in_allowlist(self):
        """Sanity check: DEFAULT_openamer_INFERENCE_URL must itself validate.

        If anyone retargets the default away from
        ``inference-api.openamer.com``, they MUST update the allowlist
        in the same change — otherwise the allowlist would reject the
        Portal's own legitimate default and break every install.
        """
        assert (
            _validate_openamer_inference_url_from_network(DEFAULT_openamer_INFERENCE_URL)
            == DEFAULT_openamer_INFERENCE_URL.rstrip("/")
        )

    def test_allowlist_contains_inference_api_host(self):
        """The default's host must be in the allowlist set."""
        from urllib.parse import urlparse
        host = urlparse(DEFAULT_openamer_INFERENCE_URL).hostname
        assert host in _ALLOWED_openamer_INFERENCE_HOSTS
...
class TestCallSiteWiring:
    ...
    ``_validate_openamer_inference_url_from_network`` with the un-validated
    ``_optional_base_url`` again, the test catches it.

    Each site lives inside ``resolve_openamer_runtime_credentials`` and one
    ...
    def _read_auth_source(self):
        import openamer_cli.auth as _auth_mod
    ...
    def test_no_unvalidated_inference_base_url_assignments_remain(self):
        ...
        for needle in (
            ...
            assert needle not in source, (
                ...
                f"Use _validate_openamer_inference_url_from_network() instead."
            )
    ...
    def test_validator_wired_at_all_known_call_sites(self):
        ...
        refresh_count = source.count(
            '_validate_openamer_inference_url_from_network(refreshed.get("inference_base_url"))'
        )
        mint_count = source.count(
            '_validate_openamer_inference_url_from_network(mint_payload.get("inference_base_url"))'
        )
    ...
    def test_proxy_adapter_also_validates(self):
        """The OpenAmer proxy adapter applies the validator as defense-in-depth
        ...
        boundary."""
        from pathlib import Path
        import openamer_cli.proxy.adapters.openamer_portal as _openamer_adapter
        source = Path(_openamer_adapter.__file__).read_text(encoding="utf-8")
        assert "_validate_openamer_inference_url_from_network" in source


class TestEnvOverrideNotGated:
    """The documented dev/staging env-var override must keep working.

    ``OPENAMER_INFERENCE_BASE_URL`` is read by ``resolve_openamer_runtime_credentials``
    ...
    def test_env_override_path_does_not_call_validator(self):
        """In resolve_openamer_runtime_credentials, the env override is
        ...
        for line in source.splitlines():
            if "OPENAMER_INFERENCE_BASE_URL" in line and "os.getenv" in line:
                assert "_validate_openamer_inference_url_from_network" not in line, (
...
class TestHealsPoisonedStoredValue:
    ...
    stale ``stg-inference-api.openamer.com`` persisted before the
    ...
    staging endpoint (real incident: opus-4.8 routed to openamer, openamer pinned
    ...
    def test_refresh_resets_rejected_url_to_default(self, monkeypatch):
        ...
        poisoned = "https://stg-inference-api.openamer.com/v1"
        state = {
            "access_token": "tok",
            "refresh_token": "rtok",
            "client_id": "openamer-cli",
            "portal_base_url": auth.DEFAULT_openamer_PORTAL_URL,
        ...
        monkeypatch.setattr(auth, "_openamer_invoke_jwt_status", lambda *a, **k: "needs_refresh")
        monkeypatch.setattr(
            auth,
        ...
        monkeypatch.setattr(auth, "_assert_openamer_inference_jwt_usable", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_select_openamer_invoke_jwt", lambda *a, **k: None)

        result = auth.refresh_openamer_oauth_from_state(state, force_refresh=True)

        assert result["inference_base_url"] == auth.DEFAULT_openamer_INFERENCE_URL, (
    ...
    def test_refresh_keeps_valid_url(self, monkeypatch):
        ...
        good = "https://inference-api.openamer.com/v1"
        state = {
            "access_token": "tok",
            "refresh_token": "rtok",
            "client_id": "openamer-cli",
...
class TestEnvOverrideWins:
    """``OPENAMER_INFERENCE_BASE_URL`` must win over the stored value for the
    ...
    STAGING = "https://stg-inference-api.openamer.com/v1"

    def _patch_no_refresh(self, monkeypatch, auth, state):
        import contextlib

        # No refresh fires: the stored access token is a usable invoke JWT.
        monkeypatch.setattr(auth, "_openamer_invoke_jwt_status", lambda *a, **k: None)
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(auth, "_write_shared_openamer_state", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_sync_openamer_pool_from_auth_store", lambda *a, **k: None)
    ...
    def test_no_refresh_env_override_wins_over_prod_stored(self, monkeypatch):
        ...
        state = self._base_state(auth, auth.DEFAULT_openamer_INFERENCE_URL)
        self._patch_no_refresh(monkeypatch, auth, state)
        monkeypatch.setenv("OPENAMER_INFERENCE_BASE_URL", self.STAGING)

        result = auth.resolve_openamer_runtime_credentials()
    ...
    def test_no_refresh_env_override_not_persisted(self, monkeypatch):
        ...
        auth.resolve_openamer_runtime_credentials()

        assert state["inference_base_url"] == auth.DEFAULT_openamer_INFERENCE_URL, (
    ...
    def test_no_refresh_no_env_uses_stored_default(self, monkeypatch):
        ...
        state = self._base_state(auth, auth.DEFAULT_openamer_INFERENCE_URL)
        self._patch_no_refresh(monkeypatch, auth, state)
        monkeypatch.delenv("OPENAMER_INFERENCE_BASE_URL", raising=False)

        result = auth.resolve_openamer_runtime_credentials()
        assert result["base_url"] == auth.DEFAULT_openamer_INFERENCE_URL
    ...
    def test_refresh_env_override_wins_but_persists_validated(self, monkeypatch):
        ...
        result = auth.resolve_openamer_runtime_credentials(force_refresh=True)
...
class TestProxyAdapterEnvOverride:
    """The OpenAmer proxy adapter is the second chokepoint: it re-validates the
    base_url returned by resolve_openamer_runtime_credentials() against the prod
    allowlist. That re-validation must not clobber a legitimate
    OPENAMER_INFERENCE_BASE_URL staging override.
    """
    ...
    def test_proxy_adapter_consults_env_override(self):
        ...
        source = Path(_openamer_adapter.__file__).read_text(encoding="utf-8")
        assert "_openamer_inference_env_override()" in source, (
