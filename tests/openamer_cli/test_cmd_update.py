...
from openamer_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateNpmLockfileCache:
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `openamer update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".openamer"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(openamer_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            openamer_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("OPENAMER_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("OPENAMER_HOME", str(named_profile))
            hm._update_node_dependencies()
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $OPENAMER_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("openamer_cli.managed_uv.resolve_uv", lambda: None)
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_already_up_to_date(
        ...
        with patch("openamer_cli.managed_uv.update_managed_uv") as mock_uv_update, \
             patch(
                 "openamer_cli.managed_uv.ensure_uv",
                 return_value=None,
             ) as mock_uv_ensure:
            cmd_update(mock_args)
    ...
    def test_zero_commit_runtime_repair_requires_process_restart(
        ...
        from openamer_cli.managed_uv import RuntimeRepairResult
        ...
        with patch(
            "openamer_cli.managed_uv.update_managed_uv",
            side_effect=fake_update,
        ), patch(
            "openamer_cli.managed_uv.ensure_uv",
            return_value="/managed/uv",
        ), patch(
            "openamer_cli.main._is_windows",
            return_value=False,
        ):
            cmd_update(mock_args)
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind NousResearch/openamer-agent silently misses updates.
        """
        from openamer_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/openamer-agent.git",
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "openamer_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "openamer_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("openamer_cli.config.check_config_version", return_value=(1, 2)), patch(
            "openamer_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("openamer_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "openamer_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "openamer_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "openamer_cli.config.check_config_version", return_value=(5, 24)
        ), patch(
            "openamer_cli.config.migrate_config",
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "openamer_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "openamer_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "openamer_cli.config.check_config_version", return_value=(1, 24)
        ), patch(
            "openamer_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("openamer_cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.openamer"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.openamer/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.openamer/profiles/work"))
        ...
        with (
            patch("openamer_cli.profiles.list_profiles", return_value=all_profiles),
            patch("openamer_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("openamer_cli.profiles.list_profiles", return_value=[default_p]),
            patch("openamer_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``openamer update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``openamer update --check --branch <name>`` honors the branch override.
...
class TestCmdUpdateZipBranchRefusal:
    """``openamer update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from openamer_cli.main import _update_via_zip
