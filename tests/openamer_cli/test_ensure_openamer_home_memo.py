"""ensure_openamer_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".openamer"
    second = tmp_path / "b" / ".openamer"

    monkeypatch.setenv("OPENAMER_HOME", str(first))
    cfg.ensure_openamer_home()

    # Profile switch: OPENAMER_HOME moves → the new path is ensured too.
    monkeypatch.setenv("OPENAMER_HOME", str(second))
    cfg.ensure_openamer_home()
