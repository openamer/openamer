...
The notice prints the most recent curator run summary on `openamer update`,
...
session start) so users normally never see the rename map. `openamer update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(openamer_main)
