"""Regression tests for #60328: --yolo must set OPENAMER_YOLO_MODE in
...
def _run_main_and_capture_yolo_at_startup(monkeypatch, argv):
    """Run main() with *argv*, capturing OPENAMER_YOLO_MODE at the
    ...
    def spy_prepare_startup(args):
        yolo_at_startup["value"] = os.environ.get("OPENAMER_YOLO_MODE")

    monkeypatch.setattr(
        "openamer_cli.main._prepare_agent_startup", spy_prepare_startup
    )
    # Stub cmd_chat so main() returns cleanly without entering chat.
    monkeypatch.setattr("openamer_cli.main.cmd_chat", lambda args: None)
    monkeypatch.delenv("OPENAMER_YOLO_MODE", raising=False)
    monkeypatch.setattr(sys, "argv", argv)

    from openamer_cli.main import main as cli_main
...
def test_chat_subcommand_yolo_flag_sets_env_before_startup(monkeypatch):
    """openamer chat --yolo must also set OPENAMER_YOLO_MODE before
...
def test_no_yolo_flag_leaves_env_unset_at_startup(monkeypatch):
    """Without --yolo, OPENAMER_YOLO_MODE must not be set at startup."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["openamer"]
    )
    assert result is None, (
        "OPENAMER_YOLO_MODE was unexpectedly set at startup without --yolo."
    )
