...
openamer_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_gateway_config_loader_honors_managed(homes, monkeypatch):
    ...
    # load_gateway_config resolves home via get_openamer_home() (OPENAMER_HOME env).
...
def test_logging_config_honors_managed(homes, monkeypatch):
    ...
    level, _max, _bk = openamer_logging._read_logging_config()
    assert level == "DEBUG"


def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # openamer_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("OPENAMER_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import openamer_time

    assert openamer_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    # writing OPENAMER_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
