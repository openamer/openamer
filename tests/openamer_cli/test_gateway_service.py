...
class TestSystemdServiceRefresh:
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``/tmp/pytest-of-.../hermes_test`` OPENAMER_HOME — silently breaking
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        ``hermes gateway restart`` with a /tmp OPENAMER_HOME exported; the
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="OPENAMER_HOME=/tmp/hermes-e2e-41264"\n'
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="OPENAMER_HOME=/var/tmp/hermes-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="OPENAMER_HOME={tmp_path}/hermes-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None
    ...
    def test_accepts_real_home(self):
        unit = '[Service]\nEnvironment="OPENAMER_HOME=/home/alice/.hermes"\n'
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="OPENAMER_HOME=/tmpfs-data/.hermes"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestSystemUnitHermesHome:
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'OPENAMER_HOME=/home/alice/.hermes' in unit
    ...
    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("OPENAMER_HOME", "/root/.hermes/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'OPENAMER_HOME=/home/alice/.hermes/profiles/coder' in unit
    ...
    def test_system_unit_preserves_custom_hermes_home(self, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("OPENAMER_HOME", "/opt/hermes-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'OPENAMER_HOME=/opt/hermes-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's OPENAMER_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        hermes_home = str(gateway_cli.get_openamer_home().resolve())
        assert f'OPENAMER_HOME={hermes_home}' in unit
...
class TestSystemUnitRefreshSyncsHermesHome:
    ...
    def test_refresh_adopts_unit_hermes_home_before_rewriting(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENAMER_HOME", str(alice_hermes))
        ...
        monkeypatch.setenv("OPENAMER_HOME", str(root_hermes))
        ...
        assert os.environ["OPENAMER_HOME"] == str(alice_hermes)
...
class TestHermesHomeForTargetUser:
    ...
    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("OPENAMER_HOME", "/opt/hermes")
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        assert f'OPENAMER_HOME={target_home / ".hermes" / "profiles" / "mybot"}' in unit
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("OPENAMER_HOME", str(root_home / ".hermes"))
...
class TestLegacyHermesUnitDetection:
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        variants = [
            ...
            "ExecStart=/venv/bin/python /opt/hermes/openamer_cli/main.py gateway run",
