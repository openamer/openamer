...
from openamer_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<openamer_home>/plugins/``; we derive
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <OPENAMER_HOME>/config.yaml.
...
class TestPluginDiscovery:
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENAMER_ENABLE_PROJECT_PLUGINS", "true")
...
class TestPreToolCallDirective:
    ...
    def test_approve_directive_returned(self, monkeypatch):
        from openamer_cli.plugins import get_pre_tool_call_directive
        monkeypatch.setattr(
            "openamer_cli.plugins.invoke_hook",
            lambda hook_name, **kwargs: [
...
class TestResolvePreToolBlock:
    ...
    def test_no_directive_returns_none(self, monkeypatch):
        from openamer_cli.plugins import resolve_pre_tool_block
        monkeypatch.setattr(
            "openamer_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("openamer_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
        assert get_pre_verify_continue_message() is None
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("openamer_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        """
        from tools.registry import registry
        from openamer_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from openamer_cli.plugins import PluginContext, PluginManifest
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("openamer_cli.plugins._plugin_manager", mgr):
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("openamer_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("openamer_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("openamer_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("openamer_cli.plugins.PluginContext.dispatch_tool.__module__", "openamer_cli.plugins"):
...
class TestPluginDebugLogging:
    """OPENAMER_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENAMER_PLUGINS_DEBUG", raising=False)
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With OPENAMER_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("OPENAMER_PLUGINS_DEBUG", "1")
        from openamer_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from OPENAMER_HOME in every context."""
