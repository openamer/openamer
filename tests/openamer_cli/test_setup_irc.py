"""Tests for IRC gateway configuration via `openamer setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh OpenAmer install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new OpenAmer install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#openamer")
            monkeypatch.setenv("IRC_NICKNAME", "openamer-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `openamer setup gateway` experience with IRC present."""
