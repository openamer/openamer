...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.openamer.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.openamer.gateway.plist",
        "launchctl stop ai.openamer.gateway",
        "systemctl restart openamer-gateway",
        "systemctl stop openamer-gateway.service",
        "systemctl start openamer-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill openamer gateway process",
        "pkill -f openamer.*gateway",
        "pkill -f gateway.*openamer",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "openamer cron list",
        "openamer update",
        "openamer config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `openamer gateway start` is benign — starting a gateway from inside a
        ...
        "openamer gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway openamer
        # services must not be falsely blocked (the old `.*openamer` matched any
        # openamer token).
        "launchctl unload ai.openamer.update-checker.plist",
        "launchctl restart ai.openamer.daemon",
        "systemctl restart openamer-meta.service",
        "systemctl restart openamer-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_openamer_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade openamer then run openamer gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.openamer.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_openamer_gateway_once.sh). The script must live under
        # OPENAMER_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("OPENAMER_HOME", str(tmp_path / ".openamer"))
        scripts_dir = tmp_path / ".openamer" / "scripts"
...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart openamer-gateway",
        "systemctl --user restart openamer-gateway",
        "systemctl stop openamer-gateway.service",
        "openamer gateway restart",
        "launchctl kickstart gui/501/ai.openamer.gateway",
        "pkill -f openamer.*gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart openamer-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-openamer systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart openamer-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart openamer-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run openamer gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("openamer gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeopenamer gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under OPENAMER_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.openamer.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    openamer_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run openamer gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OPENAMER_HOME", str(tmp_path / ".openamer"))
        (tmp_path / ".openamer").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run openamer gateway restart nightly",
        ))
