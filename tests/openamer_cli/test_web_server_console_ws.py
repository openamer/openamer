"""Dashboard OpenAmer Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_openamer_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from openamer_cli.console_engine import ConsoleResult, OpenAmerConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.5)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(OpenAmerConsoleEngine, "execute", slow_execute)
