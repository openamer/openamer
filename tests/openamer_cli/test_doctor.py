...
import openamer_cli.doctor as doctor
import openamer_cli.gateway as gateway_cli
from openamer_cli import doctor as doctor_mod
from openamer_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `openamer doctor` crashed on Windows
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("OPENAMER_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_openamer_home(self, tmp_path, provider=""):
        """Create a minimal OPENAMER_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_openamer_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "OPENAMER_HOME", home)
        ...
        try:
            from openamer_cli import auth as _auth_mod
            monkeypatch.setattr(_auth_mod, "get_openamer_auth_status", lambda: {})
...
def test_run_doctor_accepts_openamer_provider_ids_that_catalog_aliases(
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp OPENAMER_HOME.

        ``run_doctor`` reads the module-level ``OPENAMER_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("OPENAMER_HOME")`` alone
        leaves doctor probing the REAL ~/.openamer. On a dev machine with a
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(_auth_mod, "get_openamer_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from openamer_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("openamer_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "openamer_cli.doctor", raising=False)
        from openamer_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the OpenAmer / Codex / Gemini /
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        from openamer_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_openamer_auth_status", lambda: {"logged_in": False})
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """OpenAmer / Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".openamer"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "OpenAmer Portal auth" in out
        assert "logged in" in out
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir(parents=True)
        ...
        monkeypatch.setattr(doctor_mod, "OPENAMER_HOME", openamer_home)
        monkeypatch.setattr(doctor_mod, "get_openamer_home", lambda: openamer_home)
        ...
        return buf.getvalue(), openamer_home
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`openamer doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken OpenAmer install.
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir(parents=True)
        (openamer_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (openamer_home / ".env").write_text(env_body, encoding="utf-8")
...
"""
        out, openamer_home = self._run_doctor_with_config(monkeypatch, tmp_path, config_yaml=cfg)
        ...
        on_disk = (openamer_home / "config.yaml").read_text(encoding="utf-8")
