"""Tests for openamer_cli.auth._update_config_for_provider clearing stale fields.
...
from openamer_cli.config import clear_model_endpoint_credentials, get_config_path
...
class TestUpdateConfigForProviderClearsStaleCustomFields:
    ...
    def test_switching_to_openamer_clears_stale_api_mode(self):
        _seed_custom_provider_config()
        _update_config_for_provider("openamer", "https://inference-api.openamer.com/v1")
        model_cfg = _read_model_cfg()
        assert model_cfg.get("provider") == "openamer"
        assert "api_mode" not in model_cfg
        assert "api_key" not in model_cfg
