...
def container_env(tmp_path, monkeypatch):
    """Set up a fake OPENAMER_HOME with .container-mode file."""
    ...
    container_mode.write_text(
        ...
        "backend=podman\n"
        "container_name=openamer-agent\n"
        "exec_user=openamer\n"
        "openamer_bin=/data/current-package/bin/openamer\n"
    )
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    assert info["container_name"] == "openamer-agent"
    assert info["exec_user"] == "openamer"
    assert info["openamer_bin"] == "/data/current-package/bin/openamer"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("openamer_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        openamer_home = Path(tmpdir) / ".openamer"
        openamer_home.mkdir()
        (openamer_home / ".container-mode").write_text(
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=openamer-custom\n"
        "exec_user=myuser\n"
        "openamer_bin=/opt/openamer/bin/openamer\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "openamer-custom"
    assert info["exec_user"] == "myuser"
    assert info["openamer_bin"] == "/opt/openamer/bin/openamer"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from openamer_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "openamer"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "openamer-agent" in cmd
    assert "/data/current-package/bin/openamer" in cmd
    assert "chat" in cmd
