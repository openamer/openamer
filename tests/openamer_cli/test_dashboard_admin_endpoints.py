...
def _client():
    ...
    # Keep the state DB under the isolated OPENAMER_HOME for any handler that
    # touches it.
...
class TestCredentialPoolEndpoints:
    ...
    def test_delete_does_not_clobber_other_providers(self):
        ...
        from agent.credential_pool import load_pool
        from openamer_cli.auth import _load_auth_store, read_credential_pool
        from openamer_cli.config import save_env_value
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_hermes_home):
        ...
        (get_openamer_home() / "memories").mkdir(parents=True, exist_ok=True)
    ...
    def test_reset_targets(self):
        ...
        mem = get_openamer_home() / "memories"
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "openamer_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestDebugShareEndpoint:
    ...
    def test_returns_structured_urls(self, monkeypatch):
        import openamer_cli.debug as dbg
        ...
        monkeypatch.setattr("openamer_cli.dump.run_dump", lambda a: None)
