"""Tests for openamer_cli.mcp_catalog and openamer_cli.mcp_picker.
...
def _isolate_openamer_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "openamer_cli.config.get_env_path", lambda: hh / ".env"
    )
...
def _entry(name: str):
    ...
    from openamer_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from openamer_cli.mcp_catalog import list_catalog
    ...
    def test_transport_env_parsed_and_written_to_server_config(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from openamer_cli.mcp_catalog import _build_server_config
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from openamer_cli.mcp_catalog import install_entry
    ...
    def test_install_rejects_exfil_shaped_stdio_manifest(self, catalog_dir):
        ...
        from openamer_cli.config import load_config
        from openamer_cli.mcp_catalog import CatalogError, install_entry
    ...
    def test_install_required_env_missing_raises(self, catalog_dir, monkeypatch):
        ...
        from openamer_cli import mcp_catalog
        from openamer_cli.mcp_catalog import install_entry, CatalogError
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from openamer_cli.mcp_catalog import install_entry, uninstall_entry
        from openamer_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from openamer_cli.mcp_catalog import uninstall_entry
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from openamer_cli.mcp_catalog import list_catalog, catalog_diagnostics
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from openamer_cli import mcp_catalog
        from openamer_cli.mcp_catalog import _do_git_install
    ...
    def test_branch_ref_uses_branch_clone(self, catalog_dir, monkeypatch):
        ...
        from openamer_cli import mcp_catalog
        from openamer_cli.mcp_catalog import _do_git_install, get_entry
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "openamer_cli.curses_ui.curses_checklist",
