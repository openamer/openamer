...
2. Spawn: _default_spawn sets the OPENAMER_KANBAN_GOAL_MODE env vars only
...
def test_spawn_sets_goal_env_only_when_enabled(kanban_home, monkeypatch):
    ...
    env = captured["env"]
    assert env.get("OPENAMER_KANBAN_GOAL_MODE") == "1"
    assert env.get("OPENAMER_KANBAN_GOAL_MAX_TURNS") == "5"
...
def test_spawn_no_goal_env_for_plain_task(kanban_home, monkeypatch):
    ...
    kb._default_spawn(task, str(kanban_home))
    env = captured["env"]
    assert "OPENAMER_KANBAN_GOAL_MODE" not in env
    assert "OPENAMER_KANBAN_GOAL_MAX_TURNS" not in env
...
# ---------------------------------------------------------------------------
# CLI judge gate tests (openamer kanban complete bypass fix)
# ---------------------------------------------------------------------------

class TestCLIJudgeGate:
    """openamer kanban complete must apply the same goal_mode judge gate as the
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from openamer_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("openamer_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("openamer_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("openamer_cli.kanban.kb.connect_closing", fake_connect_closing)
        monkeypatch.setattr("openamer_cli.kanban._worker_run_id_for", lambda _: None)
