...
def openamer_home(tmp_path, monkeypatch):
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO openamer_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from openamer_cli.debug import _upload_paste_rs
        ...
        with patch("openamer_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "openamer_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from openamer_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from openamer_cli.debug import upload_to_pastebin

        with patch("openamer_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("openamer_cli.debug._upload_paste_rs",
             ...
             patch("openamer_cli.debug._upload_dpaste_com",
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of OPENAMER_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    def test_force_true_works_when_redaction_disabled(
        ...
        input unchanged when OPENAMER_REDACT_SECRETS=false, and the share-time
        ...
        assert os.environ.get("OPENAMER_REDACT_SECRETS", "") == "false"
...
class TestRunDebugShare:
    ...
    def test_share_survives_sweep_failure(self, openamer_home, capsys):
        ...
        with patch("openamer_cli.dump.run_dump"), \
             patch(
                 "openamer_cli.debug._sweep_expired_pastes",
    ...
    def test_share_keeps_report_and_full_log_on_same_snapshot(self, openamer_home, capsys):
        ...
        with patch("openamer_cli.dump.run_dump"), \
             patch("openamer_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("openamer_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        ...
        with patch("openamer_cli.dump.run_dump"), \
             patch("openamer_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("openamer_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from openamer_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from openamer_cli.debug import delete_paste
        ...
        with patch("openamer_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    def test_does_not_spawn_subprocess(self, openamer_home):
        ...
        import ast
        import inspect
        from openamer_cli.debug import _schedule_auto_delete
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, openamer_home):
        ...
        with patch("openamer_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, openamer_home):
        ...
        with patch("openamer_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, openamer_home):
        ...
        with patch(
            "openamer_cli.debug.delete_paste",
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, openamer_home):
        ...
        with patch("openamer_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from openamer_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("openamer_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("openamer_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, openamer_home, capsys):
        ...
        with patch("openamer_cli.dump.run_dump"), \
             ...
             patch("openamer_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, openamer_home, capsys):
        ...
        with patch("openamer_cli.dump.run_dump"), \
             patch("openamer_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("openamer_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
...
class TestBuildDebugShare:
    ...
    def test_returns_structured_urls(self, openamer_home):
        ...
        with patch("openamer_cli.dump.run_dump"), patch(
            "openamer_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("openamer_cli.debug._schedule_auto_delete"):
...
class TestRunDebugShareNous:
    ...
    def test_openamer_success_prints_view_url(self, openamer_home, capsys):
        ...
        with patch("openamer_cli.dump.run_dump"), patch(
            "openamer_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_openamer_failure_suggests_local(self, openamer_home, capsys):
        ...
        with patch("openamer_cli.dump.run_dump"), patch(
            "openamer_cli.diagnostics_upload.share_to_nous",
    ...
    def test_openamer_does_not_touch_pastebin(self, openamer_home):
        ...
        with patch("openamer_cli.dump.run_dump"), patch(
            "openamer_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("openamer_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    def _captured(self, cmd_original):
        ...
        with patch("openamer_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    ...
    def test_aborts_on_user_decline(self, openamer_home, capsys, monkeypatch):
        ...
        with patch("openamer_cli.dump.run_dump"), \
             patch("openamer_cli.debug.upload_to_pastebin") as mock_upload:
            run_debug_share(self._args())
