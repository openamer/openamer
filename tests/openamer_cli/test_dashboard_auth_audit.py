...
Profile-aware location: ``$OPENAMER_HOME/logs/dashboard-auth.log``.
...
from openamer_cli.dashboard_auth.audit import audit_log, AuditEvent


@pytest.fixture
def profile_home(tmp_path, monkeypatch):
    """Redirect $OPENAMER_HOME and ~ to a tmp dir for the duration of the test."""
...
def test_audit_write_failure_does_not_raise(monkeypatch, tmp_path):
    ...
    # Point OPENAMER_HOME at a file (not a dir) so mkdir/open will fail.
