...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ...
        ("openamer_cli.setup.save_config", {}),
        ("openamer_cli.setup.get_env_value", {"return_value": None}),
        ("openamer_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("openamer_cli.setup._print_setup_summary", {}),
        ("openamer_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("openamer_cli.setup.load_config", {"return_value": {}}),
        ("openamer_cli.setup.save_config", {}),
        ("openamer_cli.auth.get_active_provider", {"return_value": None}),
        ("openamer_cli.setup.get_env_value", {"return_value": None}),
        ("openamer_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="openamer_cli.setup.prompt_choice",
                quick="openamer_cli.setup._run_quick_setup",
                model="openamer_cli.setup.setup_model_provider",
                terminal="openamer_cli.setup.setup_terminal_backend",
                agent="openamer_cli.setup.setup_agent_settings",
                gateway="openamer_cli.setup.setup_gateway",
                tools="openamer_cli.setup.setup_tools",
            )
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("openamer_cli.setup.prompt_choice", {"return_value": 0}),
                first="openamer_cli.setup._run_first_time_quick_setup",
            )
            from openamer_cli.setup import run_setup_wizard
            run_setup_wizard(args)
