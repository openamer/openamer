...
from openamer_cli import gateway as gateway_cli
...
def test_positive_watchdog_config_generates_notify_system_unit(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda _user: ("openamer", "openamer", str(tmp_path)),
    )

    unit = gateway_cli.generate_systemd_unit(system=True, run_as_user="openamer")
...
def test_system_unit_reads_watchdog_from_target_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENAMER_HOME", str(caller_home))
    monkeypatch.setattr(
        gateway_cli,
        "_system_service_identity",
    ...
    monkeypatch.setattr(
        gateway_cli,
        "_openamer_home_for_target_user",
