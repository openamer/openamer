...
def test_project_env_overrides_stale_shell_values_when_user_env_missing(tmp_path, monkeypatch):
    ...
    loaded = load_openamer_dotenv(openamer_home=home, project_env=project_env)
...
def test_utf32_le_bom_leaves_file_untouched(tmp_path, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="openamer_cli.env_loader"):
...
def test_utf32_warning_fires_once_per_path(tmp_path, caplog, monkeypatch):
    ...
    import openamer_cli.env_loader as env_loader
    from openamer_cli.env_loader import _sanitize_env_file_if_needed
