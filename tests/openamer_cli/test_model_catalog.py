"""Tests for openamer_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate OPENAMER_HOME + reset any module-level catalog cache per test."""
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from openamer_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://github.com/openamer/openamer/blob/main/website/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/openamer/openamer"
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import openamer_cli.model_catalog as model_catalog
        from openamer_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestIntegrationWithModelsModule:
    ...
    def test_picker_openamer_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        # ``OPENAMER_HOME / auth.json`` then resolves to the same path the
        ...
        try:
            ...
            active_home = Path(os.environ["OPENAMER_HOME"])
