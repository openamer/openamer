"""Tests for openamer_cli configuration management."""
...
class TestEnsureHermesHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from openamer_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from openamer_cli.default_soul import _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from openamer_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"OPENAMER_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="openamer_cli.config"):
                config = load_config()
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from openamer_cli.config import _deep_merge
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"OPENAMER_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"OPENAMER_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"OPENAMER_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"OPENAMER_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"OPENAMER_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("openamer_cli.config.sanitize_env_file", return_value=2),
            patch(
                "openamer_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("openamer_cli.config.read_raw_config", return_value={}),
            patch("openamer_cli.config.get_missing_env_vars", return_value=[]),
            patch("openamer_cli.config.get_missing_config_fields", return_value=[]),
            patch("openamer_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from openamer_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from openamer_cli.config import _persist_migration, read_raw_config
