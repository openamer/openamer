...
from openamer_cli.copilot_auth import _try_gh_cli_token
...
class TestProviderRegistry:
    ...
    def test_oauth_providers_unchanged(self):
        ...
        assert "openamer" in PROVIDER_REGISTRY
        assert PROVIDER_REGISTRY["openamer"].auth_type == "oauth_device_code"
...
from openamer_cli.auth import PROVIDER_REGISTRY as _REGISTRY

_EXTRA_ENV_VARS = (
    # Checked directly in resolve_provider("auto"), not via the registry.
    "OPENROUTER_API_KEY", "OPENAMER_API_KEY",
    ...
    "OPENAMER_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "OPENAMER_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.auth._load_auth_store", lambda: {})
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("openamer_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_copilot_acp_status_detects_local_cli(self, monkeypatch):
        monkeypatch.setenv("OPENAMER_COPILOT_ACP_ARGS", "--acp --stdio --debug")
        monkeypatch.setattr("openamer_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("openamer_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
    ...
    def test_non_api_key_provider(self):
        status = get_api_key_provider_status("openamer")
        assert status["configured"] is False
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_zai_with_key(self, monkeypatch):
        ...
        monkeypatch.setattr("openamer_cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
    ...
    def test_resolve_copilot_with_gh_cli_fallback(self, monkeypatch):
        monkeypatch.setattr("openamer_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        creds = resolve_api_key_provider_credentials("copilot")
        assert creds["provider"] == "copilot"
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("openamer_cli.copilot_auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "openamer_cli.copilot_auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "openamer_cli.copilot_auth.os.access",
        ...
        monkeypatch.setattr("openamer_cli.copilot_auth.subprocess.run", _fake_run)
    ...
    def test_resolve_copilot_acp_with_local_cli(self, monkeypatch):
        monkeypatch.setenv("OPENAMER_COPILOT_ACP_ARGS", "--acp --stdio")
        monkeypatch.setattr("openamer_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
    ...
    def test_resolve_invalid_provider_raises(self):
        with pytest.raises(AuthError):
            resolve_api_key_provider_credentials("openamer")
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("openamer_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "openamer_cli.runtime_provider._get_model_config",
            lambda: {"provider": "copilot", "default": "gpt-5.4"},
        )
        monkeypatch.setattr(
            "openamer_cli.models.fetch_github_model_catalog",
...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        from openamer_cli import config as config_module
        monkeypatch.setenv("GLM_API_KEY", "test-key")
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: openamer_home / ".env")
        monkeypatch.setattr(config_module, "get_openamer_home", lambda: openamer_home)
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when OpenAmer is unconfigured."""
        from openamer_cli import config as config_module
        ...
        monkeypatch.setattr(config_module, "get_env_path", lambda: openamer_home / ".env")
        monkeypatch.setattr(config_module, "get_openamer_home", lambda: openamer_home)
        monkeypatch.setattr("openamer_cli.copilot_auth.resolve_copilot_token", lambda: ("", ""))
        ...
        monkeypatch.setattr("openamer_cli.auth.get_auth_status", lambda _pid: {})
    ...
    def test_claude_code_creds_counted_when_openamer_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when OpenAmer has been explicitly configured."""
        import yaml
        from openamer_cli import config as config_module
        openamer_home = tmp_path / ".openamer"
        openamer_home.mkdir()
...
class TestZaiEndpointAutoDetect:
    ...
    def test_probe_success_returns_detected_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "openamer_cli.auth.detect_zai_endpoint",
            lambda *a, **kw: {
                "id": "coding-global",
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("openamer_cli.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from openamer_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from openamer_cli.main import _PROVIDER_MODELS as main_models
        from openamer_cli.models import _PROVIDER_MODELS as models_models
...
class TestNovitaProvider:
    ...
    def test_novita_pricing_cache(self, monkeypatch):
        ...
        from openamer_cli import models as models_mod
...
class TestFetchDeepInfraModels:
    ...
    def test_returns_filtered_models_on_success(self, monkeypatch):
        ...
        import openamer_cli.models as models
        monkeypatch.setattr(
            models, "_urlopen_model_catalog_request", lambda *a, **kw: _Resp()
        )
        from openamer_cli.models import _fetch_deepinfra_models
...
class TestDeepInfraTagFiltering:
    ...
    def test_filters_by_surface_tag_and_handles_rollout_states(self, monkeypatch):
        ...
        from openamer_cli.models import _fetch_deepinfra_models_by_tag
        import openamer_cli.models as _m
    ...
    def test_returns_none_on_network_failure(self, monkeypatch):
        ...
        from openamer_cli.models import _fetch_deepinfra_models_by_tag, _fetch_deepinfra_pricing
...
class TestDeepInfraProviderProfile:
    ...
    def test_profile_registered_with_alias_and_aux(self):
        ...
        from openamer_cli.auth import PROVIDER_REGISTRY, resolve_provider
