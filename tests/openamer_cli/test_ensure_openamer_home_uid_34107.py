"""Regression tests for #34107 — Docker UID/GID handling in ensure_openamer_home.

When OpenAmer runs in Docker with ``OPENAMER_UID=1000`` / ``OPENAMER_GID=911``,
the entrypoint chowns the top-level ``OPENAMER_HOME`` once at startup. But
...
The fix is a ``_chown_to_openamer_uid`` helper that reads the env vars and
...
class TestResolveOpenAmerUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        ...
        from openamer_cli.config import _resolve_openamer_uid_gid
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("OPENAMER_UID", "")
        monkeypatch.setenv("OPENAMER_GID", "")
        from openamer_cli.config import _resolve_openamer_uid_gid
        uid, gid = _resolve_openamer_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToOpenAmerUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("OPENAMER_UID", "1000")
        monkeypatch.setenv("OPENAMER_GID", "911")
        from openamer_cli import config as cfg
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_openamer_uid(d)  # must not raise
