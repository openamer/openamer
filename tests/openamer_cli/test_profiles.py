...
def profile_env(tmp_path, monkeypatch):
    ...
    * OPENAMER_HOME  -> tmp_path/.hermes  (so get_openamer_home() agrees)
...
class TestGetActiveProfileName:
    ...
    def test_default_hermes_home_returns_default(self, profile_env):
        # OPENAMER_HOME points to tmp_path/.hermes which is the default
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        monkeypatch.setattr("openamer_cli.profiles.shutil.which", lambda name: "/opt/hermes/bin/hermes")
...
class TestInternalHelpers:
    ...
    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (OPENAMER_HOME outside ~/.hermes), profiles go under OPENAMER_HOME."""
    ...
    def test_default_hermes_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_hermes_home() returns OPENAMER_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OPENAMER_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (OPENAMER_HOME under ~/.hermes), profiles root is still ~/.hermes/profiles."""
