"""Tests for openamer_cli.managed_uv — one path, no guessing."""
...
def _runtime_info(
    ...
    from openamer_cli.sqlite_runtime import SQLiteRuntimeInfo
...
class TestEnsureUv:
    ...
    def test_install_reports_runtime_repair_to_observer(self, tmp_path):
        ...
        with patch(
            ...
            "openamer_cli.managed_uv.repair_vulnerable_runtime",
            return_value=repair,
        ):
...
class TestManagedPythonStore:
    def test_store_is_checkout_scoped_across_profiles(self, tmp_path, monkeypatch):
        from openamer_cli.managed_uv import managed_python_install_dir

        checkout = tmp_path / "checkout"
        monkeypatch.setenv("OPENAMER_HOME", str(tmp_path / "profiles" / "alpha"))
    ...
    def test_environment_is_private_and_sanitized(self, tmp_path):
        from openamer_cli.managed_uv import managed_python_env
...
class TestRuntimeRepair:
    def test_safe_runtime_is_a_noop(self, tmp_path):
        from openamer_cli.managed_uv import repair_vulnerable_runtime

        root, live, sentinel = _make_runtime_install(tmp_path)
        current = _runtime_info(live / "bin" / "python", (3, 53, 1))
        with patch("openamer_cli.managed_uv.platform.system", return_value="Linux"), \
             patch(
                 "openamer_cli.managed_uv.probe_sqlite_runtime",
                 return_value=current,
             ), \
             patch(
                 "openamer_cli.managed_uv._install_safe_python_generation"
             ) as mock_install:
    ...
    def test_failed_candidate_preserves_live_venv(self, tmp_path):
        ...
        with patch("openamer_cli.managed_uv.platform.system", return_value="Linux"), \
             ...
             patch(
                 "openamer_cli.managed_uv._stage_candidate_venv",
                 return_value=None,
             ):
            result = repair_vulnerable_runtime("uv", project_root=root)
    ...
    def test_windows_holders_refuse_runtime_mutation(self, tmp_path, monkeypatch):
        ...
        with patch("openamer_cli.managed_uv.platform.system", return_value="Windows"), \
             patch(
                 "openamer_cli.managed_uv.probe_sqlite_runtime",
                 return_value=current,
             ), \
...
class TestRuntimeCutover:
    ...
    def test_failed_smoke_with_empty_output_has_stable_detail(self, tmp_path):
        from openamer_cli.managed_uv import _smoke_candidate_venv
        ...
        with patch(
            "openamer_cli.managed_uv.probe_sqlite_runtime",
            return_value=fixed,
        ), patch("openamer_cli.managed_uv.subprocess.run", return_value=failed):
    ...
    def test_successfully_renames_candidate_into_live_path(self, tmp_path):
        ...
        with patch(
            "openamer_cli.managed_uv._smoke_candidate_venv",
    ...
    def test_interrupt_during_promotion_restores_live_venv(self, tmp_path):
        ...
        with patch(
            "openamer_cli.managed_uv._rename_with_retry",
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("openamer_cli.managed_uv._install_uv_posix") as mock_posix:
            from openamer_cli.managed_uv import _install_uv
            _install_uv(target)
    ...
    def test_windows_sets_uv_install_dir(self, tmp_path):
        ...
        with patch("openamer_cli.managed_uv.platform.system", return_value="Windows"), \
             patch("openamer_cli.managed_uv._install_uv_windows") as mock_windows:
            from openamer_cli.managed_uv import _install_uv
            _install_uv(target)
...
class TestRuntimeRequestMinorLine:
    ...
    def test_requests_minor_line(self):
        from openamer_cli.managed_uv import _runtime_request
    ...
    def _run_generation(tmp_path, monkeypatch, current_version, candidate_version):
        ...
        import openamer_cli.managed_uv as managed_uv
        from openamer_cli.sqlite_runtime import SQLiteRuntimeInfo
