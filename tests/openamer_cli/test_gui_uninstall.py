"""Tests for openamer_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    OPENAMER_HOME with all the destructive externals stubbed out.
    """
...
def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m openamer_cli.uninstall --mode gui` runs the GUI-only path.
