...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("openamer_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("openamer_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)
