...
Every aux picker (``openamer model`` → Configure auxiliary models, the
``openamer tools`` vision picker, and any future one) must route through
``openamer_cli.inventory.build_aux_picker_rows()`` so it shows the same
...
def configured_home(tmp_path, monkeypatch):
    """A OPENAMER_HOME with one ``providers:`` entry and one legacy
...
def test_aux_picker_surfaces_user_defined_providers(configured_home):
    ...
    """
    from openamer_cli.inventory import build_aux_picker_rows
...
def test_aux_picker_requests_exhausted_pool_visibility(configured_home):
    ...
    with patch("openamer_cli.model_switch.list_authenticated_providers", _capture):
...
def test_format_entries_marks_current_provider():
    from openamer_cli.inventory import format_aux_picker_entries
...
def test_aux_pickers_route_through_the_shared_substrate(configured_home):
    ...
    with (
        patch("openamer_cli.model_switch.list_authenticated_providers", _direct),
        patch("openamer_cli.inventory.build_aux_picker_rows", _substrate),
        patch("openamer_cli.main._prompt_provider_choice", return_value=None),
    ):
    ...
    assert direct_calls == [], (
        ...
        "route through openamer_cli.inventory.build_aux_picker_rows so custom "
