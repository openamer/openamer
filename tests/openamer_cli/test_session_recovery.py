...
from openamer_state import FTS_STORAGE_VERSION, SCHEMA_VERSION, SessionDB
from openamer_cli import session_recovery
...
def test_snapshot_blocks_connections_opened_during_the_copy(
    ...
    from openamer_cli import session_recovery as recovery_module
...
def test_recovery_refuses_to_snapshot_a_live_database(tmp_path: Path) -> None:
    ...
    openamer_cli.sqlite_safe_read). Recovery normally runs against an offline
    file, but the check must exist so this path cannot drift away from
    openamer_state._backup_db_file, which refuses the same situation.
    """
    from openamer_cli.sqlite_safe_read import connect_tracked
