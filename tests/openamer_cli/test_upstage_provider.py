...
Regression guard for the bug where `openamer model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`openamer_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageResolver:
    ...
    def test_solar_alias_normalizes_to_upstage(self):
        from openamer_cli.providers import normalize_provider, resolve_provider_full
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in OPENAMER_OVERLAYS
        overlay = OPENAMER_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `openamer_cli/main.py` (cmd_model): read
