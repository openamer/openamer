"""Tests for the openamer_cli models module."""
...
from openamer_cli.models import (
    ...
    is_openamer_free_tier, partition_openamer_models_by_tier,
    check_openamer_free_tier, _FREE_TIER_CACHE_TTL,
    union_with_portal_free_recommendations,
    union_with_portal_paid_recommendations,
)
import openamer_cli.models as _models_mod
...
class TestModelIds:
    def test_returns_non_empty_list(self):
        with patch("openamer_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            ids = model_ids()
...
class TestFetchOpenRouterModels:
    def test_live_fetch_recomputes_free_tags(self, monkeypatch):
        ...
        with patch("openamer_cli.models._urlopen_model_catalog_request", return_value=_Resp()):
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        ...
        with patch("openamer_cli.model_catalog.get_curated_openrouter_models", return_value=None), \
             patch("openamer_cli.models._urlopen_model_catalog_request", side_effect=OSError("boom")):
            models = fetch_openrouter_models(force_refresh=True)
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        openamer-agent is tool-calling-first — surfacing a non-tool model leads to
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with (
            patch("openamer_cli.model_catalog.get_curated_openrouter_models", return_value=[]),
            patch("openamer_cli.models._urlopen_model_catalog_request", return_value=_Resp()),
        ):
            models = fetch_openrouter_models(force_refresh=True)
    ...
    def test_permissive_when_supported_parameters_missing(self, monkeypatch):
        ...
        Some OpenRouter-compatible gateways (OpenAmer Portal, private mirrors, older
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from openamer_cli.models import _openrouter_model_supports_tools
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from openamer_cli.models import _find_openrouter_slug
        with patch("openamer_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
...
class TestDetectProviderForModel:
    ...
    def test_aggregator_not_suggested(self):
        """openamer/openrouter should never be auto-suggested as target provider."""
        with patch("openamer_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            result = detect_provider_for_model("claude-opus-4-6", "openai-codex")
        assert result is not None
        assert result[0] not in {"openamer",}  # openamer has claude models but shouldn't be suggested
...
class TestIsNousFreeTier:
    """Tests for is_openamer_free_tier — account tier detection."""

    def test_paid_service_access_allowed_true_is_not_free(self):
        assert is_openamer_free_tier({"paid_service_access": {"allowed": True}}) is False

    def test_paid_service_access_allowed_false_is_free(self):
        assert is_openamer_free_tier({"paid_service_access": {"allowed": False}}) is True

    def test_paid_service_access_paid_access_fallback(self):
        assert is_openamer_free_tier({"paid_service_access": {"paid_access": False}}) is True

    def test_paid_plus_tier(self):
        assert is_openamer_free_tier({"subscription": {"plan": "Plus", "tier": 2, "monthly_charge": 20}}) is False

    def test_free_tier_by_charge(self):
        assert is_openamer_free_tier({"subscription": {"plan": "Free", "tier": 0, "monthly_charge": 0}}) is True

    def test_no_charge_field_not_free(self):
        """Missing monthly_charge defaults to not-free (don't block users)."""
        assert is_openamer_free_tier({"subscription": {"plan": "Free", "tier": 0}}) is False

    def test_plan_name_alone_not_free(self):
        """Plan name alone is not enough — monthly_charge is required."""
        assert is_openamer_free_tier({"subscription": {"plan": "free"}}) is False

    def test_empty_subscription_not_free(self):
        """Empty subscription dict defaults to not-free (don't block users)."""
        assert is_openamer_free_tier({"subscription": {}}) is False

    def test_no_subscription_not_free(self):
        """Missing subscription key returns False."""
        assert is_openamer_free_tier({}) is False
...
class TestPartitionNousModelsByTier:
    """Tests for partition_openamer_models_by_tier — free vs paid tier model split."""
    ...
    def test_paid_tier_all_selectable(self):
        ...
        sel, unav = partition_openamer_models_by_tier(models, pricing, free_tier=False)
        assert sel == models
        assert unav == []
    ...
    def test_free_tier_splits_correctly(self):
        ...
        sel, unav = partition_openamer_models_by_tier(models, pricing, free_tier=True)
    ...
    def test_no_pricing_returns_all(self):
        ...
        sel, unav = partition_openamer_models_by_tier(models, {}, free_tier=True)
        assert sel == models
        assert unav == []
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "openamer_cli.models.fetch_openamer_recommended_models",
    ...
    def test_then_partition_keeps_portal_free_model(self):
        ...
        with patch(
            ...
            ids, p = union_with_portal_free_recommendations(curated, pricing, "")
        sel, unav = partition_openamer_models_by_tier(ids, p, free_tier=True)
    ...
    def test_empty_payload_returns_inputs_unchanged(self):
        ...
        with patch("openamer_cli.models.fetch_openamer_recommended_models", return_value={}):
            ids, p = union_with_portal_free_recommendations(curated, pricing, "")
        assert ids == curated
        assert p == pricing
...
class TestUnionWithPortalPaidRecommendations:
    ...
    def test_does_not_synthesize_pricing_for_paid_models(self):
        ...
        :func:`partition_openamer_models_by_tier` into treating them as free;
...
class TestCheckNousFreeTierCache:
    """Tests for the TTL cache on check_openamer_free_tier()."""
    ...
    @patch("openamer_cli.openamer_account.get_openamer_portal_account_info")
    def test_result_is_cached(self, mock_account):
        """Second call within TTL returns cached result without account lookup."""
        mock_account.return_value = OpenAmerPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
            paid_service_access=False,
        )
        result1 = check_openamer_free_tier()
        result2 = check_openamer_free_tier()
    ...
    def test_force_fresh_bypasses_cache(self, mock_account):
        ...
        assert check_openamer_free_tier() is False
        assert check_openamer_free_tier(force_fresh=True) is False
...
class TestNousRecommendedModels:
    """Tests for fetch_openamer_recommended_models + get_openamer_recommended_aux_model."""
    ...
    def setup_method(self):
        _models_mod._openamer_recommended_cache.clear()
    ...
    def test_fetch_caches_per_portal_url(self):
        from openamer_cli.models import fetch_openamer_recommended_models
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("openamer_cli.models._urlopen_model_catalog_request", return_value=mock_cm) as mock_urlopen:
            a = fetch_openamer_recommended_models("https://portal.example.com")
            b = fetch_openamer_recommended_models("https://portal.example.com")
    ...
    def test_fetch_cache_is_keyed_per_portal(self):
        ...
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("openamer_cli.models._urlopen_model_catalog_request", return_value=mock_cm) as mock_urlopen:
            fetch_openamer_recommended_models("https://portal.example.com")
            fetch_openamer_recommended_models("https://portal.staging-openamer.com")
        assert mock_urlopen.call_count == 2  # different portals → separate fetches

    def test_fetch_returns_empty_on_network_failure(self):
        from openamer_cli.models import fetch_openamer_recommended_models
        with patch("openamer_cli.models._urlopen_model_catalog_request", side_effect=OSError("boom")):
            result = fetch_openamer_recommended_models("https://portal.example.com")
        assert result == {}
    ...
    def test_fetch_force_refresh_bypasses_cache(self):
        ...
        with patch("openamer_cli.models._urlopen_model_catalog_request", return_value=mock_cm) as mock_urlopen:
            fetch_openamer_recommended_models("https://portal.example.com")
            fetch_openamer_recommended_models("https://portal.example.com", force_refresh=True)
    ...
    def test_get_aux_model_returns_vision_recommendation(self):
        ...
        with patch(
            ...
            model = get_openamer_recommended_aux_model(vision=True, free_tier=True)
    ...
    def test_get_aux_model_returns_compaction_recommendation(self):
        ...
        with patch(
            ...
            model = get_openamer_recommended_aux_model(vision=False, free_tier=True)
    ...
    def test_get_aux_model_returns_none_on_empty_payload(self):
        from openamer_cli.models import get_openamer_recommended_aux_model
        with patch("openamer_cli.models.fetch_openamer_recommended_models", return_value={}):
            assert get_openamer_recommended_aux_model(vision=False, free_tier=True) is None
            assert get_openamer_recommended_aux_model(vision=True, free_tier=False) is None
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("openamer_cli.models.fetch_openamer_recommended_models", return_value=payload):
            text = get_openamer_recommended_aux_model(vision=False, free_tier=False)
            vision = get_openamer_recommended_aux_model(vision=True, free_tier=False)
    ...
    def test_auto_detects_tier_when_not_supplied(self):
        """Default behaviour: call check_openamer_free_tier() to pick the tier."""
        from openamer_cli.models import get_openamer_recommended_aux_model
        payload = {
        ...
        with (
            patch("openamer_cli.models.fetch_openamer_recommended_models", return_value=payload),
            patch("openamer_cli.models.check_openamer_free_tier", return_value=True),
        ):
            assert get_openamer_recommended_aux_model(vision=False) == "free-model"
        with (
            patch("openamer_cli.models.fetch_openamer_recommended_models", return_value=payload),
            patch("openamer_cli.models.check_openamer_free_tier", return_value=False),
        ):
            assert get_openamer_recommended_aux_model(vision=False) == "paid-model"
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("openamer_cli.models.fetch_openamer_recommended_models", return_value=payload),
            patch("openamer_cli.models.check_openamer_free_tier", side_effect=RuntimeError("boom")),
        ):
            assert get_openamer_recommended_aux_model(vision=False) == "paid-model"
...
class TestCodexSoftAcceptPlausibilityGate:
    ...
    def test_unrelated_name_rejected_on_openai_codex(self):
        from openamer_cli.models import validate_requested_model
...
class TestClaudeSonnet5InCuratedLists:
    ...
    def test_openamer_list_includes_sonnet_5(self):
        from openamer_cli.models import _PROVIDER_MODELS
        assert "anthropic/claude-sonnet-5" in _PROVIDER_MODELS["openamer"]
