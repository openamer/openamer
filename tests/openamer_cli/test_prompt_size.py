"""Tests for the ``openamer prompt-size`` diagnostic (issue #34667)."""
...
from openamer_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(openamer_home, memory_text="", user_text=""):
    mem_dir = openamer_home / "memories"
...
def _seed_skill(openamer_home, name, description):
    skill_dir = openamer_home / "skills" / "demo" / name
...
def test_runs_offline_without_credentials(isolated_home, monkeypatch):
    ...
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "OPENAMER_API_KEY",
                "ANTHROPIC_API_KEY"):
