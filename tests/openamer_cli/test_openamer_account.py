"""Tests for normalized OpenAmer Portal account entitlement helpers."""
...
from openamer_cli.openamer_account import (
    ...
    reset_openamer_portal_account_info_cache,
)
...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    token = _jwt(
        {
            ...
            "org_id": "org_123",
            "client_id": "openamer-cli",
            "product_id": "openamer-openamer-agent",
            "openamer_client": "openamer-agent",
            "exp": int(time.time()) + 900,
            "paid_access": True,
            "subscription_tier": 2,
        }
    )
    monkeypatch.setattr("openamer_cli.auth.get_provider_auth_state", lambda provider: _state(token))

    info = get_openamer_portal_account_info()
    ...
    assert info.product_id == "openamer-openamer-agent"
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("openamer_cli.auth.resolve_openamer_access_token", lambda: "fresh-token")
    monkeypatch.setattr("openamer_cli.openamer_account._fetch_openamer_account_info", lambda *a, **kw: payload)
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    assert isinstance(info, OpenAmerPortalAccountInfo)
    assert info.source == "account_api"
    assert info.fresh is True
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("openamer_cli.auth.get_provider_auth_state", lambda provider: {})

    class _Entry:
        label = "manual-openamer"
        access_token = ""
    ...
    assert info.credential_source == "pool:manual-openamer"
    assert info.paid_service_access is None
...
def test_entitlement_message_returns_none_for_paid_access():
    info = OpenAmerPortalAccountInfo(
        logged_in=True,
        source="account_api",
        fresh=True,
    ...
    assert format_openamer_portal_entitlement_message(info, capability="paid models") is None
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "OpenAmer inference credentials are configured" in message
    assert "cannot verify your OpenAmer Portal paid access" in message
...
def test_entitlement_message_for_no_subscription_or_credits():
    ...
    message = format_openamer_portal_entitlement_message(info, capability="paid models")
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    message = format_openamer_portal_entitlement_message(info, capability="Tool Gateway")
...
def test_entitlement_message_for_account_missing():
    ...
    assert message is not None
    assert "could not find a OpenAmer Portal account or organisation" in message
...
def test_topup_url_is_org_pinned_when_slug_present():
    ...
    assert (
        openamer_portal_topup_url(info)
...
def test_topup_url_falls_back_to_legacy_when_slug_null():
    ...
    url = openamer_portal_topup_url(info)
...
def test_topup_url_defaults_to_production_portal_for_none():
    url = openamer_portal_topup_url(None)
    assert url == "https://portal.openamer.com/billing?topup=open"
