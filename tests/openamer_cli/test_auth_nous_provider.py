...
from openamer_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
def test_get_nous_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from openamer_cli.auth import get_nous_auth_status, invalidate_nous_auth_status_cache
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    openamer_home = tmp_path / "openamer"
    _setup_nous_auth(openamer_home, access_token="at-123")
    monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    openamer_home = tmp_path / "openamer"
    _setup_nous_auth(openamer_home, access_token="at-fresh")
    monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    openamer_home = tmp_path / "openamer"
    _setup_nous_auth(openamer_home, refresh_token="refresh-old")
    monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))
    from agent.credential_pool import load_pool
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `openamer model` → Nous Portal → Skip (keep current) after
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `openamer auth add nous`
    ...
    """
    from openamer_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "OpenAmer is not logged into Nous Portal").
    ...
    # doesn't raise "OpenAmer is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `openamer auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    assert "openamer auth add nous" in message.lower()
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    ...
    """
    from openamer_cli.auth import _nous_shared_store_path
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    ...
    from openamer_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from openamer_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from openamer_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from openamer_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `openamer auth add nous
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from openamer_cli.auth import _try_import_shared_nous_state
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from openamer_cli.auth import _load_auth_store, DEFAULT_NOUS_PORTAL_URL
