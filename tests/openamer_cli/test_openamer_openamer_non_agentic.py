"""Tests for the OpenAmer-OpenAmer-3/4 non-agentic warning detector.
...
``is_openamer_openamer_non_agentic`` should only match the actual OpenAmer
...
from openamer_cli.model_switch import (
    ...
    is_openamer_openamer_non_agentic,
)
...
@pytest.mark.parametrize(
    "model_name",
    [
        "openamer/OpenAmer-3-Llama-3.1-70B",
        "openamer/OpenAmer-3-Llama-3.1-405B",
        "openamer-3",
        "OpenAmer-3",
        "openamer-4",
        "openamer-4-405b",
        ...
        "openrouter/openamer/openamer-4-405b",
        "openamer/OpenAmer3",
        "openamer-3.1",
    ],
)
def test_matches_real_openamer_openamer_chat_models(model_name: str) -> None:
    assert is_openamer_openamer_non_agentic(model_name), (
        f"expected {model_name!r} to be flagged as OpenAmer OpenAmer 3/4"
    )
...
def test_does_not_match_unrelated_models(model_name: str) -> None:
    assert not is_openamer_openamer_non_agentic(model_name), (
        f"expected {model_name!r} NOT to be flagged as OpenAmer OpenAmer 3/4"
    )
    assert _check_openamer_model_warning(model_name) == ""


def test_none_like_inputs_are_safe() -> None:
    assert is_openamer_openamer_non_agentic("") is False
