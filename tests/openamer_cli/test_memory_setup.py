...
import openamer_cli.memory_setup as memory_setup
from openamer_cli.memory_setup import _CANCELLED, _curses_select
...
def test_curses_select_cancel_defaults_to_selected(monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.curses_ui.curses_radiolist", fake_radiolist)
...
def test_cmd_setup_top_level_cancel_writes_nothing(monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.config.load_config", load_config)
...
def test_cmd_setup_builtin_selection_still_saves_builtin(monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.config.load_config", lambda: config)
    monkeypatch.setattr("openamer_cli.config.save_config", save_config)
