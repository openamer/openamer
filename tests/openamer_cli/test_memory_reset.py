"""Tests for the `openamer memory reset` CLI command.
...
- Profile-scoped reset (uses OPENAMER_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake OPENAMER_HOME with memory files."""
    ...
    (memories / "MEMORY.md").write_text(
        "§\nOpenAmer repo is at ~/.openamer/openamer-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `openamer memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        openamer_home = tmp_path / ".openamer"
        (openamer_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("OPENAMER_HOME", str(openamer_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's OPENAMER_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_openamer_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
