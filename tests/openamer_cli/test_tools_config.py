"""Tests for openamer_cli.tools_config platform tool persistence."""
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "openamer_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _OPENAMER_CORE_TOOLS but aren't user-toggleable) are still
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("openamer_cli.tools_config.save_config"):
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/openamer/openamer/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.tools_config._run_post_setup", lambda key: None)
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "openamer_cli.tools_config._configure_tool_category_for_reconfig",
...
def test_configure_all_platforms_configures_selected_tool_missing_provider(monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.tools_config._prompt_choice", choose_by_label)
    monkeypatch.setattr(
        "openamer_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web"},
    )
    monkeypatch.setattr(
        "openamer_cli.tools_config._toolset_needs_configuration_prompt",
        lambda ts_key, config, **kwargs: ts_key == "web",
    )
    monkeypatch.setattr(
        "openamer_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from openamer_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from openamer_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/openamer/openamer/issues/6901
    """
    config = {
...
def test_computer_use_respects_custom_cua_driver_command():
    """The setup gate should match runtime's OPENAMER_CUA_DRIVER_CMD override."""
    def fake_which(name: str):
        return "/opt/bin/custom-cua" if name == "custom-cua" else None

    with patch.dict("os.environ", {"OPENAMER_CUA_DRIVER_CMD": "custom-cua"}), \
...
def test_computer_use_blank_custom_driver_command_falls_back_to_default():
    ...
    with patch.dict("os.environ", {"OPENAMER_CUA_DRIVER_CMD": "   "}), \
         patch("shutil.which", side_effect=fake_which):
        assert _toolset_needs_configuration_prompt("computer_use", {}) is False
...
def test_computer_use_post_setup_respects_custom_driver_command_when_installed():
    ...
    # bare-command miss after OPENAMER_CUA_DRIVER_CMD was looked up via which().
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from openamer_cli.tools_config import IMAGEGEN_BACKENDS
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from openamer_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("openamer_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("openamer_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("openamer_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from openamer_cli.tools_config import _toolset_allowed_for_platform
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from openamer_cli.tools_config import _save_platform_tools
    config = {}
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("openamer_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("openamer_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("openamer_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("openamer_cli.tools_config.save_env_value", lambda k, v: None)
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from openamer_cli.tools_config import apply_provider_selection
...
def test_apply_provider_selection_does_not_prompt_or_post_setup(monkeypatch):
    ...
    from openamer_cli import tools_config
...
def test_vision_picker_writes_provider_and_model(tmp_path, monkeypatch):
    ...
    """
    monkeypatch.setenv("OPENAMER_HOME", str(tmp_path))
    import openamer_cli.tools_config as tc
    ...
    seq = iter([1, 0, 1])
    with patch("openamer_cli.model_switch.list_authenticated_providers",
...
def test_provider_readiness_env_vars_gate_keys(monkeypatch):
    ...
    monkeypatch.setattr("openamer_cli.tools_config.get_env_value", lambda key: None)
    assert provider_readiness_status(provider, {}) == "needs_keys"

    monkeypatch.setattr("openamer_cli.tools_config.get_env_value", lambda key: "sk-x")
...
def test_provider_readiness_local_install_rows_track_module_presence(monkeypatch):
    ...
    monkeypatch.setattr(
        "openamer_cli.tools_config._module_installed", lambda name: False
    )
    assert provider_readiness_status(kitten, {}) == "needs_setup"
    assert provider_readiness_status(piper, {}) == "needs_setup"

    monkeypatch.setattr(
        "openamer_cli.tools_config._module_installed", lambda name: True
    )
...
def test_post_setup_no_window_flags_zero_on_posix(monkeypatch):
    ...
    from openamer_cli.tools_config import _post_setup_no_window_flags
