"""Tests for the Command Installation check in openamer doctor."""
...
import openamer_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal OPENAMER_HOME + PROJECT_ROOT for doctor tests."""
    ...
    openamer_bin = venv_bin_dir / "openamer"
    openamer_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    openamer_bin.chmod(0o755)
    ...
    return home, project, openamer_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, openamer_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "openamer"
        cmd_link.symlink_to(openamer_bin)
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        assert "openamer doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "openamer"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == openamer_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "openamer"
        wrong_target = tmp_path / "wrong_openamer"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        openamer_bin = project / ".venv" / "bin" / "openamer"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "openamer"
        cmd_link.symlink_to(openamer_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/openamer" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/openamer is a regular file (not symlink), accept it."""
        home, project, openamer_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "openamer"
        cmd_link.write_text("#!/bin/sh\nexec python -m openamer_cli.main \"$@\"\n")
