...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        ...
        with patch("openamer_cli.main._is_windows", return_value=True), \
             ...
             patch("openamer_cli.main._run_quarantined_install") as mock_install:
            from openamer_cli.main import _verify_console_scripts_installed
    ...
    def test_skips_off_windows(self, temp_pyproject, fake_scripts_dir):
        with patch("openamer_cli.main._is_windows", return_value=False), \
             patch("openamer_cli.main._run_quarantined_install") as mock_install:
            from openamer_cli.main import _verify_console_scripts_installed
    ...
    def test_primary_install_success_still_verifies_scripts(self):
        ...
        with patch("openamer_cli.main._is_windows", return_value=False), \
             patch("openamer_cli.main._run_quarantined_install") as mock_install, \
             patch("openamer_cli.main._verify_console_scripts_installed") as mock_verify:
