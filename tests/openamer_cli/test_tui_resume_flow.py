...
def main_mod(monkeypatch):
    import openamer_cli.main as mod
...
def test_termux_fast_cli_launch_bare_defers_agent_startup(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("OPENAMER_TUI", raising=False)
    monkeypatch.delenv("OPENAMER_DEFER_AGENT_STARTUP", raising=False)
    monkeypatch.delenv("OPENAMER_FAST_STARTUP_BANNER", raising=False)
    ...
    assert os.environ["OPENAMER_DEFER_AGENT_STARTUP"] == "1"
    assert os.environ["OPENAMER_FAST_STARTUP_BANNER"] == "1"
...
def test_termux_ultrafast_version_runs_before_heavy_startup(
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("OPENAMER_TERMUX_DISABLE_FAST_CLI", raising=False)
    monkeypatch.setattr(sys, "argv", ["openamer", "--version"])
...
def test_termux_fast_cli_launch_can_be_disabled(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setenv("OPENAMER_TERMUX_DISABLE_FAST_CLI", "1")
    monkeypatch.delenv("OPENAMER_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["openamer", "version"])
...
def test_termux_bundled_skills_stamp_controls_sync(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setenv("OPENAMER_TERMUX_FORCE_SKILLS_SYNC", "1")
    assert main_mod._termux_bundled_skills_sync_needed() is True
...
def test_run_and_exit_oneshot_hard_exits_when_oneshot_import_fails(
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "openamer_cli.oneshot" and "run_oneshot" in (fromlist or ()):
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import openamer_cli.oneshot as oneshot
        from openamer_cli.main import _exit_after_oneshot
...
def test_main_oneshot_path_bypasses_late_atexit_abort():
    ...
    program = textwrap.dedent(
        ...
        sys.modules["openamer_cli.oneshot"] = types.SimpleNamespace(
...
def test_oneshot_run_agent_closes_agent_after_chat(monkeypatch):
    import openamer_cli.oneshot as oneshot_mod
...
def test_oneshot_exit_code_when_failed_without_response(monkeypatch):
    ...
    monkeypatch.setattr(
        "openamer_cli.oneshot._run_agent",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from openamer_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "openamer_state", mod("openamer_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "openamer_cli.config",
        mod("openamer_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "openamer_cli.models",
        mod("openamer_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "openamer_cli.runtime_provider",
        mod(
            "openamer_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "openamer_cli.tools_config",
        mod("openamer_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["OPENAMER_TUI_ACTIVE_SESSION_FILE"])
...
def test_launch_tui_worktree_validates_relative_python_against_final_cwd(
    ...
    monkeypatch.setenv("OPENAMER_CWD", str(parent_cwd))
    ...
    assert captured["env"]["OPENAMER_CWD"] == str(worktree)
    assert captured["env"]["OPENAMER_PYTHON"] == str(relative_python)
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("openamer_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("OPENAMER_TUI_RESUME", "stale-missing-session")
    monkeypatch.setattr(
        main_mod,
        "_make_tui_argv",
        lambda tui_dir, tui_dev: (["node", "dist/entry.js"], Path(".")),
    ...
    assert "OPENAMER_TUI_RESUME" not in captured["env"]
...
def test_launch_tui_sets_resume_env_from_resume_arg(monkeypatch, main_mod):
    ...
    assert captured["env"]["OPENAMER_TUI_RESUME"] == "20260518_000000_goodid"
...
def test_make_tui_argv_dev_prebuilds_openamer_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("OPENAMER_TUI_DIR", raising=False)
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "openamer_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
