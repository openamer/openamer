...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $OPENAMER_HOME.
...
def test_home_docker_stamp_ignored_when_not_containerized(tmp_path):
    ...
    'docker' into the shared $OPENAMER_HOME. With no code-scoped stamp, a host
...
def test_home_non_docker_stamp_still_honored_for_backcompat(tmp_path):
    ...
    install that historically stamped 'git' into $OPENAMER_HOME keeps
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    """stamp_install_method writes next to the code, not into $OPENAMER_HOME."""
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    home.mkdir()
...
def test_recommended_update_command_docker():
    from openamer_cli.config import recommended_update_command_for_method
