...
def test_fire_cron_job_scopes_store_and_runtime_home_together(
    ...
    class RecordingProvider:
        def fire_due(self, job_id, *, adapters=None, loop=None):
            ...
            captured["runtime_home"] = scheduler._get_openamer_home()
    ...
    outer_token = set_openamer_home_override(default_home)
    try:
        ...
        assert scheduler._get_openamer_home() == default_home
    finally:
        reset_openamer_home_override(outer_token)
...
async def test_update_cron_job_refreshes_snapshots_when_unpinning(
    ...
    from openamer_cli import runtime_provider, web_server
...
async def test_create_cron_job_without_profile_uses_backend_own_profile(
    ...
    monkeypatch.setenv(
        "OPENAMER_HOME", str(isolated_profiles["worker_alpha"])
    )
...
async def test_create_cron_job_without_profile_defaults_when_unscoped(
    ...
    """OPENAMER_HOME at the default home (or unrecognized) keeps the legacy
    ``default`` fallback."""
    from openamer_cli import web_server

    monkeypatch.setenv("OPENAMER_HOME", str(isolated_profiles["default"]))
