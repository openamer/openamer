...
class TestToolsetConsistency:
    ...
    def test_openamer_platforms_share_core_tools(self):
        """All openamer-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on openamer-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["openamer-cli", "openamer-telegram", "openamer-discord", "openamer-whatsapp", "openamer-slack", "openamer-signal", "openamer-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_openamer_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("openamer-whatsapp")

    def test_openamer_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("openamer-api-server")
