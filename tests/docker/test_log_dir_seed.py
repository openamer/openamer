"""Runtime smoke test for Docker $OPENAMER_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the openamer user after container boot.
...
registered later runs its log service as the dropped openamer user and
...
def test_logs_gateways_seeded_and_openamer_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by openamer after boot."""
    ...
    # Both must be owned by openamer
    r = docker_exec_sh(
        container_name,
    ...
    assert "logs=openamer" in r.stdout, (
        f"logs/ not owned by openamer: {r.stdout}"
    )
    assert "gateways=openamer" in r.stdout, (
        f"logs/gateways/ not owned by openamer: {r.stdout}"
    )
