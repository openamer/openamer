...
def test_puid_pgid_remaps_openamer_user(
    built_image: str, container_name: str,
) -> None:
    """PUID=1000 PGID=1000 must remap the openamer user to UID 1000."""
    ...
    assert r.stdout.strip() == "1000", (
        f"expected openamer UID 1000 after PUID remap, got: {r.stdout.strip()}"
    )
    ...
    assert r.stdout.strip() == "1000", (
        f"expected openamer GID 1000 after PGID remap, got: {r.stdout.strip()}"
    )
...
def test_nas_low_uid_accepted(
    ...
    assert r.stdout.strip() == "99", (
        f"expected openamer UID 99, got: {r.stdout.strip()}"
    )

    r = docker_exec_sh(container_name, "id -g openamer", timeout=10)
    assert r.stdout.strip() == "100", (
        f"expected openamer GID 100, got: {r.stdout.strip()}"
    )
...
def test_remap_enables_data_volume_writes(
    ...
    """After remap, the openamer user must be able to write to /opt/data."""
    start_container(built_image, container_name, "PUID=1000", "PGID=1000")
    ...
    assert "WRITE_OK" in r.stdout, (
        f"openamer user cannot write to /opt/data after remap: {r.stdout}"
    )