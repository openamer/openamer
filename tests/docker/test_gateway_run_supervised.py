...
Before the s6 migration, ``docker run nousresearch/openamer-agent gateway
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'openamer.*gateway'`` (the gateway is running) or
    ...
    while time.monotonic() < end:
        ...
        if status == "running":
            ...
            pgrep = docker_exec_sh(
                container, "pgrep -f 'openamer.*gateway' >/dev/null 2>&1",
            )
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → openamer gateway run) can take
    ...
    # (main-wrapper.sh → python → openamer gateway run) can take well over
    ...
    # top), NOT the static main-openamer service's sleep — a bare grep
    # for `sleep infinity` would false-positive on the main-openamer
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``openamer gateway run`` invocation
        (not ``sleep infinity``).
...
def test_supervised_gateway_does_not_recurse(
    ...
    gateway slot would never actually run a python ``openamer gateway
    ...
    # (main-wrapper.sh → python → openamer gateway run) can take well
    ...
    # started either — so we'd see the CMD's `openamer gateway run`
    ...
    # `openamer gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*openamer.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `openamer gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-openamer`
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${OPENAMER_HOME}/logs/gateways/<profile>/current``.
