...
Override the image with ``OPENAMER_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("OPENAMER_TEST_IMAGE", "openamer-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``OPENAMER_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("OPENAMER_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged openamer user
# ---------------------------------------------------------------------------
#
# Background: every OpenAmer runtime path inside the container drops to UID
# 10000 (the ``openamer`` user) via ``s6-setuidgid openamer``. ``docker exec``
...
# fails (PermissionError swallowed) for openamer, so a test that ran as root
...
# helpers below run every probe under ``-u openamer`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: openamer).
