...
The shim (docker/openamer-exec-shim.sh, installed at /opt/openamer/bin/openamer)
...
1. ``docker exec <c> openamer …`` (defaulting to root) gets dropped to the
   openamer user before the real binary runs.
2. ``docker exec --user openamer <c> openamer …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $OPENAMER_HOME from a ``docker exec`` session land
   as openamer:openamer — the actual user-visible invariant.
4. The OPENAMER_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-openamer-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown openamer:openamer`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to openamer:openamer,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$OPENAMER_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (openamer_cli.container_boot), which s6 runs
    *strictly after* ``01-openamer-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_openamer_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `openamer` with a Python-style `-c` shim equivalent — there's no
    pure-openamer "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `openamer` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `openamer`
    do anything that writes to disk, then check the file's owner.

    Use `openamer config set` which writes config.yaml under OPENAMER_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "openamer", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by openamer, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "openamer:openamer", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected openamer:openamer. "
        "The shim did not drop privileges before invoking openamer."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user openamer already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "openamer", sleep_container,
         "openamer", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user openamer failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up openamer:openamer — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """OPENAMER_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", "OPENAMER_DOCKER_EXEC_AS_ROOT=1",
         sleep_container,
         "openamer", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "root:root", (
        f"With OPENAMER_DOCKER_EXEC_AS_ROOT=1, expected root:root, "
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``OPENAMER_DOCKER_EXEC_AS_ROOT=0``) doesn't
    silently keep the user as root. Mirrors the policy used by
    ``OPENAMER_GATEWAY_NO_SUPERVISE`` in #33583.
    """
    subprocess.run(
        ["docker", "exec", "--user", "root", sleep_container,
         "rm", "-f", "/opt/data/config.yaml"],
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", f"OPENAMER_DOCKER_EXEC_AS_ROOT={falsy_value}",
         sleep_container,
         "openamer", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "openamer:openamer", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected openamer:openamer"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/openamer/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid openamer openamer <args>` which resolves `openamer`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    `docker exec` path produces is openamer-readable.

    Specifically: pretend the operator ran `openamer login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the openamer UID. We use `openamer auth list` since that touches the
    ...
    # `openamer auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `openamer config set` first to
    # provoke a write into OPENAMER_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "openamer", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # OPENAMER_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "openamer", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the openamer user "
