...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves openamer_cli.kanban_db on its own
  ...
  - worker logs are captured to OPENAMER_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "OPENAMER_KANBAN_TASK": task.id,
            "OPENAMER_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="openamer_e2e_")
    os.environ["OPENAMER_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from openamer_cli import kanban_db as kb

    # Point the `openamer` CLI child processes will run at the worktree
    # openamer_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "openamer")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m openamer_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/openamer_e2e_*/worker_*.log
    import glob
