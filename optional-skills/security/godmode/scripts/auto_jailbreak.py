...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("OPENAMER_HOME", os.path.expanduser("~/.openamer")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("OPENAMER_HOME", Path.home() / ".openamer")) / "skills" / "red-teaming" / "godmode"
...
MODEL_STRATEGIES = {
    ...
    # OpenAmer/OpenAmer models — already uncensored, just needs clean prompt
    "openamer": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "openamer" in model_lower or "openamer" in model_lower:
        return "openamer"
...
def _get_current_model() -> tuple:
    """Read current model and provider from OpenAmer config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.openamer/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart OpenAmer for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart OpenAmer for changes to take effect.")
