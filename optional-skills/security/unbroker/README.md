...
The design is **OpenAmer-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
openamer skills install official/security/unbroker
```

Then start a new OpenAmer session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a OpenAmer session:
...
- **PII handling.** Dossiers live under the OpenAmer home directory (`0600`, optionally
