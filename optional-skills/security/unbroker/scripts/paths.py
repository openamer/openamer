...
All per-subject data lives under PDD_DATA_DIR (default: $OPENAMER_HOME/unbroker),
which is the same trust boundary OpenAmer uses for .env and OAuth tokens.
"""
...
def openamer_home() -> Path:
    return Path(os.environ.get("OPENAMER_HOME") or (Path.home() / ".openamer"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else openamer_home() / "unbroker"
