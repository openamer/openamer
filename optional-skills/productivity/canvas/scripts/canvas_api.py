#!/usr/bin/env python3
"""Canvas LMS API CLI for OpenAmer Agent.
...
def _check_config():
    ...
    if missing:
        openamer_env = os.path.join(
            os.environ.get("OPENAMER_HOME", os.path.expanduser("~/.openamer")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {openamer_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for OpenAmer Agent"
    )
