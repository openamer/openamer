...
### 1. Start order: editor first, OpenAmer second

OpenAmer probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW OpenAmer session.
...
restart the editor, restart the OpenAmer session.
...
server fails to bind (Output Log) or OpenAmer' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.openamer/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for OpenAmer

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. OpenAmer' connection
lives in `~/.openamer/config.yaml` via `openamer mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms OpenAmer — just don't
mistake it for the OpenAmer setup step.
...
explicitly warns against overlapping calls. OpenAmer executes same-turn tool
...
### 9. Timeouts: OpenAmer gives up before Unreal does

OpenAmer' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If OpenAmer runs elsewhere (SSH backend,
