...
the actual `setup.sh` that creates the project workspace, configures OpenAmer
...
> [openamer/kanban-video-pipeline](https://github.com/openamer/kanban-video-pipeline).
...
2. **Create profiles** — `openamer profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.openamer/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `openamer kanban create` assigned to the director
...
```bash
openamer profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.openamer/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.openamer/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most OpenAmer
...
Each profile gets a `SOUL.md` at `~/.openamer/profiles/<name>/SOUL.md` that
...
```bash
openamer kanban create "Direct production of <video title>" \
    --assignee director \
...
check_key() {
    ...
    echo "ERROR: ${var} not set in ${_openamer_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY openamer ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY openamer OPENROUTER_API_KEY || exit 1
# ...
```
