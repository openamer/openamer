...
Maps each role archetype to the OpenAmer skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public openamer-agent
...
## OpenAmer skills relevant to video production

### Visual / rendering skills (`openamer-agent/skills/creative/`)
...
### Design / pre-production skills (`openamer-agent/skills/creative/`)
...
### Audio / media skills (`openamer-agent/skills/creative/` + `skills/media/`)
...
These are **not** OpenAmer skills but external CLIs / APIs that profiles invoke.
...
## Built-in OpenAmer tools for media review

These are native OpenAmer tools — not invoked via terminal but through their own
...
key is present in `${OPENAMER_HOME:-~/.openamer}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every OpenAmer profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${OPENAMER_HOME:-~/.openamer}/.env` → environment variable.
...
When a new OpenAmer-public video skill ships:
