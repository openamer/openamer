...
# 5. Symlinks the 'openamer' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ OpenAmer Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/openamer-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink openamer into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up openamer command..."

OPENAMER_BIN="$SCRIPT_DIR/venv/bin/openamer"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$OPENAMER_BIN" "$COMMAND_LINK_DIR/openamer"
echo -e "${GREEN}✓${NC} Symlinked openamer → $COMMAND_LINK_DISPLAY_DIR/openamer"
...
# ============================================================================
# Seed bundled skills into ~/.openamer/skills/
# ============================================================================

OPENAMER_SKILLS_DIR="${OPENAMER_HOME:-$HOME/.openamer}/skills"
mkdir -p "$OPENAMER_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.openamer/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$OPENAMER_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  openamer status        # Check configuration"
if is_termux; then
    echo "  openamer gateway       # Run gateway in foreground"
else
    echo "  openamer gateway install # Install gateway service (messaging + cron)"
fi
echo "  openamer cron list     # View scheduled jobs"
echo "  openamer doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m openamer_cli.main setup
fi
