...
def run_cmd(cmd, timeout=10):
    ...
    ``cmd`` must be a *constant* command string -- callers pass literals such
    as ``"git rev-parse HEAD"``. That is why ``shell=True`` is acceptable here:
    nothing user- or path-supplied is interpolated, so there is no injection
    surface. Callers that need a path must use ``run_argv`` instead, which
    never involves a shell.

    Marked ``# noqa:SEC`` for the scanner: the shell is genuine and intended,
    and the invariant above is what keeps it safe.
    """
    try:
        r = subprocess.run(
            cmd, capture_output=True, text=True, timeout=timeout,
            shell=True, cwd=str(REPO_DIR) if REPO_DIR.exists() else None,  # noqa:SEC constant commands only
        )
...
def run_argv(argv, timeout=10):
    """Run a command WITHOUT a shell, passing arguments as a list.

    Use this whenever a path or any other variable value takes part in the
    command: no quoting to get wrong, and no shell interpretation of the
    value. Returns the same (stdout, stderr, exit_code) tuple as ``run_cmd``.
    """
    try:
        r = subprocess.run(
...
def collect_os_info():
    ...
    if pyproject_path.exists():
        try:
            ...
            # fallback: read the file directly. Previously this shelled out to
            # `grep "^version" "<path>"` -- a path interpolated into a shell
            # string, i.e. a real injection surface. Reading the file needs no
            # shell at all, so the surface is gone rather than annotated away.
            try:
