...
#   2. ~/.openamer/node/ from a prior OpenAmer-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.openamer/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$OPENAMER_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   OPENAMER_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   OPENAMER_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   OPENAMER_HOME               (default: $HOME/.openamer)
# ============================================================================

OPENAMER_NODE_MIN_VERSION="${OPENAMER_NODE_MIN_VERSION:-20}"
OPENAMER_NODE_TARGET_MAJOR="${OPENAMER_NODE_TARGET_MAJOR:-22}"
OPENAMER_HOME="${OPENAMER_HOME:-$HOME/.openamer}"
OPENAMER_NODE_AVAILABLE=false
...
# Redirect a OpenAmer-managed Node's `npm install -g` to the command link dir
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$OPENAMER_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $OPENAMER_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$OPENAMER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$OPENAMER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $OPENAMER_NODE_TARGET_MAJOR..."
    proto install node "$OPENAMER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $OPENAMER_NODE_TARGET_MAJOR..."
    nvm install "$OPENAMER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$OPENAMER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${OPENAMER_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${OPENAMER_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${OPENAMER_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${OPENAMER_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${OPENAMER_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $OPENAMER_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
...
# ---------------------------------------------------------------------------
# Heal a broken OpenAmer-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
# absent. Used by openamer_constants.find_openamer_node_executable() and safe
...
heal_managed_node() {
    ...
    _nb_log "OpenAmer-managed Node is broken — redownloading to $OPENAMER_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if [ -x "$OPENAMER_HOME/node/bin/node" ]; then
        export PATH="$OPENAMER_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (OpenAmer-managed)"
            OPENAMER_NODE_AVAILABLE=true
            return 0
        fi
    fi
    ...
    _nb_try_fnm   && { OPENAMER_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { OPENAMER_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { OPENAMER_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { OPENAMER_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { OPENAMER_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { OPENAMER_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $OPENAMER_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
