#!/usr/bin/env python3
"""Live test harness for OpenAmer Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any openamer imports.
ORIGINAL_HOME = os.environ.get("OPENAMER_HOME")
ORIGINAL_AUTH = Path.home() / ".openamer" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.openamer/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.openamer/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="openamer_ts_live_"))
    openamer_home = home_dir / ".openamer"
    openamer_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, openamer_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".openamer" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, openamer_home / ".env")
        ...
        load_openamer_dotenv(openamer_home=str(Path.home() / ".openamer"))
    ...
    (openamer_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new OPENAMER_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "openamer_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["OPENAMER_HOME"] = str(home)
...
def main():
    ...
    # Restore original OPENAMER_HOME
    if ORIGINAL_HOME is not None:
        os.environ["OPENAMER_HOME"] = ORIGINAL_HOME
    else:
