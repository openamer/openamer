...
Background
----------
A Nous bootstrap session (client_id ``openamer-cli-vps``) can take a terminal
...
``OPENAMER_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Nous
...
def _nous_entry_is_terminal(nous_state: Any) -> bool:
    ...
    Mirrors the ``terminal`` predicate in ``openamer_cli.auth.get_nous_session_validity``:
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.nous`` block out of a OPENAMER_AUTH_JSON_REBOOTSTRAP
