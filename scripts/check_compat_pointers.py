...
SKIP_DIRS = {".git", "node_modules", "website", "skills", "optional-skills", "apps", "evals", "build", "MagicMock", ".worktrees", "__pycache__",
             # Installed dependencies are not in-tree code. Without these the rglob walks
             # ~23k site-package files in .venv and the check takes >6 minutes instead of
             # ~2s; CI runs in a tree without a venv, which is why upstream never noticed.
             ".venv", "venv", "site-packages", ".tox", ".nox", "dist", "egg-info",
             # A local interpreter install dropped at the repo root on this machine
             # (pythoncore-3.14-64, untracked). Not in-tree code either.
             "Python"}
...
def _py_files():
    for p in ROOT.rglob("*.py"):
        ...
        # Any skipped dir anywhere in the path, not just the first component — a nested
        # tools/.venv is just as much third-party code as a top-level one.
        if SKIP_DIRS & set(parts[:-1]) or parts[0] in SKIP_DIRS or p.name == "check_compat_pointers.py":
            continue
