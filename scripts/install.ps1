# ============================================================================
# OpenAmer Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$OpenAmerHome = $(if ($env:OPENAMER_HOME) { $env:OPENAMER_HOME } else { "$env:LOCALAPPDATA\openamer" }),
    [string]$InstallDir = $(if ($env:OPENAMER_HOME) { "$env:OPENAMER_HOME\openamer-agent" } else { "$env:LOCALAPPDATA\openamer\openamer-agent" }),
...
function Write-Banner {
    ...
    Write-Host "|             * OpenAmer Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by OpenAmer.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $OpenAmerHome)) {
        New-Item -ItemType Directory -Force -Path $OpenAmerHome | Out-Null
    }
    $envFile = Join-Path $OpenAmerHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $OpenAmerHome "node"
...
function Install-Uv {
    # OpenAmer owns its own uv at $OpenAmerHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (openamer_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `openamer update` stay in sync.
    $managedUv = Join-Path $OpenAmerHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $OpenAmerHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $OpenAmerHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $OpenAmerHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $OpenAmerHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and OPENAMER_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under OpenAmer-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun OpenAmer setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which OpenAmer uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\openamer\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    OpenAmer needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the OpenAmer copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\openamer\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``OPENAMER_GIT_BASH_PATH`` (User scope) so OpenAmer can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a OpenAmer-managed PortableGit install instead..."
    }

    # Download PortableGit into $OpenAmerHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $OpenAmerHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent OpenAmer features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  OpenAmer needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``OPENAMER_GIT_BASH_PATH`` (User env scope) so OpenAmer can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $OpenAmerHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $OpenAmerHome\git\usr\bin\bash.exe
    $candidates += "$OpenAmerHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$OpenAmerHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("OPENAMER_GIT_BASH_PATH", $candidate, "User")
            $env:OPENAMER_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set OPENAMER_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- OpenAmer may not find Git Bash."
    Write-Info "If needed, set OPENAMER_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    # Prefer a OpenAmer-managed Node from a previous run over a too-old system one.
    $managedNode = "$OpenAmerHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$OpenAmerHome\node;$env:Path"
        Write-Success "Node.js $version found (OpenAmer-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing OpenAmer-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $OpenAmerHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $OpenAmerHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\openamer-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$OpenAmerHome\node") { Remove-Item -Recurse -Force "$OpenAmerHome\node" }
                Move-Item $extractedDir.FullName "$OpenAmerHome\node"
                ...
                $version = & "$OpenAmerHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $OpenAmerHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\openamer-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `openamer
                    # update` path (#4735).
                    ...
                    $stashName = "openamer-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed -- mirror ``openamer update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if OpenAmer behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running openamer processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\openamer-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `openamer update` (see the notes at the shared clone-path
    ...
    # next `openamer update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under OpenAmer-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running openamer process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running openamer processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*OpenAmer_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (openamer.exe) plus its child tree.
            & taskkill /F /T /IM openamer.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM openamer.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m openamer_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not openamer.exe.
            ...
            # user (see openamer_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `openamer update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `openamer.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'openamer-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install openamer-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and openamer.exe broken with
    ...
    # here, not 30 seconds later when the user runs `openamer`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register openamer.exe in dist-info/RECORD but fail to
        ...
        # `openamer` command while openamer-agent.exe / openamer-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m openamer_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `openamer dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `openamer dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `openamer dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up openamer command..."
    
    if ($NoVenv) {
        $openamerBin = "$InstallDir"
    } else {
        $openamerBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so openamer is globally available
    # On Windows, the openamer.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$openamerBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$openamerBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $openamerBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set OPENAMER_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\openamer instead
    # of the Unix default ~/.openamer
    $currentOpenAmerHome = [Environment]::GetEnvironmentVariable("OPENAMER_HOME", "User")
    if (-not $currentOpenAmerHome -or $currentOpenAmerHome -ne $OpenAmerHome) {
        ...
        Write-Success "Set OPENAMER_HOME=$OpenAmerHome"
    }
    $env:OPENAMER_HOME = $OpenAmerHome
    
    # Update current session
    $env:Path = "$openamerBin;$env:Path"
    
    Write-Success "openamer command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.openamer-bootstrap-complete which tells the OpenAmer
    ...
    # OpenAmer-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `openamer update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".openamer-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the OPENAMER_HOME directory structure ($OpenAmerHome, default %LOCALAPPDATA%\openamer)
    New-Item -ItemType Directory -Force -Path "$OpenAmerHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpenAmerHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpenAmerHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpenAmerHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpenAmerHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpenAmerHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpenAmerHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpenAmerHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$OpenAmerHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$OpenAmerHome\.env"
    ...
    # Create config.yaml
    $configPath = "$OpenAmerHome\config.yaml"
    ...
    # (the default PS5 behaviour), and OpenAmer's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$OpenAmerHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in openamer_cli/default_soul.py. The runtime
        ...
        $soulContent = @"
You are OpenAmer Agent, an intelligent AI assistant created by OpenAmer. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
"@
        $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    ...
    Write-Success "Configuration directory ready: $OpenAmerHome"
    
    # Seed bundled skills into $OpenAmerHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $OpenAmerHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $OpenAmerHome\skills"
        } catch {
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'openamer setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\openamer-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\openamer-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\openamer-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `OpenAmer` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    ...
    #
    # The Tauri bootstrap installer's launch_openamer_desktop command
    # resolves apps/desktop/release/win-unpacked/OpenAmer.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the OpenAmer-managed
    ...
    # The --dir mode produces an unpacked OpenAmer.exe in
    ...
    # are NOT trying to work around). The OpenAmer icon + product name are
    # stamped onto OpenAmer.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\openamer-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\OpenAmer.exe",
        "$desktopDir\release\win-arm64-unpacked\OpenAmer.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no OpenAmer.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The OpenAmer icon + identity are stamped onto OpenAmer.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, openamer-agent#38216).
    ...
    #    OpenAmer.exe. We deliberately do NOT point them at `openamer desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'OpenAmer.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'OpenAmer.lnk')
        )
        ...
        # with the OpenAmer icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.openamer/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'openamer setup'."
        return
    }
    ...
    # Run openamer setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m openamer_cli.main setup
    } else {
        python -m openamer_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $openamerCmd = "$InstallDir\venv\Scripts\openamer.exe"
    if (-not (Test-Path $openamerCmd)) {
        $openamerCmd = "openamer"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$OpenAmerHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'openamer whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: openamer gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$OpenAmerHome\logs\gateway.log"
            Start-Process -FilePath $openamerCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$OpenAmerHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: openamer gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: openamer gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$OpenAmerHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$OpenAmerHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$OpenAmerHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$OpenAmerHome\openamer-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   openamer              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   openamer setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   openamer config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   openamer config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   openamer gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   openamer update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning OpenAmer repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
$InstallStages += @(
    @{ Name = "path";             Title = "Adding OpenAmer to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
