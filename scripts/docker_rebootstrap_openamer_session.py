#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead OpenAmer bootstrap session.

Background
----------
A OpenAmer bootstrap session (client_id ``openamer-cli-vps``) can take a terminal
``invalid_grant`` and be quarantined locally — the refresh path clears the dead
tokens from ``auth.json`` and stamps
``providers.openamer.last_auth_error.relogin_required = true``. From then on every
...
``stage2-hook.sh`` seeds ``auth.json`` from ``OPENAMER_AUTH_JSON_BOOTSTRAP`` only
...
``OPENAMER_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the OpenAmer
...
Design constraints
------------------
- Pure stdlib, no openamer_cli imports: runs early in the boot hook, before the
  app venv/modules are guaranteed importable, as its own subprocess.
- Surgical: replaces ONLY ``providers.openamer`` in the existing auth.json, leaving
...
# from OPENAMER_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
# paths can never be confused: BOOTSTRAP seeds a fresh volume; REBOOTSTRAP
# overwrites a terminally-dead OpenAmer entry on an existing volume.
REBOOTSTRAP_ENV = "OPENAMER_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "openamer-cli-vps"


def _openamer_entry_is_terminal(openamer_state: Any) -> bool:
    """True iff the on-disk OpenAmer provider entry is in the terminal/quarantined
    state AND holds no usable credential.

    Mirrors the ``terminal`` predicate in ``openamer_cli.auth.get_openamer_session_validity``:
    ...
    """
    if not isinstance(openamer_state, dict):
        return False
    last_err = openamer_state.get("last_auth_error")
    ...
    if openamer_state.get("access_token") or openamer_state.get("refresh_token"):
        return False
    return True


def _extract_openamer_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.openamer`` block out of a OPENAMER_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    OPENAMER_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
    ...
    if not isinstance(openamer, dict) or not openamer:
        return None
    if openamer.get("client_id") != BOOTSTRAP_CLIENT_ID:
        return None
    if not (
        isinstance(openamer.get("access_token"), str)
        and openamer["access_token"].strip()
        and isinstance(openamer.get("refresh_token"), str)
        and openamer["refresh_token"].strip()
    ):
        return None
    return openamer
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "bad_seed"         — seed present but unparseable / no openamer entry
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             OPENAMER_AUTH_JSON_BOOTSTRAP path handle it)
    ...
    seed_nous = _extract_openamer_from_seed(seed_raw)
    ...
    local_nous = providers.get("openamer")
    terminal = _openamer_entry_is_terminal(local_nous)
    ...
    # Surgical replacement: swap ONLY providers.openamer, preserve everything else.
    providers["openamer"] = seed_nous
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] OpenAmer bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    elif result == "reseeded_newer":
        print("[rebootstrap] Applied newer orchestrator-issued OpenAmer bootstrap session from "
              f"{REBOOTSTRAP_ENV}")
    else:
