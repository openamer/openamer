...
running under the dispatcher (env var ``OPENAMER_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``openamer chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``openamer kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``openamer kanban complete …``
   inside the container, where ``openamer`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.openamer/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``openamer kanban …``), the dashboard
(``openamer dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``OPENAMER_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``openamer chat`` without the kanban toolset see zero
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``OPENAMER_KANBAN_TASK`` set
    ...
    ``OPENAMER_KANBAN_TASK`` in env) aren't subject to this check — their
...
#   - No-op outside dispatcher-spawned worker context (no ``OPENAMER_KANBAN_TASK``).
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set OPENAMER_KANBAN_TASK in the env)"
        )
        ...
        try:
            ...
            if task and task.goal_mode and _goal_judge_available():
                ...
                try:
                    ...
                    # openamer_cli/goals.py. Unpacking fewer raises ValueError,
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``openamer-system`` and poison
...
def _download_url_with_cap(url: str, max_bytes: int) -> tuple[bytes, Optional[str]]:
    ...
    for _ in range(_MAX_ATTACH_URL_REDIRECTS + 1):
        ...
        with httpx.stream(
            "GET",
            current_url,
            headers={"User-Agent": "openamer-kanban/attach"},
            timeout=30,
...
def _handle_attach_url(args: dict, **kw) -> str:
    ...
    The agent passes a URL; OpenAmer downloads it (with the shared size cap)
    ...
    try:
        ...
        try:
            ...
            if _inherit_project and project_id is None:
                _self_tid = os.environ.get("OPENAMER_KANBAN_TASK")
                if _self_tid:
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to OPENAMER_KANBAN_TASK from the env "
...
KANBAN_ATTACH_URL_SCHEMA = {
    ...
    "description": (
        "Attach a file to a task by URL — OpenAmer downloads it server-side "
