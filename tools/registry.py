"""Central registry for all openamer-agent tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``openamer tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``openamer tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("openamer_plugins."):
            return mod
        return None
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``openamer_plugins.{name}``), not the exact module string.
                # A handler defined in ``openamer_plugins.pkg.handlers`` is
                # still owned by the ``openamer_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("openamer_plugins.")
                    and not same_plugin
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``openamer tools enable foo``)
