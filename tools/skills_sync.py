...
def _get_bundled_dir() -> Path:
    ...
    Checks OPENAMER_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    if (OPENAMER_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``OPENAMER_HOME``. Require a strict-child relationship so both escape
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = OPENAMER_HOME / NO_BUNDLED_SKILLS_MARKER
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (OPENAMER_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
