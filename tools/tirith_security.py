...
it is automatically downloaded from GitHub releases to $OPENAMER_HOME/bin/tirith.
...
def _get_openamer_home() -> str:
    """Return the OpenAmer home directory, respecting OPENAMER_HOME env var."""
    return str(get_openamer_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_openamer_home(), ".tirith-install-failed")
...
def _openamer_bin_dir() -> str:
    """Return $OPENAMER_HOME/bin, creating it if needed."""
    d = os.path.join(_get_openamer_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $OPENAMER_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_openamer_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $OPENAMER_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $OPENAMER_HOME/bin/tirith
    ...
    openamer_bin = os.path.join(_openamer_bin_dir(), "tirith")
    if os.path.isfile(openamer_bin) and os.access(openamer_bin, os.X_OK):
        _resolved_path = openamer_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return openamer_bin
