#!/usr/bin/env python3
"""Image generation via FAL.ai (model picked in ``openamer tools``, persisted to ``image_gen.model``).
...
def _resolve_managed_fal_gateway():
    """Managed gateway config for the stored `openamer tools` selection, or ``None`` for direct FAL.
...
_CACHE_BASE_BY_BACKEND = {"docker": "/root/.openamer", "singularity": "/root/.openamer",
                          "modal": "/root/.openamer", "ssh": "~/.openamer"}
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed:
        lines.append("  2. Sign in to a Nous account that has the managed FAL gateway enabled "
                     "(`openamer setup`)")
    lines.append("  3. Configure a different image_gen provider via `openamer tools` → Image Generation "
                 "(run `openamer plugins list` to see installed backends)")
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    if provider is None:
        return _provider_error(
            ...
            f"Run `openamer plugins list` to see available image gen backends.", "provider_not_registered")
    ...
    try:
        ...
        if is_type_error and ("image_url" in kwargs or "reference_image_urls" in kwargs):
            ...
            return _provider_error(
                ...
                f"text-to-image, or pick a backend that supports editing via `openamer tools` → "
