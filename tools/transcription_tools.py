...
LOCAL_STT_COMMAND_ENV = "OPENAMER_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "OPENAMER_LOCAL_STT_LANGUAGE"
...
# DeepInfra STT base URL now resolved via openamer_cli.models.deepinfra_base_url (shared).
...
#
# The single-env-var ``OPENAMER_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__OPENAMER_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"openamer-cmd-stt-{provider_name}-") as tmpdir:
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set OPENAMER_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="openamer-local-stt-") as output_dir:
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `openamer model` or set XAI_API_KEY",
        }
    ...
    language = str(
        ...
        or os.getenv("OPENAMER_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()
...
def _transcribe_deepinfra(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    ``openamer_cli.models`` helpers so every DeepInfra surface resolves the
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    ...
    return managed_gateway.openamer_user_token, urljoin(
