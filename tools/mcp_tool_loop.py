...
def _try_acquire_mcp_discovery_lock() -> Any:
    ...
    try:
        from openamer_constants import get_openamer_home, get_openamer_home_override
        if get_openamer_home_override() is not None:
            lock_path = str(get_openamer_home() / ".mcp-discovery.lock")
        else:
            if _origin._MCP_DISCOVERY_LOCK_PATH is None:
                _origin._MCP_DISCOVERY_LOCK_PATH = str(get_openamer_home() / ".mcp-discovery.lock")
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local OPENAMER_HOME override into ``coro`` (task-local on the MCP
    loop, so concurrent scopes don't interfere)."""
    try:
        from openamer_constants import get_openamer_home_override, reset_openamer_home_override, set_openamer_home_override
