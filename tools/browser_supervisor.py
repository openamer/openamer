...
One ``CDPSupervisor`` runs per OpenAmer ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach OpenAmer logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "openamer-dialog-bridge.invalid"
...
(() => {
  if (window.__openamerDialogBridgeInstalled) return;
  window.__openamerDialogBridgeInstalled = true;
  const ENDPOINT = "http://openamer-dialog-bridge.invalid/";
