#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the OpenAmer Skills Hub.
...
Used by openamer_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _openamer_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            openamer_meta = metadata.get("openamer", {})
            if isinstance(openamer_meta, dict):
                tags = openamer_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            openamer_meta = metadata.get("openamer", {})
            if isinstance(openamer_meta, dict):
                raw_tags = openamer_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by OpenAmer) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.openamer/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                openamer_meta = meta_block.get("openamer", {})
                if isinstance(openamer_meta, dict):
                    tags = openamer_meta.get("tags", [])
...
def _openamer_index_cache_file() -> Path:
    return _index_cache_dir() / "openamer-index.json"


def _load_openamer_index() -> Optional[dict]:
    ...
    """
    # Check local cache
    openamer_index_cache_file = _openamer_index_cache_file()
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("OpenAmer index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "OpenAmer index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("OpenAmer index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        openamer_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        openamer_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class OpenAmerIndexSource(SkillSource):
    """Skill source backed by the centralized OpenAmer Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_openamer_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "openamer-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "openamer-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        OpenAmerIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "openamer-index"
