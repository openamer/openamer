...
transport, discovers their tools, and registers them into the openamer-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.openamer/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.openamer/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach OpenAmer' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    OpenAmer explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so OpenAmer and
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            openamer_home = os.path.expanduser(
                os.getenv(
                    "OPENAMER_HOME", os.path.join(os.path.expanduser("~"), ".openamer")
                )
            )
            candidates = [
                os.path.join(openamer_home, "node", "bin", resolved_command),
                ...
                # image (which the OpenAmer Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → OpenAmer MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that OpenAmer gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the OpenAmer document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`openamer_cli.mcp_config._unwrap_exception_group` with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through OpenAmer' existing approval
...
class MCPServerTask:
    ...
    def _make_logging_callback(self):
        ...
        server into OpenAmer' logging (agent.log via openamer_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "it is not installed. Run `openamer setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this OpenAmer process (kill -9, crash, force-quit)
                ...
                if (
                    ...
                    post_resp = await client.post(
                        ...
                        content=(
                            ...
                            '"capabilities":{},'
                            '"clientInfo":{"name":"openamer-probe",'
                            '"version":"0.1"}}}'
                        ),
                    )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through OpenAmer' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until OpenAmer is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `openamer mcp login {server_name}` (or delete the tokens "
            f"file under ~/.openamer/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local OPENAMER_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_openamer_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_openamer_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local OPENAMER_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_openamer_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the OpenAmer config file.
    ...
    ``os.environ`` (which includes ``~/.openamer/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through OpenAmer' MEDIA: tag convention and out to messaging
            ...
            # OpenAmer' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves OpenAmer's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. OpenAmer uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the OpenAmer registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``openamer tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the OpenAmer registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple openamer
