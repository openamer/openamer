...
def _get_mcp_stderr_log() -> Any:
    ...
    from openamer_constants import get_openamer_home, openamer_home_key, mkdir_under_openamer_home
    home_key = openamer_home_key()
    with _mcp_stderr_log_lock:
        fh = _mcp_stderr_log_fh.get(home_key)
        if fh is None or fh.closed:
            try:
                log_dir = get_openamer_home() / "logs"
                mkdir_under_openamer_home(log_dir)
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    from openamer_cli.env_loader import secret_source_names
    env = {
    ...
    for key in secret_source_names():
        ...
        if value is not None:
            env[key] = value
    for key in ("OPENAMER_KANBAN_DB", "OPENAMER_KANBAN_BOARD"):
...
def _node_fallback(command: str) -> str:
    ...
    openamer_home = os.path.expanduser(os.getenv("OPENAMER_HOME", os.path.join(home, ".openamer")))
    # /usr/local/bin: canonical Node location (from-source Linux, OpenAmer Docker image, Intel Homebrew),
    # needed when a hand-authored env.PATH omits it — npx's shebang re-execs /usr/bin/env node.
    candidates = (os.path.join(openamer_home, "node", "bin", command), os.path.join(home, ".local", "bin", command),
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    extension (same precedence as ``openamer_constants._candidate_node_command_names``). ``windows``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    for nothing (~48 MB private memory per MCP server, measured); OpenAmer already supervises the
