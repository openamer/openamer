"""
`openamer computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the OpenAmer side — the only
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with OpenAmer provider secrets stripped.
...
def run_doctor(
    ...
    *,
    ...
    Honors `OPENAMER_CUA_DRIVER_CMD` via the shared runtime resolver, so the
