...
etc.) surface as specific blocked checks via `openamer computer-use doctor`
...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and NousResearch/openamer-agent#67052.
    """
...
# fetches the latest release, so a `OPENAMER_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `OPENAMER_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "OPENAMER_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when OpenAmer should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when OpenAmer should disable cua-driver telemetry for this user.
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of NousResearch/openamer-agent#47072: instead of hardcoding
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``OPENAMER_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    OpenAmer Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `openamer computer-use doctor` succeeds from a login shell.
    """
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``OPENAMER_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `openamer computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        f"{installer}\n"
        "Or run `openamer tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of NousResearch/openamer-agent#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(self, bridge: _AsyncBridge) -> None:
        ...
        self._started = False
        # Surface 4 of NousResearch/openamer-agent#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize OpenAmer-managed secrets out of the child env.
                env=_sanitize_subprocess_env(cua_driver_child_env()),
            )
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `openamer computer-use doctor` and check "
                f"{display_openamer_home()}/logs/agent.log for the phase timings."
            )
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    NousResearch/openamer-agent#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        # Surface 6 of NousResearch/openamer-agent#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per OpenAmer run, with
        ...
        #     concurrent OpenAmer runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one OpenAmer run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"openamer-{uuid.uuid4().hex[:12]}"

    # ── Lifecycle ──────────────────────────────────────────────────
    def start(self) -> None:
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of OpenAmer' minimal core.
    ...
    def capture(
        ...
        Maps openamer `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of NousResearch/openamer-agent#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of NousResearch/openamer-agent#47072: prefer the
    ...
    def _apply_delivery(
        ...
        to proceed. See NousResearch/openamer-agent#67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # NousResearch/openamer-agent#47072). cua-driver-rs gained an explicit
