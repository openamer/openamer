...
-----------------------------------------------------------------------------
    ...
    ~/.openamer/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/openamer/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_openamer_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/openamer"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (OpenAmer convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("OPENAMER_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal OpenAmer infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``openamer checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `openamer checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "openamer@local"], store, cfg_wd)
    _run_git(["config", "user.name", "OpenAmer Checkpoint"], store, cfg_wd)
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir():
        ...
        wd_marker = child / "OPENAMER_WORKDIR"
        if wd_marker.exists():
            try:
...
# look for ``HEAD``/``info/exclude``/``OPENAMER_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at OPENAMER_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "OPENAMER_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `openamer checkpoints` CLI
# ---------------------------------------------------------------------------
