...
# from shadowing OpenAmer's own top-level modules.  ``openamer_bootstrap`` lives at
...
# openamer_cli.mcp_startup.start_background_mcp_discovery can actually fire for
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `OPENAMER_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("OPENAMER_TUI_SIDECAR_URL")
...
# tested; override via ``OPENAMER_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("OPENAMER_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``OPENAMER_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``openamer --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``openamer_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from openamer_cli.mcp_startup import _resolve_discovery_timeout
    ...
    # openamer_cli.mcp_startup.wait_for_mcp_discovery directly, and delegating
    ...
    try:
        from openamer_cli.mcp_startup import (
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``openamer --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``openamer dashboard`` spawn theirs via
    ``openamer_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``openamer_cli.mcp_startup`` thread used by the
    ...
    try:
        from openamer_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def main():
    ...
    # server stalled the whole shell on "summoning openamer…" for the full
    ...
    if _has_mcp_servers:
        # Spawn via the shared owner in openamer_cli.mcp_startup instead of
    ...
    # Live-apply skins OpenAmer activates mid-conversation.
