...
class ComputeHost:
    def __init__(
        ...
        *,
        ...
        self._heartbeat_secs = (
            ...
            else float(os.environ.get("OPENAMER_COMPUTE_HOST_HEARTBEAT_SECS") or "15")
        )
    ...
    def _ensure_server_session(self, server: Any, frame: dict[str, Any]) -> dict:
        ...
        try:
            if profile_home:
                from openamer_constants import set_openamer_home_override
            ...
            if home_token is not None:
                try:
                    from openamer_constants import reset_openamer_home_override
...
def _default_workers() -> int:
    try:
        return max(2, int(os.environ.get("OPENAMER_TUI_RPC_POOL_WORKERS") or "8"))
    except (TypeError, ValueError):
        return 8


def run_host(stdin: Any = None, stdout: Any = None) -> None:
    os.environ["OPENAMER_COMPUTE_HOST_CHILD"] = "1"
