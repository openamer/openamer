...
load_openamer_dotenv(
    openamer_home=_openamer_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.openamer/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_openamer_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("OPENAMER_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("OPENAMER_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("OPENAMER_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
class _SlashWorker:
    """Persistent OpenAmerCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the OpenAmer agent → needs provider credentials.
        ...
        if profile_home:
            ...
            # gateway's launch OPENAMER_HOME (#40677).
            env["OPENAMER_HOME"] = str(profile_home)
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from openamer_cli.active_sessions import transfer_active_session
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `openamer --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("OPENAMER_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `openamer --tui`, so it must NOT count as dead here (doing
...
def _max_live_sessions() -> int:
    try:
        from openamer_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its OPENAMER_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_openamer_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s OPENAMER_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_openamer_home``).
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``openamer dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("OPENAMER_COMPUTE_HOST_CHILD") == "1"
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `openamer` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # OPENAMER_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_openamer_home_override(profile_home)
                try:
                    from openamer_state import SessionDB
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from openamer_cli.runtime_provider import canonical_custom_identity
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from openamer_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    overlay; it does not deep-merge ``openamer_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _openamer_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_openamer_home_override()
        home = override if isinstance(override, str) and override else _openamer_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    openamer_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _save_cfg(cfg: dict):
    ...
    from openamer_cli.config import atomic_config_write

    path = _openamer_home / "config.yaml"
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read OPENAMER_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # OPENAMER_SESSION_ID even though agent_init set it via
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["OPENAMER_EXEC_ASK"] = "1"
...
def resolve_skin() -> dict:
    try:
        from openamer_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``openamer config set display.skin``) OR edited the active skin's colors in
...
def _ensure_skin_watcher() -> None:
    ...
    OpenAmer activates (``openamer config set display.skin``) or recolors goes live on
    ...
    threading.Thread(target=_loop, name="openamer-skin-watcher", daemon=True).start()


def _resolve_model() -> str:
    env = (
        os.environ.get("OPENAMER_MODEL", "")
        or os.environ.get("OPENAMER_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from openamer_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``OPENAMER_DESKTOP=1`` and ``OPENAMER_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``OPENAMER_DESKTOP_TERMINAL=1`` → "tui"
        (``openamer --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``openamer --tui``.)
    """
    if is_truthy_value(os.environ.get("OPENAMER_DESKTOP")) and not is_truthy_value(
        os.environ.get("OPENAMER_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads OPENAMER_MODEL /
    OPENAMER_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`openamer --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads OPENAMER_MODEL /
    # OPENAMER_INFERENCE_MODEL, which `openamer --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("OPENAMER_TUI_PROVIDER", "").strip()
    ...
    try:
        from openamer_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("OPENAMER_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _runtime_model_config(agent, existing: dict | None = None) -> dict:
    ...
    if provider:
        if provider.strip().lower() == "custom":
            ...
            try:
                from openamer_cli.runtime_provider import (
...
def _load_approval_mode() -> str:
    from openamer_cli.config import DEFAULT_CONFIG, _deep_merge
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`openamer_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("OPENAMER_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("OPENAMER_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base OpenAmer): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `openamer --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] OPENAMER_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from openamer_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown OPENAMER_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in OPENAMER_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid OPENAMER_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The desktop Project tools are off _OPENAMER_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off openamer-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid OPENAMER_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    cfg = None
    try:
        from openamer_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (OPENAMER_MODEL /
    # OPENAMER_INFERENCE_MODEL / OPENAMER_TUI_PROVIDER / OPENAMER_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `openamer --tui -m`
...
def _get_usage(agent) -> dict:
    ...
    # OPENAMER_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("OPENAMER_DEV_CREDITS")):
        try:
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from openamer_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from openamer_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from openamer_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from openamer_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from openamer_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("OPENAMER_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("OPENAMER_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``OpenAmerCLI.__init__`` and ``gateway/run.py``:
    ...
    try:
        ...
        for entry in fb_chain:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from openamer_cli.fallback_config import resolve_entry_api_key
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses openamer_cli.mcp_startup; TUI stdio keeps
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("OPENAMER_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("OPENAMER_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("OPENAMER_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("OPENAMER_IGNORE_RULES")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped OPENAMER_HOME for app-global remote mode; None =
...
def _session_home(session: dict) -> Path:
    """The OPENAMER_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
    return Path(profile_home) if profile_home else Path(_openamer_home)
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind OPENAMER_HOME. None/own profile → launch (unchanged).
    profile = (params.get("profile") or "").strip() or None
    profile_home = _profile_home(profile)
...
def _(rid, params: dict) -> dict:
    with _profile_db(params) as db:
        ...
        try:
            ...
            # custom `OPENAMER_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_openamer_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_openamer_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_openamer_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds OPENAMER_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``openamer --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``openamer gateway`` process, whose
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    OpenAmer has to support both the legacy 8-row petdex atlas and the current
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from openamer_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from openamer_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from openamer_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from openamer_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from openamer_cli.pets import set_pet_scale
...
def _pet_gen_root():
    ...
    root = get_openamer_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("OPENAMER_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the OpenAmer profile home
    # (~/.openamer/sessions/saved/) rather than the project/workspace CWD, and
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        home_token = (
            set_openamer_home_override(parent_home) if parent_home else None
        )
        try:
...
#
# Layout:  $OPENAMER_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_openamer_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _(rid, params: dict) -> dict:
    from openamer_cli.input_sanitize import sanitize_user_prompt_text
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``OPENAMER_UI_SESSION_ID`` as
...
def _run_prompt_submit(
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn OPENAMER_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_openamer_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from openamer_cli.config import load_config as _tui_load_config
            ...
            # billing_url, is_openamer, message) so the TUI/desktop render a
            ...
            if (
                ...
                try:
                    from openamer_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: openamer_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from openamer_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _openamer_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".openamer" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.openamer/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.openamer/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not OpenAmer Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the OpenAmer/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not OpenAmer Desktop. If it serves OpenAmer/Desktop UI or another unrelated app, stop that process and report failure.",
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from openamer_cli.model_switch import parse_model_flags_detailed
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from openamer_cli.models import resolve_fast_mode_overrides
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("OPENAMER_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under OPENAMER_HOME (~/.openamer by default) — config/sessions/skills,
    ...
    openamer_home = os.path.realpath(str(get_openamer_home()))

    return real == home or real == openamer_home or real.startswith(openamer_home + os.sep)
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    OPENAMER_HOME may be an intentional prose/data workspace. The pre-Projects
    ...
    openamer_home = os.path.normcase(os.path.realpath(str(get_openamer_home())))
    return real == home or real == openamer_home
...
def _discover_repos_payload(
    ...
    repos even with zero openamer sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (openamer home subtree + bare
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from openamer_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_openamer_home), "display": display_openamer_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _openamer_home / "config.yaml"
        try:
...
def _(rid, params: dict) -> dict:
    try:
        from openamer_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from openamer_cli.runtime_provider import resolve_runtime_provider
        from openamer_cli.auth import has_usable_secret
        from openamer_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No OpenAmer provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.openamer/.env`` into the gateway process via
    ``openamer_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from openamer_cli.commands import (
            COMMAND_REGISTRY,
            SUBCOMMANDS,
            _build_description,
        )
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `openamer` is interactive — use `/openamer chat -q …` or run `openamer` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`openamer setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`openamer gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`openamer sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`openamer config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m openamer_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "openamer_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m openamer_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from openamer_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from openamer_cli.commands import SlashCommandCompleter
...
def _model_picker_context(agent):
    ...
    from openamer_cli.inventory import load_picker_context
...
def _(rid, params: dict) -> dict:
    try:
        from openamer_cli.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from openamer_cli.config import is_managed
        from openamer_cli.inventory import build_models_payload
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `openamer model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.openamer/.env via the unified credential lifecycle
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from openamer_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "OpenAmer" if role == "assistant" else role.title()
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("OPENAMER_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("OPENAMER_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["OPENAMER_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from openamer_cli.voice import stop_continuous
            ...
            os.environ["OPENAMER_VOICE_TTS"] = "0"
    ...
    if action == "tts":
        ...
        os.environ["OPENAMER_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from openamer_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from openamer_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from openamer_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("OPENAMER_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("OPENAMER_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_openamer_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``openamer journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from openamer_cli.skills_hub import do_install
        ...
        if action == "browse":
            from openamer_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from openamer_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``openamer plugins`` / the dashboard, so the three surfaces
