...
OpenAmer Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``OPENAMER_NIX_BUILD=1`` on the
...
_IN_NIX_BUILD = os.environ.get("OPENAMER_NIX_BUILD") == "1"
...
_BLOCK_MESSAGE = (
    ...
    "the OpenAmer Nix derivation sets OPENAMER_NIX_BUILD=1. If it does, file a bug."
)
