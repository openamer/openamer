...
When running OpenAmer inside Docker, the default `network_mode: host` gives the
...
The OpenAmer [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ openamer-agent │   │ openamer-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   OPENAMER_UID=$(id -u) OPENAMER_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=openamer,openamer-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://openamer-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — OpenAmer trust model and vulnerability reporting
