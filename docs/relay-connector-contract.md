...
This document is the formal interface between the **OpenAmer gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`openamer/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `openamer gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`openamer gateway enroll` → connector `/relay/enroll`, or
...
(`openamer/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `openamer` CLI.
