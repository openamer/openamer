...
OpenAmer supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.openamer/config.yaml`:
...
Or set the env var: `OPENAMER_KANBAN_DISPATCH_IN_GATEWAY=false`
