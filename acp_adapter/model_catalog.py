"""ACP model picker: deduplicated ``provider:model`` rows from the OpenAmer inventory + named endpoints."""
...
def _choice_provider(model_id: str) -> str:
    ...
    if parts[:1] == ["custom"] and len(parts) > 1:
        from openamer_cli.models import _configured_custom_provider_ids
