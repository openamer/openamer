...
from acp_adapter.commands import OPENAMER_VERSION, SlashCommandsMixin, _estimate_tokens
...
from tools.approval_context import reset_openamer_interactive_context, set_openamer_interactive_context
...
class OpenAmerACPAgent(SlashCommandsMixin, acp.Agent):
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        """Authenticated providers + models, from the shared OpenAmer inventory (same substrate
        as ``openamer model``/TUI/dashboard) so the selector isn't just the current curated list."""
    ...
    def _switch_model(
        ...
        Resolution goes through ``openamer_cli.model_switch.switch_model`` seeded with the live
    ...
    def _provenance_meta(
        self, acp_session_id: str, current_openamer_session_id: str, previous_openamer_session_id: Optional[str] = None
    ) -> Optional[dict]:
        """Best-effort ``_meta.openamer.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(), acp_session_id, current_openamer_session_id,
    ...
    async def _send_session_info_update(
        self, session_id: str, *,
        current_openamer_session_id: Optional[str] = None, previous_openamer_session_id: Optional[str] = None,
    ) -> None:
        """Session metadata update; pass ``previous_openamer_session_id`` when the internal head
        ...
        update = SessionInfoUpdate(
            ...
            field_meta=self._provenance_meta(
                session_id, current_openamer_session_id or session_id, previous_openamer_session_id
            ),
        )
        await self._send(
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            agent.enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(agent, "enabled_toolsets", None) or ["openamer-acp"],
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        try:
            from openamer_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from openamer_cli.mcp_startup import join_mcp_discovery
    ...
    async def initialize(
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="openamer-agent", version=OPENAMER_VERSION),
    ...
    def _run_agent_turn(
        ...
        Interactive routing is a ``tools.approval`` contextvar, not ``OPENAMER_INTERACTIVE`` in
        ...
        with contextlib.ExitStack() as stack:
            # OPENAMER_SESSION_KEY scopes per-session caches (interactive sudo password) to this
            # session, not the reused thread. ``cwd`` pins what the system prompt reports as the
            # working directory — otherwise it advertises the OpenAmer workspace while tools are
            # rooted at the client's project and edits land outside it. ``cron_session=""`` masks
            # any leaked process-global OPENAMER_CRON_SESSION.
            ...
            stack.callback(reset_openamer_interactive_context, set_openamer_interactive_context(True))
    ...
    async def prompt(self, prompt: list[PromptBlock], session_id: str, **kwargs: Any) -> PromptResponse:
        ...
        return await self._finish_turn(state, session_id, conn, result, pre_turn_openamer_id, cbs.streamed)
    ...
    async def _finish_turn(
        self, state: SessionState, session_id: str, conn: Any, result: dict, pre_turn_openamer_id: Any,
        ...
        if conn and post_turn_openamer_id and pre_turn_openamer_id and post_turn_openamer_id != pre_turn_openamer_id:
            try:
                await self._send_session_info_update(
                    session_id, current_openamer_session_id=post_turn_openamer_id,
...
def __getattr__(name):  # PEP 562 — lazy so no import cycles
    ...
    import importlib
    from openamer_cli.plugin_compat import warn_once
