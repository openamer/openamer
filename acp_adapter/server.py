...
class OpenAmerACPAgent(acp.Agent):
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # os.environ["OPENAMER_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue OPENAMER_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind OPENAMER_SESSION_KEY for this session so per-session caches
            ...
            previous_session_id = os.environ.get("OPENAMER_SESSION_ID")
            ...
            try:
                ...
                # Restore OPENAMER_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("OPENAMER_SESSION_ID", None)
                else:
                    os.environ["OPENAMER_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (OPENAMER_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"OpenAmer Agent v{OPENAMER_VERSION}"
