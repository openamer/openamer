"""CLI entry point for the openamer-agent ACP adapter.

Loads ``~/.openamer/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: openamer acp / openamer-acp
"""
...
try:
    ...
    # Partial ``openamer update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing OpenAmer modules.
    openamer_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from OPENAMER_HOME (default ``~/.openamer``)."""
    from openamer_cli.env_loader import load_openamer_dotenv

    openamer_home = get_openamer_home()
    loaded = load_openamer_dotenv(openamer_home=openamer_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", openamer_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="openamer-acp", description="Run OpenAmer Agent as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print OpenAmer version and exit")
    parser.add_argument("--check", action="store_true", help="Verify ACP dependencies and adapter imports, then exit")
    parser.add_argument("--setup", action="store_true",
                        help="Run interactive OpenAmer provider/model setup for ACP terminal auth")
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.openamer/node/ "
...
def _print_version() -> None:
    ...
    print(openamer_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import OpenAmerACPAgent  # noqa: F401

    print("OpenAmer ACP check OK")


def _run_setup() -> None:
    from openamer_cli.main import main as openamer_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "openamer", "model"]
        openamer_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting openamer-agent ACP adapter")
    ...
    import acp
    from .server import OpenAmerACPAgent
