...
This is an additive OpenAmer extension surfaced under ACP ``_meta.openamer`` so
...
compression rotates the internal OpenAmer head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    Args:
        ...
        current_openamer_session_id: The live internal OpenAmer DB session id
            (``state.agent.session_id``).
        previous_openamer_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"openamer": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_openamer_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_openamer_session_id
    compression_depth = 0
    ...
    rotated = bool(
        ...
        and previous_openamer_session_id != current_openamer_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentOpenAmerSessionId": current_openamer_session_id,
        "rootOpenAmerSessionId": root_id,
        "parentOpenAmerSessionId": parent_id,
...
def session_provenance_meta(
    ...
    *,
    ...
    if prov is None:
        return None
    return {"openamer": {"sessionProvenance": prov}}
