...
Additive OpenAmer extension under ACP ``_meta.openamer`` (unknown to other clients,
...
rotates the internal OpenAmer head, ``build_session_provenance`` exposes the
...
def build_session_provenance(
    db: Any, acp_session_id: str, current_openamer_session_id: str, *, previous_openamer_session_id: Optional[str] = None,
) -> Optional[Dict[str, Any]]:
    """Build ``_meta.openamer.sessionProvenance`` for an ACP session.

    ``db`` must expose ``get_session``. ``current_openamer_session_id`` is the live
    internal id (``state.agent.session_id``); ``previous_openamer_session_id`` is
    the id before the most recent turn, supplied by ``prompt()`` to flag a
    rotation. Returns ``None`` if the session can't be read."""
    row = _get_row(db, current_openamer_session_id)
    if not row:
        return None
    ...
    root_id, compression_depth, cursor_parent = current_openamer_session_id, 0, parent_id
    seen = {current_openamer_session_id}
    ...
    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id, "currentOpenAmerSessionId": current_openamer_session_id,
        "rootOpenAmerSessionId": root_id, "parentOpenAmerSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root", "compressionDepth": compression_depth,
    }
    if previous_openamer_session_id:
        provenance["previousOpenAmerSessionId"] = previous_openamer_session_id
        if previous_openamer_session_id != current_openamer_session_id:
...
def session_provenance_meta(
    ...
    """Return a ready ``_meta`` payload: ``{"openamer": {"sessionProvenance": ...}}``."""
    prov = build_session_provenance(db, acp_session_id, current_openamer_session_id,
                                    previous_openamer_session_id=previous_openamer_session_id)
    return None if prov is None else {"openamer": {"sessionProvenance": prov}}
