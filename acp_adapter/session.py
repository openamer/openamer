...
from openamer_constants import get_openamer_home, translate_cwd_for_wsl_backend, windows_path_to_wsl
...
def _translate_acp_cwd(cwd: str) -> str:
    ...
    when OpenAmer runs in WSL so agents, tools, and persisted sessions agree; no-op elsewhere."""
...
def _expand_acp_enabled_toolsets(toolsets: List[str] | None = None,
    ...
    names = [n for n in (toolsets or ["openamer-acp"]) if n]
...
class SessionManager:
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        the runtime provider config. ``db``: SessionDB; default lazily opens ``~/.openamer/state.db``."""
    ...
    def _get_db(self):
        ...
        error in a minimal test env). ``OPENAMER_HOME`` is resolved here, not via the import-time
        ...
        if self._db_instance is None:
            try:
                from openamer_state_registry import acquire
                self._db_instance = acquire(get_openamer_home() / "state.db")
            except Exception:
    ...
    def _make_agent(self, *, session_id: str, cwd: str, model: str | None = None,
        ...
        kwargs = {
            ...
            "enabled_toolsets": _expand_acp_enabled_toolsets(["openamer-acp"], mcp_server_names=configured_mcp_servers),
        ...
        # picked up by OpenAmerACPAgent._schedule_mcp_late_refresh.
        try:
            from openamer_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
