"""ACP auth helpers — detect and advertise OpenAmer authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "openamer-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active OpenAmer runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for OpenAmer.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no OpenAmer credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate OpenAmer using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure OpenAmer provider", type="terminal", args=["--setup"],
        description=("Open OpenAmer' interactive model/provider setup in a terminal. "
...
def has_provider() -> bool:
    """Return True if OpenAmer can resolve any runtime provider credentials."""
