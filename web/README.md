# OpenAmer Agent — Web UI

Browser-based dashboard for managing OpenAmer Agent configuration, API keys, and monitoring active sessions.
...
cd ../
python -m openamer_cli.main web --no-open
...
`openamer dashboard` on port 9119 serves the **built** bundle from `openamer_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
This outputs to `../openamer_cli/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “OpenAmer Agent” — mixed case, no Mondwest, no `text-display` |
