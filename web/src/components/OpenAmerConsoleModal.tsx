...
interface OpenAmerConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function OpenAmerConsoleModal({ open, onClose }: OpenAmerConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("openamer> ");
  const inputPromptRef = useRef("openamer> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "openamer> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to OpenAmer Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="openamer-console-title"
    >
      <div
        className={cn(
          themedBody,
