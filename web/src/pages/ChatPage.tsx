/**
 * ChatPage — embeds `openamer --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (openamer_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "openamer.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__OPENAMER_SESSION_TOKEN__ &&
    !window.__OPENAMER_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `openamer dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__OPENAMER_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner OpenAmer TUI has enabled xterm mouse-events
    ...
    //      payloads upload to OPENAMER_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to OPENAMER_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[openamer-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__OPENAMER_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets OPENAMER_HOME pointed at the
    ...
    // forwarding them into OpenAmer. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="openamer-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
