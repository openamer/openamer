...
import { SelectionSwitcher } from "@openamer-research/ui/ui/components/selection-switcher";
import { Spinner } from "@openamer-research/ui/ui/components/spinner";
import { Typography } from "@openamer-research/ui/ui/components/typography/index";
import { ConfirmDialog } from "@openamer-research/ui/ui/components/confirm-dialog";
import { cn } from "@/lib/utils";
import { SidebarFooter } from "@/components/SidebarFooter";
import { SidebarStatusStrip, gatewayLine } from "@/components/SidebarStatusStrip";
import { useBelowBreakpoint } from "@openamer-research/ui/hooks/use-below-breakpoint";
...
const SIDEBAR_COLLAPSED_KEY = "openamer-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="openamer-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="openamer-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateOpenAmer = status?.can_update_openamer === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'openamer update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "openamer update";
    return (
      t.status.updateOpenAmerConfirmMessage ??
      `This will run 'openamer update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateOpenAmerConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateOpenAmer) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateOpenAmer,
      runningLabel: t.status.updatingOpenAmer,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the OpenAmer gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateOpenAmerConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateOpenAmerConfirmTitle ?? `${t.status.updateOpenAmer}?`}
    />
    </>
  );
}
