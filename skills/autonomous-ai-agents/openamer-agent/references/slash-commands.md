...
Registry of record: `openamer_cli/commands.py` (`COMMAND_REGISTRY`) — every
...
/snapshot [sub]          Create/restore OpenAmer config+state snapshots (CLI)
...
/subscription (/upgrade) a hosted provider plan info (CLI)
/topup                   a hosted provider balance / billing
