...
Author widget apps for the OpenAmer TUI (`openamer --tui`): glanceable ambient
...
- The TUI must be in use (`openamer --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.openamer/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.openamer/logs/tui_gateway_crash.log` if a widget never appears.
