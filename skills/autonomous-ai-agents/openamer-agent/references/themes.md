# Themes / Skins — Author a OpenAmer Color Theme

Author a OpenAmer **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`openamer_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.openamer/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for OpenAmer itself.
...
- Write access to the OpenAmer home dir — `~/.openamer` by default, or `$OPENAMER_HOME`
  / the active profile's dir. Skins live in `<openamer-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `openamer config set`).
...
3. `write_file` it to `<openamer-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `openamer config set display.skin default` (or they can `/skin default`).
...
```
openamer skin set <key> <hex>      # e.g. openamer skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `openamer skin set background "#08201f"` changes
only the background; `openamer skin use <name>` / `openamer skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.openamer`** when a profile is active — resolve the real home
  from `$OPENAMER_HOME` first, falling back to `~/.openamer`.
...
- **You apply it, not the user.** `openamer config set display.skin <name>` is
...
- `read_file` the written `<openamer-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `openamer config get display.skin` and confirm it reports `<name>`.
