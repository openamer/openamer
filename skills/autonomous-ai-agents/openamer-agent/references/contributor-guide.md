...
Config: `~/.openamer/config.yaml` (settings), `~/.openamer/.env` (API keys) — both under `$OPENAMER_HOME` when it is set.
...
`_OPENAMER_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_openamer_home()` for paths,
never hardcode `~/.openamer`. For custom/local-only tools, write a plugin in
`~/.openamer/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `openamer_cli/commands.py`
...
- Tests auto-redirect `OPENAMER_HOME` to temp dirs — never touch real `~/.openamer/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/openamer_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_openamer_home()` from `openamer_constants` for all paths (profile-safe)
