...
Write plugins for the OpenAmer desktop app: statusbar items, layout panes,
...
system (`~/.openamer/plugins/`) is otherwise documented separately.
...
The ONLY import surface is `@openamer/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.openamer/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@openamer/plugin-sdk`, `react`, and
