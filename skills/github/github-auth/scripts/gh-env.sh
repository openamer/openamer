#!/usr/bin/env bash
# GitHub environment detection helper for OpenAmer Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _openamer_env="${OPENAMER_HOME:-$HOME/.openamer}/.env"; [ -f "$_openamer_env" ] && grep -q "^GITHUB_TOKEN=" "$_openamer_env" 2>/dev/null; then
