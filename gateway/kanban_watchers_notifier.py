...
def _kbn():
    from openamer_cli import kanban_db_notify
...
def _warn_anchorless_thread_sub_once(sub: dict, platform: str) -> None:
    ...
    logger.warning(
        ...
        "profile route; it will not be delivered. Re-subscribe with `openamer kanban notify-subscribe ... "
...
class _Collector:
    ...
    def collect(self) -> list[dict]:
        ...
        # OPENAMER_KANBAN_DB pins the board path.
        kb = self.kb
...
def _fmt_gave_up(ev, n) -> tuple:
    ...
    return (
        f"⛔ {n.head} is now blocked: {count}{last}. Fix the cause, then `openamer kanban unblock "
        f"{n.task_id}` (or `openamer kanban reassign {n.task_id}`). Logs: `openamer kanban log {n.task_id}`.",
        None, None,
    )
