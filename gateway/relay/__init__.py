"""Relay/connector support package for the OpenAmer gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``openamer gateway enroll`` writes them to
    ``~/.openamer/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``OPENAMER_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``openamer gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        ...
        # multi-agent scope it would prefix every reply "**OpenAmer Agent:**" —
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.openamer/.env``.
    ...
    sets neither ``OPENAMER_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'openamer'}"
