...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``openamer setup`` display).
    ...
    # built-ins).  Used by ``openamer gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `openamer`
        # invocation -- including plain `openamer chat`, which never touches any
        ...
        # `openamer setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``openamer setup``/``gateway status``, channel
