...
``<OPENAMER_HOME>/state/gateway.lifecycle.json`` closes the gap:
...
def _process_openamer_home() -> Path:
    """OPENAMER_HOME for process-level identity files (ignore task overrides)."""
    from openamer_constants import get_openamer_home

    val = os.environ.get("OPENAMER_HOME", "").strip()
    return Path(val) if val else get_openamer_home()


def _home_path(home: Optional[Path], *relative: str) -> Path:
    return (_process_openamer_home() if home is None else home).joinpath(*relative)


def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<OPENAMER_HOME>/state/gateway.lifecycle.json``."""
...
def _pid_is_sentinel_owner(pid: Any, start_time: Any, create_time: Any) -> bool:
    ...
    try:
        ...
        return False
    from openamer_cli.process_identity import _process_create_time
...
def _report_unclean_exit(evidence: Dict[str, Any], home: Optional[Path]) -> None:
    ...
    if verdict not in ("ok", "absent"):
        logger.error(
            ...
            "missing until it is repaired. Run `openamer doctor`.",
            verdict,
        )
