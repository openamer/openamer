...
def _kbc():
    from openamer_cli import kanban_db_connect
    return kanban_db_connect


def _kbd():
    from openamer_cli import kanban_db_dispatch
...
def _default_profile_secret_scope():
    ...
    LLM reads ``auxiliary.*`` from ``get_openamer_home()``, so its credentials come
    ...
    if not is_multiplex_active():
        yield
        return
    token = set_secret_scope(build_profile_secret_scope(Path(get_openamer_home())))
    try:
        yield
    finally:
