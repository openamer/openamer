    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if openamer_home_str:
            takeover_path = Path(openamer_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(openamer_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded openamer-agent but never re-ran ``openamer setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../openamer-gateway.service"
    ...
    # that's the common case for openamer.
    ...
    # constant in openamer_cli/gateway.py.
    headroom = 30.0
