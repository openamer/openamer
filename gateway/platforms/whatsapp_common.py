...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *OpenAmer Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to OPENAMER_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/openamer), this function
    mirrors the bridge source to a writable OPENAMER_HOME location and returns that
    ...
    # Try OPENAMER_HOME location first
    openamer_home = get_openamer_home()
    openamer_home_bridge = openamer_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to OPENAMER_HOME if needed
    if openamer_home_bridge.exists():
        return openamer_home_bridge

    # Mirror the bridge source to OPENAMER_HOME
    try:
        openamer_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            openamer_home_bridge,
            dirs_exist_ok=False,
        )
        return openamer_home_bridge
    except Exception:
