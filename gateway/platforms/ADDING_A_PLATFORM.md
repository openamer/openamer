...
There are two ways to add a platform to the OpenAmer gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.openamer/plugins/` (or under `plugins/platforms/`
...
**zero changes to core OpenAmer code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `openamer gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `openamer_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the OpenAmer core.
...
And add it to the `openamer-gateway` composite:

```python
"openamer-gateway": {
    "includes": [..., "openamer-your-platform"]
}
```
...
## 12. Status Display (`openamer_cli/status.py`)
...
## 13. Gateway Setup Wizard (`openamer_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ openamer_cli/ toolsets.py \
  --include="*.py" -l | sort -u
