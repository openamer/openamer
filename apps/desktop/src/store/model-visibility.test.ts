...
describe('model visibility', () => {
  ...
  it('preserves hidden-provider sentinel without re-adding defaults', () => {
    // User explicitly hid all models for "openamer" — sentinel marks this choice.
    const stored = new Set([emptyProviderSentinelKey('openamer')])

    const visible = effectiveVisibleKeys(stored, [
      provider('openamer', ['openamer-3-llama-3.1-70b', 'openamer-3-llama-3.1-8b']),
      provider('ollama', ['qwen3:latest'])
    ])

    expect(visible.has(modelVisibilityKey('openamer', 'openamer-3-llama-3.1-70b'))).toBe(false)
    expect(visible.has(modelVisibilityKey('openamer', 'openamer-3-llama-3.1-8b'))).toBe(false)
    // Sentinel itself is stripped from the result.
    expect(visible.has(emptyProviderSentinelKey('openamer'))).toBe(false)
  ...
  it('restores model when toggling on after hiding all', () => {
    // Simulates: user hid all "openamer" models, then toggles one back on.
    const stored = new Set([emptyProviderSentinelKey('openamer'), modelVisibilityKey('ollama', 'qwen3:latest')])

    // After toggle: sentinel removed, one model added.
    const afterToggle = new Set(stored)
    afterToggle.delete(emptyProviderSentinelKey('openamer'))
    afterToggle.add(modelVisibilityKey('openamer', 'openamer-3-llama-3.1-70b'))
    ...
    expect(visible.has(modelVisibilityKey('openamer', 'openamer-3-llama-3.1-70b'))).toBe(true)
    expect(visible.has(modelVisibilityKey('openamer', 'openamer-3-llama-3.1-8b'))).toBe(false)
  })
  ...
  it('resolveVisibleKeys preserves sentinels that effectiveVisibleKeys strips', () => {
    const stored = new Set([emptyProviderSentinelKey('openamer')])
    const providers = [provider('openamer', ['openamer-x', 'openamer-y']), provider('ollama', ['qwen3:latest'])]

    const resolved = resolveVisibleKeys(stored, providers)
    expect(resolved.has(emptyProviderSentinelKey('openamer'))).toBe(true)
    expect(resolved.has(modelVisibilityKey('openamer', 'openamer-x'))).toBe(false)
    ...
    expect(effectiveVisibleKeys(stored, providers).has(emptyProviderSentinelKey('openamer'))).toBe(false)
  })
})

describe('toggleModelVisibility', () => {
  const providers = [provider('openai', ['gpt-a', 'gpt-b']), provider('openamer', ['openamer-x', 'openamer-y'])]
  ...
  it('keeps a hidden provider hidden when a different provider is toggled (regression for #43485)', () => {
    // Hide ALL of openamer — its sentinel is now stored.
    let stored: Set<string> | null = null
    stored = apply(stored, 'openamer', 'openamer-x')
    stored = apply(stored, 'openamer', 'openamer-y')
    expect(stored.has(emptyProviderSentinelKey('openamer'))).toBe(true)

    // Toggle a model in another provider. openamer must NOT snap back on.
    stored = apply(stored, 'openai', 'gpt-a')

    expect(stored.has(emptyProviderSentinelKey('openamer'))).toBe(true)
    const visible = effectiveVisibleKeys(stored, providers)
    expect(visible.has(modelVisibilityKey('openamer', 'openamer-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('openamer', 'openamer-y'))).toBe(false)
  })

  it('clears only the toggled provider sentinel when a model is re-enabled', () => {
    let stored: Set<string> | null = new Set([emptyProviderSentinelKey('openai'), emptyProviderSentinelKey('openamer')])
  ...
  it('re-enabling one model of a hidden-all provider restores ONLY that model, not the curated defaults', () => {
    // openai hidden-all, openamer untouched.
  ...
  it('toggling from an empty (non-null) stored set adds the model without expanding defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('openamer', 'openamer-x'))).toBe(false)
  })
  ...
  it('toggling off one default model from null stored keeps the rest of the curated defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('openamer', 'openamer-x'))).toBe(true)
