 ...
 * app's bounds and stays visible while OpenAmer is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'openamer.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'openamer.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.openamerDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.openamerDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.openamerDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.openamerDesktop?.petOverlay?.close()
}
