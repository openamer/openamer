...
//
// Scope is always "uncommitted" — OpenAmer' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'openamer.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'openamer.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'openamer.desktop.reviewTreeMode'
const SELECTED_KEY = 'openamer.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<OpenAmerReviewFile[]>([])
...
export const $reviewShipInfo = atom<OpenAmerReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['openamerDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: OpenAmerReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.openamerDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.openamerDesktop?.openExternal?.(url)
    }
