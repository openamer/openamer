...
const STORAGE_KEY = 'openamer:native-notifications'
...
// "Backgrounded" = the user isn't on OpenAmer. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.openamerDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.openamerDesktop
