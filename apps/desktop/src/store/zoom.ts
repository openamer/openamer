...
export function setZoomPercent(percent: number): void {
  window.openamerDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.openamerDesktop?.zoom) {
  void window.openamerDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.openamerDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
