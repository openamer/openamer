...
import { getOpenAmerConfigRecord, saveOpenAmerConfig } from '@/openamer'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getOpenAmerConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveOpenAmerConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
