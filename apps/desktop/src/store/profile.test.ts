...
const { getProfiles } = await import('@/openamer')
...
const getConnection = vi.fn<(profile?: string | null) => Promise<OpenAmerConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { openamerDesktop: { getConnection } })
