...
import type { OpenAmerGitBaseBranch, OpenAmerGitBranch } from '@/global'
import { translateNow } from '@/i18n'
import { desktopDefaultCwd, isDesktopFsRemoteMode, selectDesktopPaths, writeDesktopFileText } from '@/lib/desktop-fs'
import { desktopGit } from '@/lib/desktop-git'
import { isMissingRpcMethod } from '@/lib/gateway-rpc'
import { persistentAtom } from '@/lib/persisted'
import { getOpenAmerConfig, type OpenAmerGateway } from '@/openamer'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/openamer'
...
const PROJECT_SCOPE_KEY = 'openamer.desktop.projectScope'
...
export function projectNameForCwd(cwd: string, tree: readonly SidebarProjectTree[] = $projectTree.get()): null | string {
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('OpenAmer gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active OpenAmer profile changed while connecting')
  }
...
async function refreshProjectTreeOn(gateway: OpenAmerGateway): Promise<void> {
...
const repoScanStates = new WeakMap<OpenAmerGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<OpenAmerGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getOpenAmerConfig(context.profile))
...
// repo, returning where OpenAmer should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<OpenAmerGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<OpenAmerGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.openamerDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.openamerDesktop?.writeClipboard?.(path)
  }
}
