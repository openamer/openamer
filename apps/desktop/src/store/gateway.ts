import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@openamer/shared'
...
const isOpen = (gateway: OpenAmerGateway | null): boolean => gateway?.connectionState === 'open'
...
interface GatewayRegistryState {
  ...
  primaryGateway: OpenAmerGateway | null
  primaryProfile: string
  activeKey: string
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<OpenAmerGateway | null>>
}

const STATE_KEY = Symbol.for('openamer.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<OpenAmerGateway | null>(null)
  }
}
...
export function setPrimaryGateway(gateway: OpenAmerGateway | null, profile = 'default'): void {
...
export function activeGateway(): OpenAmerGateway | null {
...
function createSecondary(profile: string): Secondary {
  const gateway = new OpenAmerGateway()
...
export async function ensureActiveGatewayOpen(): Promise<OpenAmerGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
