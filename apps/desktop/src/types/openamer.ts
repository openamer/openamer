...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when OpenAmer doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in openamer_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `openamer model` uses — instead of
...
export interface OpenAmerConfig {
  agent?: {
...
export type OpenAmerConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `openamer model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors openamer_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with OpenAmer, 'hub' = installed. */
 ...
 *  not OpenAmer); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/openamer/update/check` — the backend's own update state.
