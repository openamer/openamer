...
import { getOpenAmerConfigRecord, type OpenAmerConfigRecord, saveOpenAmerConfig } from '@/openamer'
...
export interface I18nConfigClient {
  getConfig: () => Promise<OpenAmerConfigRecord>
  saveConfig: (config: OpenAmerConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.openamerDesktop?.api) {
      return Promise.resolve({})
    }

    return getOpenAmerConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.openamerDesktop?.api) {
    ...
    return saveOpenAmerConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: OpenAmerConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: OpenAmerConfigRecord, locale: Locale): OpenAmerConfigRecord {
