/**
 * Live skin sync from the OpenAmer backend.
 *
 * The backend resolves the active skin (built-in or `$OPENAMER_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (OpenAmer
 ...
 */
export function ingestBackendSkin(skin: OpenAmerSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  // (openamer), so we never register a converted theme under `default`. It is still a
  // valid apply TARGET though: a runtime switch back to `default` must repaint the
  // desktop to its own default (setTheme normalizes `default` → openamer). So we only
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as OpenAmerSkin)
