...
import { BUILTIN_THEME_LIST, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, openamerTheme } from './presets'
...
const SKIN_KEY = 'openamer-desktop-theme-v2'
const MODE_KEY = 'openamer-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'openamer-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'openamer-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'openamer-desktop-active-profile-v1'
const RETIRED_SKINS = new Set(['openamer-light', 'default', 'gold'])
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? openamerTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...openamerTheme.typography, ...theme.typography }
  ...
  root.dataset.openamerTheme = skinName
  root.dataset.openamerMode = rendered
  ...
  window.openamerDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('openamer-boot-background', chromeBg)
    window.localStorage.setItem('openamer-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.openamerThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.openamerDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (OpenAmer authoring/activating a skin from a
