import type { BillingBlock } from '@openamer/shared'
...
import type { RpcEvent } from '@/types/openamer'
 ...
 * billing-specific toast — never the generic "OpenAmer error" — with a smart CTA
 * (OpenAmer → in-app Settings → Billing, other providers → their billing page).
 */
...
interface GatewayEventDeps {
  ...
  refreshOpenAmerConfig: () => Promise<void>
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshOpenAmerConfig — two REST calls (config + defaults) per event, per
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        ingestBackendSkin((payload as { skin?: OpenAmerSkin } | undefined)?.skin, { apply: false })

        return
      } else if (event.type === 'skin.changed') {
        // A runtime skin switch (OpenAmer activating an authored skin, or `/skin`
        ...
        if (fromActiveProfile) {
          ingestBackendSkin(payload as OpenAmerSkin | undefined, { apply: true })
        }
        ...
        if (apply) {
          ...
          // everything refreshOpenAmerConfig applies is either active-session
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'OpenAmer reported an error',
        ...
        // native OS notification when OpenAmer is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'OpenAmer reported an error'
