...
interface OpenAmerConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useOpenAmerConfig({ activeSessionIdRef }: OpenAmerConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshOpenAmerConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        ...
        // rendering/applying OpenAmer' built-in medium over the user's config.
  ...
  return { refreshOpenAmerConfig, sttEnabled, voiceMaxRecordingSeconds }
}
