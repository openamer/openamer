...
import { getSessionMessages, triggerCronJob } from '@/openamer'
...
import { useOpenAmerConfig } from '../session/hooks/use-openamer-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshOpenAmerConfig, sttEnabled, voiceMaxRecordingSeconds } = useOpenAmerConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('openamer:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('openamer:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshOpenAmerConfig(true)
    void refreshActiveProfile()
  }, [activeGatewayProfile, refreshCurrentModel, refreshOpenAmerConfig])
