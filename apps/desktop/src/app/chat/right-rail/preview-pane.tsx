        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.openamerDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask OpenAmer to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.openamerDesktop?.watchPreviewFile ||
      !window.openamerDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.openamerDesktop.onPreviewFileChanged(payload => {
    ...
    void window.openamerDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.openamerDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.openamerDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:openamer-preview')
