...
import type { OpenAmerReadDirResult } from '@/global'
import { $connection } from '@/store/session'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<OpenAmerReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { openamerDesktop: { readDir: typeof readDir } }).openamerDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): OpenAmerReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { openamerDesktop: unknown }).openamerDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: OpenAmerReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<OpenAmerReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { openamerDesktop: unknown }).openamerDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.openamerDesktop is missing', async () => {
    delete (window as unknown as { openamerDesktop?: unknown }).openamerDesktop
