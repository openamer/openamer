...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.openamerDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: OpenAmerGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: OpenAmerConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting OpenAmer...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting OpenAmer…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to OpenAmer gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshOpenAmerConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshOpenAmerConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startOpenAmer re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new OpenAmerGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshOpenAmerConfig(),
          ...
          // retry it), not brick boot behind the "OpenAmer couldn't start"
