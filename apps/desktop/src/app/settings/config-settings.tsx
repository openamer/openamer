...
import type { ConfigFieldSchema, OpenAmerConfigRecord } from '@/types/openamer'
...
export function voiceFieldVisible(key: string, config: OpenAmerConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<OpenAmerConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useOpenAmerConfigRecord()
  ...
  const {
    ...
    queryKey: ['openamer-config-schema'],
    queryFn: getOpenAmerConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveOpenAmerConfig(config)
  ...
  const updateConfig = (next: OpenAmerConfigRecord) => {
