...
import type { ToolsetConfig } from '@/types/openamer'
...
const getOpenAmerConfigSchema = vi.fn()
...
vi.mock('@/openamer', () => ({
  ...
  getOpenAmerConfigSchema: () => getOpenAmerConfigSchema(),
...
beforeEach(() => {
  ...
  getOpenAmerConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getOpenAmerConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveOpenAmerConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveOpenAmerConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
