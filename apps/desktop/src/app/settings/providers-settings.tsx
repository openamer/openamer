...
import { disconnectOAuthProvider, listOAuthProviders } from '@/openamer'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.openamerDesktop?.terminal)
...
//      in openamer_cli/provider_catalog.py) — the SAME provider identity
//      `openamer model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // OpenAmer can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider OpenAmer can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // OpenAmer never deletes creds another tool owns behind a silent API call.
