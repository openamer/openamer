...
describe('BillingSettings', () => {
  ...
  it('rejects auto-refill amounts outside the billing bounds', async () => {
    ...
    expect(screen.getByText((content) => content.includes('minimum is') && content.includes('$'))).toBeTruthy()
  ...
  it('opens auto-refill edit without a validation error even when the saved config is below the minimum', async () => {
    ...
    expect(screen.queryByText((content) => content.includes('minimum is') && content.includes('$'))).toBeNull()
  ...
  it('disables buy controls while polling and renders the settled outcome', async () => {
    ...
    await waitFor(() => expect(screen.getByText((content) => content.includes('added') && content.includes('Balance is refreshing'))).toBeTruthy())
  })
  ...
  it('renders logged-out as a connect card without normal account rows', async () => {
    ...
    expect(await screen.findByText('Connect your OpenAmer account')).toBeTruthy()
    expect(screen.getByText('Run /portal in the TUI or open the OpenAmer portal to connect your account.')).toBeTruthy()
