...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'openamer:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.openamerDesktop?.saveImageFromUrl) {
        if (await window.openamerDesktop.saveImageFromUrl(src)) {
