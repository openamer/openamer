...
const ICON_PATHS: Record<OpenAmerRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as OpenAmerRefType] ?? ICON_FALLBACK
...
export const openamerDirectiveFormatter: Unstable_DirectiveFormatter = {
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(HERMES_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as OpenAmerRefType, id),
        id
      }
    }),
...
function shortLabel(type: OpenAmerRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...openamerDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing OpenAmer directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.openamerDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.openamerDesktop?.readFileDataUrl(id)
