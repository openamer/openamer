...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to OpenAmer gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.openamerDesktop
  Object.defineProperty(window, 'openamerDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'openamerDesktop', { configurable: true, value: original })
}
