import type { GatewayWsUrlResult } from '@openamer/shared'
...
declare global {
  interface Window {
    openamerDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<OpenAmerConnection>
      ...
      // OpenAmer Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new OPENAMER_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: OpenAmerApiRequest) => Promise<T>
      notify: (payload: OpenAmerNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<OpenAmerReadFileTextResult>
      selectPaths: (options?: OpenAmerSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<OpenAmerPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<OpenAmerPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: OpenAmerTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<OpenAmerReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<OpenAmerGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<OpenAmerGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<OpenAmerGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<OpenAmerRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: OpenAmerReviewScope, baseRef?: null | string) => Promise<OpenAmerReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: OpenAmerReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<OpenAmerReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: OpenAmerTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<OpenAmerTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: OpenAmerWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: OpenAmerPreviewFileChanged) => void) => () => void
...
export interface OpenAmerTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface OpenAmerTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `openamer update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  openamerRoot?: string
...
export interface OpenAmerConnection {
  baseUrl: string
...
export interface OpenAmerTitleBarTheme {
  background: string
  foreground: string
}

export interface OpenAmerWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a OpenAmer Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted OpenAmer Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected OpenAmer Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // Whether the OAuth partition holds a live OpenAmer portal (Privy) session — the
  // portal authenticates via Privy, so this reflects the privy-token cookie, NOT
  // the openamer gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered OpenAmer Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface OpenAmerApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface OpenAmerNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface OpenAmerPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface OpenAmerReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface OpenAmerPreviewWatch {
  id: string
  path: string
}
...
export interface OpenAmerGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface OpenAmerGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface OpenAmerGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface OpenAmerRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface OpenAmerRepoStatus {
  branch: null | string
...
// turn began.
export type OpenAmerReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface OpenAmerReviewFile {
  path: string
  added: number
  removed: number
...
export interface OpenAmerReviewList {
  files: OpenAmerReviewFile[]
...
export interface OpenAmerReviewPr {
  url: string
  state: string
  number: number
}
...
export interface OpenAmerReviewShipInfo {
  ghReady: boolean
  pr: OpenAmerReviewPr | null
}

export interface OpenAmerReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface OpenAmerReadDirResult {
  entries: OpenAmerReadDirEntry[]
  error?: string
}

export interface OpenAmerPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface OpenAmerSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
