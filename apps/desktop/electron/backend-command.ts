// Backend subcommand routing for the desktop-managed OpenAmer process.
//
// The desktop app launches its own headless backend via `openamer serve` — it
...
// app hasn't updated yet, or an older `openamer` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional OpenAmer profile to pin via `--profile`.
 */
 ...
 * `-m openamer_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `openamer_cli/subcommands/dashboard.py` source registers
