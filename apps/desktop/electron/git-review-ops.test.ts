...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'openamer-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'openamer-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'OpenAmer Test'], { cwd: dir })
