...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'openamer-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/openamer-runner-test',
    sourceRepoRoot: null,
    openamerHome: '/tmp/openamer-runner-test',
    logRoot: '/tmp/openamer-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'openamer-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'openamer-agent')
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/openamer-agent', '--openamer-home', '/tmp/openamer', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/openamer',
      openamerHome: '/tmp/home'
    }),
    ['--dir', '/tmp/openamer', '--openamer-home', '/tmp/home', '--branch', 'main']
  )
})
