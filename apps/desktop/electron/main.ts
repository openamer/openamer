...
import { buildDesktopBackendEnv, normalizeOpenAmerHomeRoot } from './backend-env'
import { isReauthRequiredError, waitForOpenAmerReady } from './backend-health'
import { canImportOpenAmerCli, shouldTrustOpenAmerOverride, verifyOpenAmerCli } from './backend-probes'
...
import {
  ...
  resolveVenvOpenAmerCommand
} from './windows-openamer-path'
import {
...
const USER_DATA_OVERRIDE = process.env.OPENAMER_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.OPENAMER_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.OPENAMER_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with OPENAMER_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[openamer] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[openamer] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `openamer desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose OpenAmer sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[openamer] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[openamer] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[openamer] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[openamer] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[openamer] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('openamer:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[openamer] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[openamer] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[openamer] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[openamer] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}
...
//   macOS / Linux: ~/.openamer (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.openamer directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\openamer yet, prefer the legacy path so we don't orphan their
...
function resolveOpenAmerHome() {
  ...
  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'openamer-home')
  }
  ...
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'openamer')
    const legacy = path.join(app.getPath('home'), '.openamer')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.openamer setup (no LOCALAPPDATA install yet) so users don't lose state.
...
function pathWithOpenAmerManagedNode(...entries) {
...
// boots, going straight to `resolveOpenAmerBackend()`. Missing or stale marker
...
// Mirrors openamer_cli.profiles._PROFILE_ID_RE so we never hand the backend a
...
//
// Mirror the Python logs (openamer_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.OPENAMER_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.OPENAMER_DESKTOP_BOOT_FAKE_ERROR || ''

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.OPENAMER_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.OPENAMER_DESKTOP_APP_NAME || 'OpenAmer'
...
// 'openamer:native-theme' ('dark' | 'light' | 'system'); we pin
...
// an in-place `openamer update` mid-session is reflected without an app restart;
...
const MEDIA_PROTOCOL = 'openamer-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(openamer-media://…)`.
...
// startOpenAmer(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.OPENAMER_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.OPENAMER_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startOpenAmer() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning openamer serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start OpenAmer backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  const lines = text.split(/\r?\n/).map(line => `[openamer] ${line}`)
  openamerLog.push(...lines)

  if (openamerLog.length > 300) {
    openamerLog.splice(0, openamerLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-openamer-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('openamer:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the openamer:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('openamer:bootstrap:event', ev)
}
...
// (`force_kill_other_openamer`, taskkill /IM openamer.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — OpenAmer will start automatically when it completes…',
      12
    )
...
function findOnPath(command) {
  ...
  // shell-script shim named `openamer` — must not shadow `openamer.cmd`/`openamer.exe`.
...
function unwrapWindowsVenvOpenAmerCommand(command, backendArgs) {
  return resolveVenvOpenAmerCommand(command, backendArgs, {
...
// spawns `openamer serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `openamer` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'openamer_cli', 'subcommands', 'dashboard.py'), 'utf8')
...
function isOpenAmerSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'openamer_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.OPENAMER_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several OpenAmer deps (notably pywinpty's Rust-built
...
// install only has PortableGit under %LOCALAPPDATA%\openamer\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'openamer', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'openamer', 'git', 'bin', 'git.exe'))
  }
...
function recentOpenAmerLog() {
  return openamerLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's openamer root) ──────
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('openamer:updates:progress', merged)
  }
}
...
// Path to the venv shim whose lock decides whether `openamer update` can write
// fresh entry points. On Windows this is the file the running backend
// `openamer.exe` holds open; on POSIX it's never mandatory-locked.
function venvOpenAmerShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'openamer.exe')
    : path.join(updateRoot, 'venv', 'bin', 'openamer')
}
...
// only signals the direct child, so on Windows a backend `openamer.exe` that
// spawned its own grandchildren (a `openamer` REPL, a pty terminal session, the
...
// `openamerProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `openamer.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a openamer
// REPL, a pty terminal, the gateway) keep `openamer.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  const openamerProcess = backendConnectionState.getProcess()

  if (openamerProcess && Number.isInteger(openamerProcess.pid)) {
    pids.push(openamerProcess.pid)
  }
  ...
  if (openamerProcess && !openamerProcess.killed) {
    try {
      openamerProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvOpenAmerShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentOpenAmerProcess = backendConnectionState.getProcess()

    if (currentOpenAmerProcess && Number.isInteger(currentOpenAmerProcess.pid)) {
      stragglers.push(currentOpenAmerProcess.pid)
    }
...
// `openamer update`). Instead we spawn the staged OpenAmer-Setup binary with
// --update and quit, so it can run `openamer update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri openamer-setup. Unlike Windows
      ...
      // whole update itself: `openamer update` (backend) + `openamer desktop
    ...
    if (!updater) {
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `openamer update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `openamer update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, openamerRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating OpenAmer — this window will close and the updater will open. Don’t reopen OpenAmer yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // openamer.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running openamer, an unkillable child). Handing off anyway
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvOpenAmer = path.join(venvBin, IS_WINDOWS ? 'openamer.exe' : 'openamer')
  ...
  // not just the `openamer.exe` console-script shim. That shim is generated at the
...
// Resolve the openamer CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `openamer` on PATH.
function resolveOpenAmerCliBinary(updateRoot) {
  const venvOpenAmer = path.join(updateRoot, 'venv', 'bin', 'openamer')

  if (fileExists(venvOpenAmer)) {
    return venvOpenAmer
  }

  return findOnPath('openamer') || null
}
...
// Python-level pre-update snapshot inside `openamer update`.
function preflightStateDb(openamerHome, rememberLog) {
  const stateDbPath = path.join(openamerHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(openamerHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(openamerHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(openamerHome, old))
            } catch {
              void 0
            }
          }
...
// macOS/Linux in-app update: backend (`openamer update`) + OS-aware GUI rebuild
// (`openamer desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const openamer = resolveOpenAmerCliBinary(updateRoot)

  if (!openamer) {
    emitUpdateProgress({ stage: 'manual', message: 'openamer update', percent: null })

    return { ok: true, manual: true, command: 'openamer update', openamerRoot: updateRoot }
  }
  ...
  // Node lives directly under %LOCALAPPDATA%\\openamer\\node, not node\\bin.
  // PYTHONUNBUFFERED: `openamer update` writes to a pipe here, so CPython
  ...
  // `openamer update` reaps stale `openamer serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads OPENAMER_DESKTOP_CHILD_PID and excludes
  ...
  if (openamerProcess && Number.isInteger(openamerProcess.pid)) {
    desktopChildPids.push(openamerProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.OPENAMER_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating OpenAmer (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(openamer, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'openamer update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'openamer update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(openamer, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart OpenAmer to retry.',
  ...
  // Linux in-app update terminal state (#45205). `openamer desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting OpenAmer…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `openamer-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen OpenAmer to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the OpenAmer desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen OpenAmer to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'OpenAmer.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'OpenAmer.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart OpenAmer to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.openamer-update-new"; then
    rm -rf "$DST.openamer-update-old" 2>/dev/null || true
    mv "$DST" "$DST.openamer-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.openamer-update-new" "$DST"
    rm -rf "$DST.openamer-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart OpenAmer to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `openamer
...
function resolveWebDist() {
  const override = process.env.OPENAMER_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set OPENAMER_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: openamer desktop --force-build`
  )
...
function resolveOpenAmerCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/OpenAmer.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.OPENAMER_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveOpenAmerCwd(), sanitized: Boolean(trimmed) }
  }
...
function resolveOpenAmerBackend(backendArgs) {
  ...
  //    installed `openamer` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isOpenAmerSourceRoot.)
  if (!IS_PACKAGED && isOpenAmerSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `OpenAmer source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  //    to spawning openamer. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `openamer update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `openamer` on PATH -- installed via install.ps1 / install.sh from
  ...
  if (process.env.OPENAMER_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (openamerOverride) {
      ...
      if (resolvedOverride) {
        openamerCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(openamerOverride, { isWsl: IS_WSL })) {
        openamerCommand = openamerOverride
      } else {
        rememberLog(`Ignoring Windows OpenAmer override under WSL: ${openamerOverride}`)
      }
    } else {
      openamerCommand = findOnPath('openamer')
    }

    if (openamerCommand) {
      if (looksLikeDesktopAppBinary(openamerCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving OpenAmer CLI: ${openamerCommand}`)
        openamerCommand = null
      }
    }

    if (openamerCommand) {
      const unwrapped = unwrapWindowsVenvOpenAmerCommand(openamerCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `openamer` shim
      ...
      rememberLog(
        `Ignoring existing OpenAmer CLI at ${openamerCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed openamer_cli module via system Python.
  ...
  if (python) {
    ...
    // having openamer_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\openamer\openamer-agent\venv.
    if (canImportOpenAmerCli(python)) {
      return {
        kind: 'python',
        label: `installed openamer_cli module via ${python}`,
        command: python,
        args: ['-m', 'openamer_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: openamer_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveOpenAmerBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'OpenAmer Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveOpenAmerBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no OpenAmer install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'OpenAmer recovery was handed off to OpenAmer Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('OpenAmer install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      ...
      // Latch the failure so subsequent startOpenAmer() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // openamer:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveOpenAmerBackend(backend.args))
  }
  ...
  // On Windows, preflight Git Bash. OpenAmer' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\openamer\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `openamer` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for OpenAmer on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch OpenAmer.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isOpenAmerSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `OpenAmer venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }
...
function multipartBody(upload) {
  const boundary = `----openamer-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported OpenAmer backend URL protocol: ${parsed.protocol}`))
    ...
    const req = client.request(
      parsed,
      {
        ...
        headers: {
          ...
          'X-OpenAmer-Session-Token': token,
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the OpenAmer backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to OpenAmer backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-OpenAmer-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('openamer:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveOpenAmerCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('openamer:preview-file-changed', payload)
}
...
// waitForOpenAmerReady how to read a 401 — rejected session vs gated route.
...
async function waitForOpenAmer(baseUrl, token, signal?, authMode?) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForOpenAmerReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('openamer:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('openamer:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting OpenAmer...".
function sendPowerResume() {
  ...
  webContents.send('openamer:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('openamer:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('openamer:window-state-changed', state)
}
...
const OAUTH_SESSION_PARTITION = 'persist:openamer-remote-oauth'
...
// "not signed in", which on the initial boot path (startOpenAmer → the renderer's
// single-shot boot() with no retry) surfaced as the "OpenAmer couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "OpenAmer
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to OpenAmer Cloud agent…' : 'Sign in to OpenAmer gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to OpenAmer backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in openamer_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// hasLiveOauthSession, which looks for openamer_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a OpenAmer Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to OpenAmer Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (OpenAmer Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to OpenAmer Cloud. Open Settings → Gateway, choose OpenAmer Cloud, and sign in.'
    ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your OpenAmer Cloud session has expired. Sign in to OpenAmer Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the OpenAmer Cloud org tag on cloud-mode entries so Settings can
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.OPENAMER_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.OPENAMER_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted OpenAmer Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteOpenAmerPath: (ssh || savedSsh)?.remoteOpenAmerPath || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by OPENAMER_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the OpenAmer Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered OpenAmer Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteOpenAmerPath: input.sshRemoteOpenAmerPath ?? existingBlock.remoteOpenAmerPath
  })
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (openamer #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote OpenAmer gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote OpenAmer gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote OpenAmer gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.OPENAMER_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteOpenAmerPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: connectionScopeKey(profile) || '',
      remoteOpenAmerPath: sshConfig.remoteOpenAmerPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForOpenAmer: (baseUrl, token) => waitForOpenAmer(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    openamerVersion: result.openamerVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.openamerVersion || 'openamer (version unknown)'} at ${result.openamerPath || '?'}`
  )
  ...
  return { ...connection, remoteOpenAmerVersion: result.openamerVersion || '' }
}
...
//   2. env override (OPENAMER_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.OPENAMER_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.OPENAMER_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'OPENAMER_DESKTOP_REMOTE_URL is set but OPENAMER_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote OpenAmer backend.'
      )
    }
...
// env → global) so a startOpenAmer() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every OpenAmer
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteOpenAmerPath: input.sshRemoteOpenAmerPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteOpenAmerPath || '')
          let openamerPath
          let openamerVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            openamerPath = runtime.openamerPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.openamerPath])
            openamerVersion = inspection.version
            supported = inspection.supported
          } else {
            openamerPath = await remoteLifecycle.locateOpenAmer(ssh, sshConfig.remoteOpenAmerPath || '')
            openamerVersion = await remoteLifecycle.probeOpenAmerVersion(ssh, openamerPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, openamerPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update OpenAmer on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteOpenAmerPath: openamerPath,
            remoteOpenAmerVersion: openamerVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startOpenAmer())
  ...
  // connect to OpenAmer gateway". Mirror the renderer's connect here so the test
...
function resetOpenAmerConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const openamerProcess = backendConnectionState.invalidate()
  stopBackendChild(openamerProcess)
...
// startOpenAmer() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetOpenAmerConnection() invalidates it.
  const openamerProcess = backendConnectionState.getProcess()
  const dying = openamerProcess && !openamerProcess.killed ? openamerProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('openamer:connection:applied')
}
...
// profile to startOpenAmer() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startOpenAmer() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForOpenAmer(remote.baseUrl, remote.token, undefined, remote.authMode)

    return {
      ...remote,
      profile,
      logs: openamerLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  const backend = await ensureRuntime(resolveOpenAmerBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const openamerCwd = resolveOpenAmerCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting OpenAmer backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`OpenAmer backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`OpenAmer backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`OpenAmer backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForOpenAmer(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `OpenAmer backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_openamer_home() recreates the deleted profile directory.
//
...
async function startOpenAmer() {
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving OpenAmer backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      await advanceBootProgress('backend.remote', `Connecting to remote OpenAmer backend at ${remote.baseUrl}`, 24)
      await waitForOpenAmer(remote.baseUrl, remote.token, undefined, remote.authMode)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote OpenAmer backend is ready',
        progress: 94,
        running: true,
        error: null
      })
      ...
      return {
        ...
        remoteOpenAmerVersion: remote.remoteOpenAmerVersion,
        token: remote.token,
        wsUrl: remote.wsUrl,
        logs: openamerLog.slice(-80),
        ...getWindowState()
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving OpenAmer runtime', 28)

        return resolveOpenAmerBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting OpenAmer backend via ${backend.label}`, 84)
    rememberLog(`Starting OpenAmer backend via ${backend.label}`)

    const openamerProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: openamerCwd,
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, openamerProcess)

    if (!processOwner) {
      stopBackendChild(openamerProcess)
      throw new Error('OpenAmer backend start was superseded by a newer connection attempt.')
    }

    openamerProcess.stdout.on('data', rememberLog)
    openamerProcess.stderr.on('data', rememberLog)
    ...
    openamerProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale OpenAmer backend error: ${error.message}`)
        rejectBackendStart?.(new Error('OpenAmer backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`OpenAmer backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `OpenAmer backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    openamerProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale OpenAmer backend exit (${signal || code})`)
      ...
      rememberLog(`OpenAmer backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `OpenAmer backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `OpenAmer backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentOpenAmerLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for OpenAmer backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(openamerProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for OpenAmer backend to become ready', 90)
    ...
    updateBootProgress({
      phase: 'backend.ready',
      message: 'OpenAmer backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while OpenAmer is
...
// pushes pet state over IPC (openamer:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via openamer:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main OpenAmer app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the OpenAmer icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see openamer:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep OpenAmer in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('openamer:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  startOpenAmer().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('openamer:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting OpenAmer…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('openamer:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const result = await revalidateRemoteConnection({
      ...
      resetConnection: resetOpenAmerConnection,
...
ipcMain.handle('openamer:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('openamer:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('openamer:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('openamer:window:openInstance', async () => {
...
ipcMain.handle('openamer:zoom:get', event => {
...
ipcMain.on('openamer:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('openamer:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('openamer:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('openamer:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('openamer:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('openamer:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('openamer:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('openamer:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('openamer:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('openamer:pet-overlay:control', payload)
})
ipcMain.handle('openamer:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startOpenAmer() call restarts the
...
ipcMain.handle('openamer:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startOpenAmer() re-runs the full installer (refreshing a broken/partial
  ...
  resetOpenAmerConnection()

  return { ok: true }
})
ipcMain.handle('openamer:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('openamer:bootstrap:cancel', async () => {
...
ipcMain.handle('openamer:boot-progress:get', async () => bootProgressState)
ipcMain.handle('openamer:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('openamer:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('openamer:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('openamer:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('openamer:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('openamer:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('openamer:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startOpenAmer() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('openamer:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- OpenAmer Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('openamer:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('openamer:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('openamer:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('openamer:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('openamer:profile:set', async (_event, name) => {
...
ipcMain.on('openamer:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('openamer:requestMicrophoneAccess', async () => {
...
ipcMain.handle('openamer:api', async (_event, request) => {
  ...
  // backend calls ensure_openamer_home() which recreates the profile directory,
...
ipcMain.handle('openamer:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('openamer:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('openamer:focus-session', payload.sessionId)
    }
  })
...
ipcMain.handle('openamer:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('openamer:readFileText', async (_event, filePath) => {
...
ipcMain.handle('openamer:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('openamer:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('openamer:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('openamer:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('openamer:saveClipboardImage', async () => {
...
ipcMain.handle('openamer:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('openamer:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('openamer:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('openamer:titlebar-theme', (_event, payload) => {
...
ipcMain.on('openamer:native-theme', (_event, mode) => {
...
ipcMain.on('openamer:translucency', (_event, payload) => {
...
ipcMain.on('openamer:keep-awake', (_event, on) => {
...
ipcMain.handle('openamer:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('openamer:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveOpenAmerCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('openamer:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveOpenAmerCwd()
}))

ipcMain.handle('openamer:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('openamer:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('openamer:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('openamer:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('openamer:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('openamer:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: openamerLog.slice(-200) }))
...
function terminalShellCommand() {
  // OPENAMER_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.OPENAMER_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip OpenAmer' TUI into
...
function terminalChannel(id, suffix) {
  return `openamer:terminal:${id}:${suffix}`
}
...
ipcMain.handle('openamer:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('openamer:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('openamer:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('openamer:fs:openDir', async (_event, dirPath) => {
...
ipcMain.handle('openamer:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('openamer:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('openamer:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('openamer:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('openamer:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('openamer:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('openamer:git:branchSwitch', async (_event, repoPath, branch) =>
...
ipcMain.handle('openamer:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('openamer:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('openamer:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('openamer:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('openamer:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('openamer:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('openamer:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('openamer:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('openamer:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('openamer:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('openamer:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('openamer:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('openamer:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('openamer:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('openamer:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('openamer:terminal:write', (_event, id, data) => {
...
ipcMain.handle('openamer:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('openamer:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('openamer:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('openamer:updates:check', async () =>
...
ipcMain.handle('openamer:updates:apply', async (_event, payload) =>
...
ipcMain.handle('openamer:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('openamer:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical OpenAmer version (the one `release.py` bumps in
// openamer_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real OpenAmer version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveOpenAmerVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'openamer_cli', '__init__.py')
...
// Re-resolve the live OpenAmer version and push it into the native About panel
// just before showing it, so an in-place `openamer update` is reflected without
...
ipcMain.handle('openamer:version', async () => ({
  appVersion: resolveOpenAmerVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  openamerRoot: resolveUpdateRoot()
}))
...
// the actual removal via `openamer uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (openamer_cli/uninstall.py + openamer_cli/gui_uninstall.py).
//
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no OpenAmer agent venv at ${VENV_ROOT}.`
    }
  }
...
function _spawnSessionToBrain(baseUrl: string): void {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `openamer-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `openamer-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('openamer:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('openamer:uninstall:run', async (_event, payload) => {
...
function handleDeepLink(url) {
  ...
  // openamer://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('openamer:deep-link', payload)
...
ipcMain.handle('openamer:deep-link-ready', () => {
...
// second-instance argv. Without the lock a second `openamer://` launch spawns a
