...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'openamer_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'openamer_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'openamer_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/OpenAmer.app/Contents/MacOS/OpenAmer', 'darwin'),
    '/Applications/OpenAmer.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/OpenAmer.app/Contents/MacOS/OpenAmer', 'darwin'),
    '/Users/x/Applications/OpenAmer.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\OpenAmer\\OpenAmer.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\OpenAmer'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\openamer-desktop\\OpenAmer.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\openamer-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\OpenAmer.exe', 'win32'), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/OpenAmer.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/OpenAmer.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.openamer/openamer-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.openamer/openamer-agent',
    uninstallArgs: ['-m', 'openamer_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/openamer/linux-unpacked',
    openamerHome: '/home/x/.openamer'
  })
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.openamer/openamer-agent',
    agentRoot: '/home/x/.openamer/openamer-agent',
    uninstallArgs: ['-m', 'openamer_cli.uninstall', '--mode', 'full'],
    appPath: null,
    openamerHome: '/home/x/.openamer'
  })

  // System python + source on PYTHONPATH so import openamer_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.openamer\/openamer-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'openamer_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'openamer_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    openamerHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'openamer_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    pythonPath: 'C:\\openamer',
    agentRoot: 'C:\\openamer',
    uninstallArgs: ['-m', 'openamer_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\OpenAmer',
    openamerHome: 'C:\\Users\\x\\AppData\\Local\\openamer'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import openamer_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\openamer;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "openamer_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\OpenAmer" >nul 2>&1/)
