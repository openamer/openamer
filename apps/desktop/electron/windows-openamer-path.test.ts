// Unit tests for the pure Windows `openamer` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvOpenAmerCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `openamer` shim shadows
//      the real openamer.cmd/openamer.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the openamer.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvOpenAmerCommand() — must probe the venv python via
//      canImportOpenAmerCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvOpenAmerCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportOpenAmerCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    openamerHome: '/fake/openamer-home',
...
test('resolveVenvOpenAmerCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvOpenAmerCommand('/root/venv/Scripts/openamer.exe', [], deps), null)
})

test('resolveVenvOpenAmerCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvOpenAmerCommand('/root/venv/Scripts/openamer.cmd', [], deps), null)
})

test('resolveVenvOpenAmerCommand: returns null when the basename is not openamer/openamer.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvOpenAmerCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvOpenAmerCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvOpenAmerCommand('/root/venv/bin/openamer.exe', [], deps), null)
})

test('resolveVenvOpenAmerCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvOpenAmerCommand('/root/venv/Scripts/openamer.exe', [], deps), null)
})

test('resolveVenvOpenAmerCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportOpenAmerCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvOpenAmerCommand('/root/venv/Scripts/openamer.exe', ['serve'], deps)
...
test('resolveVenvOpenAmerCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvOpenAmerCommand('/root/venv/Scripts/openamer.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'openamer_cli.main', 'serve', '--port', '0'])
