...
const READY_RE = /^OPENAMER_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitOpenAmerPath = '') {
  const explicit = psLiteral(explicitOpenAmerPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$openamerHome=$env:OPENAMER_HOME',
    'if(-not $openamerHome){$openamerHome=Join-Path $env:LOCALAPPDATA "openamer"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command openamer.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $openamerHome "openamer-agent\\venv\\Scripts\\openamer.exe")',
    '$candidates+=(Join-Path $HOME "openamer-agent\\.venv\\Scripts\\openamer.exe")',
    '$openamer=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $openamer){throw "OpenAmer is not installed on the remote Windows host."}',
    'if($explicit -and $openamer -ne $explicit){throw "The configured OpenAmer path is not an executable file."}',
    '$python=Join-Path (Split-Path $openamer) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote OpenAmer Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;openamerHome=$openamerHome;openamerPath=$openamer;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitOpenAmerPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitOpenAmerPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'openamer_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.openamerPath === 'string' &&
    typeof lock.openamerHome === 'string'
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertCurrent(signal)
  const runtime = await probeWindowsRemote(ssh, remoteOpenAmerPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.openamerPath])

  if (!inspection.supported) {
    const error: any = new Error('Update OpenAmer on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.openamerPath = inspection.path
  const openamerVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located openamer at ${runtime.openamerPath}`)
  ...
  if (validLock(lock, ownershipId)) {
    ...
    const reusable =
      ...
      lock.openamerPath === runtime.openamerPath &&
      lock.openamerHome === runtime.openamerHome
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, openamerPath: runtime.openamerPath })
    )
  } catch (error) {
  ...
  const owned = {
    ...
    profile,
    openamerPath: runtime.openamerPath,
    openamerHome: runtime.openamerHome,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForOpenAmer(baseUrl, token)
    assertCurrent(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="OpenAmer SSH"', 'powershell.exe -NoLogo')
