/**
 * windows-openamer-path.ts
 *
 * Pure, dependency-injected pieces of Windows `openamer` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvOpenAmerCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `openamer` shim shadowed the real
 *      openamer.cmd/openamer.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\openamer.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvOpenAmerCommand() — unwrapWindowsVenvOpenAmerCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `openamer`) must
 * not shadow `openamer.cmd`/`openamer.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv openamer shim, bootstrap-complete
 * marker) — gating on just the openamer.exe console-script shim alone is the
...
export interface ResolveVenvOpenAmerCommandDeps {
  ...
  canImportOpenAmerCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `openamer`/`openamer.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/openamer(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m openamer_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportOpenAmerCli(). A
 * venv whose update died mid-`pip install` still has python.exe + openamer.exe
 ...
 *
 * Returns null when `command` is not a venv openamer shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvOpenAmerCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvOpenAmerCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^openamer(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv OpenAmer at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing OpenAmer Python at ${python}`,
    command: python,
