 ...
 * startOpenAmer() re-throw the cached error without re-attempting the connect —
...
/**
 * Whether a startOpenAmer() failure should latch into `backendStartFailure`.
 ...
 * Every subsequent `getConnection`/`api` call re-runs `startOpenAmer`, re-emits
