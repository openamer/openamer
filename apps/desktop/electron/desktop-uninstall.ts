 ...
 * the three user-facing uninstall modes to the `openamer uninstall` CLI flags,
 ...
 *              `openamer uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `openamer uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `openamer uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `openamer` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m openamer_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `openamer_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'openamer_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/OpenAmer.app/Contents/MacOS/OpenAmer  → …/OpenAmer.app
 *   Windows: …\OpenAmer\OpenAmer.exe                 → …\OpenAmer  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/OpenAmer.app/Contents/MacOS/OpenAmer → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/OpenAmer.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs OpenAmer.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]OpenAmer$/i.test(dir) || /[\\/]openamer-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/openamer
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import openamer_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, openamerHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import openamer_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but OpenAmer install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
