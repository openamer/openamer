 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of OpenAmer Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.openamer-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(openamerHome) {
  return path.join(openamerHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.openamer/openamer-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(openamerHome) {
  if (!openamerHome) {
    return null
  }

  const candidate = path.join(openamerHome, 'openamer-agent', 'scripts', installScriptName())
...
function cachedScriptPath(openamerHome, commit) {
  return path.join(bootstrapCacheDir(openamerHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/openamer-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(openamerHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(openamerHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, openamerHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass OPENAMER_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          OPENAMER_HOME: openamerHome || process.env.OPENAMER_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, openamerHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, openamerHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--openamer-home', openamerHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, openamerHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, openamerHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, openamerHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(openamerHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, openamerHome, emit })
