...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'OpenAmer E2E'], { cwd: repo })
...
function configureRepoCwd(openamerHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(openamerHome, mockUrl)
  fs.appendFileSync(path.join(openamerHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(openamerHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.openamerHome, mock.url, repo)
