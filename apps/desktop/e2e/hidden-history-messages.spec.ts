...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.openamerHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.openamerHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.openamerHome)
  const builder = await RealSessionBuilder.start(sandbox.openamerHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.openamerHome, mock.url)
  fs.appendFileSync(path.join(sandbox.openamerHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.openamerHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
