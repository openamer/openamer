 ...
 * AIAgent loop, using the E2E mock provider configured in `openamerHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(openamerHome: string) {
    this.child = spawn('uv', ['run', '--active', '--no-sync', 'python', '-m', 'tui_gateway.entry'], {
      cwd: REPO_ROOT,
      env: {
        ...process.env,
        OPENAMER_HOME: openamerHome,
  ...
  static async start(openamerHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(openamerHome)
