/**
 * Shared E2E fixtures for the OpenAmer desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → openamer serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `openamer desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `openamer-e2e-${prefix}-${Math.random()}`))
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(openamerHome, 'config.yaml')
 ...
 */
export function writeEnvFile(openamerHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(openamerHome, '.env')
 ...
 */
function writeEmptyConfig(openamerHome: string): void {
  const configPath = path.join(openamerHome, 'config.yaml')
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.openamerHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via OPENAMER_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`openamer serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.openamerHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { OPENAMER_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to OpenAmer backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'OpenAmer.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'OpenAmer.app', 'Contents', 'MacOS', 'OpenAmer')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'openamer')
}
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    OPENAMER_DESKTOP_BOOT_FAKE: '1',
    OPENAMER_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })
