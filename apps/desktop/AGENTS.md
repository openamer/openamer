...
How to build OpenAmer Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another OpenAmer surface can also
...
- A **runtime home change** (switching the underlying `OPENAMER_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across OpenAmer — do not assume
