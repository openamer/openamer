...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `openamer-dev-mock-${Date.now()}`))
  const openamerHome = path.join(root, 'openamer-home')
  const userDataDir = path.join(root, 'electron-user-data')
  fs.mkdirSync(openamerHome, { recursive: true })
  fs.mkdirSync(userDataDir, { recursive: true })
  return { root, openamerHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(openamerHome, mockUrl) {
  fs.writeFileSync(
    path.join(openamerHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(openamerHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
