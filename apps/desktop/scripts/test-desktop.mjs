...
// no longer bundles the OpenAmer Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'OpenAmer.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'OpenAmer'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'OpenAmer.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'OpenAmer'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default OPENAMER_HOME for non-sandboxed runs -- matches main.ts's
// resolveOpenAmerHome(). On Windows it's %LOCALAPPDATA%\openamer; elsewhere
// it's ~/.openamer. The fresh-install sandbox launchFresh() sets its own
// OPENAMER_HOME and never touches this.
const DEFAULT_OPENAMER_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'openamer')
  }
  return path.join(os.homedir(), '.openamer')
})()
const VENV_ROOT = path.join(DEFAULT_OPENAMER_HOME, 'openamer-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'openamer-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.OPENAMER_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `OpenAmer-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `OpenAmer-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `OpenAmer-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'OpenAmer-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.OPENAMER_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.OPENAMER_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const openamerHome = path.join(sandbox, 'openamer-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(openamerHome, { recursive: true })
  ...
  console.log(`  OPENAMER_HOME: ${openamerHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(openamerHome, 'openamer-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The OpenAmer Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of openamer_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'openamer-agent', 'openamer_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing OpenAmer
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + OPENAMER_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  OPENAMER_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
