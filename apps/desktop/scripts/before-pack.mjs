 ...
 * product name (`OpenAmer`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/OpenAmer'
 *
 * This is a hard failure with no obvious cause for the user — `openamer desktop`
 ...
 * (the mac-unpacked OpenAmer.app bundle) and Windows (win-unpacked), so we
 ...
 * OpenAmer.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `openamer desktop --build-only` (openamer_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'OpenAmer.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'OpenAmer'}.exe`
