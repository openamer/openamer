...
export const $openamerHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.openamer/logs/bootstrap-installer.log')
    $openamerHome.set('~/.openamer')
  ...
  try {
    const [logPath, openamerHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_openamer_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $openamerHome.set(openamerHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch OpenAmer" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (OpenAmer-Setup.exe --update);
...
export async function launchOpenAmerDesktop(): Promise<void> {
  ...
  await invoke('launch_openamer_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'OpenAmer repository' },
