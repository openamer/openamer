...
//!
//! Mirrors `openamer_constants.get_openamer_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\openamer
//!   macOS:   ~/.openamer
//!   Linux:   ~/.openamer  (override via $OPENAMER_HOME)
//!
//! NOTE (macOS): Python's get_openamer_home(), scripts/install.sh, and the
//! Electron desktop's resolveOpenamerHome() ALL use ~/.openamer on macOS — there
...
/// Returns the canonical OpenAmer home directory, respecting $OPENAMER_HOME if set.
pub fn openamer_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("OPENAMER_HOME") {
    ...
    {
        // %LOCALAPPDATA%\openamer — matches scripts/install.ps1's $OPENAMER_HOME.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("openamer");
        }
    }

    // macOS + Linux + fallback: ~/.openamer (matches Python get_openamer_home(),
    // install.sh, and the Electron desktop's resolveOpenamerHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".openamer");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".openamer")
}

pub fn log_dir() -> PathBuf {
    openamer_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    openamer_home().join("bootstrap-cache")
}
...
/// OPENAMER_HOME so it survives repo checkout deletion (unlike anything under
/// openamer-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\openamer\openamer-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_openamer`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under OPENAMER_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves OPENAMER_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    openamer_home().join(".openamer-update-in-progress")
}
...
pub fn copy_self_to_openamer_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to OPENAMER_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_OPENAMER_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".openamer-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under OPENAMER_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[openamer-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("OPENAMER_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_openamer_home() -> String {
    openamer_home().to_string_lossy().into_owned()
}
