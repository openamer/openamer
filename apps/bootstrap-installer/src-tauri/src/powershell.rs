...
///
/// `openamer_home_override` propagates to the child as $OPENAMER_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, openamer_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = openamer_home_override {
        cmd.env("OPENAMER_HOME", home);
    }
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_openamer_home() {
