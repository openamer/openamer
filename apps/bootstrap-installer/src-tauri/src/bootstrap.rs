...
#[tauri::command]
pub async fn launch_openamer_desktop(app: AppHandle, install_root: String) -> Result<(), String> {
...
async fn run_bootstrap(
    ...
    if let Err(err) = crate::paths::copy_self_to_openamer_home() {
        tracing::warn!(
            ?err,
            "failed to copy installer into OPENAMER_HOME (non-fatal)"
        );
        emit_log(&format!(
