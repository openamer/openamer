# OpenAmer Agent - Development Guide

Instructions for AI coding assistants and developers working on the openamer-agent codebase.

**Never give up on the right solution.**

## What OpenAmer Is

OpenAmer is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: OpenAmer ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`openamer tools`, `openamer setup`, auto-install) rather than bolting on a raw
  env var.
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `openamer tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.openamer/plugins/` (or via a
  pip entry point), and promote them in the OpenAmer Discord
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `openamer <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `openamer webhook`, `openamer cron`, `openamer tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.openamer/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.openamer/openamer-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── openamer_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── openamer_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── openamer-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `openamer --tui`
...
**User config:** `~/.openamer/config.yaml` (settings), `~/.openamer/.env` (API keys only).
**Logs:** `~/.openamer/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_openamer_home()`.
Browse with `openamer logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across OpenAmer: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`openamer_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `OpenAmerCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.openamer/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`openamer_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/openamer` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `openamer_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `OpenAmerCLI.process_command()` in `cli.py`:
```python
...
npm run dev       # watch mode (rebuilds openamer-ink + tsx --watch)
npm start         # production
npm run build     # full build (openamer-ink + tsc)
...
### TUI in the Dashboard (`openamer dashboard` → `/chat`)

The dashboard embeds the real `openamer --tui` — **not** a rewrite.  See `openamer_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `openamer_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `openamer --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `openamer --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.
...
be core tools. For custom or local-only tools, do **not** edit OpenAmer core.
Use the plugin route instead: create `~/.openamer/plugins/<name>/plugin.yaml`
and `~/.openamer/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core OpenAmer tool that should ship in the base system.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `openamer_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `openamer_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `openamer tools`, `openamer setup`, most CLI subcommands | `openamer_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`openamer_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
openamer_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.openamer/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic OpenAmer gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `openamer_cli/skin_engine.py`:
...
Users create `~/.openamer/skins/<name>.yaml`:
...
OpenAmer has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.openamer/plugins/` and pip-installed entry points.

### General plugins (`openamer_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.openamer/plugins/`, `./.openamer/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `openamer` at startup so
  `openamer <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(openamer_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `openamer <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `openamer --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `openamer_cli/main.py`, etc.).
...
into `~/.openamer/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `openamer memory setup` / `post_setup()` without
...
`~/.openamer/plugins/` (or via pip entry points). They register through
...
don't own. Promote standalone plugins in the OpenAmer Discord
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `openamer skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.openamer.tags`, `metadata.openamer.category`,
`metadata.openamer.related_skills`, `metadata.openamer.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.openamer.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native OpenAmer tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "OpenAmer Agent" is the secondary collaborator. If the
   contributor's commit shows "OpenAmer Agent" as author (because they
   used OpenAmer to draft the skill), replace it with their actual name
...
lives in the `openamer-agent-dev` skill at
...
Enable/disable per platform via `openamer tools` (the curses UI) or the
...
OpenAmer returns a delegation id immediately and the result re-enters the
...
go to `~/.openamer/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `openamer_cli/curator.py` wires `openamer curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.openamer/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `openamer cron <verb>`
...
- File lock at `~/.openamer/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `openamer kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `openamer_cli/kanban.py` wires `openamer kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`openamer-kanban-dispatcher.service` for
...
OpenAmer-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
1. **Use `get_openamer_home()` for all OPENAMER_HOME paths.** Import from `openamer_constants`.
   ...
   config_path = get_openamer_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".openamer" / "config.yaml"
   ```

2. **Use `display_openamer_home()` for user-facing messages.** Import from `openamer_constants`.
   This returns `~/.openamer` for default or `~/.openamer/profiles/<name>` for profiles.
   ```python
   # GOOD
   from openamer_constants import display_openamer_home
   print(f"Config saved to {display_openamer_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.openamer/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_openamer_home()` at import time,
...
interactive menus must use `openamer_cli/curses_ui.py` — see
`openamer_cli/tools_config.py` for the canonical pattern.
...
Use the pattern from `tests/openamer_cli/test_profiles.py`:
```python
@pytest.fixture
...
| HOME / `~/.openamer/` | Your real config+auth.json                  | Temp dir per test                         |
