# Política de Seguridad de OpenAmer Agent

Este documento describe el modelo de confianza de OpenAmer Agent, identifica el
...
o **security@openamer.com**. No abras issues públicos para
...
- Detalles del entorno (`openamer version`, SHA del commit, SO, versión de Python).
...
OpenAmer Agent es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta OpenAmer Agent,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a OpenAmer Agent al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de OpenAmer Agent
...
OpenAmer Agent admite dos posturas de aislamiento a nivel de SO. Abordan
...
OpenAmer Agent admite esto de dos maneras:

- **La propia imagen Docker de OpenAmer Agent y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de OpenAmer Agent
...
OpenAmer Agent filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en OpenAmer Agent
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de OpenAmer Agent
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   OpenAmer Agent no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de OpenAmer Agent o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  OpenAmer Agent ha documentado una postura sobre cómo su salida debe ser
...
- **Configuraciones documentadas de emergencia.** Compensaciones seleccionadas por el operador
  ...
  la seguridad de openamer-home, y similares. Los informes contra esas
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de OpenAmer Agent
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en OpenAmer Agent. Los errores en la ruta de
  instalación de habilidades o plugins de OpenAmer Agent que impidan al
...
- OpenAmer Agent incluye guardias de cadena de suministro para lanzamientos de servidores
