...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time OpenAmer starts and finds ``~/.openclaw/``.

    Points users at ``openamer claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``openamer claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to OpenAmer, run "
        "`openamer claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `openamer claw cleanup` (renames it to ~/.openclaw.pre-migration — "
