"""Cross-session rate limit guard for OpenAmer Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether OpenAmer Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from OpenAmer triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 OpenAmer retries), and every one of those calls counts
...
_STATE_FILENAME = "openamer.json"


def _state_path() -> str:
    """Return the path to the OpenAmer rate limit state file."""
    try:
        from openamer_constants import get_openamer_home
        base = get_openamer_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".openamer")
...
def record_openamer_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that OpenAmer Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "OpenAmer rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write OpenAmer rate limit state: %s", exc)


def openamer_rate_limit_remaining() -> Optional[float]:
    """Check if OpenAmer Portal is currently rate-limited.
...
def clear_openamer_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful OpenAmer request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear OpenAmer rate limit state: %s", exc)
...
def is_genuine_openamer_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from OpenAmer Portal is a real account rate limit.

    OpenAmer Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, OpenAmer, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on OpenAmer is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on OpenAmer.

    Tripping the cross-session breaker on (b) blocks ALL OpenAmer requests
    (and all models, since OpenAmer is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  OpenAmer emits
