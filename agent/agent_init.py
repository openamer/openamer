...
from openamer_cli.timeouts import get_provider_request_timeout
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    try:
        ...
        from openamer_cli.models import normalize_provider as normalize_model_provider
        from openamer_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: openamer config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$OPENAMER_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_openamer_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$OPENAMER_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .openamer.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / OPENAMER_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.openamer/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from openamer_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.openamer/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from openamer_logging import setup_logging, setup_verbose_logging
    setup_logging(openamer_home=_ra()._openamer_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that openamer_logging.setup_logging() does not
        ...
        # handler level inside openamer_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from openamer_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from openamer_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `openamer model` to "
                        "select a provider, or run `openamer setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff OPENAMER_KANBAN_TASK is set).
    ...
    agent.logs_dir = openamer_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.openamer/sessions/session_{sid}.json)
    ...
    try:
        from openamer_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from openamer_cli.config import load_config as _load_agent_config
    ...
    # management API (the historical OpenAmer behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "openamer"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace OpenAmer' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "openamer", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, openamer, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from openamer_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from openamer_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from openamer_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by OpenAmer Agent.  Choose a model with at least "
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from openamer_cli.model_switch import _check_openamer_model_warning
            ...
            if _openamer_warn:
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_openamer_warn)
        except Exception:
            pass
