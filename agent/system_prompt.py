...
upstream prefix cache warm.  See ``openamer-agent-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, openamer subscription block, tool-use
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_openamer_subscription_prompt``,
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``OPENAMER_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("OPENAMER_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  OpenAmer never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want OPENAMER_HOME persona while keeping
    ...
    # OpenAmer agent goals / wishes — loaded from the active OpenAmer
    # home and kept in the stable tier because the goal set changes rarely.
    # This gives the agent its own persistent priorities (language, autonomy,
    # verification, skill preservation, honest blocker reporting).
    try:
        from agent.goal_engine import build_goal_block
        _goal_block = build_goal_block()
        if _goal_block:
            stable_parts.append(_goal_block)
    except Exception:
        # Goal engine failure must never block prompt build.
        pass

    # Pointer to the openamer-agent skill + docs for user questions about OpenAmer itself.
    stable_parts.append(OPENAMER_AGENT_HELP_GUIDANCE)
    ...
    # OPENAMER_KANBAN_TASK env var). Normal chat sessions never see
    ...
    openamer_subscription_prompt = _r.build_openamer_subscription_prompt(agent.valid_tool_names)
    if openamer_subscription_prompt:
        stable_parts.append(openamer_subscription_prompt)
    ...
    # Coding posture (base OpenAmer, any interactive coding surface in a code
    ...
    # Active-profile hint — names the OpenAmer profile the agent is running
    # under so it doesn't conflate ~/.openamer/skills/ (default profile) with
    # ~/.openamer/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        post_workspace_parts.append(
            ...
            "under " + str(get_openamer_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        post_workspace_parts.append(
            f"Active OpenAmer profile: {active_profile}. This session reads "
            f"and writes {get_openamer_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_openamer_home()}/skills/, {get_openamer_home()}/plugins/, "
            f"{get_openamer_home()}/cron/, {get_openamer_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing OpenAmer). Every other surface (desktop chat panel,
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — OpenAmer never rebuilds or reinjects parts of it
