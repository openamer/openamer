...
# Parameters OpenAmer sends on SOME routes only → hosts where that is deliberate.
...
def _plugin_verdict(c: _Ctx) -> Optional[Verdict]:
    ...
    try:
        from openamer_cli.plugins import get_plugin_error_classification
...
def _model_id_missing_known_prefix(model: str, provider: str) -> bool:
    ...
    if not name or "/" in name:
        return False
    try:
        from openamer_cli.model_normalize import suggest_prefixed_model_id
