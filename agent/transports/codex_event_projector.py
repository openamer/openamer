"""Projects codex app-server events into OpenAmer' messages list.

The translator that lets OpenAmer' memory/skill review keep working under the
...
OpenAmer' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape OpenAmer
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way OpenAmer' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but OpenAmer stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way OpenAmer already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — OpenAmer' messages store text only.
        text_parts: list[str] = []
