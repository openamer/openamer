...
Owns one Codex thread per OpenAmer session. Drives `turn/start`, consumes
...
# OpenAmer' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_OPENAMER_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse OpenAmer/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per OpenAmer session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _OPENAMER_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("OPENAMER_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="openamer",
            client_title="OpenAmer Agent",
            client_version=_get_openamer_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but OpenAmer' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into OpenAmer' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own openamer-tools callback we
            # auto-accept — the user already approved OpenAmer' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map OpenAmer approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    OpenAmer returns 'once', 'session', 'always', or 'deny'.
...
def _get_openamer_version() -> str:
    """Best-effort OpenAmer version string for codex's userAgent line."""
    try:
