...
as ``kind: backend``) or ``~/.openamer/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``openamer tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$OPENAMER_HOME/cache/videos/``, creating parents as needed."""
    from openamer_constants import get_openamer_home

    path = get_openamer_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$OPENAMER_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$OPENAMER_HOME/cache/videos/``.
