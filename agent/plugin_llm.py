...
Plugins built on OpenAmer Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing OpenAmer
...
:class:`~openamer_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors OpenAmer' main config
...
handles every provider, fallback chain, and per-task override OpenAmer
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`openamer_cli.plugins.PluginContext`
