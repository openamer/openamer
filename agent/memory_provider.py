...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `openamer backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - openamer_home (str): The active OPENAMER_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.openamer``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "openamer").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'openamer memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], openamer_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'openamer memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``openamer_home`` is the active OPENAMER_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE OPENAMER_HOME.

        ``openamer backup`` only walks OPENAMER_HOME, so any provider state kept
        ...
        of the archive, and ``openamer import`` restores them to their original
