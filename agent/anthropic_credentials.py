...
``ANTHROPIC_API_KEY``, OpenAmer-owned OAuth grants in the ``auth.json`` credential
pool, then ``~/.claude/.credentials.json`` / macOS Keychain as a borrowed fallback.
``~/.openamer/.anthropic_oauth.json`` (OpenAmer PKCE) and
...
_SPENT_ROTATION_SIDECAR_COMMENT = (
    ...
    "consumed server-side but never durably committed. Written by OpenAmer so sibling "
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    return source_path.with_name(source_path.name + ".openamer-spent-rotations.json")
...
_SINGLETON_SOURCE_PATHS = {
    "claude_code": lambda: claude_code_credentials_path(), "openamer_pkce": lambda: _get_openamer_oauth_file()
}
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from openamer_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
        refresh_timeout_seconds = env_float("OPENAMER_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20)
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    """Prefer refreshable Claude Code creds over a static env OAuth token: OpenAmer historically persisted setup tokens
...
def _resolve_anthropic_pool_token(*, skip_borrowed: bool = False) -> Optional[str]:
    ...
    refresh=False`` (never ``select()``) so diagnostic call sites (account_usage, ``openamer models``) never mutate
...
# ── OpenAmer-native PKCE OAuth flow (~/.openamer/.anthropic_oauth.json); mirrors Claude Code / pi-ai / OpenCode ──
...
def _root_openamer_oauth_file() -> Optional[Path]:
    ...
    try:
        ...
        root = get_default_openamer_root()
        return None if root.resolve(strict=False) == get_openamer_home().resolve(strict=False) else root / ".anthropic_oauth.json"
    except Exception:
        return None
...
def run_openamer_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print("\n".join([
        "", "Authorize OpenAmer with your Claude Pro/Max subscription.", "",
