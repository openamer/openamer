...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into OpenAmer with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.openamer/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of OpenAmer
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``openamer tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``openamer setup`` for
