...
``kind: backend``) or ``~/.openamer/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class BrowserProvider(abc.ABC):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by :mod:`openamer_cli.tools_config` to inject this provider as a
        ...
        expose API key prompts, badges, managed-OpenAmer gating, and the
