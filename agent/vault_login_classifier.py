...
INSPECTION_STAMP_ATTR = "data-openamer-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-openamer-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-openamer-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-openamer-vault-slot]").forEach((n) => n.removeAttribute("data-openamer-vault-slot"));
