...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``openamer`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's OPENAMER_HOME is ``~/.openamer/profiles/<name>``; the
    default profile (``~/.openamer``) returns "".
    """
