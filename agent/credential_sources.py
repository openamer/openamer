...
``openamer auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.openamer/.env; hint when the shell exports it."""
    ...
    # .env as utf-8-sig like openamer_cli/config.py: a BOM-sensitive read would
...
def _remove_auth_store_oauth(provider: str, removed) -> RemovalResult:
    ...
    """
    from openamer_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
