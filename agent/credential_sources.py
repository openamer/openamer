"""Unified removal contract for every credential source OpenAmer reads from.

OpenAmer seeds its credential pool from many places:
    ...
    manual        — user ran `openamer auth add`
...
unify here is **removal**:
    ...
    ``openamer auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.openamer/.env  → clear it
    ...
    """
    from openamer_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.openamer/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to OpenAmer.",
            f"  The pool entry is now suppressed — OpenAmer will ignore "
            f"{env_var} until you run `openamer auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so OpenAmer stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `openamer auth add anthropic` to re-enable if needed.",
    ])


def _remove_openamer_pkce(provider: str, removed) -> RemovalResult:
    """~/.openamer/.anthropic_oauth.json is ours — delete it outright."""
    from openamer_constants import get_openamer_home

    result = RemovalResult()
    oauth_file = get_openamer_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared OpenAmer Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``openamer auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `openamer model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the OpenAmer auth store is not enough — _seed_from_singletons() would
    ...
    via ``openamer auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `openamer auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `openamer auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    OpenAmer stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `openamer auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="openamer_pkce",
        remove_fn=_remove_openamer_pkce,
