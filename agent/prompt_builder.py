...
from openamer_constants import get_openamer_home, get_skills_dir, is_wsl
...
def _find_openamer_md(cwd: Path) -> Optional[Path]:
    ...
    # could pick up a .openamer.md planted in /tmp, /home, etc.
...
KANBAN_GUIDANCE = (
    ...
    "every assignee in a real profile (`openamer profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `openamer kanban <verb>` for board operations. Use "
...
#
# The openamer-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to openamer-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `openamer computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that OpenAmer "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the OpenAmer terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the OpenAmer desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the OpenAmer WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where OpenAmer itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host OpenAmer runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where OpenAmer itself is running. The host OS, home, and cwd "
                f"of the OpenAmer process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where OpenAmer "
    ...
    # Embedder-supplied environment description. Lets a host that wraps OpenAmer
    ...
    # OpenAmer self-system knowledge (guarded; never breaks the prompt)
    try:
        from openamer_cli import system_info as _si
        hints.append(_si.describe())
    except Exception:
        pass
...
def _skills_prompt_snapshot_path() -> Path:
    return get_openamer_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.openamer/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot OpenAmer Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `openamer-agent` skill "
            "first. It has the actual commands (e.g. `openamer config set …`, `openamer tools`, "
            "`openamer setup`) so you don't have to guess or invent workarounds.\n"
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    ...
    try:
        ...
        logger.debug("Could not ensure OPENAMER_HOME before loading SOUL.md: %s", e)

    soul_path = get_openamer_home() / "SOUL.md"
...
def _load_openamer_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    ...
    if not openamer_md_path:
        return ""
    try:
        content = openamer_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = openamer_md_path.name
        try:
            rel = str(openamer_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".openamer.md", context_length=context_length,
            read_path=str(openamer_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", openamer_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    SOUL.md from OPENAMER_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the OpenAmer install/source
    ...
    # explicitly configured cwd is honored verbatim — the OpenAmer tree is a
    ...
    # their launch dir IS the user's shell cwd (developing OpenAmer in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the OpenAmer install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_openamer_md(cwd_path, context_length)
    ...
    # SOUL.md from OPENAMER_HOME only — skip when already loaded as identity
    if not skip_soul:
