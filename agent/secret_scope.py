...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the OPENAMER_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "OPENAMER_REDACT_SECRETS", "OPENAMER_openamer_TIMEOUT_SECONDS",
    "_OPENAMER_GATEWAY",
    # OS / interpreter
...
def build_profile_secret_scope(openamer_home: Path) -> Dict[str, str]:
    ...
    try:
        from openamer_cli.env_loader import get_secret_source_values
