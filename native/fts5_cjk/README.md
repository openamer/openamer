...
storage discipline as the other indexes). On a populated database, run
    ...
    openamer sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.openamer/config.yaml` to disable. Override
the .so location with `OPENAMER_FTS5_CJK_SO`.
