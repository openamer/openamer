# nix/desktop.nix — OpenAmer Desktop (Electron) app build + wrapper
#
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "openamer-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "openamer-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "openamer-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/openamer-desktop $out/bin
    cp -r ${renderer}/* $out/share/openamer-desktop/
    ...
    substituteInPlace $out/share/openamer-desktop/dist/electron-main.mjs \
