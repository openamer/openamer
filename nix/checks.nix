...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      openamer-agent = self'.packages.default;
      openamerVenv = openamer-agent.openamerVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "openamer-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${openamerVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "openamer-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "openamer-build-package" { } ''
          echo "PASS: package built at ${openamer-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "openamer-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "openamer-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${openamer-agent}/bin/openamer || (echo "FAIL: openamer binary missing"; exit 1)
          test -x ${openamer-agent}/bin/openamer-agent || (echo "FAIL: openamer-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${openamer-agent}/bin/openamer version 2>&1 | grep -qi "openamer" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "openamer-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in openamer openamer-agent openamer-acp; do
            test -x ${openamer-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "openamer-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking openamer --help ==="
          ${openamer-agent}/bin/openamer --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${openamer-agent}/bin/openamer --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "openamer-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${openamer-agent}/share/openamer-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${openamer-agent}/share/openamer-agent/skills -name "SKILL.md" | wc -l)
          ...
          test -d ${openamer-agent}/share/openamer-agent/optional-skills || \
        ...
        bundled-plugins = pkgs.runCommand "openamer-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${openamer-agent}/share/openamer-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${openamer-agent}/share/openamer-agent/plugins/platforms/irc/plugin.yaml || \
        ...
        bundled-locales = pkgs.runCommand "openamer-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${openamer-agent}/share/openamer-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${openamer-agent}/share/openamer-agent/locales -name "*.yaml" | wc -l)
        ...
        bundled-mcps = pkgs.runCommand "openamer-bundled-mcps" { } ''
          ...
          test -d ${openamer-agent}/share/openamer-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${openamer-agent}/share/openamer-agent/optional-mcps -name "manifest.yaml" | wc -l)
          ...
          test -n "$CATALOG" || (echo "FAIL: openamer mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "openamer-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${openamer-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${openamer-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
        ...
        extra-python-packages = let
          ...
          openamerWithExtra = openamer-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "openamer-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${openamerWithExtra}/bin/openamer || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${openamerWithExtra}/bin/openamer || \
          ...
          if grep -q "PYTHONPATH" ${openamer-agent}/bin/openamer; then
        ...
        extra-dependency-groups = let
          openamerWithGroups = openamer-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "openamer-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${openamerWithGroups}"
          echo "venv: ${openamerWithGroups.openamerVenv}"
        ...
        messaging-variant = pkgs.runCommand "openamer-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.openamerVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "openamer-config-roundtrip" {
          ...
          merge_and_load() {
            local openamer_home="$1"
            export OPENAMER_HOME="$openamer_home"
            ${configMergeScript} ${nixSettings} "$openamer_home/config.yaml"
            ${openamerVenv}/bin/python3 -c '
import json, sys
